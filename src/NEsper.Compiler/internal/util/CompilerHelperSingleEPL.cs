			...
			StatementCompileTimeServices compileTimeServices)
		{
			...
			try {
				if (compilable is CompilableEPL) {
					...
					var classesInlined = CompileAddExtensions(parseResult.Classes, compilable, inlinedClassConsumer, compileTimeServices);
			...
			StatementCompileTimeServices compileTimeServices)
		{
			...
			if (option != null) {
				classFileConsumer = compilerResult => {
					option.Visit(new InlinedClassInspectionContext());
				};
			}
