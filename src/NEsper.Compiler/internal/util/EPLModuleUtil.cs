...
namespace com.espertech.esper.compiler.@internal.util
{
	public class EPLModuleUtil
	{
			...
			string resourceName)
		{
			...
			ISet<Import> imports = new LinkedHashSet<Import>();
			count = 0;
			foreach (var node in nodes) {
				...
				if ((node is ParseNodeComment) || (node is ParseNodeExpression)) {
					...
					items.Add(
						new ModuleItem(
							...
							node.Item.StartChar,
							node.Item.EndChar,
							node.Item.LineNumEnd,
							isComments ? -1 : node.Item.LineNumContent,
							isComments ? -1 : node.Item.LineNumContentEnd));
				}
			}
		...
		public static ParseNode GetModule(
			...
			string resourceName)
		{
			var input = CaseChangingCharStreamFactory.Make(item.Expression);
		...
		{
			var input = CaseChangingCharStreamFactory.Make(module);
			var lex = ParseHelper.NewLexer(input);
			...
			int? startLineNum = null;
			int? startLineNumContent = null;
			var charPosStart = 0;
			var charPos = 0;
			...
			foreach (var token in tokenList) {
				...
				if (semi) {
					...
					if (startLineNum == null && t.Type != EsperEPL2GrammarParser.WS) {
						startLineNum = t.Line;
						charPosStart = charPos;
					}

					if (startLineNumContent == null &&
					    t.Type != EsperEPL2GrammarParser.WS &&
					    t.Channel == TokenConstants.DefaultChannel) {
						startLineNumContent = t.Line;
					}
        ...
        public static Module ReadFile(FileInfo file)
		{
			using (var inputStream = file.OpenRead()) {
				return ReadInternal(inputStream, file.FullName);
			}
		}
		...
		{
			// Handle class """{text}"""
			var indexFirstTriple = IndexFirstToken(index, tokens, EsperEPL2GrammarParser.TRIPLEQUOTE);
			if (indexFirstTriple == -1) {
				return index;
			}

			var indexCloseTriple = IndexFirstToken(indexFirstTriple + 1, tokens, EsperEPL2GrammarParser.TRIPLEQUOTE);
