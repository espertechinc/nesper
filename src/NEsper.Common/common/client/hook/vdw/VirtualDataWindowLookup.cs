...
namespace com.espertech.esper.common.client.hook.vdw
{
    ...
    {
        /// <summary>Invoked by an EPL statement that queries a virtual data window to perform a lookup. <para />Keys passed are the actual query lookup values. For range lookups, the key passed is an instance of <seealso cref="VirtualDataWindowKeyRange" />. <para />Value values follow <seealso cref="VirtualDataWindowLookupContext" />. <para />EventsPerStream contains the events participating in the subquery or join. It is not necessary to use eventsPerStream and the events are provided for additional information. Please consider eventsPerStream for Esper internal use. </summary>
