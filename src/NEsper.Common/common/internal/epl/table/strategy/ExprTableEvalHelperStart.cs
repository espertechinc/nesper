...
namespace com.espertech.esper.common.@internal.epl.table.strategy
{
    public class ExprTableEvalHelperStart
    {
            ...
            ExprEvaluatorContext exprEvaluatorContext)
        {
            ...
            var evals = new Dictionary<int, ExprTableEvalStrategy>(tableAccesses.Count);
            foreach (var entry in tableAccesses) {
                var table = entry.Value.Table;
                var provider = table.GetStateProvider(exprEvaluatorContext.AgentInstanceId, exprEvaluatorContext.IsWritesToTables);
