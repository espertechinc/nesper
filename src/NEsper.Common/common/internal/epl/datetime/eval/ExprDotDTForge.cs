    ...
    public class ExprDotDTForge : ExprDotForge
    {
            ...
            IntervalForge intervalForge)
        {
            if (inputEventType == null) {
                ...
                throw new ExprValidationException("Invalid input type '" + inputTypeBoxed + "'");
            }
            ...
            var getterEndTimestamp =
                ((EventTypeSPI) inputEventType).GetGetterSPI(propertyNameEnd);
        ...
        private void CheckNotNull(Type getterResultType, string propertyName)
        {
            if (getterResultType.IsNullTypeSafe()) {
