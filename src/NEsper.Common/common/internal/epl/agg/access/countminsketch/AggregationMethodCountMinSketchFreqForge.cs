...
namespace com.espertech.esper.common.@internal.epl.agg.access.countminsketch
{
	public class AggregationMethodCountMinSketchFreqForge : AggregationMethodForge
	{
		...
		{
			CodegenMethod method = parent.MakeChild(typeof(AgregationMethodCountMinSketchFreq), GetType(), classScope);
			method.Block
				.DeclareVarNewInstance<AgregationMethodCountMinSketchFreq>("strat")
				.SetProperty(Ref("strat"), "FrequencyEval", ExprNodeUtilityCodegen.CodegenEvaluator(_frequencyEval.Forge, method, GetType(), classScope))
