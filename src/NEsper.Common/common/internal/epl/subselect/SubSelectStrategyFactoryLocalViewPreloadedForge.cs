    ...
    public class SubSelectStrategyFactoryLocalViewPreloadedForge : SubSelectStrategyFactoryForge
    {
            ...
            CodegenClassScope classScope)
        {
            ...
            var groupKeyEval = MultiKeyCodegen.CodegenExprEvaluatorMayMultikey(
                _groupKeys,
                null,
                _groupByMultiKeyClasses,
                method,
                classScope);

            var aggregationServiceFactoryExpr = MakeAggregationService(
                _subqueryNumber,
            ...
            method.Block
                ...
                .SetProperty(
                    Ref("factory"),
                    "ViewFactories",
                    ViewFactoryForgeUtil.CodegenForgesWInit(
                        _viewForges,
                        0,
                        _subqueryNumber,
                        method,
                ...
                .SetProperty(
                    ...
                    _lookupStrategy.First.Make(method, symbols, classScope))
                .SetProperty(
                    Ref("factory"),
                    "LookupStrategyFactory",
                    _lookupStrategy.Second.Make(method, symbols, classScope))
                .SetProperty(Ref("factory"), "AggregationServiceFactory", aggregationServiceFactoryExpr)
                .SetProperty(Ref("factory"), "CorrelatedSubquery", Constant(_correlatedSubquery))
                .SetProperty(Ref("factory"), "GroupKeyEval", groupKeyEval)
                .SetProperty(
                    Ref("factory"),
                    "FilterExprEval",
                    _filterExprNode == null
                        ? ConstantNull()
                        : ExprNodeUtilityCodegen.CodegenEvaluatorNoCoerce(
                            _filterExprNode.Forge,
                            method,
                            GetType(),
                            classScope));
        ...
        public bool HasPrior => _viewResourceDelegateDesc.PriorRequests != null &&
                                !_viewResourceDelegateDesc.PriorRequests.IsEmpty();
