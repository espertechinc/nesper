...
namespace com.espertech.esper.common.@internal.epl.resultset.select.eval
{
	public class SelectEvalNoWildcardJson : SelectExprProcessorForge
	{
			...
			CodegenClassScope codegenClassScope)
		{
			...
			methodNode.Block.DeclareVar(_jsonEventType.UnderlyingType, "und", NewInstanceNamed(_jsonEventType.Detail.UnderlyingClassName));
			for (var i = 0; i < _selectContext.ColumnNames.Length; i++) {
				...
				if (evalType == typeof(EventBean)) {
					var conversion = methodNode.MakeChild(fieldClassBoxed, GetType(), codegenClassScope).AddParam(typeof(object), "value");
					conversion.Block
					...
					var conversion = methodNode.MakeChild(underlyingType, GetType(), codegenClassScope).AddParam(typeof(object), "value");
					conversion.Block
						.IfRefNullReturnNull("value")
					...
					var conversion = methodNode.MakeChild(underlyingArrayType, GetType(), codegenClassScope).AddParam(typeof(object), "value");
					conversion.Block
						.IfRefNullReturnNull("value")
