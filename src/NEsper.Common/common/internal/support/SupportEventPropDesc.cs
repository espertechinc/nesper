...
namespace com.espertech.esper.common.@internal.support
{
	public class SupportEventPropDesc
	{
		...
		{
			if (_propertyType.IsNullType()) {
				return;
			}

			var propertyClass = _propertyType;
			if (propertyClass.IsArray) {
				WithIndexed().WithComponentType(propertyClass.GetElementType());
			}

			if (propertyClass.IsGenericStringDictionary()) {
				WithMapped().WithComponentType(TypeHelper.GetSecondParameterTypeOrObject(_propertyType));
			}

			if (propertyClass.IsGenericEnumerable()) {
				WithIndexed().WithComponentType(TypeHelper.GetSingleParameterTypeOrObject(_propertyType));
			}
		}
