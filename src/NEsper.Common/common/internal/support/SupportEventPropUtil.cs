...
namespace com.espertech.esper.common.@internal.support
{
	public class SupportEventPropUtil
	{
			...
			params SupportEventPropDesc[] expected)
		{
			...
			var receivedProps = new Dictionary<string, EventPropertyDescriptor>();
			...
			var expectedProps = new Dictionary<string, SupportEventPropDesc>();
			...
			Type classes)
		{
			var count = 0;
			foreach (var field in fields) {
				AssertEquals("position " + count, classes, type.GetPropertyType(field));
			}
		}
