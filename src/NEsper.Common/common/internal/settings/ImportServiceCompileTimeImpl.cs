...
namespace com.espertech.esper.common.@internal.settings
{
		...
		, ImportServiceCompileTime
	{
		...
		private readonly IList<Pair<ISet<string>, ConfigurationCompilerPlugInAggregationMultiFunction>> _aggregationAccess;
		private readonly IDictionary<string, ImportSingleRowDesc> _singleRowFunctions = new Dictionary<string, ImportSingleRowDesc>();
		private readonly IDictionary<string, AdvancedIndexFactoryProvider> _advancedIndexProviders = new LinkedHashMap<string, AdvancedIndexFactoryProvider>();
			...
			string functionName)
		{
			...
			foreach (var pairs in _aggregationAccess) {
				if (pairs.First.Contains(functionNameLower)) {
					throw new ImportException("Aggregation multi-function by name '" + functionName + "' is already defined");
				}
			}
			...
			ConfigurationCompilerPlugInAggregationFunction aggregationDesc)
		{
			...
			if (aggregationDesc.ForgeClassName == null || !IsClassName(aggregationDesc.ForgeClassName)) {
				throw new ImportException("Invalid class name for aggregation function forge '" + aggregationDesc.ForgeClassName + "'");
			}
