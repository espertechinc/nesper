...
{
	public class ResultSetProcessorRowPerGroupRollupOutputLastHelperImpl : ResultSetProcessorRowPerGroupRollupOutputLastHelper
    {
	        ...
	        if (newData != null) {
	            foreach (var aNewData in newData) {
	                ...
	                foreach (var level in _processor.Prototype.GroupByRollupDesc.Levels) {
	                    ...
	                    _outputLimitGroupRepsPerLevel[level.LevelNumber].Put(groupKey, eventsPerStream);
	        ...
	        if (newEvents != null) {
	            foreach (var newEvent in newEvents) {
	                ...
	                foreach (var level in _processor.Prototype.GroupByRollupDesc.Levels) {
	                    ...
	                    _outputLimitGroupRepsPerLevel[level.LevelNumber].Put(groupKey, aNewData);
	                    if (_processor.Prototype.IsSelectRStream && !_groupRepsOutputLastUnordRStream[level.LevelNumber].ContainsKey(groupKey)) {
	        ...
	        if (oldEvents != null) {
	            foreach (var oldEvent in oldEvents) {
	                ...
	                foreach (var level in _processor.Prototype.GroupByRollupDesc.Levels) {
	                    ...
	                    _outputLimitGroupRepsPerLevel[level.LevelNumber].Put(groupKey, aOldData);
	                    if (_processor.Prototype.IsSelectRStream && !_groupRepsOutputLastUnordRStream[level.LevelNumber].ContainsKey(groupKey)) {
