...
namespace com.espertech.esper.regression.view
{
	...
	public class TestAggregateLocalGroupBy 
	{
	    ...
	    public void TestInvalid() {
	        ...
	        string eplSubq = "select (select sum(intPrimitive, group_by:theString) from SupportBean.win:keepall()) from SupportBean_S0";
	    ...
	    public void TestPlanning() {
	        ...
	        string theEpl = PLAN_CALLBACK_HOOK + "select window(*, group_by:theString), last(*, group_by:theString) from SupportBean.win:length(2)";
	    ...
	    private void RunAssertionUngroupedRowRemove() {
	        ...
	        string epl = "create window MyWindow.win:keepall() as SupportBean;\n" +
	    ...
	    private void RunAssertionUngroupedAggAdditionalAndPlugin() {
	        ...
	        ConfigurationPlugInAggregationMultiFunction mfAggConfig = new ConfigurationPlugInAggregationMultiFunction(SupportAggMFFuncExtensions.GetFunctionNames(), typeof(SupportAggMFFactory).FullName);
	    ...
	    private void RunAssertionUngroupedUnidirectionalJoin() {
	        string epl = "select theString, sum(intPrimitive, group_by:theString) as c0 from SupportBean.win:keepall(), SupportBean_S0 unidirectional";
	        _epService.EPAdministrator.CreateEPL(epl).AddListener(_listener);
