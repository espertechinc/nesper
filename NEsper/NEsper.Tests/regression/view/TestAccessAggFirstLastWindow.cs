...
namespace com.espertech.esper.regression.view
{
	...
	public class TestAccessAggFirstLastWindow
    {
	    ...
	    public void TestNoParamChainedAndProperty() {
	        ...
	        var stmt = _epService.EPAdministrator.CreateEPL("select first().property as val0, first().MyMethod() as val1, window() as val2 from ChainEvent.std:lastevent()");
	    ...
	    public void TestLastMaxMixedOnSelect() {
	        ...
	        var epl = "on SupportBean(TheString like 'B%') select last(mw.IntPrimitive) as li, max(mw.IntPrimitive) as mi from MyWindow mw";
	        var stmt = _epService.EPAdministrator.CreateEPL(epl);
	        stmt.AddListener(_listener);
	        var fields = "li,mi".Split(',');
	    ...
	    public void TestPrevNthIndexedFirstLast() {
	        ...
	        var stmt = _epService.EPAdministrator.CreateEPL(epl);
	        stmt.AddListener(_listener);
	        var fields = "p0,p1,p2,n0,n1,n2,l1,l2,l3".Split(',');
	    ...
	    private void RunAssertionFirstLastIndexed() {
	        var fields = "f0,f1,f2,f3,l0,l1,l2,l3".Split(',');
	        _epService.EPRuntime.SendEvent(new SupportBean("E1", 10));
	    ...
	    public void TestInvalid() {
	        TryInvalid("select window(distinct IntPrimitive) from SupportBean",
                       "Incorrect syntax near '(' ('distinct' is a reserved keyword) at line 1 column 13 near reserved keyword 'distinct' [select window(distinct IntPrimitive) from SupportBean]");
	    ...
	    public void TestSubquery() {
	        var epl = "select id, (select window(sb.*) from SupportBean.win:length(2) as sb) as w from SupportBean_A";
	        var stmt = _epService.EPAdministrator.CreateEPL(epl);
	        stmt.AddListener(_listener);
	        var fields = "id,w".Split(',');

	        _epService.EPRuntime.SendEvent(new SupportBean_A("A1"));
	        EPAssertionUtil.AssertProps(_listener.AssertOneGetNewAndReset(), fields, new object[]{"A1", null});

	        var beanOne = SendEvent(_epService, "E1", 0, 1);
	        ...
	        var beanTwo = SendEvent(_epService, "E2", 0, 1);
	        ...
	        var beanThree = SendEvent(_epService, "E2", 0, 1);
	    ...
	    public void TestMethodAndAccessTogether() {
	        var epl = "select sum(IntPrimitive) as si, window(sa.IntPrimitive) as wi from SupportBean.win:length(2) as sa";
	        var stmt = _epService.EPAdministrator.CreateEPL(epl);
	        stmt.AddListener(_listener);
	        var fields = "si,wi".Split(',');
	    ...
	    public void TestOutputRateLimiting() {
	        var epl = "select sum(IntPrimitive) as si, window(sa.IntPrimitive) as wi from SupportBean.win:keepall() as sa output every 2 events";
	        var stmt = _epService.EPAdministrator.CreateEPL(epl);
	        stmt.AddListener(_listener);
	        var fields = "si,wi".Split(',');
	    ...
	    private void RunAssertionType(bool isCheckStatic) {
	        var fields = "f1,f2,w1,l1".Split(',');

	        var beanOne = SendEvent(_epService, "E1", 10d, 100);
	        ...
	        if (isCheckStatic) {
	            object[] parameters = SupportStaticMethodLib.GetInvocations()[0];
	    ...
	    public void TestJoin2Access() {
	        ...
	        var fields = "ast,bst,fas,was,las,fbs,wbs,lbs".Split(',');
	    ...
	    public void TestOuterJoin1Access() {
	        ...
	        var fields = "aid,bid,fb,wb,lb".Split(',');
	    ...
	    public void TestBatchWindowGrouped()
	    {
	        ...
	        var fields = "TheString,fi,wi,li".Split(',');
            ...
            var result = _listener.GetAndResetLastNewData();
	    ...
	    public void TestLateInitialize()
	    {
	        ...
	        var fields = "firststring,windowstring,laststring".Split(',');
	        var epl = "select " +
	    ...
	    public void TestOnDemandQuery()
	    {
	        ...
	        var q = _epService.EPRuntime.PrepareQuery("select first(IntPrimitive) as f, window(IntPrimitive) as w, last(IntPrimitive) as l from MyWindow as s");
	    ...
	    private void RunAssertionStar() {
	        var fields = "firststar,firststarsb,laststar,laststarsb,windowstar,windowstarsb".Split(',');
	    ...
	    public void TestWindowedUnGrouped()
	    {
	        ...
	        var stmtWNull = _epService.EPAdministrator.CreateEPL("select window(IntBoxed).take(10) from SupportBean.win:length(2)");
	    ...
	    public void TestWindowedGrouped()
	    {
	        ...
	        // test SODA indexes
	        var eplFirstLast = "select " +
	                "last(IntPrimitive), " +
	                "last(IntPrimitive, 1), " +
	                "first(IntPrimitive), " +
	                "first(IntPrimitive, 1) " +
	                "from SupportBean.win:length(3)";
	        var modelFirstLast = _epService.EPAdministrator.CompileEPL(epl);
	        Assert.AreEqual(epl, modelFirstLast.ToEPL());
	    }

	    private void RunAssertionGrouped() {
	        var fields = "TheString,firststring,firstint,laststring,lastint,allint".Split(',');
	        ...
	        var received = _listener.GetAndResetLastNewData();
	    ...
	    private void RunAssertionUngrouped() {
	        var fields = "firststring,firstint,laststring,lastint,allint".Split(',');
