...
namespace com.espertech.esper.regression.nwtable
{
    ...
    public class TestInfraOnMerge
    {
        ...
        public void TestUpdateNestedEvent() {
            ...
            _epService.EPAdministrator.CreateEPL("create window AInfra.win:keepall() as (c Composite)");
        ...
        private void RunAssertionFlow(bool namedWindow) 
        {
            ...
            var createEPL = namedWindow ?
                    "@Name('Window') create window MyMergeInfra.std:unique(TheString) as SupportBean" :
            ...
            var eplCreateInfraTwo = namedWindow ?
                    "create window MyInfraTwo.std:unique(id) as select * from TypeOne" :
        ...
        private void RunAssertionMultipleInsert(bool namedWindow) {
            ...
            var eplCreate = namedWindow ?
                    "create window MyInfra.win:keepall() as MySchema" :
            ...
            SendMyEvent(EventRepresentationEnumExtensions.GetEngineDefault(_epService), "E1", 0);
            Assert.IsFalse(_mergeListener.IsInvoked);

            SendMyEvent(EventRepresentationEnumExtensions.GetEngineDefault(_epService), "A1", 1);
            EPAssertionUtil.AssertProps(_mergeListener.AssertOneGetNewAndReset(), fields, new object[]{"A1", 1});

            SendMyEvent(EventRepresentationEnumExtensions.GetEngineDefault(_epService), "B1", 2);
            EPAssertionUtil.AssertProps(_mergeListener.AssertOneGetNewAndReset(), fields, new object[]{"B1", 2});
    
            SendMyEvent(EventRepresentationEnumExtensions.GetEngineDefault(_epService), "C1", 3);
            EPAssertionUtil.AssertProps(_mergeListener.AssertOneGetNewAndReset(), fields, new object[]{"Z", -1});
    
            SendMyEvent(EventRepresentationEnumExtensions.GetEngineDefault(_epService), "D1", 4);
        ...
        private void RunAssertionNoWhereClause(bool namedWindow) 
        {
            ...
            SendMyEvent(EventRepresentationEnumExtensions.GetEngineDefault(_epService), "E1", 2);
            EPAssertionUtil.AssertPropsPerRowAnyOrder(namedWindowStmt.GetEnumerator(), fields, new object[][] { new object[] { "xE1x", -2 } });
    
            SendMyEvent(EventRepresentationEnumExtensions.GetEngineDefault(_epService), "A1", 3);   // matched : no where clause
            EPAssertionUtil.AssertPropsPerRowAnyOrder(namedWindowStmt.GetEnumerator(), fields, new object[][] { new object[] { "xE1x", -2 } });
            ...
            SendMyEvent(EventRepresentationEnumExtensions.GetEngineDefault(_epService), "A1", 4);
            EPAssertionUtil.AssertPropsPerRowAnyOrder(namedWindowStmt.GetEnumerator(), fields, new object[][] { new object[] { "A1", 4 } });
    
            SendMyEvent(EventRepresentationEnumExtensions.GetEngineDefault(_epService), "B1", 5);   // matched : no where clause
            EPAssertionUtil.AssertPropsPerRowAnyOrder(namedWindowStmt.GetEnumerator(), fields, new object[][] { new object[] { "A1", 4 } });
    
            _epService.EPRuntime.SendEvent(new SupportBean_A("Ax1"));
            EPAssertionUtil.AssertPropsPerRowAnyOrder(namedWindowStmt.GetEnumerator(), fields, null);
    
            SendMyEvent(EventRepresentationEnumExtensions.GetEngineDefault(_epService), "B1", 5);
            EPAssertionUtil.AssertPropsPerRowAnyOrder(namedWindowStmt.GetEnumerator(), fields, new object[][] { new object[] { "B1", 5 } });
    
            SendMyEvent(EventRepresentationEnumExtensions.GetEngineDefault(_epService), "C", 6);
        ...
        private void RunAssertionInvalid(bool namedWindow) {
            ...
            var eplCreateMergeInfra = namedWindow ?
                    "create window MergeInfra.std:unique(TheString) as SupportBean" :
            ...
            var eplCreateABCInfra = namedWindow ?
                    "create window ABCInfra.win:keepall() as ABCSchema" :
            ...
            if (namedWindow) {
                ...
                SupportMessageAssertUtil.TryInvalid(_epService, epl, "Error starting statement: Validation failed in when-not-matched (clause 1): Expression-returned event type 'SupportBean_A' with underlying type 'com.espertech.esper.support.bean.SupportBean_A' cannot be converted to target event type 'MergeInfra' with underlying type 'com.espertech.esper.support.bean.SupportBean' [on SupportBean_A as up merge MergeInfra as mv where mv.TheString=id when matched then insert select *]");
            }
        ...
        private void RunAssertionInnerTypeAndVariable(bool namedWindow, EventRepresentationEnum eventRepresentationEnum) {
            ...
            var eplCreate = namedWindow ?
                    eventRepresentationEnum.GetAnnotationText() + " create window MyInfra.win:keepall() as (c1 string, c2 MyInnerSchema)" :
            ...
            var subscriber = new esper.support.util.SupportSubscriberMRD();
        ...
        private void RunAssertionPatternMultimatch(bool namedWindow) {
            ...
            var eplCreate = namedWindow ?
                    "create window MyInfra.win:keepall() as (c1 string, c2 string)" :
        ...
        private void RunAssertionOnMergeInsertStream(bool namedWindow) {
            ...
            var eplCreate = namedWindow ?
                    "create window Win.win:keepall() as WinSchema " :
        ...
        private void RunAssertionMultiactionDeleteUpdate(bool namedWindow) {
            var eplCreate = namedWindow ?
                    "create window Win.win:keepall() as SupportBean" :
        ...
        private void RunAssertionSubqueryNotMatched(bool namedWindow) {
            var eplCreateOne = namedWindow ?
                    "create window InfraOne.std:unique(string) (string string, IntPrimitive int)" :
            ...
            var eplCreateTwo = namedWindow ?
                    "create window InfraTwo.std:unique(val0) (val0 string, val1 int)" :
        ...
        private void RunAssertionInsertOtherStream(bool namedWindow, EventRepresentationEnum eventRepresentationEnum) {
            var epl = eventRepresentationEnum.GetAnnotationText() + " create schema MyEvent as (name string, value double);\n" +
                    (namedWindow ? 
                        eventRepresentationEnum.GetAnnotationText() + " create window MyInfra.std:unique(name) as MyEvent;\n":
        ...
        private void MakeSendNameValueEvent(EPServiceProvider engine, EventRepresentationEnum eventRepresentationEnum, string typeName, string name, double value) {
            ...
            if (eventRepresentationEnum.IsObjectArrayEvent()) {
                engine.EPRuntime.SendEvent(theEvent.Values.ToArray(), typeName);
            }
            else {
        ...
        private void RunUpdateNestedEvent(bool namedWindow, string metaType) {
            var eplTypes =
                    ...
                    (namedWindow ?
                            "create window AInfra.std:lastevent() as AInfraType;\n":
