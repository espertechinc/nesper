...
namespace com.espertech.esper.regression.epl
{
    ...
    public class TestSubselectUnfiltered
    {
        ...
        {
            var stmtText = "select id from S0 where (select true from S1.win:length(1000))";
        ...
        {
            var stmtText = "select id from S0 where (select p10='X' from S1.win:length(1000))";
        ...
        public void TestInvalidSubselect()
        {
            ...
            TryInvalid("select id in (select * from S1.win:length(1000)) as value from S0",
                       "Error starting statement: Failed to validate select-clause expression subquery number 1 querying S1: Implicit conversion from datatype '" + Name.Of<SupportBean_S1>() + "' to '" + Name.Of<int>() + "' is not allowed [select id in (select * from S1.win:length(1000)) as value from S0]");
        }
        ...
        {
            var stmtText = "select (select " + typeof(SupportStaticMethodLib).FullName + ".MinusOne(id) from S1.win:length(1000)) as idS1 from S0";
        ...
        {
            var stmtText = "select 100*(select id from S1.win:length(1000)) as idS1 from S0";
        ...
        {
            var stmtText = "select (select id from S1(p10='A').win:length(1000)) as idS1 from S0";
        ...
        {
            var stmtText = "select (select id from S1.win:length(1000)) as idS1 from S0";
        ...
        {
            var stmtText = "select (select id from S1.win:length(2)) as idS1 from S0";
            RunAssertMultiRowUnfiltered(stmtText, "idS1");
        }
        ...
        {
            var stmtText = "select (select id from S1.std:lastevent()) as idS1 from S0";
        ...
        {
            var stmtText = "select (select id as myId from S1.std:lastevent()) from S0";
        ...
        {
            var stmtText = "select (select id from S1.std:lastevent()) from S0";
        ...
        {
            var stmtText = "select (select p10 || p11 from S1.std:lastevent()) as value from S0";
        ...
        {
            var stmtText = "select (select id+1 as myId from S1.std:lastevent()) as idS1_0, " +
