...
namespace com.espertech.esper.regression.epl
{
    ...
    public class TestInsertIntoPopulateUnderlying 
    {
        ...
        public void TestCtor()
        {
            ...
            // test join wildcard
            var eplThree = "insert into SupportBeanCtorTwo select * from SupportBean_ST0.std:lastevent(), SupportBean_ST1.std:lastevent()";
            ...
            var epl = "insert into MyEventWithCtorSameType select c1,c2 from SupportBean(TheString='b1').std:lastevent() as c1, SupportBean(TheString='b2').std:lastevent() as c2";
        ...
        public void TestInvalid()
        {
            ...
            TryInvalid("Error starting statement: Failed to find a suitable constructor for type 'SupportBeanCtorOne': Could not find constructor in class 'com.espertech.esper.support.bean.SupportBeanCtorOne' with matching parameter number and expected parameter type(s) '" + Name.Clean<int>() + "' (nearest matching constructor taking type(s) 'System.String') [insert into SupportBeanCtorOne select 1 from SupportBean]", text);

            text = "insert into SupportBean(IntPrimitive) select 1L from SupportBean";
            TryInvalid("Error starting statement: Invalid assignment of column 'IntPrimitive' of type '" + Name.Of<long>() + "' to event property 'IntPrimitive' typed as '" + Name.Of<int>(false) + "', column and parameter types mismatch [insert into SupportBean(IntPrimitive) select 1L from SupportBean]", text);

            text = "insert into SupportBean(IntPrimitive) select null from SupportBean";
            TryInvalid("Error starting statement: Invalid assignment of column 'IntPrimitive' of null type to event property 'IntPrimitive' typed as '" + Name.Of<int>(false) + "', nullable type mismatch [insert into SupportBean(IntPrimitive) select null from SupportBean]", text);
            
            text = "insert into SupportBeanReadOnly select 'a' as geom from SupportBean";
            TryInvalid("Error starting statement: Failed to find a suitable constructor for type 'SupportBeanReadOnly': Could not find constructor in class 'com.espertech.esper.support.bean.SupportBeanReadOnly' with matching parameter number and expected parameter type(s) 'System.String' (nearest matching constructor taking no parameters) [insert into SupportBeanReadOnly select 'a' as geom from SupportBean]", text);
            ...
            TryInvalid("Error starting statement: Event type named 'ABCStream' has already been declared with differing column name or type information: Type by name 'ABCStream' is not a compatible type (target type underlying is 'Pair`2') [insert into ABCStream(string) select 'E1' from MyMap]", text);

            text = "insert into xmltype select 1 from SupportBean";
            TryInvalid("Error starting statement: Event type named 'xmltype' has already been declared with differing column name or type information: Type by name 'xmltype' is not a compatible type (target type underlying is 'XmlNode') [insert into xmltype select 1 from SupportBean]", text);
        ...
        public void TestPopulateMap()
        {
            ...
            var stmtOrig = _epService.EPAdministrator.CreateEPL("select * from MyMapType");
    
            var stmtTextOne = "insert into MyMapType select IntPrimitive as intVal, TheString as stringVal, DoubleBoxed as doubleVal from SupportBean";
            var stmtOne = _epService.EPAdministrator.CreateEPL(stmtTextOne);
            stmtOne.Events += _listener.Update;
            ...
            _epService.EPAdministrator.CreateEPL("create schema ConcreteType as (value System.String)");
            _epService.EPAdministrator.CreateEPL("insert into ConcreteType select \"Test\" as value from pattern[every timer:interval(1 second)]");
        }
        ...
        public void TestPopulateObjectArray()
        {
            ...
            var stmtTextOne = "insert into MyOAType select IntPrimitive as intVal, TheString as stringVal, DoubleBoxed as doubleVal from SupportBean";
            var stmtOne = _epService.EPAdministrator.CreateEPL(stmtTextOne);
            stmtOne.Events += _listener.Update;
            Assert.AreSame(stmtOrig.EventType, stmtOne.EventType);
            ...
            EPAssertionUtil.AssertProps(_listener.AssertOneGetNewAndReset(), "intVal,stringVal,doubleVal,nullVal".Split(','), new Object[]{1000, "E1", 1001d, null});
        }
        ...
        public void TestBeanFactoryMethod()
        {
            ...
            _epService.EPRuntime.SendEvent(new Dictionary<string, object>(), "MyMap");
            EPAssertionUtil.AssertProps(_listener.AssertOneGetNewAndReset(), "Id,Type,Device,Measurement,Confidence".Split(','), new Object[]{2, "A01", "DHC1000", 100.0, 5.0});
        ...
        public void TestArrayPONOInsert()
        {
            ...
            try {
                ...
                Assert.AreEqual("Error starting statement: Invalid assignment of column 'EndEvent' of type 'com.espertech.esper.support.bean.SupportBean[]' to event property 'EndEvent' typed as 'com.espertech.esper.support.bean.SupportBean', column and parameter types mismatch [INSERT INTO FinalEventInvalidNonArray SELECT s as StartEvent, e as EndEvent FROM PATTERN [every s=SupportBean_S0 -> e=SupportBean(TheString=s.p00) until timer:interval(10 sec)]]", ex.Message);
            }
            ...
            try {
                ...
                Assert.AreEqual("Error starting statement: Invalid assignment of column 'StartEvent' of type 'com.espertech.esper.support.bean.SupportBean_S0' to event property 'StartEvent' typed as 'com.espertech.esper.support.bean.SupportBean_S0[]', column and parameter types mismatch [INSERT INTO FinalEventInvalidArray SELECT s as StartEvent, e as EndEvent FROM PATTERN [every s=SupportBean_S0 -> e=SupportBean(TheString=s.p00) until timer:interval(10 sec)]]", ex.Message);
            }
        }
        ...
        private void SendEventTwo(EPServiceProvider epService, EventRepresentationEnum eventRepresentationEnum, String id, int val)
        {
            ...
            if (eventRepresentationEnum.IsObjectArrayEvent()) {
                epService.EPRuntime.SendEvent(theEvent.Values.ToArray(), "EventTwo");
            }
            else {
