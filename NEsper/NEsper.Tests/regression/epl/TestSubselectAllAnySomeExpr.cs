...
namespace com.espertech.esper.regression.epl
{
    ...
    public class TestSubselectAllAnySomeExpr 
    {
        ...
        public void TestRelationalOpAll()
        {
            ...
            try
            {
                _epService.EPAdministrator.CreateEPL("select intArr > all (select IntPrimitive from SupportBean.win:keepall()) from ArrayBean");
                Assert.Fail();
            }
            catch (EPStatementException ex)
            {
                Assert.AreEqual("Error starting statement: Failed to validate select-clause expression subquery number 1 querying SupportBean: Collection or array comparison is not allowed for the IN, ANY, SOME or ALL keywords [select intArr > all (select IntPrimitive from SupportBean.win:keepall()) from ArrayBean]", ex.Message);
            }
        ...
        public void TestInvalid()
        {
            ...
            {
                Assert.AreEqual("Error starting statement: Failed to validate select-clause expression subquery number 1 querying SupportBean: Collection or array comparison is not allowed for the IN, ANY, SOME or ALL keywords [select intArr = all (select IntPrimitive from SupportBean.win:keepall()) as r1 from ArrayBean]", ex.Message);
            }
        }
