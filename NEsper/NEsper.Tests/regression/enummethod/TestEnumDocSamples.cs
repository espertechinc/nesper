...
namespace com.espertech.esper.regression.enummethod
{
    ...
    public class TestEnumDocSamples
    {
        ...
        public void TestSubquery()
        {
            ...
            _epService.EPAdministrator.CreateEPL("select (select name, count(*) as cnt from Zone.win:keepall() group by name).where(v => cnt > 1) from LocationReport");
        }

        [Test]
        public void TestNamedWindow()
        {
            _epService.EPAdministrator.CreateEPL("create window ZoneWindow.win:keepall() as Zone");
