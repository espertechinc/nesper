	...
	public class TestExpressionDefConfigurations
    {
	    ...
	    private void RunAssertionExpressionCacheSize(int? configuredCacheSize, int expectedInvocationCount)
        {
	        ...
	        if (configuredCacheSize != null) {
	            config.EngineDefaults.ExecutionConfig.DeclaredExprValueCacheSize = configuredCacheSize.Value;
	        }
	        ...
	        EPStatement stmt = epService.EPAdministrator.CreateEPL(
	                "expression myExpr {v => alwaysTrue(null) } select myExpr(st0) as c0, myExpr(st1) as c1, myExpr(st0) as c2, myExpr(st1) as c3 from SupportBean_ST0.std:lastevent() as st0, SupportBean_ST1.std:lastevent() as st1");
