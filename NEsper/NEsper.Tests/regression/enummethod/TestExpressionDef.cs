...
namespace com.espertech.esper.regression.enummethod
{
	...
	public class TestExpressionDef
    {
	    ...
	    public void TestNestedExpressionMultiSubquery() {
	        var fields = "c0".Split(',');
	        _epService.EPAdministrator.CreateEPL("create expression F1 { (select IntPrimitive from SupportBean.std:lastevent())}");
	        _epService.EPAdministrator.CreateEPL("create expression F2 { param => (select a.IntPrimitive from SupportBean.std:unique(TheString) as a where a.TheString = param.TheString) }");
	    ...
	    public void TestSubqueryCross() {
	        ...
	        var eplAlias = "expression subq alias for { (select TheString from SupportBean.win:keepall() where TheString = one.id and IntPrimitive = two.p10) }" +
	                "select subq as val1 " +
	                "from SupportBean_ST0.std:lastevent() as one, SupportBean_ST1.std:lastevent() as two";
	    ...
	    public void TestSubqueryCorrelated() {
	        ...
	        var eplAlias = "expression subqOne alias for {(select id from SupportBean_ST0.win:keepall() where p00 = t.IntPrimitive)} " +
	                "select TheString as val0, subqOne(t) as val1 from SupportBean as t";
	    ...
	    public void TestSubqueryUncorrelated() {
	        var eplDeclare = "expression subqOne {(select id from SupportBean_ST0.std:lastevent())} " +
	                "select TheString as val0, subqOne() as val1 from SupportBean as t";
	        RunAssertionSubqueryUncorrelated(eplDeclare);

	        var eplAlias = "expression subqOne alias for {(select id from SupportBean_ST0.std:lastevent())} " +
	                "select TheString as val0, subqOne as val1 from SupportBean as t";
	    ...
	    private void RunAssertionSubqueryNamedWindowUncorrelated(string epl)
        {
	        ...
	        var stmt = _epService.EPAdministrator.CreateEPL(epl);
	        stmt.AddListener(_listener);
	        LambdaAssertionUtil.AssertTypes(stmt.EventType, fieldsSelected, new Type[]{typeof(ICollection<object>), typeof(ICollection<object>)});
	    ...
	    public void TestSubqueryNamedWindowCorrelated() {
	        ...
	        _epService.EPAdministrator.CreateEPL(EventRepresentationEnum.MAP.GetAnnotationText() + " create window MyWindowTwo.win:keepall() as (id string, p00 int)");
	    ...
	    public void TestAggregationAccess() {
	        ...
	        var eplAlias = "expression wb alias for {window(*).where(y => y.IntPrimitive > 2)}" +
	                "select wb as val1 from SupportBean.win:keepall() as t";
	    ...
	    private void RunAssertionNamedWindowCast(string epl) {
	        ...
	        _epService.EPAdministrator.CreateEPL("create window MyWindow.win:keepall() as (myObject long)");
	    ...
	    public void TestInvalid() {
	        ...
	        var epl = "expression abc {(select * from SupportBean_ST0.std:lastevent() as st0 where p00=IntPrimitive)} select abc() from SupportBean";
	        TryInvalid(epl, "Error starting statement: Failed to plan subquery number 1 querying SupportBean_ST0: Failed to validate filter expression 'p00=IntPrimitive': Property named 'IntPrimitive' is not valid in any stream [expression abc {(select * from SupportBean_ST0.std:lastevent() as st0 where p00=IntPrimitive)} select abc() from SupportBean]");
	        ...
	        TryInvalid(epl, "Error starting statement: Failed to plan subquery number 1 querying SupportBean_ST0: Failed to validate filter expression 'p00=sb.IntPrimitive': Failed to find a stream named 'sb' (did you mean 'st0'?) [expression abc {(select * from SupportBean_ST0.std:lastevent() as st0 where p00=sb.IntPrimitive)} select abc() from SupportBean sb]");
