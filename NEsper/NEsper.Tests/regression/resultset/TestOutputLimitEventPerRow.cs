...
namespace com.espertech.esper.regression.resultset
{
	...
	public class TestOutputLimitEventPerRow 
	{
	    ...
	    public void TestOutputFirstHavingJoinNoJoin() {
	        ...
	        var stmtTextJoin = "select TheString, LongPrimitive, sum(IntPrimitive) as value from MyWindow mv, SupportBean_A.win:keepall() a where a.id = mv.TheString " +
	        ...
	        var stmtTextOrderJoin = "select TheString, LongPrimitive, sum(IntPrimitive) as value from MyWindow mv, SupportBean_A.win:keepall() a where a.id = mv.TheString " +
