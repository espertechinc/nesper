...
namespace com.espertech.esper.regression.resultset
{
	...
	public class TestOutputLimitSimple 
	{
	    ...
	    public void TestOutputEveryTimePeriod()
	    {
	        ...
	        var stmtText = "select Symbol from MarketData.win:keepall() output snapshot every 1 day 2 hours 3 minutes 4 seconds 5 milliseconds";
	    ...
	    public void TestOutputEveryTimePeriodVariable()
	    {
	        ...
	        var stmtText = "select Symbol from MarketData.win:keepall() output snapshot every D days H hours M minutes S seconds MS milliseconds";
	        var stmt = _epService.EPAdministrator.CreateEPL(stmtText);
	        stmt.AddListener(_listener);
	        SendMDEvent("E1", 0);
	    ...
	    public void TestLimitTime(){
	    	...
	    	var selectStatement = "select * from " + eventName + ".win:length(5)";
	    ...
	    {
	        var stmtText = "select * from " + typeof(SupportBean).FullName + ".win:time_batch(10 seconds) output every 10 seconds";
	        var stmt = _epService.EPAdministrator.CreateEPL(stmtText);
	        var listener = new SupportUpdateListener();
	        stmt.AddListener(listener);
	    ...
	    public void TestLimitEventSimple()
	    {
	        ...
	        var eventName = typeof(SupportBean).FullName;
	        var selectStmt = "select * from " + eventName + ".win:length(5)";
	    ...
	    public void TestLimitSnapshot()
	    {
	        ...
	        SendTimer(0);
	        var selectStmt = "select * from " + typeof(SupportBean).FullName + ".win:time(10) output snapshot every 3 events";
	    ...
	    public void TestLimitSnapshotJoin()
	    {
	        ...
	        SendTimer(0);
	        var selectStmt = "select TheString from " + typeof(SupportBean).FullName + ".win:time(10) as s," +
	                typeof(SupportMarketDataBean).FullName + ".win:keepall() as m where s.TheString = m.Symbol output snapshot every 3 events order by Symbol asc";
	    ...
	    public void TestSnapshotMonthScoped() {
	        ...
	        _epService.EPAdministrator.CreateEPL("select * from SupportBean.std:lastevent() output snapshot every 1 month").AddListener(_listener);
	    ...
	    public void TestFirstMonthScoped() {
	        _epService.EPAdministrator.Configuration.AddEventType<SupportBean>();
	        SendCurrentTime("2002-02-01T9:00:00.000");
	        _epService.EPAdministrator.CreateEPL("select * from SupportBean.std:lastevent() output first every 1 month").AddListener(_listener);
