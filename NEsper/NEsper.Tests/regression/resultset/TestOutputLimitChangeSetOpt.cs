...
namespace com.espertech.esper.regression.resultset
{
	...
	public class TestOutputLimitChangeSetOpt 
	{
        ...
        private void RunAssertion(
            ...
            string orderBy)
        {
	        var epl = string.Format("{0}select irstream {1} " + "from SupportBean.win:length(2) {2}{3}output {4} every 1 seconds {5}", (withHint ? "@Hint('enable_outputlimit_opt') " : ""), selectClause, (groupBy == null ? "" : groupBy + " "), (having == null ? "" : having + " "), outputKeyword, (orderBy ?? ""));
