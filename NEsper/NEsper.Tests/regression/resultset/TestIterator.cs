	...
	public class TestIterator 
	{
	    ...
	    public void TestPatternWithWindow()
	    {
			var cepStatementString =	"select * from pattern " +
										"[every ( addressInfo = " + typeof(SupportBean).FullName + "(TheString='address') " +
										"-> txnWD = " + typeof(SupportBean).FullName + "(TheString='txn') ) ].std:lastevent() " +
										"where addressInfo.IntBoxed = txnWD.IntBoxed";
			var epStatement = _epService.EPAdministrator.CreateEPL(cepStatementString);
	    ...
	    {
	        var stmtText = "select * from " + typeof(SupportMarketDataBean).FullName + ".win:length(5) order by Symbol, Volume";
	    ...
	    public void TestOrderByProps()
	    {
	        ...
	        var stmtText = "select Symbol, Volume from " + typeof(SupportMarketDataBean).FullName + ".win:length(3) order by Symbol, Volume";
	        var stmt = _epService.EPAdministrator.CreateEPL(stmtText);
	        Assert.IsFalse(stmt.GetEnumerator().MoveNext());
	    ...
	    public void TestFilter()
	    {
	        ...
	        var stmtText = "select Symbol, Volume * 10 as vol from " + typeof(SupportMarketDataBean).FullName + ".win:length(5)" +
	                      " where Volume < 0";
	        var stmt = _epService.EPAdministrator.CreateEPL(stmtText);
	        Assert.IsFalse(stmt.GetEnumerator().MoveNext());
