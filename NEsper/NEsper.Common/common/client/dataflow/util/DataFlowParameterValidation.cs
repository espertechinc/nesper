...
namespace com.espertech.esper.common.client.dataflow.util
{
	...
	public class DataFlowParameterValidation {
	    ...
	    public static ExprNode Validate(string name, ExprNode expr, Type expectedReturnType, DataFlowOpForgeInitializeContext context) {
	        if (expr == null) {
	            return null;
	        }
	    ...
	    /// <returns>validated expression node</returns>
	    /// <throws>ExprValidationException when validation failed</throws>
	    public static ExprNode Validate(string name, ExprNode expr, EventType eventType, Type expectedReturnType, DataFlowOpForgeInitializeContext context) {
	        if (expr == null) {
	            return null;
	        }
	        ExprNode validated = EPLValidationUtil.ValidateSimpleGetSubtree(ExprNodeOrigin.DATAFLOWFILTER, expr, eventType, false, context.StatementRawInfo, context.Services);
	        ValidateReturnType(name, validated, expectedReturnType);
	        return validated;
	    }

	    private static void ValidateReturnType(string name, ExprNode validated, Type expectedReturnType) {
