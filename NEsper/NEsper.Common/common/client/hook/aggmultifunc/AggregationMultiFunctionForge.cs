...
namespace com.espertech.esper.common.client.hook.aggmultifunc
{
    ...
    /// <para />This API allows adding one or more related aggregation functions that can share state,
    /// share parameters or exhibit related behavior.
    /// <para />Please use <seealso cref="ConfigurationCompilerPlugInAggregationMultiFunction" />to register this factory class in the runtimetogether with one or more function names.
    /// <para />The runtimeinstantiates a single instance of this class at the time it encounters the first
    ...
    /// <para />At the time of statement parsing, each aggregation multi-function encountered during parsing
    ...
    /// <para />The runtimeinvokes {@link #validateGetHandler(AggregationMultiFunctionValidationContext)}}
    ...
    /// For multiple aggregation functions, the order in which such calls occur is not well defined
    /// and should not be relied on by the implementation.
    /// <para />Usually a single <seealso cref="AggregationMultiFunctionHandler" /> handler class can handle the needs
    ...
    /// <para />The function class must be Serializable only when used with EsperHA.
    /// </summary>
