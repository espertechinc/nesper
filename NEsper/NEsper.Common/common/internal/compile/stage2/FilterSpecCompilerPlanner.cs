...
namespace com.espertech.esper.common.@internal.compile.stage2
{
	public class FilterSpecCompilerPlanner {
	    ...
	    public static IList<FilterSpecParamForge>[] PlanFilterParameters(IList<ExprNode> validatedNodes, FilterSpecCompilerArgs args)
	            {
	        ...
	        Hint hint = HintEnum.MAX_FILTER_WIDTH.GetHint(args.statementRawInfo.Annotations);
	        if (hint != null) {
	    ...
	    private static IList<FilterSpecParamForge>[] PlanRemainingNodesIfFeasible(FilterSpecParaForgeMap overallExpressions, FilterSpecCompilerArgs args, int filterServiceMaxFilterWidth)
	            {
	        ...
	        // combine
	        IList<FilterSpecParamForge>[] result = (IList<FilterSpecParamForge>[]) new IList[sizeFactorized];
	    ...
	    private static IList<FilterSpecParamForge> ComputePermutation(FilterSpecParaForgeMap filterParamExprMap, object[] permutation, FilterSpecParaForgeMap[][] orNodesMaps, FilterSpecCompilerArgs args)
	            {
	    ...
	    private static void DecomposePopulateConsolidate(FilterSpecParaForgeMap filterParamExprMap, IList<ExprNode> validatedNodes, FilterSpecCompilerArgs args)
	            {
	        IList<ExprNode> constituents = DecomposeCheckAggregation(validatedNodes);

	        // Make filter parameter for each expression node, if it can be optimized
	        foreach (ExprNode constituent in constituents) {
	            FilterSpecParamForge param = FilterSpecCompilerMakeParamUtil.MakeFilterParam(constituent, args.arrayEventTypes, args.statementRawInfo.StatementName);
	    ...
	    private static FilterSpecParamForge MakeRemainingNode(IList<ExprNode> unassignedExpressions, FilterSpecCompilerArgs args)
	            {
	    ...
	    private static IList<FilterSpecParamForge>[] AllocateListArray(int i) {
	        return (IList<FilterSpecParamForge>[]) new IList[i];
	    }
	    ...
	    private static FilterSpecParamForge MakeBooleanExprParam(ExprNode exprNode, FilterSpecCompilerArgs args) {
	        ...
	        ExprFilterSpecLookupableForge lookupable = new ExprFilterSpecLookupableForge(PROPERTY_NAME_BOOLEAN_EXPRESSION, null, exprNode.Forge.EvaluationType, false);
	    ...
	    private static IList<ExprNode> DecomposeCheckAggregation(IList<ExprNode> validatedNodes) {
	        ...
	        foreach (ExprNode validated in validatedNodes) {
	            ...
	            IList<ExprAggregateNode> aggregateExprNodes = new LinkedList<ExprAggregateNode>();
