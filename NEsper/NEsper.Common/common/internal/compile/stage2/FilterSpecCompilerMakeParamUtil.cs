...
namespace com.espertech.esper.common.@internal.compile.stage2
{
    ...
    public class FilterSpecCompilerMakeParamUtil
    {
        ...
        protected internal static FilterSpecParamForge MakeFilterParam(
            ExprNode constituent, LinkedHashMap<string, Pair<EventType, string>> arrayEventTypes, string statementName)
        {
        ...
        private static FilterSpecParamForge HandleAdvancedIndexDescProvider(
            ...
            LinkedHashMap<string, Pair<EventType, string>> arrayEventTypes, string statementName)
        {
            FilterSpecCompilerAdvIndexDesc filterDesc = provider.FilterSpecDesc;
            if (filterDesc == null) {
                return null;
            }

            ExprNode[] keyExpressions = filterDesc.KeyExpressions;
            ...
            ExprNode[] indexExpressions = filterDesc.IndexExpressions;
        ...
        private static FilterSpecParamFilterForEvalDoubleForge ResolveFilterIndexDoubleEval(
            string indexName, ExprNode indexExpression, LinkedHashMap<string, Pair<EventType, string>> arrayEventTypes,
        ...
        private static EventPropertyGetterSPI ResolveFilterIndexRequiredGetter(string indexName, ExprNode keyExpression)
        {
        ...
        {
            if (Boxing.GetBoxedType(constituent.Forge.EvaluationType) != typeof(bool?)) {
                return null;
            }
            ...
            ExprFilterSpecLookupableForge lookupable = constituent.FilterLookupable;
            ...
            ExprNode constituent, LinkedHashMap<string, Pair<EventType, string>> arrayEventTypes, string statementName)
        {
            ...
            if (right.Forge.ForgeConstantType.IsCompileTimeConstant && left is ExprFilterOptimizableNode) {
                ...
                if (filterOptimizableNode.FilterLookupEligible) {
                    ExprFilterSpecLookupableForge lookupable = filterOptimizableNode.FilterLookupable;
                    object constant = right.Forge.ExprEvaluator.Evaluate(null, true, null);
            ...
            if (left.Forge.ForgeConstantType.IsCompileTimeConstant && right is ExprFilterOptimizableNode) {
                ...
                if (filterOptimizableNode.FilterLookupEligible) {
                    ExprFilterSpecLookupableForge lookupable = filterOptimizableNode.FilterLookupable;
                    object constant = left.Forge.ExprEvaluator.Evaluate(null, true, null);
                    constant = HandleConstantsCoercion(lookupable, constant);
            ...
            if (left is ExprFilterOptimizableNode && right is ExprContextPropertyNode) {
                ...
                ExprFilterSpecLookupableForge lookupable = filterOptimizableNode.FilterLookupable;
                if (filterOptimizableNode.FilterLookupEligible) {
                    SimpleNumberCoercer numberCoercer = GetNumberCoercer(
            ...
            if (left is ExprFilterOptimizableNode && right.Forge.ForgeConstantType.IsDeployTimeTimeConstant &&
                ...
                ExprFilterSpecLookupableForge lookupable = filterOptimizableNode.FilterLookupable;
                if (filterOptimizableNode.FilterLookupEligible) {
                    Type returnType = right.Forge.EvaluationType;
                    SimpleNumberCoercer numberCoercer = GetNumberCoercer(lookupable.ReturnType, returnType, lookupable.Expression);
            ...
            if (left.Forge.ForgeConstantType.IsDeployTimeTimeConstant && left is ExprNodeDeployTimeConst &&
                ...
                ExprFilterSpecLookupableForge lookupable = filterOptimizableNode.FilterLookupable;
                if (filterOptimizableNode.FilterLookupEligible) {
                    Type returnType = left.Forge.EvaluationType;
                    op = GetReversedOperator(
                        constituent, op); // reverse operators, as the expression is "stream1.prop xyz stream0.prop"
                    SimpleNumberCoercer numberCoercer = GetNumberCoercer(lookupable.ReturnType, returnType, lookupable.Expression);
                    return new FilterSpecParamDeployTimeConstParamForge(
        ...
        private static FilterSpecParamForge HandleProperty(
            FilterOperator op, ExprIdentNode identNodeLeft, ExprIdentNode identNodeRight,
            LinkedHashMap<string, Pair<EventType, string>> arrayEventTypes, string statementName)
        {
            var propertyName = identNodeLeft.ResolvedPropertyName;

            Type leftType = identNodeLeft.Forge.EvaluationType;
            Type rightType = identNodeRight.Forge.EvaluationType;
        ...
        private static EventType GetArrayInnerEventType(
            LinkedHashMap<string, Pair<EventType, string>> arrayEventTypes, string streamName)
        {
        ...
        private static SimpleNumberCoercer GetNumberCoercer(Type leftType, Type rightType, string expression)
        {
            Type numericCoercionType = Boxing.GetBoxedType(leftType);
        ...
        private static FilterSpecParamForge HandleRangeNode(
            ExprBetweenNode betweenNode, LinkedHashMap<string, Pair<EventType, string>> arrayEventTypes,
            string statementName)
        {
        ...
        private static FilterSpecParamFilterForEvalForge HandleRangeNodeEndpoint(
            ExprNode endpoint, LinkedHashMap<string, Pair<EventType, string>> arrayEventTypes, string statementName)
        {
            // constant
            if (endpoint.Forge.ForgeConstantType.IsCompileTimeConstant) {
                object value = endpoint.Forge.ExprEvaluator.Evaluate(null, true, null);
                if (value == null) {
                    return null;
                }
        ...
        private static FilterSpecParamFilterForEvalDoubleForge GetIdentNodeDoubleEval(
            ExprIdentNode node, LinkedHashMap<string, Pair<EventType, string>> arrayEventTypes, string statementName)
        {
            if (node.StreamId == 0) {
                return null;
            }
            ...
            ExprInNode constituent, LinkedHashMap<string, Pair<EventType, string>> arrayEventTypes)
        {
            ...
            it.Current; // ignore the first node as it's the identifier
            while (it.MoveNext()) {
                ...
                if (subNode.Forge.ForgeConstantType.IsCompileTimeConstant) {
                    ...
                    if (constant != null && constant.GetType().IsArray) {
                        for (var i = 0; i < Array.GetLength(constant); i++) {
