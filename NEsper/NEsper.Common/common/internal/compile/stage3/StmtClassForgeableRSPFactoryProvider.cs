	...
	public class StmtClassForgeableRSPFactoryProvider : StmtClassForgeable {
	    ...
	    public CodegenClass Forge(bool includeDebugSymbols, bool fireAndForget) {
	        ...
	        try {
		        ...
		        throw new EPException("Fatal exception during code-generation for " + debugInformationProvider() + " : " + t.Message, t);
	        }
	    }
		    ...
		    ResultSetProcessorDesc spec)
	    {
	        ...
	        var processViewResultMethod = CodegenMethod
		        .MakeMethod(typeof(UniformPair<EventBean[]>), forge.GetType(), CodegenSymbolProviderEmpty.INSTANCE, classScope)
	        ...
	        var getEnumeratorMethodView = CodegenMethod
		        .MakeMethod(typeof(IEnumerator<EventBean>), forge.GetType(), CodegenSymbolProviderEmpty.INSTANCE, classScope)
	        ...
	        var getEnumeratorMethodJoin = CodegenMethod
		        .MakeMethod(typeof(IEnumerator<EventBean>), forge.GetType(), CodegenSymbolProviderEmpty.INSTANCE, classScope)
		        .AddParam(typeof(ISet<MultiKeyArrayOfKeys<EventBean>>), NAME_JOINSET);
	        if (!spec.IsJoin) {
	        ...
	        var processOutputLimitedJoinMethod = CodegenMethod
		        .MakeMethod(typeof(UniformPair<EventBean[]>), forge.GetType(), CodegenSymbolProviderEmpty.INSTANCE, classScope)
		        .AddParam(typeof(IList<UniformPair<ISet<MultiKeyArrayOfKeys<EventBean>>>>), NAME_JOINEVENTSSET)
		        .AddParam(typeof(bool), NAME_ISSYNTHESIZE);
	        if (!spec.IsJoin || !spec.HasOutputLimit || spec.HasOutputLimitSnapshot) {
	        ...
	        CodegenStackGenerator.RecursiveBuildStack(resultEventTypeGetter, "ResultEventType", innerMethods, innerProperties);
	        CodegenStackGenerator.RecursiveBuildStack(processViewResultMethod, "ProcessViewResult", innerMethods, innerProperties);
	        CodegenStackGenerator.RecursiveBuildStack(processJoinResultMethod, "ProcessJoinResult", innerMethods, innerProperties);
	        CodegenStackGenerator.RecursiveBuildStack(getEnumeratorMethodView, "GetEnumerator", innerMethods, innerProperties);
	        CodegenStackGenerator.RecursiveBuildStack(getEnumeratorMethodJoin, "GetEnumerator", innerMethods, innerProperties);
	        ...
	        CodegenStackGenerator.RecursiveBuildStack(processOutputLimitedLastAllNonBufferedJoinMethod, "ProcessOutputLimitedLastAllNonBufferedJoin", innerMethods, innerProperties);
	        CodegenStackGenerator.RecursiveBuildStack(continueOutputLimitedLastAllNonBufferedViewMethod, "ContinueOutputLimitedLastAllNonBufferedView", innerMethods, innerProperties);
	        CodegenStackGenerator.RecursiveBuildStack(continueOutputLimitedLastAllNonBufferedJoinMethod, "ContinueOutputLimitedLastAllNonBufferedJoin", innerMethods, innerProperties);
	        CodegenStackGenerator.RecursiveBuildStack(acceptHelperVisitorMethod, "AcceptHelperVisitor", innerMethods, innerProperties);
		    ...
		    SelectExprProcessorForge[] forges)
	    {
		    ...
		    outerClassCtor.Block.AssignRef("selectExprProcessorArray", NewArrayByLength(typeof(SelectExprProcessor), Constant(forges.Length)));
		    for (var i = 0; i < forges.Length; i++) {
