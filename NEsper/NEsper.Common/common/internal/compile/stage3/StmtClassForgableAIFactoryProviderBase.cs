...
namespace com.espertech.esper.common.@internal.compile.stage3
{
	public abstract class StmtClassForgableAIFactoryProviderBase : StmtClassForgable {
	    ...
	    public CodegenClass Forge(bool includeDebugSymbols) {
	        ...
	        codegenCtor.Block.AssignRef(MEMBERNAME_STATEMENTAIFACTORY, LocalMethod(CodegenConstructorInit(codegenCtor, classScope), SAIFFInitializeSymbol.REF_STMTINITSVC));

	        CodegenMethod getFactoryMethod = CodegenMethod.MakeParentNode(typeof(StatementAgentInstanceFactory), this.GetType(), CodegenSymbolProviderEmpty.INSTANCE, classScope);
	        getFactoryMethod.Block.MethodReturn(@Ref(MEMBERNAME_STATEMENTAIFACTORY));

	        CodegenMethod assignMethod = CodegenMethod.MakeParentNode(typeof(void), this.GetType(), CodegenSymbolProviderEmpty.INSTANCE, classScope).AddParam(typeof(StatementAIFactoryAssignments), "assignments");
	        if (packageScope.FieldsClassNameOptional != null) {
	            assignMethod.Block.StaticMethod(packageScope.FieldsClassNameOptional, "assign", @Ref("assignments"));
	        }

	        CodegenMethod unassignMethod = CodegenMethod.MakeParentNode(typeof(void), this.GetType(), CodegenSymbolProviderEmpty.INSTANCE, classScope);
	        if (packageScope.FieldsClassNameOptional != null) {
	            unassignMethod.Block.StaticMethod(packageScope.FieldsClassNameOptional, "unassign");
	        }

	        CodegenMethod setValueMethod = CodegenMethod.MakeParentNode(typeof(void), typeof(StmtClassForgableStmtFields), classScope).AddParam(typeof(int), "index").AddParam(typeof(object), "value");
	        ...
	        return new CodegenClass(typeof(StatementAIFactoryProvider), packageScope.PackageName, className, classScope, members, codegenCtor, methods, Collections.GetEmptyList<object>());
	    }
