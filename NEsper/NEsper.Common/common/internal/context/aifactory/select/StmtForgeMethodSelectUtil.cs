...
namespace com.espertech.esper.common.@internal.context.aifactory.select
{
    public class StmtForgeMethodSelectUtil
    {
            ...
            StatementCompileTimeServices services)
        {
            ...
            StreamJoinAnalysisResultCompileTime joinAnalysisResult = VerifyJoinViews(
                statementSpec, services.NamedWindowCompileTimeResolver);
            ...
            for (var stream = 0; stream < numStreams; stream++)
            {
                ...
                var args = new ViewFactoryForgeArgs(
                    stream, false, -1, streamSpec.Options, null, @base.StatementRawInfo, services);
                ...
                else if (streamSpec is FilterStreamSpecCompiled)
                {
                    ...
                    viewableActivatorForges[stream] = new ViewableActivatorFilterForge(
                        filterSpecCompiled, isCanIterateUnbound, stream, false, -1);
                    viewForges[stream] = ViewFactoryForgeUtil.CreateForges(
                        streamSpec.ViewSpecs, args, streamEventTypes[stream]);
                ...
                else if (streamSpec is PatternStreamSpecCompiled)
                {
                    ...
                    MapEventType patternType = ViewableActivatorPatternForge.MakeRegisterPatternType(
                        @base, stream, patternStreamSpec, services);
                    ...
                    viewableActivatorForges[stream] = new ViewableActivatorPatternForge(
                        patternType, patternStreamSpec, patternContext, isCanIterateUnbound);
                ...
                else if (streamSpec is NamedWindowConsumerStreamSpec)
                {
                    ...
                    var typesFilterValidation = new StreamTypeServiceImpl(
                        namedWindowType, namedSpec.OptionalStreamName, false);
                    var filterSingle =
                    ...
                    QueryGraphForge filterQueryGraph = EPLValidationUtil.ValidateFilterGetQueryGraphSafe(
                        filterSingle, typesFilterValidation, @base.StatementRawInfo, services);

                    namedWindowConsumers.Add(namedSpec);
                    viewableActivatorForges[stream] = new ViewableActivatorNamedWindowForge(
                        namedSpec, namedWindow, filterSingle, filterQueryGraph, true, namedSpec.OptPropertyEvaluator);
                ...
                else if (streamSpec is TableQueryStreamSpec)
                {
                    ...
                    EPLValidationUtil.ValidateContextName(
                        true, table.TableName, table.OptionalContextName, statementSpec.Raw.OptionalContextName, false);
                    var filter =
                ...
                else if (streamSpec is DBStatementStreamSpec)
                {
                    ...
                    var viewable = HistoricalEventViewableDatabaseForgeFactory.CreateDBStatementView(
                        stream, sqlStreamSpec, typeConversionHook, outputRowConversionHook, @base, services);
                ...
                else if (streamSpec is MethodStreamSpec)
                {
                    ...
                    var viewable = HistoricalEventViewableMethodForgeFactory.CreateMethodStatementView(
                        stream, methodStreamSpec, @base, services);
                    historicalEventViewables[stream] = viewable;
                    streamEventTypes[stream] = viewable.EventType;
                    viewForges[stream] = Collections.GetEmptyList<ViewFactoryForge>();
                    viewableActivatorForges[stream] = new ViewableActivatorHistoricalForge(viewable);
            ...
            if (statementSpec.Raw.MatchRecognizeSpec != null)
            {
                ...
                var desc = RowRecogNFAViewPlanUtil.ValidateAndPlan(eventType, isUnbound, @base, services);
                var forge = new RowRecogNFAViewFactoryForge(desc);
            ...
            bool[] hasIStreamOnly = GetHasIStreamOnly(isNamedWindow, viewForges);
            bool optionalStreamsIfAny = OuterJoinAnalyzer.OptionalStreamsIfAny(statementSpec.Raw.OuterJoinDescList);
            StreamTypeService typeService = new StreamTypeServiceImpl(
                streamEventTypes, streamNames, hasIStreamOnly, false, optionalStreamsIfAny);
            ...
            ExprNode whereClauseValidated = EPStatementStartMethodHelperValidate.ValidateNodes(
                statementSpec.Raw, typeService, viewResourceDelegateExpr, @base.StatementRawInfo, services);
            ...
            ResultSetProcessorDesc resultSetProcessorDesc = ResultSetProcessorFactoryFactory.GetProcessorPrototype(
                new ResultSetSpec(statementSpec), typeService, viewResourceDelegateExpr,
                joinAnalysisResult.UnidirectionalInd, true, @base.ContextPropertyRegistry, false, false,
            ...
            OutputProcessViewFactoryForge outputProcessViewFactoryForge = OutputProcessViewForgeFactory.Make(
                typeService.EventTypes, resultSetProcessorDesc.ResultEventType,
                resultSetProcessorDesc.ResultSetProcessorType, statementSpec, @base.StatementRawInfo, services);
            ...
            if (numStreams > 1)
            {
                ...
                joinForge = JoinSetComposerPrototypeForgeFactory.MakeComposerPrototype(
                    statementSpec, joinAnalysisResult, typeService, historicalViewableDesc, false, hasAggregations,
                    @base.StatementRawInfo, services);
            ...
            var outputProcessViewProviderClassName = CodeGenerationIDGenerator.GenerateClassNameSimple(
                typeof(OutputProcessViewFactoryProvider), classPostfix);
            ...
            var forgableResult = new StmtForgeMethodResult(
                forgables, filterSpecCompileds, scheduleHandleCallbackProviders, namedWindowConsumers,
                FilterSpecCompiled.MakeExprNodeList(filterSpecCompileds, Collections.GetEmptyList<FilterSpecParamExprNodeForge>()));
        ...
        private static DataFlowActivationResult HandleDataflowActivation(
            ViewFactoryForgeArgs args, StreamSpecCompiled streamSpec)
        {
        ...
        private static void ValidateNoViews(StreamSpecCompiled streamSpec, string conceptName)
        {
            ...
            {
                throw new ExprValidationException(
                    conceptName + " joins do not allow views onto the data, view '"
                                + streamSpec.ViewSpecs[0].ObjectName + "' is not valid in this context");
            }
        }

        private static void ValidateTableAccessUse(IntoTableSpec intoTableSpec, ISet<ExprTableAccessNode> tableNodes)
        {
            if (intoTableSpec != null && tableNodes != null && tableNodes.Count > 0)
            {
                foreach (var node in tableNodes)
                {
                    ...
                    {
                        throw new ExprValidationException(
                            "Invalid use of table '" + intoTableSpec.Name +
                    ...
                    if (index.TableName != null)
                    {
                        ...
                        if (tableMeta.TableVisibility == NameAccessModifier.PUBLIC)
                        {
                            services.ModuleDependenciesCompileTime.AddPathIndex(
                                false, index.TableName, tableMeta.TableModuleName, index.IndexName,
        ...
        private class DataFlowActivationResult
        {
            public DataFlowActivationResult(
                EventType streamEventType, string eventTypeName, ViewableActivatorForge viewableActivatorForge,
