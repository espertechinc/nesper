	...
	public class ContextControllerConditionTimePeriod : ContextControllerConditionNonHA {
	    ...
	    public bool Activate(EventBean optionalTriggeringEvent, ContextControllerEndConditionMatchEventProvider endConditionMatchEventProvider) {
	        ScheduleHandleCallback scheduleCallback = new ProxyScheduleHandleCallback() {
	            ProcScheduledTrigger = () =>  {
	                ...
	                agentInstanceContext.InstrumentationProvider.QContextScheduledEval(agentInstanceContext.StatementContext.ContextRuntimeDescriptor);
	        ...
	        agentInstanceContext.AuditProvider.ScheduleAdd(timeDelta, agentInstanceContext, scheduleHandle, ScheduleObjectType.context, NAME_AUDITPROVIDER_SCHEDULE);
	    ...
	    public void Deactivate() {
	        if (scheduleHandle != null) {
	            AgentInstanceContext agentInstanceContext = controller.Realization.AgentInstanceContextCreate;
	            agentInstanceContext.AuditProvider.ScheduleRemove(agentInstanceContext, scheduleHandle, ScheduleObjectType.context, NAME_AUDITPROVIDER_SCHEDULE);
