...
namespace com.espertech.esper.common.@internal.serde.serdeset.multikey
{
	public class DIOMultiKeyArrayObjectSerde : DataInputOutputSerde<MultiKeyArrayObject> {
	    ...
	    public void Write(MultiKeyArrayObject mk, DataOutput output, byte[] unitKey, EventBeanCollatedWriter writer) {
	        WriteInternal(mk.Keys, output);
	    }

	    public MultiKeyArrayObject Read(DataInput input, byte[] unitKey) {
	        return new MultiKeyArrayObject(ReadInternal(input));
	    }

	    private void WriteInternal(object[] @object, DataOutput output) {
	        if (@object == null) {
	            output.WriteInt(-1);
	            return;
	        }
