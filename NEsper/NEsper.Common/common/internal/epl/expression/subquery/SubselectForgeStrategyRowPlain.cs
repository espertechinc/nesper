...
namespace com.espertech.esper.common.@internal.epl.expression.subquery
{
    public class SubselectForgeStrategyRowPlain : SubselectForgeStrategyRowBase
    {
                ...
                if (subselect.SelectClause == null) {
                    method.Block.MethodReturn(
                        Cast(
                            ...
                            StaticMethod(
                                typeof(EventBeanUtility), "getNonemptyFirstEventUnderlying",
                    ...
                    method.Block.ApplyTri(DECLARE_EVENTS_SHIFTED, method, symbols)
                        .AssignArrayElement(
                            ...
                            StaticMethod(
                                typeof(EventBeanUtility), "getNonemptyFirstEvent",
                                symbols.GetAddMatchingEvents(method)));
                }
            }
            else {
            ...
            CodegenClassScope classScope)
        {
                        ...
                        typeof(EventBean), "event", symbols.GetAddMatchingEvents(methodX));
                    {
                        foreachX.AssignArrayElement(REF_EVENTS_SHIFTED, Constant(0), @Ref("event"))
                            .DeclareVar(
                                ...
                                LocalMethod(
                                    subselect.EvaluateRowCodegen(methodX, classScope), REF_EVENTS_SHIFTED,
                                    ConstantTrue(), symbols.GetAddExprEvalCtx(methodX)))
                            .DeclareVar(
                                typeof(EventBean), "rowEvent",
            ...
            methodY.Block.DeclareVar(typeof(ArrayDeque<object>), "filtered", ConstantNull());
            CodegenBlock foreachY = methodY.Block.ForEach(
                typeof(EventBean), "event", symbols.GetAddMatchingEvents(methodY));
            {
                foreachY.AssignArrayElement(REF_EVENTS_SHIFTED, Constant(0), @Ref("event"));
                CodegenMethod filter = CodegenLegoMethodExpression.CodegenExpression(
                    subselect.FilterExpr, methodY, classScope);
                CodegenLegoBooleanExpression.CodegenContinueIfNullOrNotPass(
                    foreachY, typeof(bool?),
                    LocalMethod(
                        filter, REF_EVENTS_SHIFTED, symbols.GetAddIsNewData(methodY),
            ...
            CodegenClassScope classScope)
        {
            if (subselect.FilterExpr == null) {
                if (subselect.SelectClause == null) {
                    ...
                    CodegenMethod method = parent.MakeChild(typeof(ICollection<object>), this.GetType(), classScope);
                    method.Block
                        .DeclareVar(typeof(IList<object>), "result", NewInstance(typeof(List<object>)))
                    ...
                    {
                        @foreach.AssignArrayElement(REF_EVENTS_SHIFTED, Constant(0), @Ref("event"))
                            .DeclareVar(typeof(object), "value", selectClause)
                ...
                typeof(EventBean), "event", symbols.GetAddMatchingEvents(methodX));
            {
                foreachX.AssignArrayElement(REF_EVENTS_SHIFTED, Constant(0), @Ref("event"));
                CodegenLegoBooleanExpression.CodegenContinueIfNullOrNotPass(
                    foreachX, typeof(bool?),
                    LocalMethod(
                        filter, REF_EVENTS_SHIFTED, symbols.GetAddIsNewData(methodX),
                        symbols.GetAddExprEvalCtx(methodX)));
                foreachX.DeclareVar(typeof(object), "value", selectClauseX)
                    .ExprDotMethod(@Ref("result"), "add", @Ref("value"));
            }
            methodX.Block.MethodReturn(@Ref("result"));
            return LocalMethod(methodX);
            ...
            CodegenClassScope classScope)
        {
            ...
            CodegenExpression filter = ExprNodeUtilityCodegen.CodegenEvaluator(
                subselect.FilterExpr, method, this.GetType(), classScope);
            method.Block
                .ApplyTri(DECLARE_EVENTS_SHIFTED, method, symbols)
                .DeclareVar(
