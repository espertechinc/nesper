...
namespace com.espertech.esper.common.@internal.epl.expression.dot.core
{
	public class ExprDotNodeUtility {
	    ...
	    public static ObjectArrayEventType MakeTransientOAType(string enumMethod, string propertyName, Type type, StatementRawInfo statementRawInfo, StatementCompileTimeServices services) {
	        ...
	        EventTypeMetadata metadata = new EventTypeMetadata(eventTypeName, statementRawInfo.ModuleName, EventTypeTypeClass.ENUMDERIVED, EventTypeApplicationType.OBJECTARR, NameAccessModifier.TRANSIENT, EventTypeBusModifier.NONBUS, false, EventTypeIdPair.Unassigned());
	        ObjectArrayEventType oatype = BaseNestableEventUtil.MakeOATypeCompileTime(metadata, propsResult, null, null, null, null, services.BeanEventTypeFactoryPrivate, services.EventTypeCompileTimeResolver);
	    ...
	    public static ExprDotEnumerationSourceForge GetEnumerationSource(ExprNode inputExpression, StreamTypeService streamTypeService, bool hasEnumerationMethod, bool disablePropertyExpressionEventCollCache, StatementRawInfo statementRawInfo, StatementCompileTimeServices compileTimeServices) {
	    ...
	    public static ExprDotEnumerationSourceForgeForProps GetPropertyEnumerationSource(string propertyName, int streamId, EventType streamType, bool allowEnumType, bool disablePropertyExpressionEventCollCache) {
	        ...
	        if (getter != null && fragmentEventType != null) {
	            if (fragmentEventType.IsIndexed) {
	                enumEvaluator = new PropertyDotEventCollectionForge(propertyName, streamId, fragmentEventType.FragmentType, getter, disablePropertyExpressionEventCollCache);
	    ...
	    public static EventType[] GetSingleLambdaParamEventType(string enumMethodUsedName, IList<string> goesToNames, EventType inputEventType, Type collectionComponentType, StatementRawInfo statementRawInfo, StatementCompileTimeServices services) {
	    ...
	    public static object EvaluateChain(ExprDotForge[] forges, ExprDotEval[] evaluators, object inner, EventBean[] eventsPerStream, bool isNewData, ExprEvaluatorContext context) {
	    ...
	    public static ExprDotNodeRealizedChain GetChainEvaluators(
	        ...
	        while (!chainSpecStack.IsEmpty()) {
	            ...
	            if (DatetimeMethodEnum.IsDateTimeMethod(chainElement.Name)
	                ...
	                ExprDotDTMethodDesc datetimeImpl = ExprDotDTFactory.ValidateMake(validationContext.StreamTypeService, chainSpecStack, datetimeMethod, chainElement.Name, currentInputType, chainElement.Parameters, inputDesc, validationContext.ImportService.TimeAbacus, null, validationContext.TableCompileTimeResolver);
	            ...
	            if (methodTarget != null) {
	                try {
	                    ...
	                    ExprNodeUtilMethodDesc desc = GetValidateMethodDescriptor(methodTarget, chainElement.Name, chainElement.Parameters, validationContext);
	                    ...
	                    if (currentInputType is ClassEPType) {
	                        ...
	                        if (desc.ReflectionMethod.ReturnType.IsArray && !chainSpecStack.IsEmpty() && EnumMethodEnum.IsEnumerationMethod(chainSpecStack.First.Name)) {
	                            forge = new ExprDotMethodForgeNoDuck(validationContext.StatementName, desc.ReflectionMethod, paramForges, ExprDotMethodForgeNoDuck.Type.WRAPARRAY);
	                        } else {
	                            forge = new ExprDotMethodForgeNoDuck(validationContext.StatementName, desc.ReflectionMethod, paramForges, ExprDotMethodForgeNoDuck.Type.PLAIN);
	                        }
	                    } else {
	                        forge = new ExprDotMethodForgeNoDuck(validationContext.StatementName, desc.ReflectionMethod, paramForges, ExprDotMethodForgeNoDuck.Type.UNDERLYING);
	                    }
	                    ...
	                    if (!isDuckTyping) {
	                        ...
	                        ExprDotMethodForgeDuck duck = new ExprDotMethodForgeDuck(validationContext.StatementName, validationContext.ImportService, chainElement.Name, paramTypes, paramForges);
	    ...
	    public static CodegenExpression EvaluateChainCodegen(CodegenMethod parent, ExprForgeCodegenSymbol exprSymbol, CodegenClassScope codegenClassScope, CodegenExpression inner, Type innerType, ExprDotForge[] forges, ExprDotStaticMethodWrap optionalResultWrapLambda) {
	        if (forges.Length == 0) {
	            return inner;
	        }
	        ...
	        for (int i = 0; i < forges.Length; i++) {
	            ...
	            block.Apply(Instblock(codegenClassScope, "qExprDotChainElement", Constant(i), Constant(instrumentationName.MethodType), Constant(instrumentationName.MethodName)));
	    ...
	    private static ExprNodeUtilMethodDesc GetValidateMethodDescriptor(
	        Type methodTarget, string methodName, IList<ExprNode> parameters, ExprValidationContext validationContext)
	    {
