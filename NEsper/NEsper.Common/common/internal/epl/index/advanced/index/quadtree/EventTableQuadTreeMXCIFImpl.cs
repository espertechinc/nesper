...
namespace com.espertech.esper.common.@internal.epl.index.advanced.index.quadtree
{
    public class EventTableQuadTreeMXCIFImpl : EventTableQuadTree
    {
        ...
        public void Add(EventBean @event, ExprEvaluatorContext exprEvaluatorContext)
        {
            ...
            if (!added)
            {
                throw InvalidColumnValue(organization.IndexName, "(x,y,width,height)", "(" + x + "," + y + "," + width + "," + height + ")", "a value intersecting index bounding box (range-end-inclusive) " + quadTree.Root.Bb);
            }
        }
