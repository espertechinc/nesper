...
namespace com.espertech.esper.common.@internal.epl.index.inkeyword
{
	public class PropertyHashedArrayFactoryFactoryForge : EventTableFactoryFactoryForge {
	    ...
	    public PropertyHashedArrayFactoryFactoryForge(int streamNum, EventType eventType, string[] propertyNames, bool unique, bool isFireAndForget) {
	    ...
	    public CodegenExpression Make(CodegenMethodScope parent, SAIFFInitializeSymbol symbols, CodegenClassScope classScope) {
	        ...
	        method.Block.DeclareVar(typeof(EventPropertyValueGetter[]), "getters", NewArrayByLength(typeof(EventPropertyValueGetter), Constant(propertyNames.Length)));
	        for (int i = 0; i < propertyNames.Length; i++) {
	            ...
	            CodegenExpression getter = EventTypeUtility.CodegenGetterWCoerce(getterSPI, propertyType, propertyType, method, this.GetType(), classScope);
	            method.Block.AssignArrayElement(@Ref("getters"), Constant(i), getter);
	        }

	        method.Block.MethodReturn(NewInstance(typeof(PropertyHashedArrayFactoryFactory),
