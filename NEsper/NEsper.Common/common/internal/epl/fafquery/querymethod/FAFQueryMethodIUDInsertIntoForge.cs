...
namespace com.espertech.esper.common.@internal.epl.fafquery.querymethod
{
	...
	public class FAFQueryMethodIUDInsertIntoForge : FAFQueryMethodIUDBaseForge {
	    ...
	    public FAFQueryMethodIUDInsertIntoForge(StatementSpecCompiled specCompiled, Compilable compilable, StatementRawInfo statementRawInfo, StatementCompileTimeServices services)
	    ...
	    protected override void InitExec(string aliasName, StatementSpecCompiled spec, StatementRawInfo statementRawInfo, StatementCompileTimeServices services) {
	        ...
	        IList<SelectClauseElementCompiled> selectNoWildcard = InfraOnMergeHelperForge.CompileSelectNoWildcard(UuidGenerator.Generate(), Arrays.AsList(spec.SelectClauseCompiled.SelectExprList));
	        ...
	        if (spec.Raw.InsertIntoDesc.ColumnNames.IsEmpty()) {
	            FireAndForgetSpecInsert insert = (FireAndForgetSpecInsert) spec.Raw.FireAndForgetSpec;
	        ...
	        int count = -1;
	        foreach (SelectClauseElementCompiled compiled in spec.SelectClauseCompiled.SelectExprList) {
	            count++;
	            if (compiled is SelectClauseExprCompiledSpec) {
	                SelectClauseExprCompiledSpec expr = (SelectClauseExprCompiledSpec) compiled;
	                ExprNode validatedExpression = ExprNodeUtilityValidate.GetValidatedSubtree(ExprNodeOrigin.SELECT, expr.SelectExpression, validationContext);
	        ...
	        SelectProcessorArgs args = new SelectProcessorArgs(selectNoWildcard.ToArray(), null,
	    ...
	    private static StatementSpecCompiled AssociatedFromClause(StatementSpecCompiled statementSpec, StatementCompileTimeServices services) {
	        StatementSpecRaw raw = statementSpec.Raw;
	        ...
	        string infraName = statementSpec.Raw.InsertIntoDesc.EventTypeName;
	        ...
	        if (namedWindow != null) {
	            stream = new NamedWindowConsumerStreamSpec(namedWindow, null, new ViewSpec[0], Collections.GetEmptyList<ExprNode>(), StreamSpecOptions.DEFAULT, null);
	        } else {
