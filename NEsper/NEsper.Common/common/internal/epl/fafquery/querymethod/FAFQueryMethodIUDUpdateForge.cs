...
namespace com.espertech.esper.common.@internal.epl.fafquery.querymethod
{
	...
	public class FAFQueryMethodIUDUpdateForge : FAFQueryMethodIUDBaseForge {
	    ...
	    public FAFQueryMethodIUDUpdateForge(StatementSpecCompiled spec, Compilable compilable, StatementRawInfo statementRawInfo, StatementCompileTimeServices services)
	    ...
	    protected override void InitExec(string aliasName, StatementSpecCompiled spec, StatementRawInfo statementRawInfo, StatementCompileTimeServices services) {
	        StreamTypeServiceImpl assignmentTypeService = new StreamTypeServiceImpl(
	                new EventType[]{processor.EventTypeRSPInputEvents, null, processor.EventTypeRSPInputEvents},
	                new string[]{aliasName, "", INITIAL_VALUE_STREAM_NAME},
	                new bool[]{true, true, true}, true, false);
	        assignmentTypeService.StreamZeroUnambigous = true;
	        ...
	        try {
	            foreach (OnTriggerSetAssignment assignment in updateSpec.Assignments) {
	                ExprNode validated = ExprNodeUtilityValidate.GetValidatedSubtree(ExprNodeOrigin.UPDATEASSIGN, assignment.Expression, validationContext);
	        ...
	        try {
	            ...
	            updateHelper = EventBeanUpdateHelperForgeFactory.Make(processor.NamedWindowOrTableName,
