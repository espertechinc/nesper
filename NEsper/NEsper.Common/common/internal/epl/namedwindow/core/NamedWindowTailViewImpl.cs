...
namespace com.espertech.esper.common.@internal.epl.namedwindow.core
{
	...
	public class NamedWindowTailViewImpl : NamedWindowTailViewBase {
	    ...
	    public NamedWindowTailViewImpl(EventType eventType, bool isParentBatchWindow, EPStatementInitServices services) : base(eventType, isParentBatchWindow, services)
	        {
	    ...
	    public override NamedWindowConsumerView AddConsumerNoContext(NamedWindowConsumerDesc consumerDesc) {
	        NamedWindowConsumerCallback consumerCallback = new ProxyNamedWindowConsumerCallback() {
	            ...
	            ProcSnapshot = (queryGraph, annotations) => Collections.GetEmptyList<EventBean>(),
	        ...
	        NamedWindowConsumerView consumerView = new NamedWindowConsumerView(consumerDesc.NamedWindowConsumerId, consumerDesc.FilterEvaluator, consumerDesc.OptPropertyEvaluator, eventType, consumerCallback, consumerDesc.AgentInstanceContext, audit);

	        // Keep a list of consumer views per statement to accomodate joins and subqueries
	        IList<NamedWindowConsumerView> viewsPerStatements = consumersNonContext.Get(consumerDesc.AgentInstanceContext.EpStatementAgentInstanceHandle);
	    ...
	    public override void AddDispatches(NamedWindowConsumerLatchFactory latchFactory, IDictionary<EPStatementAgentInstanceHandle, IList<NamedWindowConsumerView>> consumersInContext, NamedWindowDeltaData delta, AgentInstanceContext agentInstanceContext) {
	    ...
	    public override NamedWindowConsumerLatchFactory MakeLatchFactory() {
	        return new NamedWindowConsumerLatchFactory(eventType.Name,
