...
namespace com.espertech.esper.common.@internal.epl.namedwindow.core
{
	public class NamedWindowImpl : NamedWindowWDirectConsume {
	    ...
	    public NamedWindowImpl(NamedWindowMetaData metadata, EPStatementInitServices services) {
	        ...
	        tailView = services.NamedWindowFactoryService.CreateNamedWindowTailView(metadata.EventType, metadata.IsChildBatching, services, metadata.ContextName);
	    }
	    ...
	    public NamedWindowConsumerView AddConsumer(NamedWindowConsumerDesc consumerDesc, bool isSubselect) {
	        ...
	        if (rootView.ContextName != null) {
	            ...
	            if (contextDescriptor != null && rootView.ContextName.Equals(contextDescriptor.ContextName)) {
	                StatementResourceHolder holder = statementContext.StatementResourceService.GetPartitioned(consumerDesc.AgentInstanceContext.AgentInstanceId);
	    ...
	    public void ValidateAddIndex(string deloymentId, string statementName, string explicitIndexName, string explicitIndexModuleName, QueryPlanIndexItem explicitIndexDesc, IndexMultiKey imk) {
