...
namespace com.espertech.esper.common.@internal.epl.annotation
{
	...
	public class AnnotationUtil {
	    ...
	    public static Attribute[] CompileAnnotations(IList<AnnotationDesc> annotationSpec, ImportServiceCompileTime importService, Compilable compilable) {
	        Attribute[] annotations;
	        try {
	            annotations = AnnotationUtil.CompileAnnotations(annotationSpec, importService);
	    ...
	    public static CodegenMethod MakeAnnotations(Type arrayType, Attribute[] annotations, CodegenMethod parent, CodegenClassScope classScope) {
	    ...
	    private static Attribute[] CompileAnnotations(IList<AnnotationDesc> desc, ImportServiceCompileTime importService)
	            {
	    ...
	    private static Attribute CreateProxy(AnnotationDesc desc, ImportServiceCompileTime importService)
	    {
	        // resolve class
	        ...
	        if (requiredAttributes.Count > 0) {
	            ...
	            required.Sort();
	            throw new AnnotationException("Annotation '" + annotationClass.GetSimpleName() + "' requires a value for attribute '" + required[0] + "'");
	        }
	        ...
	        if (providedValues.Count > 0) {
	            ...
	            if (allAttributes.Contains(provided[0])) {
	                throw new AnnotationException("Annotation '" + annotationClass.GetSimpleName() + "' has duplicate attribute values for attribute '" + provided[0] + "'");
	            } else {
	                throw new AnnotationException("Annotation '" + annotationClass.GetSimpleName() + "' does not have an attribute '" + provided[0] + "'");
	            }
	        }
	    ...
	    private static object GetFinalValue(Type annotationClass, AnnotationAttribute annotationAttribute, object value, ImportServiceCompileTime importService) {
	        if (value == null) {
	            if (annotationAttribute.DefaultValue == null) {
	                throw new AnnotationException("Annotation '" + annotationClass.GetSimpleName() + "' requires a value for attribute '" + annotationAttribute.Name + "'");
	            }
	            ...
	            if (!annotationAttribute.AnnotationType.IsAttribute()) {
	                ...
	                if (finalValue == null) {
	                    throw new AnnotationException("Annotation '" + annotationClass.GetSimpleName() + "' requires a " +
	                            annotationAttribute.AnnotationType.GetSimpleName() + "-typed value for attribute '" + annotationAttribute.Name + "' but received " +
	    ...
	    private static IList<AnnotationAttribute> GetAttributes(Type annotationClass) {
	        ...
	        for (var i = 0; i < methods.Length; i++) {
	            ...
	            props.Add(new AnnotationAttribute(
	    ...
	    public static IList<Attribute> FindAnnotations(Attribute[] annotations, Type annotationClass) {
	        if (!annotationClass.IsAttribute()) {
	            throw new ArgumentException("Class " + annotationClass.Name + " is not an annotation class");
	        }
	        if (annotations == null || annotations.Length == 0) {
	    ...
	    private static AnnotationException MakeArrayMismatchException(Type annotationClass, Type componentType, string attributeName, Type unexpected) {
	    ...
	    private static CodegenExpression MakeAnnotation(Attribute annotation, CodegenMethod parent, CodegenClassScope codegenClassScope) {
	        if (annotation == null) {
	            return ConstantNull();
	        } else if (annotation is NameAttribute name) {
	            return NewInstance(typeof(AnnotationNameAttribute), 
	                Constant(name.Value));
	        } else if (annotation is PriorityAttribute priority) {
	            return NewInstance(typeof(AnnotationPriorityAttribute), 
	                Constant(priority.Value));
	        } else if (annotation is TagAttribute tag) {
	            return NewInstance(typeof(AnnotationTag), 
	                Constant(tag.Name), 
	                Constant(tag.Value));
	        } else if (annotation is DropAttribute) {
	            return NewInstance(typeof(AnnotationDropAttribute));
	        } else if (annotation is DescriptionAttribute description) {
	            return NewInstance(typeof(AnnotationDescription), 
	                Constant(description.Value));
	        } else if (annotation is HintAttribute hint) {
	            return NewInstance(typeof(AnnotationHintAttribute), 
	                Constant(hint.Value), 
	                Constant(hint.Applies), 
	                Constant(hint.Model));
	        } else if (annotation is NoLockAttribute) {
	            return NewInstance(typeof(AnnotationNoLock));
	        } else if (annotation is AuditAttribute audit) {
	            return NewInstance(typeof(AnnotationAudit), Constant(audit.Value));
	        } else if (annotation is EventRepresentationAttribute anno) {
	            return NewInstance(typeof(AnnotationEventRepresentation), 
	                EnumValue(
	                    anno.Value.GetType(), 
	                    anno.Value.GetName()));
	        } else if (annotation is IterableUnboundAttribute) {
	            return NewInstance(typeof(AnnotationIterableUnbound));
	        } else if (annotation is HookAttribute hook) {
	            return NewInstance(typeof(AnnotationHookAttribute), 
	                EnumValue(typeof(HookType), hook.HookType.GetName()),
	                Constant(hook.Hook));
	        } else if (annotation is AvroSchemaFieldAttribute field) {
	            return NewInstance(typeof(AvroSchemaFieldHook), 
	                Constant(field.Name), 
	                Constant(field.Schema));
	        } else if (annotation is PrivateAttribute) {
	            return NewInstance(typeof(AnnotationPrivate));
	        } else if (annotation is ProtectedAttribute) {
	            return NewInstance(typeof(AnnotationProtected));
	        } else if (annotation is PublicAttribute) {
	            return NewInstance(typeof(AnnotationPublic));
	        } else if (annotation is BusEventTypeAttribute) {
	            return NewInstance(typeof(AnnotationBusEventType));
	        } else {
	            if (annotation.GetType().Namespace == RootNamespace) {
	                throw new IllegalStateException("Unrecognized annotation residing in the '" + RootNamespace + " namespace having type" + annotation.GetType().Name);
	            } else {
