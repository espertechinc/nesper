...
namespace com.espertech.esper.common.@internal.epl.dataflow.core
{
	public class EPDataFlowServiceImpl : EPDataFlowService
	{
	    ...
	    public EPDataFlowInstance Instantiate(string deploymentId, string dataflowName) {
	        lock (this) {
	            return Instantiate(deploymentId, dataflowName, new EPDataFlowInstantiationOptions());
	        }
	    }

	    public EPDataFlowInstance Instantiate(string deploymentId, string dataFlowName, EPDataFlowInstantiationOptions options) {
	        lock (this) {
	            DataflowDesc entry = GetEntryMayNull(deploymentId, dataFlowName);
	            if (entry == null) {
	    ...
	    public void AddDataflow(string deploymentId, DataflowDesc dataflow) {
	        lock (this) {
