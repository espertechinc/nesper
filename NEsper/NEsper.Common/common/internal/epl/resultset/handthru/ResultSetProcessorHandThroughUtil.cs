...
namespace com.espertech.esper.common.@internal.epl.resultset.handthru
{
	public class ResultSetProcessorHandThroughUtil {
	    ...
	    public static EventBean[] GetSelectEventsNoHavingHandThruView(SelectExprProcessor exprProcessor, EventBean[] events, bool isNewData, bool isSynthesize, ExprEvaluatorContext agentInstanceContext) {
	        if (events == null) {
	            return null;
	        }
	    ...
	    /// <param name="agentInstanceContext">context</param>
	    /// <returns>output events, one for each input event</returns>
	    public static EventBean[] GetSelectEventsNoHavingHandThruJoin(SelectExprProcessor exprProcessor, ISet<MultiKey<EventBean>> events, bool isNewData, bool isSynthesize, ExprEvaluatorContext agentInstanceContext) {
