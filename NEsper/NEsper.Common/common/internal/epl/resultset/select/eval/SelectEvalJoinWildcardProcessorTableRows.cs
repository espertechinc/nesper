...
namespace com.espertech.esper.common.@internal.epl.resultset.select.eval
{
	...
	public class SelectEvalJoinWildcardProcessorTableRows : SelectExprProcessorForge {
	    ...
	    public CodegenMethod ProcessCodegen(CodegenExpression resultEventType, CodegenExpression eventBeanFactory, CodegenMethodScope codegenMethodScope, SelectExprProcessorCodegenSymbol selectSymbol, ExprForgeCodegenSymbol exprSymbol, CodegenClassScope codegenClassScope) {
	        ...
	        methodNode.Block.DeclareVar(typeof(EventBean[]), "eventsPerStreamWTableRows", NewArrayByLength(typeof(EventBean), Constant(types.Length)));
	        for (int i = 0; i < types.Length; i++) {
	            if (tables[i] == null) {
	                methodNode.Block.AssignArrayElement("eventsPerStreamWTableRows", Constant(i), ArrayAtIndex(refEPS, Constant(i)));
	            } else {
	                CodegenExpressionField eventToPublic = TableDeployTimeResolver.MakeTableEventToPublicField(tables[i], codegenClassScope, this.GetType());
	                string refname = "e" + i;
