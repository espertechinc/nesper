...
namespace com.espertech.esper.common.@internal.epl.agg.access.sorted
{
	...
	{
		private readonly AggregationStateSortedForge forge;
		private readonly CodegenExpressionMember sorted;
		private readonly CodegenExpressionInstanceField sortedSerde;
		private readonly CodegenExpressionMember size;
		private readonly CodegenExpressionInstanceField comparator;
		private readonly CodegenExpressionMember joinRefs;
		private readonly CodegenExpressionInstanceField joinRefsSerde;
		...
		{
			this.forge = forge;
			sorted = membersColumnized.AddMember(col, typeof(SortedDictionary<object, object>), "sorted");
			...
			ctor.Block.AssignRef(sorted, NewInstance(typeof(SortedDictionary<object, object>), comparator));
			...
			CodegenNamedMethods namedMethods)
		{
			...
			if (joinRefs == null) {
				...
				method.Block
					.IfCondition(ExprDotMethod(joinRefs, "Add", Ref("theEvent")))
					.InstanceMethod(referenceAddToColl, Ref("theEvent"), eps, ctx);
			}
		}
		...
		{
			CodegenMethod method = parent.MakeChildWithScope(typeof(EventBean), this.GetType(), CodegenSymbolProviderEmpty.INSTANCE, classScope);
			method.Block
		...
		{
			return NewInstance(typeof(AggregationStateSortedEnumerator), sorted, ConstantTrue());
		}
			...
			CodegenClassScope classScope)
		{
			...
			if (joinRefs != null) {
				method.Block.ExprDotMethod(joinRefsSerde, "Write", RowDotMember(row, joinRefs), output, unitKey, writer);
			}
		}
		...
		private static CodegenMethod GetComparableWMultiKeyCodegen(
			...
			CodegenClassScope classScope)
		{
			string methodName = "GetComparable_" + member.Ref;
		...
		private CodegenMethod ReferenceAddToCollCodegen(
			...
			CodegenClassScope classScope)
		{
			CodegenMethod getComparable = GetComparableWMultiKeyCodegen(forge.Spec.Criteria, sorted, namedMethods, classScope);

			CodegenMethod method = parent.MakeChildWithScope(typeof(void),  this.GetType(), CodegenSymbolProviderEmpty.INSTANCE, classScope)
