...
namespace com.espertech.esper.common.@internal.epl.agg.access.linear
{
	...
	public class AggregationAccessorWindowWEval {
	    public static void GetValueCodegen(AggregationAccessorWindowWEvalForge forge, AggregationStateLinearForge accessStateFactory, AggregationAccessorForgeGetCodegenContext context) {
	        CodegenExpression size = accessStateFactory.AggregatorLinear.SizeCodegen();
	        CodegenExpression iterator = accessStateFactory.AggregatorLinear.IteratorCodegen(context.ClassScope, context.Method, context.NamedMethods);
	        ...
	        context.Method.Block.IfCondition(EqualsIdentity(size, Constant(0))).BlockReturn(ConstantNull())
	                .DeclareVar(TypeHelper.GetArrayType(forge.GetElementType()), "array", NewArrayByLength(forge.GetElementType(), size))
	    ...
	    public static void GetEnumerableScalarCodegen(AggregationAccessorWindowWEvalForge forge, AggregationStateLinearForge stateForge, AggregationAccessorForgeGetCodegenContext context) {
	        context.Method.Block.DeclareVar(typeof(int), "size", stateForge.AggregatorLinear.SizeCodegen())
	                ...
	                .DeclareVar(typeof(IEnumerator<EventBean>), "it", stateForge.AggregatorLinear.IteratorCodegen(context.ClassScope, context.Method, context.NamedMethods))
	                .DeclareVar(typeof(EventBean[]), "eventsPerStreamBuf", NewArrayByLength(typeof(EventBean), Constant(forge.StreamNum + 1)))
	                .WhileLoop(ExprDotMethod(@Ref("it"), "hasNext"))
	                .DeclareVar(typeof(EventBean), "bean", Cast(typeof(EventBean), ExprDotMethod(@Ref("it"), "next")))
	                .AssignArrayElement("eventsPerStreamBuf", Constant(forge.StreamNum), @Ref("bean"))
	                .DeclareVar(Boxing.GetBoxedType(forge.ChildNode.EvaluationType), "value", LocalMethod(CodegenLegoMethodExpression.CodegenExpression(forge.ChildNode, context.Method, context.ClassScope), @Ref("eventsPerStreamBuf"), ConstantTrue(), ConstantNull()))
