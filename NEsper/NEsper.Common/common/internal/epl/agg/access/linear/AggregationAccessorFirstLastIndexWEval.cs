...
namespace com.espertech.esper.common.@internal.epl.agg.access.linear
{
	...
	public class AggregationAccessorFirstLastIndexWEval {
	    public static void GetValueCodegen(AggregationAccessorFirstLastIndexWEvalForge forge, AggregationAccessorForgeGetCodegenContext context) {
	        ...
	        CodegenMethod getBeanFirstLastIndex = GetBeanFirstLastIndexCodegen(forge, context.Column, context.ClassScope, stateForge, context.Method, context.NamedMethods);
	        context.Method.Block.DeclareVar(typeof(EventBean), "bean", LocalMethod(getBeanFirstLastIndex))
	                ...
	                .MethodReturn(LocalMethod(CodegenLegoMethodExpression.CodegenExpression(forge.ChildNode, context.Method, context.ClassScope), @Ref("eventsPerStreamBuf"), ConstantTrue(), ConstantNull()));
	    }
	    ...
	    public static void GetEnumerableScalarCodegen(AggregationAccessorFirstLastIndexWEvalForge forge, AggregationAccessorForgeGetCodegenContext context) {
	        ...
	        context.Method.Block.DeclareVar(typeof(EventBean), "bean", LocalMethod(getBeanFirstLastIndex))
	                ...
	                .DeclareVar(typeof(EventBean[]), "eventsPerStreamBuf", NewArrayByLength(typeof(EventBean), Constant(forge.StreamNum + 1)))
	                .AssignArrayElement("eventsPerStreamBuf", Constant(forge.StreamNum), @Ref("bean"))
	                .DeclareVar(typeof(object), "value", LocalMethod(CodegenLegoMethodExpression.CodegenExpression(forge.ChildNode, context.Method, context.ClassScope), @Ref("eventsPerStreamBuf"), ConstantTrue(), ConstantNull()))
	    ...
	    private static CodegenMethod GetBeanFirstLastIndexCodegen(AggregationAccessorFirstLastIndexWEvalForge forge, int column, CodegenClassScope classScope, AggregationStateLinearForge stateForge, CodegenMethod parent, CodegenNamedMethods namedMethods) {
	        ...
	        if (forge.Constant == -1) {
	            ...
	            method.Block.DeclareVar(evalType, "indexResult", LocalMethod(CodegenLegoMethodExpression.CodegenExpression(forge.IndexNode, method, classScope), ConstantNull(), ConstantTrue(), ConstantNull()));
	        ...
	        CodegenExpression value = forge.IsFirst ? stateForge.AggregatorLinear.GetFirstNthValueCodegen(@Ref("index"), method, classScope, namedMethods) : stateForge.AggregatorLinear.GetLastNthValueCodegen(@Ref("index"), method, classScope, namedMethods);
