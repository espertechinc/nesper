...
namespace com.espertech.esper.common.@internal.epl.historical.common
{
	...
	public class HistoricalStreamIndexListForge {
	    ...
	    public Pair<HistoricalIndexLookupStrategyForge, PollResultIndexingStrategyForge> GetStrategy(int streamViewStreamNum) {
	        ...
	        if (indexesUsedByStreams == null) {
	            ...
	            if (indexesUsedByStreams.Count > 1) {
	                ...
	                foreach (KeyValuePair<HistoricalStreamIndexDesc, IList<int>> desc in indexesUsedByStreams) {
	                    int sampleStreamViewStreamNum = desc.Value.Get(0);
	                    indexingStrategies[count] = JoinSetComposerPrototypeForgeFactory.DetermineIndexing(queryGraph, typesPerStream[historicalStreamNum], typesPerStream[sampleStreamViewStreamNum], historicalStreamNum, sampleStreamViewStreamNum).Second;
	                    count++;
	                }
	        ...
	        HistoricalIndexLookupStrategyForge innerLookupStrategy = JoinSetComposerPrototypeForgeFactory.DetermineIndexing(queryGraph, typesPerStream[historicalStreamNum], typesPerStream[streamViewStreamNum], historicalStreamNum, streamViewStreamNum).First;
	        ...
	        return new Pair<>(lookupStrategy, masterIndexingStrategy);
	    }

	    private Type[] GetPropertyTypes(EventType eventType, string[] properties) {
	    ...
	    private Type[] GetPropertyTypes(IList<QueryGraphValueEntryHashKeyedForge> hashKeys) {
	        ...
	        for (int i = 0; i < hashKeys.Count; i++) {
	            types[i] = Boxing.GetBoxedType(hashKeys.Get(i).KeyExpr.Forge.EvaluationType);
	        }
	        return types;
	    }
	}
