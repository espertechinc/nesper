...
namespace com.espertech.esper.common.@internal.epl.enummethod.eval
{
	public class ExprDotForgeSetExceptUnionIntersect : ExprDotForgeEnumMethodBase {
	    ...
	    public override EnumForge GetEnumForge(StreamTypeService streamTypeService, string enumMethodUsedName, IList<ExprDotEvalParam> bodiesAndParameters, EventType inputEventType, Type collectionComponentType, int numStreamsIncoming, bool disablePropertyExpressionEventCollCache, StatementRawInfo statementRawInfo, StatementCompileTimeServices services) {
	        ...
	        ExprDotEnumerationSourceForge enumSrc = ExprDotNodeUtility.GetEnumerationSource(first.Body, streamTypeService, true, disablePropertyExpressionEventCollCache, statementRawInfo, services);
	        ...
	        if (inputEventType != null) {
	            ...
	            if (setType != inputEventType) {
	                ...
	                if (!isSubtype) {
	                    string message = "Enumeration method '" + enumMethodUsedName + "' expects event type '" + inputEventType.Name + "' but receives event type '" + setType.Name + "'";
	                    throw new ExprValidationException(message);
	                }
	            }
	        } else {
	            ...
	            if (!TypeHelper.IsAssignmentCompatible(setType, collectionComponentType)) {
	                string message = "Enumeration method '" + enumMethodUsedName + "' expects scalar type '" + collectionComponentType.Name + "' but receives event type '" + setType.Name + "'";
	                throw new ExprValidationException(message);
	            }
	        }
