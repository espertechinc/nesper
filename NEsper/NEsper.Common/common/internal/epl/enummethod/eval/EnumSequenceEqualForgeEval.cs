...
namespace com.espertech.esper.common.@internal.epl.enummethod.eval
{
    public class EnumSequenceEqualForgeEval : EnumEval
    {
        ...
        public static CodegenExpression Codegen(EnumSequenceEqualForge forge, EnumForgeCodegenParams args, CodegenMethodScope codegenMethodScope, CodegenClassScope codegenClassScope)
        {
            ...
            CodegenMethod methodNode = codegenMethodScope.MakeChildWithScope(typeof(bool), typeof(EnumSequenceEqualForgeEval), scope, codegenClassScope).AddParam(EnumForgeCodegenNames.PARAMS);

            methodNode.Block.MethodReturn(StaticMethod(typeof(EnumSequenceEqualForgeEval), "enumSequenceEqualsCompare", EnumForgeCodegenNames.REF_ENUMCOLL, forge.innerExpression.EvaluateCodegen(typeof(object), methodNode, scope, codegenClassScope)));
            return LocalMethod(methodNode, args.Eps, args.Enumcoll, args.IsNewData, args.ExprCtx);
        }

        public static bool EnumSequenceEqualsCompare(ICollection<object> enumcoll, object otherObj)
        {
            if (otherObj == null)
            {
                return false;
            ...
            if (!(otherObj is ICollection<object>))
            {
                ...
                {
                    Log.Warn("Enumeration method 'sequenceEqual' expected a Collection-type return value from its parameter but received '" + otherObj.GetType() + "'");
                    return false;
                }
            }
