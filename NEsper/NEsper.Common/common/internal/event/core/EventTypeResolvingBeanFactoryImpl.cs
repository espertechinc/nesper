...
namespace com.espertech.esper.common.@internal.@event.core
{
	public class EventTypeResolvingBeanFactoryImpl : EventTypeResolvingBeanFactory {
	    ...
	    public EventBean AdapterForMap(IDictionary<string, object> map, string eventTypeName) {
	        EventType type = eventTypeRepository.GetTypeByName(eventTypeName);
	        EventTypeUtility.ValidateTypeMap(eventTypeName, type);
	        return new MapEventBean(map, type);
	    }

	    public EventBean AdapterForXMLDOM(XmlNode node, string eventTypeName) {
	        EventType type = eventTypeRepository.GetTypeByName(eventTypeName);
	        EventTypeUtility.ValidateTypeXMLDOM(eventTypeName, type);
	        XmlNode namedNode = GetXMLNodeFromDocument(node);
