...
namespace com.espertech.esper.common.@internal.@event.json.compiletime
{
	public class StmtClassForgeableJsonSerializer : StmtClassForgeable
	{
			...
			bool fireAndForget)
		{
			...
			// set-value-method
			CodegenMethod setValueMethod = CodegenMethod
			...
			CodegenMethod newUnderlyingMethod = CodegenMethod
				.MakeParentNode(typeof(object), this.GetType(), CodegenSymbolProviderEmpty.INSTANCE, classScope);
			...
			clazz.BaseList.AssignType(typeof(JsonSerializationContext));
			return clazz;
		}

		private CodegenMethod ForgeWriteStatic(CodegenClassScope classScope)
		{
			CodegenMethod writeStaticMethod = CodegenMethod
				.MakeParentNode(typeof(void), this.GetType(), CodegenSymbolProviderEmpty.INSTANCE, classScope)
		...
		private CodegenMethod ForgeMakeMethod(
			CodegenClassScope classScope,
			CodegenClassMethods methods,
			CodegenClassProperties properties)
		{
			var makeMethod = CodegenMethod
				.MakeParentNode(
					...
					typeof(StmtClassForgeableJsonSerializer),
			...
			CodegenClassScope classScope)
		{
			...
			foreach (KeyValuePair<string, object> property in _desc.PropertiesThisType) {
				...
				CodegenExpression write = forge.SerializerForge.CodegenSerialize(
					new JsonSerializerForgeRefs(Ref("writer"), Ref("und." + fieldName), Constant(property.Key)),
					method,
					classScope);
				method.Block
