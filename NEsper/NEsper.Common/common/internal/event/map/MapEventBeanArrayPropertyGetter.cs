	...
	public class MapEventBeanArrayPropertyGetter : MapEventPropertyGetter {
	    ...
	    private CodegenMethod GetMapCodegen(CodegenMethodScope codegenMethodScope, CodegenClassScope codegenClassScope) {
	        return codegenMethodScope.MakeChild(typeof(object), this.GetType(), codegenClassScope).AddParam(typeof(IDictionary<object, object>), "map").Block
	                .DeclareVar(typeof(object), "mapValue", ExprDotMethod(@Ref("map"), "get", Constant(propertyName)))
	                .MethodReturn(LocalMethod(BaseNestableEventUtil.GetArrayPropertyAsUnderlyingsArrayCodegen(underlyingType, codegenMethodScope, codegenClassScope), Cast(typeof(EventBean[]), @Ref("mapValue"))));
	    }
