...
namespace com.espertech.esper.common.@internal.@event.bean.introspect
{
	...
	public class PropertyListBuilderPublic : PropertyListBuilder {
	    ...
	    private static void AddPublicMethods(IList<PropertyStem> result, Type clazz)
	    {
	        ...
	        for (int i = 0; i < methods.Length; i++) {
	            ...
	            PropertyStem desc = PropertyListBuilderExplicit.MakeMethodDesc(methods[i], methods[i].Name);
	            result.Add(desc);
	        }
	    ...
	    private static void AddPublicFields(IList<PropertyStem> result, Type clazz) {
	        FieldInfo[] fields = clazz.GetFields();
	        for (int i = 0; i < fields.Length; i++) {
	            PropertyStem desc = PropertyListBuilderExplicit.MakeFieldDesc(fields[i], fields[i].Name);
	            result.Add(desc);
	        }
	    }
	}
