...
namespace com.espertech.esper.common.@internal.@event.bean.introspect
{
	...
	public class PropertyListBuilderExplicit : PropertyListBuilder {
	    ...
	    /// Ctor.
	    /// </summary>
	    /// <param name="legacyConfig">is a legacy type specification containinginformation about explicitly configured fields and methods
	    /// </param>
	    ...
	    protected internal static void GetExplicitProperties(IList<PropertyStem> result,
	                                                Type clazz,
	                                                ConfigurationCommonEventTypeBean legacyConfig) {
	        foreach (ConfigurationCommonEventTypeBean.LegacyFieldPropDesc desc in legacyConfig.FieldProperties) {
	            result.Add(MakeDesc(clazz, desc));
	        }
	        foreach (ConfigurationCommonEventTypeBean.LegacyMethodPropDesc desc in legacyConfig.MethodProperties) {
	            result.Add(MakeDesc(clazz, desc));
	        }
	    }

	    private static PropertyStem MakeDesc(Type clazz, ConfigurationCommonEventTypeBean.LegacyMethodPropDesc methodDesc) {
	    ...
	    private static PropertyStem MakeDesc(Type clazz, ConfigurationCommonEventTypeBean.LegacyFieldPropDesc fieldDesc) {
	    ...
	    protected internal static PropertyStem MakeFieldDesc(FieldInfo field, string name) {
	    ...
	    /// <param name="name">is the name of the event property</param>
	    /// <returns>property descriptor</returns>
	    protected internal static PropertyStem MakeMethodDesc(MethodInfo method, string name) {
