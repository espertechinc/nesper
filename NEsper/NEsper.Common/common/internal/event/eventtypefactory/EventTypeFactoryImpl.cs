...
namespace com.espertech.esper.common.@internal.@event.eventtypefactory
{
	public class EventTypeFactoryImpl : EventTypeFactory {
	    public readonly static EventTypeFactoryImpl INSTANCE = new EventTypeFactoryImpl();

	    private EventTypeFactoryImpl() {
	    }

	    public BeanEventType CreateBeanType(BeanEventTypeStem stem, EventTypeMetadata metadata, BeanEventTypeFactory beanEventTypeFactory, EventType[] superTypes, ISet<EventType> deepSuperTypes, string startTimestampPropertyName, string endTimestampPropertyName) {
	    ...
	    public MapEventType CreateMap(EventTypeMetadata metadata, LinkedHashMap<string, object> properties,
	        ...
	        Pair<EventType[], ISet<EventType>> st = EventTypeUtility.GetSuperTypesDepthFirst(superTypes, EventUnderlyingType.MAP, eventTypeNameResolver);
	    ...
	    public ObjectArrayEventType CreateObjectArray(EventTypeMetadata metadata, LinkedHashMap<string, object> properties, string[] superTypes, string startTimestampPropertyName, string endTimestampPropertyName, BeanEventTypeFactory beanEventTypeFactory, EventTypeNameResolver eventTypeNameResolver) {
	        Pair<EventType[], ISet<EventType>> st = EventTypeUtility.GetSuperTypesDepthFirst(superTypes, EventUnderlyingType.OBJECTARRAY, eventTypeNameResolver);
	        properties = BaseNestableEventUtil.ResolvePropertyTypes(properties, eventTypeNameResolver);
	        return new ObjectArrayEventType(metadata, properties, st.First, st.Second,
	    ...
	    public WrapperEventType CreateWrapper(EventTypeMetadata metadata, EventType underlying, LinkedHashMap<string, object> properties, BeanEventTypeFactory beanEventTypeFactory, EventTypeNameResolver eventTypeNameResolver) {
	        return WrapperEventTypeUtil.MakeWrapper(metadata, underlying, properties, beanEventTypeFactory.EventBeanTypedEventFactory, beanEventTypeFactory, eventTypeNameResolver);
	    }
	    ...
	    public EventType CreateXMLType(EventTypeMetadata metadata, ConfigurationCommonEventTypeXMLDOM detail, SchemaModel schemaModel, string representsFragmentOfProperty, string representsOriginalTypeName, BeanEventTypeFactory beanEventTypeFactory, XMLFragmentEventTypeFactory xmlFragmentEventTypeFactory, EventTypeNameResolver eventTypeNameResolver) {
	        if (metadata.IsPropertyAgnostic) {
	            return new SimpleXMLEventType(metadata, detail, beanEventTypeFactory.EventBeanTypedEventFactory, eventTypeNameResolver, xmlFragmentEventTypeFactory);
	        }
	        return new SchemaXMLEventType(metadata, detail, schemaModel, representsFragmentOfProperty, representsOriginalTypeName, beanEventTypeFactory.EventBeanTypedEventFactory, eventTypeNameResolver, xmlFragmentEventTypeFactory);
	    }
