...
namespace com.espertech.esper.common.@internal.@event.variant
{
	public class VariantEventTypeUtil {
	    ...
	    public static CodegenExpressionField GetField(VariantEventType variantEventType, CodegenClassScope codegenClassScope) {
	        return codegenClassScope.AddFieldUnshared(true, typeof(VariantEventType), Cast(typeof(VariantEventType), EventTypeUtility.ResolveTypeCodegen(variantEventType, EPStatementInitServicesConstants.REF)));
	    }

	    public static void ValidateInsertedIntoEventType(EventType eventType, VariantEventType variantEventType) {
	        ...
	        EventType[] variants = variantEventType.Variants;
	        foreach (EventType variant in variants) {
