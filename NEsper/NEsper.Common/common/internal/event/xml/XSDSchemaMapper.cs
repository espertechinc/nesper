...
namespace com.espertech.esper.common.@internal.@event.xml
{
    ...
    public class XSDSchemaMapper
    {
            ...
            int maxRecusiveDepth = DEFAULT_MAX_RECURSIVE_DEPTH)
        {
            // Load schema
            try {
                var schemaLocation = string.IsNullOrEmpty(schemaResource)
                    ? null
                    : resourceManager.ResolveResourceURL(schemaResource);
        ...
        /// <returns></returns>
        private XmlSchemaType ResolveSchemaType(
            XmlSchema xsModel,
        ...
        /// <param name="name">The name.</param>
        /// <returns></returns>
        private static XmlSchemaType ResolveSimpleType(
            XmlSchema xsModel,
            XmlQualifiedName name)
        {
            XmlSchemaType schemaType = XmlSchemaType.GetBuiltInSimpleType(name);
            if (schemaType == null) {
                schemaType = XmlSchemaType.GetBuiltInComplexType(name);
                if (schemaType == null) {
        ...
        private void BuildElementDictionary(XmlSchema xsModel)
        {
            ...
            foreach (var oelement in xsModel.Items) {
                ...
                if (element != null) {
                    var elementName = element.QualifiedName;
