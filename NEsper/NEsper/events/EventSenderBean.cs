...
namespace com.espertech.esper.events
{
    ...
    public class EventSenderBean : EventSender
    {
        ...
        private readonly ICollection<Type> _compatibleClasses;
        ...
        public void SendEvent(Object theEvent)
        {
            if (theEvent == null)
            {
                throw new ArgumentNullException("event", "No event object provided to sendEvent method");
            }
        ...
        private EventBean GetEventBean(Object theEvent)
        {
            ...
            if (theEvent.GetType() != _beanEventType.UnderlyingType)
            {
                using (_iLock.Acquire())
                {
                    ...
                    {
                        if (TypeHelper.IsSubclassOrImplementsInterface(theEvent.GetType(), _beanEventType.UnderlyingType))
                        {
                            _compatibleClasses.Add(theEvent.GetType());
                        }
                        else
                        {
                            throw new EPException("Event object of type " + theEvent.GetType().FullName +
