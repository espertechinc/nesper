...
namespace com.espertech.esper.events
{
    ...
    public class EventAdapterServiceHelper
    {
        ...
        public static EventBeanManufacturer GetManufacturer(EventAdapterService eventAdapterService, EventType eventType, IList<WriteablePropertyDescriptor> properties, EngineImportService engineImportService, bool allowAnyType)
        {
            if (!(eventType is EventTypeSPI))
            {
                return null;
            ...
            {
                var beanEventType = (BeanEventType)eventType;
                return new EventBeanManufacturerBean(beanEventType, eventAdapterService, properties.ToArray(), engineImportService);
            }
            var typeSPI = (EventTypeSPI)eventType;
            if (!allowAnyType && !AllowPropulate(typeSPI))
            {
            ...
            if (eventType is ObjectArrayEventType)
            {
                ...
                return new EventBeanManufacturerObjectArray(
                    objectArrayEventType, eventAdapterService, properties);
            }
            return null;
        }
