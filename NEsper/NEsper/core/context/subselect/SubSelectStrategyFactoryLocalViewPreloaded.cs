...
namespace com.espertech.esper.core.context.subselect
{
	...
	public class SubSelectStrategyFactoryLocalViewPreloaded : SubSelectStrategyFactory
	{
	    ...
	    private readonly static SubordTableLookupStrategyNullRow NULL_ROW_STRATEGY = new SubordTableLookupStrategyNullRow();
	        ...
	        bool isRecoveringResilient)
        {
	        ...
	        if (hasPrior) {
	            var priorEventViewFactory = EPStatementStartMethodHelperPrior.GetPriorEventViewFactory(
                    agentInstanceContext.StatementContext, 1024 + _subqueryNumber, viewFactoryChain.IsEmpty(), true, subqueryNumber);
	        ...
	        var index = _pair.First.MakeEventTables(new EventTableFactoryTableIdentAgentInstanceSubq(agentInstanceContext, _subqueryNumber));
	        ...
	        if (_aggregationServiceFactory != null) {
	            aggregationService = _aggregationServiceFactory.AggregationServiceFactory.MakeService(agentInstanceContext, agentInstanceContext.StatementContext.MethodResolutionService, true, subqueryNumber);
	            ...
	            if (namedSpec.FilterExpressions != null && !namedSpec.FilterExpressions.IsEmpty()) {
	                ...
	                try {
	                    ...
	                    var filterSpecCompiled = FilterSpecCompiler.MakeFilterSpec(types.EventTypes[0], types.StreamNames[0],
	                            namedSpec.FilterExpressions, null, tagged, tagged, types, null, agentInstanceContext.StatementContext, Collections.SingletonSet(0));
