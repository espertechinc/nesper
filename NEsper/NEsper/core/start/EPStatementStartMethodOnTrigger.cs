...
namespace com.espertech.esper.core.start
{
	...
	public class EPStatementStartMethodOnTrigger : EPStatementStartMethodBase
	{
	    ...
	    public override EPStatementStartResult StartInternal(EPServicesContext services, StatementContext statementContext, bool isNewStatement, bool isRecoveringStatement, bool isRecoveringResilient)
        {
	        ...
	        // determine context
	        string contextName = _statementSpec.OptionalContextName;
	        ...
	        // obtain activator
            StreamSpecCompiled streamSpec = _statementSpec.StreamSpecs[0];
            ...
            if (_statementSpec.OnTriggerDesc is OnTriggerWindowDesc)
            {
                var onTriggerDesc = (OnTriggerWindowDesc)_statementSpec.OnTriggerDesc;
	            ...
	            if (tableMetadata != null) {
	                ...
	                services.StatementVariableRefService.AddReferences(statementContext.StatementName, onTriggerDesc.WindowName);
	                ...
	                throw new ExprValidationException("A named window or variable by name '" + onTriggerDesc.WindowName + "' does not exist");
	            }
	        }
	        // variable assignments
            else if (_statementSpec.OnTriggerDesc is OnTriggerSetDesc)
            {
                var desc = (OnTriggerSetDesc)_statementSpec.OnTriggerDesc;
                contextFactoryResult = HandleContextFactorySetVariable(_statementSpec, statementContext, services, desc, streamSpec, subSelectStreamDesc, contextPropertyRegistry, activatorResult);
	        }
	        // split-stream use case
	        else {
                var desc = (OnTriggerSplitStreamDesc)_statementSpec.OnTriggerDesc;
                contextFactoryResult = HandleContextFactorySplitStream(_statementSpec, statementContext, services, desc, streamSpec, contextPropertyRegistry, subSelectStreamDesc, activatorResult);
	        }
	        statementContext.StatementAgentInstanceFactory = contextFactoryResult.ContextFactory;
	        EventType resultEventType = contextFactoryResult.ResultSetProcessorPrototype == null ? null : contextFactoryResult.ResultSetProcessorPrototype.ResultSetProcessorFactory.ResultEventType;
	        ...
	        if (tableMetadata != null) {
	            destroyCallbacks.AddCallback(new EPStatementDestroyCallbackTableIdxRef(services.TableService, tableMetadata, statementContext.StatementName));
	            destroyCallbacks.AddCallback(new EPStatementDestroyCallbackTableUpdStr(services.TableService, tableMetadata, statementContext.StatementName));
	        }
            ...
            if (_statementSpec.OptionalContextName != null)
            {
	            ...
	            foreach (KeyValuePair<ExprSubselectNode, SubSelectStrategyFactoryDesc> entry in contextFactoryResult.SubSelectStrategyCollection.Subqueries)
                {
	                ...
	                subselectStrategyInstances.Put(entry.Key, new SubSelectStrategyHolder(specificService, subselectAggregation, subselectPriorStrategies, subselectPreviousStrategies, null, null, null));
	            }
                ...
                if (_statementSpec.TableNodes != null)
                {
                    foreach (ExprTableAccessNode tableNode in _statementSpec.TableNodes)
                    {
                ...
                var statement = new ContextManagedStatementOnTriggerDesc(_statementSpec, statementContext, mergeView, contextFactoryResult.ContextFactory);
	            ...
	            destroyCallbacks.AddCallback(new EPStatementDestroyCallbackContext(services.ContextManagementService, contextName, statementContext.StatementName, statementContext.StatementId));
	        }
	        ...
	        if (contextFactoryResult.ResultSetProcessorPrototype != null && contextFactoryResult.ResultSetProcessorPrototype.AggregationServiceFactoryDesc != null) {
	            EPStatementStartMethodHelperAssignExpr.AssignAggregations(aggregationService, contextFactoryResult.ResultSetProcessorPrototype.AggregationServiceFactoryDesc.Expressions);
	        }

	        // assign subquery nodes
	        EPStatementStartMethodHelperAssignExpr.AssignSubqueryStrategies(contextFactoryResult.SubSelectStrategyCollection, subselectStrategyInstances);
        ...
        {
	        var usedByChildViews = patternStreamSpec.ViewSpecs.Length > 0 || (_statementSpec.InsertIntoDesc != null);
	        ...
	        FilterStreamSpecCompiled filterStreamSpec)
        {
	        ...
	        var activator = services.ViewableActivatorFactory.CreateFilterProxy(services, filterStreamSpec.FilterSpec, statementContext.Annotations, false, instrumentationAgentOnTrigger, false, 0);
	        ...
	        ActivatorResult activatorResult)
	    {
	        ...
	        var subSelectStrategyCollection = EPStatementStartMethodHelperSubselect.PlanSubSelect(services, statementContext, IsQueryPlanLogging(services), subSelectStreamDesc, new string[]{streamSpec.OptionalStreamName}, new EventType[]{activatorResult.ActivatorResultEventType}, new string[]{activatorResult.TriggerEventTypeName}, statementSpec.DeclaredExpressions, contextPropertyRegistry);
	        ...
	        try {
	            ...
	            onSetVariableViewFactory = new OnSetVariableViewFactory(statementContext.StatementId, desc, statementContext.EventAdapterService, statementContext.VariableService, statementContext.StatementResultService, exprEvaluatorContext);
	        }
	        catch (VariableValueException ex) {
	            throw new ExprValidationException("Error in variable assignment: " + ex.Message, ex);
	        }

	        var outputEventType = onSetVariableViewFactory.EventType;
	        ...
	        var outputResultSetProcessorPrototype = ResultSetProcessorFactoryFactory.GetProcessorPrototype(defaultSelectAllSpec, statementContext, streamTypeService, null, new bool[0], true, contextPropertyRegistry, null, services.ConfigSnapshot, services.ResultSetProcessorHelperFactory, false, true);

	        var outputViewFactory = OutputProcessViewFactoryFactory.Make(statementSpec, services.InternalEventRouter, statementContext, null, null, services.TableService, outputResultSetProcessorPrototype.ResultSetProcessorFactory.ResultSetProcessorType, services.ResultSetProcessorHelperFactory, services.StatementVariableRefService);
	        ...
	        ActivatorResult activatorResult)
	    {
	        ...
	        if ((statementSpec.GroupByExpressions != null && statementSpec.GroupByExpressions.GroupByNodes.Length > 0) || (statementSpec.HavingExprRootNode != null) || (statementSpec.OrderByList.Length > 0))
	        {
	        ...
	        var processorFactories = new ResultSetProcessorFactoryDesc[desc.SplitStreams.Count + 1];
	        var whereClauses = new ExprNode[desc.SplitStreams.Count + 1];
	        processorFactories[0] = ResultSetProcessorFactoryFactory.GetProcessorPrototype(
	                statementSpec, statementContext, typeService, null, new bool[0], false, contextPropertyRegistry, null, 
                    services.ConfigSnapshot, 
                    services.ResultSetProcessorHelperFactory, false, true);
	        whereClauses[0] = statementSpec.FilterRootNode;
	        var isNamedWindowInsert = new bool[desc.SplitStreams.Count + 1];
	        isNamedWindowInsert[0] = false;
	        var insertIntoTableNames = new string[desc.SplitStreams.Count + 1];
	        insertIntoTableNames[0] = GetOptionalInsertIntoTableName(statementSpec.InsertIntoDesc, services.TableService);
	        ...
	        foreach (var splits in desc.SplitStreams)
	        {
	            ...
	            processorFactories[index] = ResultSetProcessorFactoryFactory.GetProcessorPrototype(
	                    splitSpec, statementContext, typeService, null, new bool[0], false, contextPropertyRegistry, null, services.ConfigSnapshot, services.ResultSetProcessorHelperFactory, false, true);
	            whereClauses[index] = splitSpec.FilterRootNode;
	            isNamedWindowInsert[index] = statementContext.NamedWindowMgmtService.IsNamedWindow(splits.InsertInto.EventTypeName);
	            insertIntoTableNames[index] = GetOptionalInsertIntoTableName(splits.InsertInto, services.TableService);

	            index++;
	        }

	        var splitDesc = new StatementAgentInstanceFactoryOnTriggerSplitDesc(processorFactories, whereClauses, isNamedWindowInsert);
	        var contextFactory = new StatementAgentInstanceFactoryOnTriggerSplit(statementContext, statementSpec, services, activatorResult.Activator, subSelectStrategyCollection, splitDesc, activatorResult.ActivatorResultEventType, insertIntoTableNames);
	        return new ContextFactoryResult(contextFactory, subSelectStrategyCollection, null);
	    }
	        ...
	        IList<StopCallback> stopCallbacks)
	    {
	        ...
	        // validate context
	        ValidateOnExpressionContext(contextName, processor.ContextName, "Named window '" + onTriggerDesc.WindowName + "'");

	        var namedWindowType = processor.NamedWindowType;
	        services.StatementEventTypeRefService.AddReferences(statementContext.StatementName, new string[] {onTriggerDesc.WindowName});
            ...
            if (_statementSpec.InsertIntoDesc != null || onTriggerDesc is OnTriggerMergeDesc)
            {
	            routerService = services.InternalEventRouter;
	        }
            if (_statementSpec.InsertIntoDesc != null)
            {
                var tableMetadata = services.TableService.GetTableMetadata(_statementSpec.InsertIntoDesc.EventTypeName);
	            if (tableMetadata != null) {
	                optionalInsertIntoTableName = tableMetadata.TableName;
	                routerService = null;
	            }
	            addToFront = statementContext.NamedWindowMgmtService.IsNamedWindow(_statementSpec.InsertIntoDesc.EventTypeName);
	        }
	        bool isDistinct = _statementSpec.SelectClauseSpec.IsDistinct;
	        var selectResultEventType = validationResult.ResultSetProcessorPrototype.ResultSetProcessorFactory.ResultEventType;
	        var createNamedWindowMetricsHandle = processor.CreateNamedWindowMetricsHandle;

	        var onExprFactory = NamedWindowOnExprFactoryFactory.Make(namedWindowType, onTriggerDesc.WindowName, validationResult.ZeroStreamAliasName,
	                onTriggerDesc,
	        ...
	        if ((_statementSpec.OnTriggerDesc.OnTriggerType == OnTriggerType.ON_DELETE) ||
	            (_statementSpec.OnTriggerDesc.OnTriggerType == OnTriggerType.ON_UPDATE) ||
	            (_statementSpec.OnTriggerDesc.OnTriggerType == OnTriggerType.ON_MERGE))
	        {
                var defaultSelectAllSpec = new StatementSpecCompiled();
	            defaultSelectAllSpec.SelectClauseSpec.SetSelectExprList(new SelectClauseElementWildcard());
	            StreamTypeService streamTypeService = new StreamTypeServiceImpl(new EventType[] {namedWindowType}, new string[] {"trigger_stream"}, new bool[] {true}, services.EngineURI, false);
	            outputResultSetProcessorPrototype = ResultSetProcessorFactoryFactory.GetProcessorPrototype(defaultSelectAllSpec, statementContext, streamTypeService, null, new bool[0], true, contextPropertyRegistry, null, services.ConfigSnapshot, services.ResultSetProcessorHelperFactory, false, true);
	        }

	        var resultEventType = validationResult.ResultSetProcessorPrototype.ResultSetProcessorFactory.ResultEventType;
	        var outputViewFactory = OutputProcessViewFactoryFactory.Make(_statementSpec, services.InternalEventRouter, statementContext, resultEventType, null, services.TableService, validationResult.ResultSetProcessorPrototype.ResultSetProcessorFactory.ResultSetProcessorType, services.ResultSetProcessorHelperFactory, services.StatementVariableRefService);

	        var contextFactory = new StatementAgentInstanceFactoryOnTriggerNamedWindow(statementContext, _statementSpec, services, activatorResult.Activator, validationResult.SubSelectStrategyCollection, validationResult.ResultSetProcessorPrototype, validationResult.ValidatedJoin, outputResultSetProcessorPrototype, onExprFactory, outputViewFactory, activatorResult.ActivatorResultEventType, processor, stopCallbacks);
	        ...
	        string optionalTableName)
	    {
	        ...
	        var subSelectStrategyCollection = EPStatementStartMethodHelperSubselect.PlanSubSelect(services, statementContext, IsQueryPlanLogging(services), subSelectStreamDesc, new string[]{zeroStreamAliasName, streamSpec.OptionalStreamName}, new EventType[]{namedWindowType, activatorResult.ActivatorResultEventType}, new string[]{namedWindowTypeName, activatorResult.TriggerEventTypeName}, _statementSpec.DeclaredExpressions, contextPropertyRegistry);

	        var typeService = new StreamTypeServiceImpl(new EventType[] {namedWindowType, activatorResult.ActivatorResultEventType}, new string[] {zeroStreamAliasName, streamName}, new bool[] {false, true}, services.EngineURI, true);
	        ...
	        if (zeroStreamAliasName.Equals(INITIAL_VALUE_STREAM_NAME) || streamName.Equals(INITIAL_VALUE_STREAM_NAME)) {
	            ...
	            assignmentTypeService = new StreamTypeServiceImpl(new EventType[] {namedWindowType, activatorResult.ActivatorResultEventType, namedWindowType}, new string[] {zeroStreamAliasName, streamName, INITIAL_VALUE_STREAM_NAME}, new bool[] {false, true, true}, services.EngineURI, false);
	        ...
	        var validatedJoin = ValidateJoinNamedWindow(services.EngineURI, statementContext, ExprNodeOrigin.WHERE, _statementSpec.FilterRootNode,
	        ...
	        EPStatementStartMethodHelperValidate.ValidateNodes(_statementSpec, statementContext, typeService, null);
	        ...
	        if (_statementSpec.SelectClauseSpec.SelectExprList.Length == 0)
	        {
	        ...
	        var resultSetProcessorPrototype = ResultSetProcessorFactoryFactory.GetProcessorPrototype(
	                _statementSpec, statementContext, typeService, null, new bool[0], true, contextPropertyRegistry, null, services.ConfigSnapshot, services.ResultSetProcessorHelperFactory, false, true);
	    ...
	    private void ValidateOnExpressionContext(string onExprContextName, string desiredContextName, string title)
	    {
	        ...
	        if (onExprContextName != desiredContextName)
            {
	            throw new ExprValidationException(string.Format("Cannot create on-trigger expression: {0} was declared with context '{1}', please use the same context instead", title, ((object) desiredContextName).Render()));
	        }
	    }
	        ...
	        SubSelectActivationCollection subSelectStreamDesc)
	    {
	        ...
	        // validate expressions and plan subselects
	        TriggerValidationPlanResult validationResult = ValidateOnTriggerPlan(services, statementContext, onTriggerDesc, metadata.InternalEventType, streamSpec, activatorResult, contextPropertyRegistry, subSelectStreamDesc, metadata.TableName);

	        // table on-action view factory
	        var onExprFactory = TableOnViewFactoryFactory.Make(metadata, onTriggerDesc, activatorResult.ActivatorResultEventType, streamSpec.OptionalStreamName,
	            ...
	            (_statementSpec.OnTriggerDesc.OnTriggerType == OnTriggerType.ON_MERGE))
	        {
	            ...
	            StreamTypeService streamTypeService = new StreamTypeServiceImpl(new EventType[] {metadata.PublicEventType}, new string[] {"trigger_stream"}, new bool[] {true}, services.EngineURI, false);
	            outputResultSetProcessorPrototype = ResultSetProcessorFactoryFactory.GetProcessorPrototype(defaultSelectAllSpec, statementContext, streamTypeService, null, new bool[0], true, contextPropertyRegistry, null, services.ConfigSnapshot, services.ResultSetProcessorHelperFactory, false, true);
	        }
...

	    {
	        ...
	        foreach (var matchedItem in mergeDesc.Items) {
	            ...
	            var twoStreamTypeSvc = new StreamTypeServiceImpl(new EventType[] {namedWindowType, triggerStreamType},
	                    new string[] {namedWindowName, triggerStreamName}, new bool[] {true, true}, statementContext.EngineURI, true);
	            StreamTypeService insertOnlyTypeSvc = new StreamTypeServiceImpl(new EventType[] {dummyTypeNoProperties, triggerStreamType},
	                    new string[] {UuidGenerator.Generate(), triggerStreamName}, new bool[] {true, true}, statementContext.EngineURI, true);
	            ...
	            if (namedWindowName.Equals(INITIAL_VALUE_STREAM_NAME) || triggerStreamName.Equals(INITIAL_VALUE_STREAM_NAME)) {
	                ...
	                assignmentStreamTypeSvc = new StreamTypeServiceImpl(new EventType[] {namedWindowType, triggerStreamType, namedWindowType},
	                        new string[] {namedWindowName, triggerStreamName, INITIAL_VALUE_STREAM_NAME}, new bool[] {true, true, true}, statementContext.EngineURI, false);
	            ...
	            if (matchedItem.OptionalMatchCond != null) {
	                ...
	                matchedItem.OptionalMatchCond = EPStatementStartMethodHelperValidate.ValidateExprNoAgg(ExprNodeOrigin.MERGEMATCHCOND, matchedItem.OptionalMatchCond, matchValidStreams, statementContext, evaluatorContextStmt, exprNodeErrorMessage, true);
	                if (!matchedItem.IsMatchedUnmatched) {
	                    EPStatementStartMethodHelperValidate.ValidateSubqueryExcludeOuterStream(matchedItem.OptionalMatchCond);
	                }
	            }
	            ...
	            foreach (var item in matchedItem.Actions) {
	                if (item is OnTriggerMergeActionDelete) {
	                    ...
	                    if (delete.OptionalWhereClause != null) {
	                        delete.OptionalWhereClause = EPStatementStartMethodHelperValidate.ValidateExprNoAgg(ExprNodeOrigin.MERGEMATCHWHERE, delete.OptionalWhereClause, twoStreamTypeSvc, statementContext, evaluatorContextStmt, exprNodeErrorMessage, true);
	                    }
	                }
	                else if (item is OnTriggerMergeActionUpdate) {
	                    var update = (OnTriggerMergeActionUpdate) item;
	                    if (update.OptionalWhereClause != null) {
	                        update.OptionalWhereClause = EPStatementStartMethodHelperValidate.ValidateExprNoAgg(ExprNodeOrigin.MERGEMATCHWHERE, update.OptionalWhereClause, twoStreamTypeSvc, statementContext, evaluatorContextStmt, exprNodeErrorMessage, true);
	                    }
	                    foreach (var assignment in update.Assignments)
	                    {
	                        assignment.Expression = EPStatementStartMethodHelperValidate.ValidateExprNoAgg(ExprNodeOrigin.UPDATEASSIGN, assignment.Expression, assignmentStreamTypeSvc, statementContext, evaluatorContextStmt, exprNodeErrorMessage, true);
	                    }
	                }
	                    ...
	                    if (insert.OptionalWhereClause != null) {
	                        insert.OptionalWhereClause = EPStatementStartMethodHelperValidate.ValidateExprNoAgg(ExprNodeOrigin.MERGEMATCHWHERE, insert.OptionalWhereClause, insertTypeSvc, statementContext, evaluatorContextStmt, exprNodeErrorMessage, true);
	                    }
	                    var colIndex = 0;
	                    foreach (var raw in insert.SelectClause)
	                    {
	                        if (raw is SelectClauseStreamRawSpec)
	                        {
	                            ...
	                            if (foundStreamNum == null) {
	                                throw new ExprValidationException("Stream by name '" + rawStreamSpec.StreamName + "' was not found");
	                            }
	                            var streamSelectSpec = new SelectClauseStreamCompiledSpec(rawStreamSpec.StreamName, rawStreamSpec.OptionalAsName);
	                        ...
	                        else if (raw is SelectClauseExprRawSpec)
	                        {
	                            ...
	                            var validationContext = new ExprValidationContext(
	                                ...
	                                statementContext.ContextDescriptor, statementContext.ScriptingService,
                                    false, false, true, false, null, false);
	                            var exprCompiled = ExprNodeUtility.GetValidatedSubtree(ExprNodeOrigin.SELECT, exprSpec.SelectExpression, validationContext);
	                            ...
	                            if (resultName == null)
	                            {
	                                if (insert.Columns.Count > colIndex) {
	                                    ...
	                                    resultName = exprCompiled.ToExpressionStringMinPrecedenceSafe();
	                                }
	                            }
	                            compiledSelect.Add(new SelectClauseExprCompiledSpec(exprCompiled, resultName, exprSpec.OptionalAsName, exprSpec.IsEvents));
	    ...
	    internal ExprNode ValidateJoinNamedWindow(
	        string engineURI,
	        StatementContext statementContext,
        ...
        internal class ActivatorResult
        {
	        ...
	        internal ActivatorResult(ViewableActivator activator, string triggerEventTypeName, EventType activatorResultEventType)
            {
	            Activator = activator;
