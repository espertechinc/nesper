...
namespace com.espertech.esper.core.start
{
    ...
    public class EPStatementStartMethodSelectUtil {
        public static EPStatementStartMethodSelectDesc Prepare(StatementSpecCompiled statementSpec,
            ...
            var stopCallbacks = new LinkedList<StopCallback>();
            var destroyCallbacks = new EPStatementDestroyCallbackList();
    
            // determine context
            string contextName = statementSpec.OptionalContextName;
            ContextPropertyRegistry contextPropertyRegistry = (contextName != null) ? Services.ContextManagementService.GetContextDescriptor(contextName).ContextPropertyRegistry : null;
    
            // Determine stream names for each stream - some streams may not have a name given
            string[] streamNames = EPStatementStartMethodHelperUtil.DetermineStreamNames(statementSpec.StreamSpecs);
            ...
            bool isJoin = statementSpec.StreamSpecs.Length > 1;
            bool hasContext = statementSpec.OptionalContextName != null;
            ...
            StreamJoinAnalysisResult joinAnalysisResult = VerifyJoinViews(statementSpec, statementContext.NamedWindowMgmtService);
            ...
            for (int i = 0; i < statementSpec.StreamSpecs.Length; i++) {
                ...
                bool isCanIterateUnbound = streamSpec.ViewSpecs.Length == 0 &&
                        (services.ConfigSnapshot.EngineDefaults.ViewResources.IsIterableUnbound ||
                                AnnotationUtil.FindAnnotation(statementSpec.Annotations, typeof(IterableUnbound)) != null);
                ...
                if (streamSpec is FilterStreamSpecCompiled) {
                    ...
                    bool filterSubselectSameStream = EPStatementStartMethodHelperUtil.DetermineSubquerySameStream(statementSpec, filterStreamSpec);
                    ...
                    if (optionalViewableActivatorFactory != null) {
                        ...
                        if (!hasContext) {
                            ...
                            if (InstrumentationHelper.ENABLED) {
                                string eventTypeName = filterStreamSpec.FilterSpec.FilterForEventType.Name;
                                int streamNumber = i;
                    ...
                    EventType resultEventType = filterStreamSpec.FilterSpec.ResultEventType;
                    ...
                    bool usedByChildViews = streamSpec.ViewSpecs.Length > 0 || (statementSpec.InsertIntoDesc != null);
                    string patternTypeName = statementContext.StatementId + "_pattern_" + i;
                    EventType eventType = services.EventAdapterService.CreateSemiAnonymousMapType(patternTypeName, patternStreamSpec.TaggedEventTypes, patternStreamSpec.ArrayEventTypes, usedByChildViews);
                    ...
                    EvalRootFactoryNode rootFactoryNode = services.PatternNodeFactory.MakeRootNode(patternStreamSpec.EvalFactoryNode);
                    PatternContext patternContext = statementContext.PatternContextFactory.CreateContext(statementContext, i, rootFactoryNode, patternStreamSpec.MatchedEventMapMeta, true);
    
                    // create activator
                    ViewableActivator patternActivator = services.ViewableActivatorFactory.CreatePattern(patternContext, rootFactoryNode, eventType, EPStatementStartMethodHelperUtil.IsConsumingFilters(patternStreamSpec.EvalFactoryNode), patternStreamSpec.IsSuppressSameEventMatches, patternStreamSpec.IsDiscardPartialsOnMatch, isCanIterateUnbound);
                    ...
                    SQLColumnTypeConversion typeConversionHook = (SQLColumnTypeConversion) TypeHelper.GetAnnotationHook(statementSpec.Annotations, HookType.SQLCOL, typeof(SQLColumnTypeConversion), statementContext.EngineImportService);
                    SQLOutputRowConversion outputRowConversionHook = (SQLOutputRowConversion) TypeHelper.GetAnnotationHook(statementSpec.Annotations, HookType.SQLROW, typeof(SQLOutputRowConversion), statementContext.EngineImportService);
                    EPStatementAgentInstanceHandle epStatementAgentInstanceHandle = defaultAgentInstanceContext.EpStatementAgentInstanceHandle;
                    HistoricalEventViewable historicalEventViewable = DatabasePollingViewableFactory.CreateDBStatementView(statementContext.StatementId, i, sqlStreamSpec, services.DatabaseRefService, services.EventAdapterService, epStatementAgentInstanceHandle, typeConversionHook, outputRowConversionHook,
                            statementContext.ConfigSnapshot.EngineDefaults.Logging.IsEnableADO, services.DataCacheFactory, statementContext);
                    ...
                    joinAnalysisResult.TablesForStream = i, metadata;
                    ...
                    EventType namedWindowType = processor.TailView.EventType;
                    ...
                    joinAnalysisResult.NamedWindow = i;
                    ...
                    EPStatementStartMethodHelperValidate.ValidateNoDataWindowOnNamedWindow(unmaterializedViewChain[i].ViewFactoryChain);
                } else {
            ...
            if (statementSpec.MatchRecognizeSpec != null) {
                ...
                bool isUnbound = (unmaterializedViewChain[0].ViewFactoryChain.IsEmpty()) && (!(statementSpec.StreamSpecs[0] is NamedWindowConsumerStreamSpec));
                EventRowRegexNFAViewFactory factory = services.RegexHandlerFactory.MakeViewFactory(unmaterializedViewChain[0], statementSpec.MatchRecognizeSpec, defaultAgentInstanceContext, isUnbound, statementSpec.Annotations, services.ConfigSnapshot.EngineDefaults.MatchRecognize);
                unmaterializedViewChain[0].ViewFactoryChain.Add(factory);
            ...
            SubSelectStrategyCollection subSelectStrategyCollection = EPStatementStartMethodHelperSubselect.PlanSubSelect(services, statementContext, queryPlanLogging, subSelectStreamDesc, streamNames, streamEventTypes, eventTypeNames, statementSpec.DeclaredExpressions, contextPropertyRegistry);
    
            // Construct type information per stream
            var typeService = new StreamTypeServiceImpl(streamEventTypes, streamNames, EPStatementStartMethodHelperUtil.GetHasIStreamOnly(isNamedWindow, unmaterializedViewChain), services.EngineURI, false);
            ...
            for (int stream = 0; stream < historicalEventViewables.Length; stream++) {
                ...
                historicalEventViewable.Validate(services.EngineImportService,
                        typeService,
                ...
                historicalViewableDesc.Historical = stream, historicalEventViewable.RequiredStreams;
            ...
            if (numStreams > 1) {
                bool selectsRemoveStream = statementSpec.SelectStreamSelectorEnum.IsSelectsRStream ||
                        statementSpec.OutputLimitSpec != null;
                bool hasAggregations = !resultSetProcessorPrototypeDesc.AggregationServiceFactoryDesc.Expressions.IsEmpty();
        ...
        private static void ValidateNoViews(StreamSpecCompiled streamSpec, string conceptName)
            ...
            if (streamSpec.ViewSpecs.Length > 0) {
                throw new ExprValidationException(conceptName + " joins do not allow views onto the data, view '"
                        + streamSpec.ViewSpecs[0].ObjectName + "' is not valid in this context");
            }
        }
        ...
        private static StreamJoinAnalysisResult VerifyJoinViews(StatementSpecCompiled statementSpec, NamedWindowMgmtService namedWindowMgmtService)
                {
            StreamSpecCompiled[] streamSpecs = statementSpec.StreamSpecs;
            ...
            for (int i = 0; i < statementSpec.StreamSpecs.Length; i++) {
                ...
                if (streamSpec.Options.IsUnidirectional) {
                    analysisResult.UnidirectionalInd = i;
                }
                if (streamSpec.ViewSpecs.Length > 0) {
                    analysisResult.HasChildViews = i;
                }
                if (streamSpec is NamedWindowConsumerStreamSpec) {
                    ...
                    analysisResult.NamedWindow = i;
                    NamedWindowProcessor processor = namedWindowMgmtService.GetProcessor(nwSpec.WindowName);
                    string[][] uniqueIndexes = processor.UniqueIndexes;
                    analysisResult.UniqueKeys[i] = uniqueIndexes;
                    if (processor.IsVirtualDataWindow) {
                        analysisResult.ViewExternal[i] = new ProxyVirtualDWViewProviderForAgentInstance() {
                            ProcGetView = (agentInstanceContext) => {
                                return Processor.GetProcessorInstance(agentInstanceContext).RootViewInstance.VirtualDataWindow;
                            };
                        };
                    }
                }
            ...
            foreach (StreamSpecCompiled streamSpec in statementSpec.StreamSpecs) {
                ...
                FilterSpecCompiled filterSpec = ((FilterStreamSpecCompiled) streamSpec).FilterSpec;
            ...
            if (pureSelfJoin && (unidirectionalFilterSpec == null)) {
                analysisResult.PureSelfJoin = true;
                return analysisResult;
            }
            ...
            for (int i = 0; i < statementSpec.StreamSpecs.Length; i++) {
                ...
                string name = streamSpec.OptionalStreamName;
                ...
                if ((unidirectionalFilterSpec != null) &&
                    ...
                    analysisResult.UnidirectionalNonDriving = i;
                    continue;
                }
                if ((streamSpec is FilterStreamSpecCompiled) ||
        ...
        private static void VerifyJoinUnidirectional(StreamJoinAnalysisResult analysisResult, StatementSpecCompiled statementSpec) {
            int numUnidirectionalStreams = analysisResult.UnidirectionalCount;
        ...
        private static bool IsFullOuterJoinAllStreams(StatementSpecCompiled statementSpec) {
            if (statementSpec.OuterJoinDescList == null || statementSpec.OuterJoinDescList.Length == 0) {
                return false;
            }
            for (int stream = 0; stream < statementSpec.StreamSpecs.Length - 1; stream++) {
                if (statementSpec.OuterJoinDescList[stream].OuterJoinType != OuterJoinType.FULL) {
                    return false;
                }
            }
            return true;
