...
namespace com.espertech.esper.epl.join.exec.@base
{
    ...
    public class HistoricalTableLookupStrategy : JoinExecTableLookupStrategy
    {
        ...
        public ICollection<EventBean> Lookup(EventBean theEvent, Cursor cursor, ExprEvaluatorContext exprEvaluatorContext)
        {
            ...
            // poll
            EventTable[][] indexPerLookupRow = _viewable.Poll(_lookupEventsPerStream, _indexingStrategy, exprEvaluatorContext);
                ...
                if (subsetIter != null)
                {
                    if (_outerJoinExprNode != null) {
                        ...
                        while(subsetIter.MoveNext())
                        {
                            ...
                            var pass = (bool?) _outerJoinExprNode.Evaluate(new EvaluateParams(_lookupEventsPerStream[0], true, exprEvaluatorContext));
