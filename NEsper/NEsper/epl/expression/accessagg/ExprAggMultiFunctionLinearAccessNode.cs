...
namespace com.espertech.esper.epl.expression.accessagg
{
        ...
        , ExprAggregateAccessMultiValueNode
    {
        ...
        private LinearAggregationFactoryDesc HandleNonIntoTable(ExprNode[] childNodes, AggregationStateType stateType, ExprValidationContext validationContext) {
            ...
            if (evaluatorIndex != null) {
                ...
                if (evaluatorIndex.IsConstantResult) {
                    constant = (int?) evaluatorIndex.ExprEvaluator.Evaluate(new EvaluateParams(null, true, null));
                }
            ...
            if (tableMetadataX == null && !isFafWindow && (istreamOnly || streamTypeService.IsOnDemandStreams)) {
                var factory = validationContext.EngineImportService.AggregationFactoryFactory.MakeLinearUnbounded(validationContext.StatementExtensionSvcContext, this, containedType, accessorResultType, streamNum);
            ...
            var stateFactory = validationContext.EngineImportService.AggregationFactoryFactory.MakeLinear(validationContext.StatementExtensionSvcContext, this, streamNum);
            var factoryX = new ExprAggMultiFunctionLinearAccessNodeFactoryAccess(this, accessor, accessorResultType, containedType,
        ...
        {
            var message = "For tables columns, the " + stateType.GetName().ToLowerInvariant() + " aggregation function requires the 'Window(*)' declaration";
            if (stateType != AggregationStateType.WINDOW) {
            ...
            var stateFactory = validationContext.EngineImportService.AggregationFactoryFactory.MakeLinear(validationContext.StatementExtensionSvcContext, this, 0);
            ...
            TableMetadataColumnAggregation tableAccess)
        {
            ...
            if (childNodes.Length == 2) {
                ...
                if (indexEvalNode.IsConstantResult) {
                    constant = (int?) indexEvalNode.ExprEvaluator.Evaluate(new EvaluateParams(null, true, null));
                }
        ...
        public ICollection<object> EvaluateGetROCollectionScalar(EventBean[] eventsPerStream, bool isNewData, ExprEvaluatorContext context) {
            return base.AggregationResultFuture.GetCollectionScalar(base.Column, eventsPerStream, isNewData, context);
        }
