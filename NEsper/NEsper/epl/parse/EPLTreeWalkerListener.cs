...
{
	/// <summary>
	/// Called during the walks of a EPL expression AST ITree as specified in the grammar file.
	...
	public class EPLTreeWalkerListener : IEsperEPL2GrammarListener
	{
	    ...
	    private static readonly ISet<int> EVENT_FILTER_WALK_EXCEPTIONS__RECURSIVE = new HashSet<int>();
        private static readonly ISet<int> WHERE_CLAUSE_WALK_EXCEPTIONS__RECURSIVE = new HashSet<int>();
        private static readonly ISet<int> EVENT_PROPERTY_WALK_EXCEPTIONS__PARENT = new HashSet<int>();
        private static readonly ISet<int> SELECT_EXPRELE_WALK_EXCEPTIONS__RECURSIVE = new HashSet<int>();
	    ...
	    private readonly IDictionary<ITree, EvalFactoryNode> _astPatternNodeMap =
            new LinkedHashMap<ITree, EvalFactoryNode>();

	    private readonly IDictionary<ITree, RowRegexExprNode> _astRowRegexNodeMap =
	        new Dictionary<ITree, RowRegexExprNode>();

	    private readonly IDictionary<ITree, object> _astGOPNodeMap =
	        new Dictionary<ITree, object>();

	    private readonly IDictionary<ITree, StatementSpecRaw> _astStatementSpecMap =
	        new Dictionary<ITree, StatementSpecRaw>();

	    private readonly LazyAllocatedMap<ConfigurationPlugInAggregationMultiFunction, PlugInAggregationMultiFunctionFactory> _plugInAggregations =
            new LazyAllocatedMap<ConfigurationPlugInAggregationMultiFunction, PlugInAggregationMultiFunctionFactory>();
	    ...
	    public void ExitMatchRecog(EsperEPL2GrammarParser.MatchRecogContext ctx) {
	        ...
	        if (ctx.matchRecogMatchesInterval() != null) {
	            if (!ctx.matchRecogMatchesInterval().i.Text.ToLower().Equals("interval")) {
	    ...
	    public void ExitSelectionListElementExpr(EsperEPL2GrammarParser.SelectionListElementExprContext ctx) {
	        ...
	        if (ASTUtil.IsRecursiveParentRule(ctx, SELECT_EXPRELE_WALK_EXCEPTIONS__RECURSIVE)) {
	            ...
	            if ((_astExprNodeMap.Count > 1) || ((_astExprNodeMap.IsEmpty()))) {
	                throw ASTWalkException.From("Unexpected AST ITree contains zero or more then 1 child element for root", _tokenStream, ctx);
	            }
	        ...
	        if (ctx.selectionListElementAnno() != null) {
	            string annotation = ctx.selectionListElementAnno().i.Text.ToLower();
	    ...
	    public void ExitUpdateExpr(EsperEPL2GrammarParser.UpdateExprContext ctx) {
	        ...
	        var streamSpec = new FilterStreamSpecRaw(new FilterSpecRaw(eventTypeName, Collections.GetEmptyList<ExprNode>(), null), ViewSpec.EMPTY_VIEWSPEC_ARRAY, eventTypeName, new StreamSpecOptions());
	    ...
	    public void ExitOnStreamExpr(EsperEPL2GrammarParser.OnStreamExprContext ctx) {
	        ...
	        if (ctx.eventFilterExpression() != null) {
	            streamSpec = new FilterStreamSpecRaw(_filterSpec, ViewSpec.EMPTY_VIEWSPEC_ARRAY, streamAsName, new StreamSpecOptions());
	        }
	        else if (ctx.patternInclusionExpression() != null) {
	            if ((_astPatternNodeMap.Count > 1) || ((_astPatternNodeMap.IsEmpty()))) {
	                throw ASTWalkException.From("Unexpected AST ITree contains zero or more then 1 child elements for root");
	            }
	            // Get expression node sub-ITree from the AST nodes placed so far
	            var evalNode = _astPatternNodeMap.Values.First();
	            var flags = GetPatternFlags(ctx.patternInclusionExpression().annotationEnum());
                streamSpec = new PatternStreamSpecRaw(evalNode, ViewSpec.ToArray(_viewSpecs), streamAsName, new StreamSpecOptions(), flags.IsSuppressSameEventMatches, flags.IsDiscardPartialsOnMatch);
	    ...
	    public void ExitPropertyExpressionAtomic(EsperEPL2GrammarParser.PropertyExpressionAtomicContext ctx) {
	        ...
	        if (ctx.propertyExpressionAnnotation() != null) {
	            var annoName = ctx.propertyExpressionAnnotation().n.Text;
	            if (!annoName.ToLower().Equals("type")) {
	                throw ASTWalkException.From("Invalid annotation for property selection, expected 'type' but found '" + annoName + "'", _tokenStream, ctx);
	            }
	            splitterEventTypeName = ctx.propertyExpressionAnnotation().v.Text;
	        }
	    ...
	    public void ExitEvalEqualsExpression(EsperEPL2GrammarParser.EvalEqualsExpressionContext ctx) {
	        ...
	        if (ctx.a == null) {
	            bool isIs = ctx.@is != null || ctx.isnot != null;
	    ...
	    public void ExitStreamExpression(EsperEPL2GrammarParser.StreamExpressionContext ctx) {
	        ...
	        if (ASTUtil.GetRuleIndexIfProvided(ctx.GetChild(0)) == EsperEPL2GrammarParser.RULE_eventFilterExpression) {
	            ...
	            try
	            {
	                ...
	                foreach (var fragment in sqlFragments)
	                {
	                    ...
	                    if (expression.ToUpper().Equals(DatabasePollingViewableFactory.SAMPLE_WHERECLAUSE_PLACEHOLDER)) {
	                        continue;
	                    }
	        ...
	        else if (ctx.methodJoinExpression() != null)
	        {
	            ...
	            var className = ASTUtil.UnescapeClassIdent(mthctx.classIdentifier());

	            var indexDot = className.LastIndexOf('.');
                ...
                streamSpec = new MethodStreamSpec(streamName, ViewSpec.ToArray(_viewSpecs), prefixIdent, classNamePart, methodNamePart, exprNodes);
	        }
	        else {
	    ...
	    public void ExitViewExpression(EsperEPL2GrammarParser.ViewExpressionContext ctx) {
	        ...
	        var viewParameters = ASTExprHelper.ExprCollectSubNodes(ctx, 2, _astExprNodeMap);
	    ...
	    public void ExitPatternFilterExpression(EsperEPL2GrammarParser.PatternFilterExpressionContext ctx) {
	        ...
	        if (anno != null) {
	            string name = ctx.patternFilterAnnotation().i.Text;
	    ...
	    public void ExitStartPatternExpressionRule(EsperEPL2GrammarParser.StartPatternExpressionRuleContext ctx) {
	        ...
	        var streamSpec = new PatternStreamSpecRaw(evalNode, ViewSpec.EMPTY_VIEWSPEC_ARRAY, null, new StreamSpecOptions(), false, false);
	        _statementSpec.StreamSpecs.Add(streamSpec);
	    ...
	    public void ExitCreateSchemaExpr(EsperEPL2GrammarParser.CreateSchemaExprContext ctx) {
	        ...
	        if (ctx.parent.RuleIndex == EsperEPL2GrammarParser.RULE_eplExpression) {
	            _statementSpec.StreamSpecs.Add(new FilterStreamSpecRaw(new FilterSpecRaw(typeof(object).FullName, Collections.GetEmptyList<ExprNode>(), null), ViewSpec.EMPTY_VIEWSPEC_ARRAY, null, new StreamSpecOptions()));
	        }
	    ...
	    public void ExitCreateIndexExpr(EsperEPL2GrammarParser.CreateIndexExprContext ctx) {
	        ...
	        foreach (var col in cols) {
	            ...
	            if (col.t != null) {
	                ...
	                try {
	                    type = EnumHelper.Parse<CreateIndexType>(typeName);
	                }
	                catch (Exception) {
	                    throw ASTWalkException.From("Invalid column index type '" + typeName + "' encountered, please use any of the following index type names " + EnumHelper.GetValues<CreateIndexType>().Render());
	                }
	            }
	        ...
	        if (ctx.u != null) {
	            ...
	            if (ident.ToLower().Trim().Equals("unique")) {
	                unique = true;
	            }
	            else {
	    ...
	    public void ExitCreateWindowExpr(EsperEPL2GrammarParser.CreateWindowExprContext ctx) {
	        ...
	        var colums = ASTCreateSchemaHelper.GetColTypeList(ctx.createColumnList());
	    ...
	    public void ExitUnaryExpression(EsperEPL2GrammarParser.UnaryExpressionContext ctx) {
            ...
            if (ctx.NEWKW() != null && ctx.classIdentifier() != null)
            {
                ...
                if (ctx.chainedFunction() != null)
                {
                    ...
                    ExprDotNode dotNode = new ExprDotNode(chainSpec, _engineImportService.IsDuckType, _engineImportService.IsUdfCache);
            ...
            if (ctx.b != null)
            {
	            ...
	            if (ctx.chainedFunction() == null) {
	                ...
	                if (pair.Second.IsEmpty()) {
	                    ...
	                    exprNode = new ExprDotNode(pair.Second, _engineImportService.IsDuckType, _engineImportService.IsUdfCache);
	    ...
	    public void ExitEventProperty(EsperEPL2GrammarParser.EventPropertyContext ctx) {
	        if (EVENT_PROPERTY_WALK_EXCEPTIONS__PARENT.Contains(ctx.Parent.RuleIndex)) {
	            return;
	        }
	        ...
	        if (ctx.eventPropertyAtomic().Length == 1 || PropertyParser.IsNestedPropertyWithNonSimpleLead(ctx))
	        {
	            ...
	            EsperEPL2GrammarParser.EventPropertyAtomicContext first = ctx.eventPropertyAtomic()[0];
	            ...
	            if (first.lp != null) {
	                ...
	                IList<ExprNode> @params = Collections.SingletonList<ExprNode>(new ExprConstantNodeImpl(key));
	        ...
	        else
	        {
	            ...
	            var variableMetaDataX = _variableService.GetVariableMetaData(leadingIdentifier);
	            if (tableNode != null) {
	    ...
	    public void ExitOnExpr(EsperEPL2GrammarParser.OnExprContext ctx) {
	        if (ctx.onMergeExpr() != null) {
	            string windowName = ctx.onMergeExpr().n.Text;
	            string asName = ctx.onMergeExpr().i != null ? ctx.onMergeExpr().i.Text : null;
	        ...
	        else if (ctx.onSetExpr() == null)
	        {
	            ...
	            if (windowName == null) {
	                ...
	                for (var ii = 1; ii < statementSpecList.Length; ii++) {
	                    ...
	                    splitStreams.Add(new OnTriggerSplitStream(raw.InsertIntoDesc, raw.SelectClauseSpec, raw.FilterExprRootNode));
	                }
	                splitStreams.Add(new OnTriggerSplitStream(_statementSpec.InsertIntoDesc, _statementSpec.SelectClauseSpec, _statementSpec.FilterExprRootNode));
	                if (statementSpecList.Length != 0) {
	                    _statementSpec = statementSpecList[0];
	                }
	                bool isFirst = ctx.outputClauseInsert() == null || ctx.outputClauseInsert().ALL() == null;
	    ...
	    public void ExitCaseExpression(EsperEPL2GrammarParser.CaseExpressionContext ctx) {
	        ...
	        if (_astExprNodeMap.IsEmpty()) {
	            throw ASTWalkException.From("Unexpected AST ITree contains zero child element for case node", _tokenStream, ctx);
	        }
	        if (_astExprNodeMap.Count == 1) {
	            throw ASTWalkException.From("AST ITree does not contain at least when node for case node", _tokenStream, ctx);
	        }
	    ...
	    public void VisitTerminal(ITerminalNode terminalNode) {
	        if (terminalNode.Symbol.Type == EsperEPL2GrammarLexer.STAR) {
                int ruleIndex = ASTUtil.GetRuleIndexIfProvided(terminalNode.Parent);
	    ...
	    public void ExitFollowedByExpression(EsperEPL2GrammarParser.FollowedByExpressionContext ctx) {
	        ...
	        var fbNode = _patternNodeFactory.MakeFollowedByNode(expressions, _configurationInformation.EngineDefaults.PatternsConfig.MaxSubexpressions != null);
        ...
        public void ExitBuiltin_firstlastwindow(EsperEPL2GrammarParser.Builtin_firstlastwindowContext ctx) {
            var stateType = AggregationStateTypeExtensions.FromString(ctx.firstLastWindowAggregation().q.Text);
	    ...
	    public void ExitBuiltin_cnt(EsperEPL2GrammarParser.Builtin_cntContext ctx) {
	        ExprAggregateNode aggregateNode = new ExprCountNode(ctx.d != null);
	        ASTExprHelper.ExprCollectAddSubNodesAddParentNode(aggregateNode, ctx, _astExprNodeMap);
	    }
	    ...
	    private UniformPair<string> GetOnExprWindowName(EsperEPL2GrammarParser.OnExprContext ctx) {
	        ...
	        if (ctx.onUpdateExpr() != null) {
	            IToken alias = ctx.onUpdateExpr().i;
	    ...
	    private string GetDefaultDialect() {
	        return _configurationInformation.EngineDefaults.ScriptsConfig.DefaultDialect;
	    }
	    ...
	    private void HandleMergeInsert(EsperEPL2GrammarParser.MergeInsertContext mergeInsertContext) {
	        ...
	        IList<SelectClauseElementRaw> expressions = new List<SelectClauseElementRaw>(_statementSpec.SelectClauseSpec.SelectExprList);
	        ...
	        var columsList = ASTUtil.GetIdentList(mergeInsertContext.columnList());
	    ...
	    private void HandleChainedFunction(ParserRuleContext parentCtx, EsperEPL2GrammarParser.ChainedFunctionContext chainedCtx, ExprNode childExpression) {
	        var chainSpec = ASTLibFunctionHelper.GetLibFuncChain(chainedCtx.libFunctionNoClass(), _astExprNodeMap);
	        var dotNode = new ExprDotNode(chainSpec, _configurationInformation.EngineDefaults.ExpressionConfig.IsDuckTyping,
	                _configurationInformation.EngineDefaults.ExpressionConfig.IsUdfCache);
	    ...
	    private void HandleFAFNamedWindowStream(EsperEPL2GrammarParser.ClassIdentifierContext node, IToken i) {
            ...
            _statementSpec.StreamSpecs.Add(new FilterStreamSpecRaw(new FilterSpecRaw(windowName, Collections.GetEmptyList<ExprNode>(), null), ViewSpec.ToArray(_viewSpecs), alias, new StreamSpecOptions()));
	    }
	    ...
	    internal void End() {
	        if (_astExprNodeMap.Count > 1)
	        {
	            throw ASTWalkException.From("Unexpected AST ITree contains left over child elements," +
	    ...
	    public void ExitCreateTableExpr(EsperEPL2GrammarParser.CreateTableExprContext ctx) {
	        ...
	        var cols = ASTTableHelper.GetColumns(ctx.createTableColumnList().createTableColumn(), _astExprNodeMap, _engineImportService);
