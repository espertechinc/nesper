...
namespace com.espertech.esper.epl.parse
{
    ...
    {
	    public static IList<ExprChainedSpec> GetLibFuncChain(IList<EsperEPL2GrammarParser.LibFunctionNoClassContext> ctxs, IDictionary<ITree, ExprNode> astExprNodeMap)
        {
	        var chained = new List<ExprChainedSpec>(ctxs.Count);
	        foreach (var ctx in ctxs) {
	            var chainSpec = ASTLibFunctionHelper.GetLibFunctionChainSpec(ctx, astExprNodeMap);
        ...
        private static void AddChainRemainderFromOffset(IList<ASTLibModelChainElement> chainElements, int offset, IList<ExprChainedSpec> specList, IDictionary<ITree, ExprNode> astExprNodeMap)
        {
	    ...
	    private static void HandleMinMax(string ident, EsperEPL2GrammarParser.LibFunctionArgsContext ctxArgs, IDictionary<ITree, ExprNode> astExprNodeMap)
	    {
	        // Determine min or max
	        var childNodeText = ident;
            var childNodeTextLower = childNodeText.ToLower();
            MinMaxTypeEnum minMaxTypeEnum;
	        var filtered = childNodeText.StartsWith("f");
	        if (childNodeTextLower.Equals("min") || childNodeTextLower.Equals("fmin")) {
	            minMaxTypeEnum = MinMaxTypeEnum.MIN;
	        }
	        else if (childNodeTextLower.Equals("max") || childNodeTextLower.Equals("fmax")) {
        ...
        {
	        public ASTLibModelChainElement(string funcName, EsperEPL2GrammarParser.LibFunctionArgsContext args, bool hasLeftParen)
            {
	            FuncName = funcName;
	            Args = args;
