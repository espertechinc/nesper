    ...
    public class StatementSpecMapper
    {
        ...
        private static StatementSpecRaw Map(EPStatementObjectModel sodaStatement, StatementSpecMapContext mapContext) {
            ...
            if (sodaStatement.CreateWindow != null && raw.StreamSpecs.Count == 0) {
                ...
                raw.StreamSpecs.Add(new FilterStreamSpecRaw(spec, ViewSpec.EMPTY_VIEWSPEC_ARRAY, null, StreamSpecOptions.DEFAULT));
            }
            return raw;
        }
        ...
        private static void UnmapCreateTable(CreateTableDesc desc, EPStatementObjectModel model, StatementSpecUnMapContext unmapContext) {
            ...
            foreach (var col in desc.Columns) {
                ...
                var coldesc = new Com.espertech.esper.client.soda.CreateTableColumn(col.ColumnName, optExpr, col.OptTypeName, col.OptTypeIsArray, col.OptTypeIsPrimitiveArray, annots, col.PrimaryKey);
                cols.Add(coldesc);
            }
            clause.Columns = cols;
        ...
        private static void UnmapSQLParameters(IEnumerable<KeyValuePair<int, IList<ExprNode>>> sqlParameters, StatementSpecUnMapContext unmapContext) {
        ...
        private static ContextDescriptor UnmapCreateContextDetail(ContextDetail contextDetail, StatementSpecUnMapContext unmapContext) {
            ...
            if (contextDetail is ContextDetailInitiatedTerminated) {
                ...
                foreach (var item in init.Items) {
                    var dot = UnmapChains(new List<ExprChainedSpec>(Collections.SingletonList(item.Function)), unmapContext, false)[0];
        ...
        private static void UnmapCreateIndex(CreateIndexDesc createIndexDesc, EPStatementObjectModel model, StatementSpecUnMapContext unmapContext) {
            ...
            var cols = new List<CreateIndexColumn>();
            foreach (var item in createIndexDesc.Columns) {
                cols.Add(new CreateIndexColumn(item.Name, CreateIndexColumnType.ValueOf(item.Type.Name().ToUpperCase())));
            }
            model.CreateIndex = new CreateIndexClause(createIndexDesc.IndexName, createIndexDesc.WindowName, cols, createIndexDesc.IsUnique);
        }
        ...
        private static DataFlowOperator UnmapGraphOperator(GraphOperatorSpec spec, StatementSpecUnMapContext unmapContext) {
            ...
            var inputs = new List<DataFlowOperatorInput>();
            foreach (GraphOperatorInputNamesAlias @in in spec.Input.StreamNamesAndAliases) {
        ...
        private static void UnmapOutputLimit(OutputLimitSpec outputLimitSpec, EPStatementObjectModel model, StatementSpecUnMapContext unmapContext) {
            ...
            clause.AndAfterTerminate = outputLimitSpec.IsAndAfterTerminate;
        ...
        private static void UnmapForClause(ForClauseSpec spec, EPStatementObjectModel model, StatementSpecUnMapContext unmapContext) {
            ...
            foreach (var itemSpec in spec.Clauses) {
                var item = new ForClauseItem(ForClauseKeyword.ValueOf(itemSpec.Keyword.ToUpperCase(Locale.ENGLISH)));
        ...
        private static void UnmapMatchRecognize(MatchRecognizeSpec spec, EPStatementObjectModel model, StatementSpecUnMapContext unmapContext) {
            ...
            var measures = new List<SelectClauseExpression>();
            ...
            clause.SkipClause = MatchRecognizeSkipClause.Values([spec.Skip.Skip.Ordinal()]);
    
            var defines = new List<MatchRecognizeDefine>();
            foreach (var define in spec.Defines) {
    
                defines.Add(new MatchRecognizeDefine(define.Identifier, UnmapExpressionDeep(define.Expression, unmapContext)));
            }
            clause.Defines = defines;
    
            if (spec.Interval != null) {
                clause.IntervalClause = new MatchRecognizeIntervalClause((TimePeriodExpression) UnmapExpressionDeep(
        ...
        private static void MapOutputLimit(OutputLimitClause outputLimitClause, StatementSpecRaw raw, StatementSpecMapContext mapContext) {
            ...
            OutputLimitLimitType displayLimit = OutputLimitLimitType.ValueOf(outputLimitClause.Selector.ToString().ToUpperCase(Locale.ENGLISH));
            ...
            if (outputLimitClause.WhenExpression != null) {
                ...
                foreach (Assignment pair in outputLimitClause.ThenAssignments) {
                    var expr = MapExpressionDeep(pair.Value, mapContext);
                    assignments.Add(new OnTriggerSetAssignment(expr));
                }
            }
            ...
            if (onExpr is OnDeleteClause) {
                ...
                foreach (var item in splitClause.Items) {
                    ...
                    if (item.PropertySelects != null) {
                        ...
                        fromClause = new OnTriggerSplitStreamFromClause(propertyEvalSpec, item.PropertySelectsStreamName);
                    }
        ...
        private static void MapForClause(ForClause clause, StatementSpecRaw raw, StatementSpecMapContext mapContext) {
            ...
            foreach (var item in clause.Items) {
                var specItem = new ForClauseItemSpec(item.Keyword.GetName(), MapExpressionDeep(item.Expressions, mapContext));
        ...
        private static void MapMatchRecognize(MatchRecognizeClause clause, StatementSpecRaw raw, StatementSpecMapContext mapContext) {
            ...
            var measures = new List<MatchRecognizeMeasureItem>();
            foreach (var item in clause.Measures) {
                measures.Add(new MatchRecognizeMeasureItem(MapExpressionDeep(item.Expression, mapContext), item.AsName));
            }
            spec.Measures = measures;
            spec.IsAllMatches = clause.IsAll;
            spec.Skip = new MatchRecognizeSkip(MatchRecognizeSkipEnum.Values([clause.SkipClause.Ordinal()]));
    
            var defines = new List<MatchRecognizeDefineItem>();
            foreach (var define in clause.Defines) {
    
                defines.Add(new MatchRecognizeDefineItem(define.Name, MapExpressionDeep(define.Expression, mapContext)));
            }
        ...
        private static GroupByClauseElement MapGroupByExpr(GroupByClauseExpression expr, StatementSpecMapContext mapContext) {
            ...
            if (expr is GroupByClauseExpressionCombination) {
                IList<ExprNode> nodes = MapExpressionDeep(((GroupByClauseExpressionCombination) expr).Expressions, mapContext);
        ...
        private static void MapCreateWindow(CreateWindowClause createWindow, FromClause fromClause, StatementSpecRaw raw, StatementSpecMapContext mapContext) {
            ...
            raw.CreateWindowDesc = new CreateWindowDesc(
                ...
                MapViews(createWindow.Views, mapContext, StreamSpecOptions.DEFAULT, createWindow.IsInsert, insertFromWhereExpr, columns, asEventTypeName));
        }
        ...
        private static void MapCreateIndex(CreateIndexClause clause, StatementSpecRaw raw, StatementSpecMapContext mapContext) {
            ...
            var cols = new List<CreateIndexItem>();
            foreach (CreateIndexColumn col in clause.Columns) {
                cols.Add(new CreateIndexItem(col.ColumnName, CreateIndexType.ValueOf(col.Type.Name().ToUpperCase(Locale.ENGLISH))));
            }
        ...
        private static void MapUpdateClause(UpdateClause updateClause, StatementSpecRaw raw, StatementSpecMapContext mapContext) {
            ...
            raw.StreamSpecs.Add(new FilterStreamSpecRaw(filterSpecRaw, ViewSpec.EMPTY_VIEWSPEC_ARRAY, null, StreamSpecOptions.DEFAULT));
        }
        ...
        private static CreateSchemaDesc MapCreateSchemaInternal(CreateSchemaClause clause, StatementSpecRaw raw, StatementSpecMapContext mapContext) {
            ...
            return new CreateSchemaDesc(clause.SchemaName, clause.Types, columns, clause.Inherits, AssignedTypeExtensions.MapFrom(clause.TypeDefinition), clause.StartTimestampPropertyName, clause.EndTimestampPropertyName, clause.CopyFrom);
        }
        ...
        private static IList<GraphOperatorOutputItemType> MapGraphOpType(IList<DataFlowOperatorOutputType> typeInfos) {
            ...
            foreach (var info in typeInfos) {
                var type = new GraphOperatorOutputItemType(info.IsWildcard, info.TypeOrClassname, MapGraphOpType(info.TypeParameters));
                types.Add(type);
            }
            return types;
        }
            ...
            if (expr is ArithmaticExpression) {
                ...
                if (indexDot == -1) {
                    ...
                    if (mapContext.TableService.GetTableMetadata(prop.PropertyName) != null) {
                        var tableNode = new ExprTableAccessNodeTopLevel(prop.PropertyName);
                        mapContext.TableExpressions.Add(tableNode);
                        return tableNode;
                    }
    
                    // maybe variable
                    var variableMetaData = mapContext.VariableService.GetVariableMetaData(prop.PropertyName);
                ...
                if (op.Operator.Equals("=")) {
                    ...
                    return new ExprRelationalOpNodeImpl(RelationalOpEnum.Parse(op.Operator));
                }
                ...
                if (op.ConstantType != null) {
                    try {
                        constantType = mapContext.EngineImportService.ClassForNameProvider.ClassForName(op.ConstantType);
                ...
                if (sub.Operator.Equals("=")) {
                    ...
                    relop = RelationalOpEnum.Parse(sub.Operator);
                }
                ...
                return new ExprPreviousNode(ExprPreviousNodePreviousType.ValueOf(prev.Type.ToString()));
                ...
                return new ExprTimePeriodImpl(mapContext.Configuration.EngineDefaults.Expression.TimeZone,
                        tpe.HasYears, tpe.HasMonths, tpe.HasWeeks, tpe.HasDays, tpe.HasHours, tpe.HasMinutes, tpe.HasSeconds, tpe.HasMilliseconds, tpe.HasMicroseconds,
                ...
                Pair<ExprTableAccessNode, IList<ExprChainedSpec>> pair = ASTTableExprHelper.CheckTableNameGetLibFunc(mapContext.TableService, mapContext.EngineImportService, mapContext.PlugInAggregations,
                ...
                if (chain.Count == 1) {
                    ...
                    var script = ExprDeclaredHelper.GetExistsScript(mapContext.Configuration.EngineDefaults.Scripts.DefaultDialect,
                            name, chain[0].Parameters, mapContext.Scripts.Values(), mapContext.ExprDeclaredService);
                    if (script != null) {
                        return script;
                    }
                }
                var dotNode = new ExprDotNodeImpl(chain,
                        mapContext.Configuration.EngineDefaults.Expression.IsDuckTyping,
                        mapContext.Configuration.EngineDefaults.Expression.IsUdfCache);
        ...
        private static IList<Expression> UnmapExpressionDeep(ExprNode[] expressions, StatementSpecUnMapContext unmapContext) {
        ...
        private static MatchRecognizeRegEx UnmapExpressionFlatRowregex(RowRegexExprNode expr, StatementSpecUnMapContext unmapContext) {
            if (expr is RowRegexExprNodeAlteration) {
                ...
                return new MatchRecognizeRegExAtom(atom.Tag, MatchRecogizePatternElementType.Values()[atom.Type.Ordinal()], repeat);
                ...
                return new MatchRecognizeRegExNested(MatchRecogizePatternElementType.Values()[nested.Type.Ordinal()], repeat);
            }
        }
        ...
        private static RowRegexExprNode MapExpressionFlatRowregex(MatchRecognizeRegEx expr, StatementSpecMapContext mapContext) {
            if (expr is MatchRecognizeRegExAlteration) {
                ...
                return new RowRegexExprNodeAtom(atom.Name, RegexNFATypeEnum.Values()[atom.Type.Ordinal()], repeat);
                ...
                return new RowRegexExprNodeNested(RegexNFATypeEnum.Values()[nested.Type.Ordinal()], repeat);
            }
        }
        ...
        private static Expression UnmapExpressionFlat(ExprNode expr, StatementSpecUnMapContext unmapContext) {
            if (expr is ExprMathNode) {
                ...
                if (sub.RelationalOp != null) {
                    @operator = sub.RelationalOp.GetExpressionText();
                }
                ...
                result.Type = PreviousExpressionType.ValueOf(prev.PreviousType.ToString());
                return result;
                ...
                if (node.Index == null) {
                    ...
                    subs = new SubstitutionParameterExpressionIndexed(node.Index);
                }
                ...
                return new CompareListExpression(node.IsAll, node.RelationalOpEnum.ExpressionText);
                ...
                if (table is ExprTableAccessNodeKeys) {
                    ...
                    dotExpression.Add(table.TableName, Collections.EMPTY_LIST, true);
                ...
                if (table is ExprTableAccessNodeSubpropAccessor) {
                    ...
                    if (sub.ChildNodes.Length == 0) {
                        var dotExpression = new DotExpression();
                        dotExpression.Add(table.TableName + "." + sub.SubpropName, Collections.EMPTY_LIST, true);
                        IList<Expression> params = UnmapExpressionDeep(sub.AggregateAccessMultiValueNode.ChildNodes, unmapContext);
            ...
            throw new ArgumentException("Could not map expression node of type " + expr.Class.SimpleName);
        }
        ...
        private static void MapFrom(FromClause fromClause, StatementSpecRaw raw, StatementSpecMapContext mapContext) {
            ...
            foreach (var qualifier in fromClause.OuterJoinQualifiers) {
                ...
                raw.OuterJoinDescList.Add(new OuterJoinDesc(qualifier.Type, left, right, additionalLeft, additionalRight));
            }
        }
        ...
        private static EvalFactoryNode MapPatternEvalFlat(PatternExpr eval, StatementSpecMapContext mapContext) {
            ...
            if (eval is PatternAndExpr) {
                ...
                return MapContext.PatternNodeFactory.MakeFollowedByNode(maxExpr, mapContext.Configuration.EngineDefaults.Patterns.MaxSubexpressions != null);
            ...
            throw new ArgumentException("Could not map pattern expression node of type " + eval.Class.SimpleName);
        }
        ...
        private static PatternExpr UnmapPatternEvalFlat(EvalFactoryNode eval, StatementSpecUnMapContext unmapContext) {
            if (eval is EvalAndFactoryNode) {
                ...
                IList<Expression> expressions = UnmapExpressionDeep(observerNode.PatternObserverSpec.ObjectParameters, unmapContext);
                ...
                IList<Expression> expressions = UnmapExpressionDeep(guardNode.PatternGuardSpec.ObjectParameters, unmapContext);
        ...
        private static PropertyEvalSpec MapPropertySelects(IList<ContainedEventSelect> propertySelects, StatementSpecMapContext mapContext) {
        ...
        private static IList<ContainedEventSelect> UnmapPropertySelects(PropertyEvalSpec propertyEvalSpec, StatementSpecUnMapContext unmapContext) {
        ...
        private static IList<AnnotationPart> UnmapAnnotations(IList<AnnotationDesc> annotations) {
            var result = new List<AnnotationPart>();
        ...
        private static IList<ExpressionDeclaration> UnmapExpressionDeclarations(ExpressionDeclDesc expr, StatementSpecUnMapContext unmapContext) {
            ...
            var result = new List<ExpressionDeclaration>();
        ...
        private static ScriptExpression UnmapScriptExpression(ExpressionScriptProvided script, StatementSpecUnMapContext unmapContext) {
            ...
            return new ScriptExpression(script.Name, script.ParameterNames, script.Expression, returnType, script.OptionalDialect, script.OptionalEventTypeName);
        }
        ...
        private static ExpressionScriptProvided MapScriptExpression(ScriptExpression decl, StatementSpecMapContext mapContext) {
            ...
            bool isArray = decl.OptionalReturnType != null ? Decl.OptionalReturnType.Contains("[]") : false;
            return new ExpressionScriptProvided(decl.Name, decl.ExpressionText, decl.ParameterNames, returnType, isArray, decl.OptionalEventTypeName, decl.OptionalDialect);
        }
    
        private static AnnotationDesc MapAnnotation(AnnotationPart part) {
            if ((part.Attributes == null) || (part.Attributes.IsEmpty())) {
                return new AnnotationDesc(part.Name, Collections.EMPTY_LIST);
            }
        ...
        private static void MapSQLParameters(FromClause fromClause, StatementSpecRaw raw, StatementSpecMapContext mapContext) {
            ...
            foreach (var stream in fromClause.Streams) {
                ...
                try {
                    ...
                    throw new EPException("Error parsing SQL placeholder expression '" + sqlStream.SqlWithSubsParams + "': ");
                }
        ...
        private static IList<ExprChainedSpec> MapChains(IEnumerable<DotExpressionItem> pairs, StatementSpecMapContext mapContext) {
            var chains = new List<ExprChainedSpec>();
            foreach (var item in pairs) {
                chains.Add(new ExprChainedSpec(item.Name, MapExpressionDeep(item.Parameters, mapContext), item.IsProperty));
            }
            return chains;
        }
    
        private static IList<DotExpressionItem> UnmapChains(IEnumerable<ExprChainedSpec> pairs, StatementSpecUnMapContext unmapContext, bool isProperty) {
            var result = new List<DotExpressionItem>();
            foreach (var chain in pairs) {
                result.Add(new DotExpressionItem(chain.Name, UnmapExpressionDeep(chain.Parameters, unmapContext), chain.IsProperty));
            }
            return result;
        }
    }
