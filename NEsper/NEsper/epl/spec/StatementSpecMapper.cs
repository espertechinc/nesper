...
using ContextDescriptor = com.espertech.esper.client.soda.ContextDescriptor;
    ...
    public class StatementSpecMapper
    {
        ...
        private static StatementSpecRaw Map(EPStatementObjectModel sodaStatement, StatementSpecMapContext mapContext)
        {
            ...
            if (sodaStatement.CreateWindow != null && raw.StreamSpecs.Count == 0)
            {
                ...
                raw.StreamSpecs.Add(new FilterStreamSpecRaw(spec, ViewSpec.EMPTY_VIEWSPEC_ARRAY, null, new StreamSpecOptions()));
            }
            return raw;
        }
        ...
        private static void MapFireAndForget(FireAndForgetClause fireAndForgetClause, StatementSpecRaw raw, StatementSpecMapContext mapContext)
        {
            ...
            else if (fireAndForgetClause is FireAndForgetUpdate)
            {
                ...
                var assignments = upd.Assignments.Select(pair => MapExpressionDeep(pair.Value, mapContext)).Select(expr => new OnTriggerSetAssignment(expr)).ToList();
        ...
        private static EPStatementObjectModel UnmapInternal(StatementSpecRaw statementSpec, StatementSpecUnMapContext unmapContext)
        {
            ...
            var selectClause = UnmapSelect(statementSpec.SelectClauseSpec, statementSpec.SelectStreamSelectorEnum, unmapContext);
        ...
        private static void UnmapCreateTable(CreateTableDesc desc, EPStatementObjectModel model, StatementSpecUnMapContext unmapContext)
        {
            ...
            var cols = new List<client.soda.CreateTableColumn>();
            foreach (var col in desc.Columns)
            {
                ...
                var coldesc = new client.soda.CreateTableColumn(col.ColumnName, optExpr, col.OptTypeName, col.OptTypeIsArray, col.OptTypeIsPrimitiveArray, annots, col.PrimaryKey);
                cols.Add(coldesc);
            }
            clause.Columns = cols;
        ...
        private static void UnmapSQLParameters(IDictionary<int, IList<ExprNode>> sqlParameters, StatementSpecUnMapContext unmapContext)
        {
        ...
        private static void UnmapOnClause(OnTriggerDesc onTriggerDesc, EPStatementObjectModel model, StatementSpecUnMapContext unmapContext)
        {
            ...
            else if (onTriggerDesc.OnTriggerType == OnTriggerType.ON_SPLITSTREAM)
            {
                ...
                foreach (var stream in trigger.SplitStreams)
                {
                    ...
                    var selectClause = UnmapSelect(stream.SelectClause, SelectClauseStreamSelectorEnum.ISTREAM_ONLY, unmapContext);
            ...
            else if (onTriggerDesc.OnTriggerType == OnTriggerType.ON_MERGE)
            {
                ...
                foreach (var matched in trigger.Items)
                {
                    ...
                    var matchCond = matched.OptionalMatchCond != null ? UnmapExpressionDeep(matched.OptionalMatchCond, unmapContext) : null;
                    ...
                    foreach (var actionitem in matched.Actions)
                    {
                        ...
                        if (actionitem is OnTriggerMergeActionDelete)
                        {
                            ...
                            var optionalCondition = delete.OptionalWhereClause == null ? null : UnmapExpressionDeep(delete.OptionalWhereClause, unmapContext);
                        ...
                        else if (actionitem is OnTriggerMergeActionUpdate)
                        {
                            ...
                            var assignments = merge.Assignments.Select(pair => UnmapExpressionDeep(pair.Expression, unmapContext)).Select(expr => new Assignment(expr)).ToList();
                            var optionalCondition = merge.OptionalWhereClause == null ? null : UnmapExpressionDeep(merge.OptionalWhereClause, unmapContext);
                        ...
                        else if (actionitem is OnTriggerMergeActionInsert)
                        {
                            ...
                            var optionalCondition = insert.OptionalWhereClause == null ? null : UnmapExpressionDeep(insert.OptionalWhereClause, unmapContext);
                            action = new OnMergeMatchedInsertAction(columnNames, select, optionalCondition, insert.OptionalStreamName);
                        }
                        else
                        {
                            throw new ArgumentException("Unrecognized merged action type '" + actionitem.GetType() + "'");
                        }
                        actions.Add(action);
                    }
        ...
        private static void UnmapUpdateClause(IList<StreamSpecRaw> desc, UpdateDesc updateDesc, EPStatementObjectModel model, StatementSpecUnMapContext unmapContext)
        {
            if (updateDesc == null)
            {
                return;
        ...
        private static ContextDescriptor UnmapCreateContextDetail(ContextDetail contextDetail, StatementSpecUnMapContext unmapContext)
        {
            ...
            else if (contextDetail is ContextDetailPartitioned)
            {
                ...
                IList<ContextDescriptorKeyedSegmentedItem> segmentedItems = new List<ContextDescriptorKeyedSegmentedItem>();
            ...
            else if (contextDetail is ContextDetailHash)
            {
                ...
                {
                    DotExpressionItem dot = UnmapChains(new List<ExprChainedSpec>(Collections.SingletonList(item.Function)), unmapContext, false)[0];
                    var dotExpression = new SingleRowMethodExpression(new List<DotExpressionItem>(Collections.SingletonList(dot)));
                    var filter = UnmapFilter(item.FilterSpecRaw, unmapContext);
        ...
        private static ContextDescriptorCondition UnmapCreateContextRangeCondition(ContextDetailCondition endpoint, StatementSpecUnMapContext unmapContext)
        {
            if (endpoint is ContextDetailConditionCrontab)
            {
                ...
                var crontabExpr = UnmapExpressionDeep(crontab.Crontab, unmapContext);
        ...
        private static void UnmapCreateWindow(CreateWindowDesc createWindowDesc, EPStatementObjectModel model, StatementSpecUnMapContext unmapContext)
        {
            ...
            var clause = new CreateWindowClause(createWindowDesc.WindowName, UnmapViews(createWindowDesc.ViewSpecs, unmapContext));
        ...
        private static void UnmapCreateVariable(CreateVariableDesc createVariableDesc, EPStatementObjectModel model, StatementSpecUnMapContext unmapContext)
        {
            ...
            var clause = new CreateVariableClause(createVariableDesc.VariableType, createVariableDesc.VariableName, assignment, createVariableDesc.IsConstant);
        ...
        private static CreateSchemaClause UnmapCreateSchemaInternal(CreateSchemaDesc desc, StatementSpecUnMapContext unmapContext)
        {
            ...
            var clause = new CreateSchemaClause(desc.SchemaName, desc.Types, columns, desc.Inherits, desc.AssignedType.MapToSoda());
        ...
        private static DataFlowOperator UnmapGraphOperator(GraphOperatorSpec spec, StatementSpecUnMapContext unmapContext)
        {
            ...
            var inputs = spec.Input.StreamNamesAndAliases.Select(@in => new DataFlowOperatorInput(@in.InputStreamNames, @in.OptionalAsName)).ToList();
            op.Input = inputs;

            IList<DataFlowOperatorOutput> outputs = new List<DataFlowOperatorOutput>();
            foreach (var @out in spec.Output.Items)
            {
                IList<DataFlowOperatorOutputType> types = @out.TypeInfo.IsEmpty() ? null : new List<DataFlowOperatorOutputType>(Collections.SingletonList(UnmapTypeInfo(@out.TypeInfo[0])));
            ...
            if (spec.Detail != null)
            {
                ...
                foreach (KeyValuePair<string, object> param in spec.Detail.Configs)
                {
        ...
        private static DataFlowOperatorOutputType UnmapTypeInfo(GraphOperatorOutputItemType typeInfo)
        {
            ...
            {
                types = typeInfo.TypeParameters.Select(UnmapTypeInfo).ToList();
            }
            return new DataFlowOperatorOutputType(typeInfo.IsWildcard, typeInfo.TypeOrClassname, types);
        }

        private static void UnmapOrderBy(IList<OrderByItem> orderByList, EPStatementObjectModel model, StatementSpecUnMapContext unmapContext)
        {
        ...
        private static void UnmapOutputLimit(OutputLimitSpec outputLimitSpec, EPStatementObjectModel model, StatementSpecUnMapContext unmapContext)
        {
            ...
            if (outputLimitSpec.RateType == OutputLimitRateType.TIME_PERIOD)
            {
                ...
                var timePeriod = (TimePeriodExpression)UnmapExpressionDeep(outputLimitSpec.TimePeriodExpr, unmapContext);
            ...
            else if (outputLimitSpec.RateType == OutputLimitRateType.AFTER)
            {
                ...
                {
                    var after = (TimePeriodExpression)UnmapExpressionDeep(outputLimitSpec.AfterTimePeriodExpr, unmapContext);
                    clause = new OutputLimitClause(OutputLimitSelector.DEFAULT, OutputLimitUnit.AFTER, after, null);
                }
                else
                {
                    clause = new OutputLimitClause(OutputLimitSelector.DEFAULT, OutputLimitUnit.AFTER, null, outputLimitSpec.AfterNumberOfEvents);
                }
            }
            ...
            if (outputLimitSpec.AfterTimePeriodExpr != null)
            {
                clause.AfterTimePeriodExpression = UnmapExpressionDeep(outputLimitSpec.AfterTimePeriodExpr, unmapContext);
            }
            clause.IsAndAfterTerminate = outputLimitSpec.IsAndAfterTerminate;
            if (outputLimitSpec.AndAfterTerminateExpr != null)
            {
                clause.AndAfterTerminateAndExpr = UnmapExpressionDeep(outputLimitSpec.AndAfterTerminateExpr, unmapContext);
            }
        ...
        private static void UnmapRowLimit(RowLimitSpec rowLimitSpec, EPStatementObjectModel model, StatementSpecUnMapContext unmapContext)
        {
            ...
            var spec = new RowLimitClause(rowLimitSpec.NumRows, rowLimitSpec.OptionalOffset,
        ...
        private static void UnmapMatchRecognize(MatchRecognizeSpec spec, EPStatementObjectModel model, StatementSpecUnMapContext unmapContext)
        {
            ...
            IList<MatchRecognizeDefine> defines = spec.Defines
                .Select(define => new MatchRecognizeDefine(define.Identifier, UnmapExpressionDeep(define.Expression, unmapContext)))
                .ToList();
            ...
            {
                clause.IntervalClause = new MatchRecognizeIntervalClause((TimePeriodExpression)UnmapExpressionDeep(spec.Interval.TimePeriodExpr, unmapContext), spec.Interval.IsOrTerminated);
            }
        ...
        private static void MapOutputLimit(OutputLimitClause outputLimitClause, StatementSpecRaw raw, StatementSpecMapContext mapContext)
        {
            ...
            OutputLimitLimitType displayLimit = EnumHelper.Parse<OutputLimitLimitType>(outputLimitClause.Selector.ToString());
            ...
            {
                afterTimePeriod = (ExprTimePeriod)MapExpressionDeep(outputLimitClause.AfterTimePeriodExpression, mapContext);
            }
            ...
            {
                afterTerminateAssignments = outputLimitClause.AndAfterTerminateThenAssignments
                    .Select(pair => MapExpressionDeep(pair.Value, mapContext))
                    .Select(expr => new OnTriggerSetAssignment(expr))
                    .ToList();
            }

            var spec = new OutputLimitSpec(frequency, frequencyVariable, rateType, displayLimit, whenExpression, assignments, timerAtExprList, timePeriod, afterTimePeriod, outputLimitClause.AfterNumberOfEvents, outputLimitClause.IsAndAfterTerminate, andAfterTerminateAndExpr, afterTerminateAssignments);
        ...
        private static void MapOnTrigger(OnClause onExpr, StatementSpecRaw raw, StatementSpecMapContext mapContext)
        {
            ...
            if (onExpr is OnDeleteClause)
            {
                ...
                raw.OnTriggerDesc = new OnTriggerWindowDesc(onDeleteClause.WindowName, onDeleteClause.OptionalAsName, OnTriggerType.ON_DELETE, false);
            }
            else if (onExpr is OnSelectClause)
            {
                var onSelectClause = (OnSelectClause)onExpr;
                raw.OnTriggerDesc = new OnTriggerWindowDesc(onSelectClause.WindowName, onSelectClause.OptionalAsName, OnTriggerType.ON_SELECT, onSelectClause.IsDeleteAndSelect);
            }
            ...
            else if (onExpr is OnUpdateClause)
            {
                ...
                var assignments = updateClause.Assignments
                    ...
                    .Select(expr => new OnTriggerSetAssignment(expr))
                    .ToList();
                var desc = new OnTriggerWindowUpdateDesc(updateClause.WindowName, updateClause.OptionalAsName, assignments);
                raw.OnTriggerDesc = desc;
            }
                ...
                foreach (var matchItem in merge.MatchItems)
                {
                    ...
                    foreach (var action in matchItem.Actions)
                    {
                        ...
                        if (action is OnMergeMatchedDeleteAction)
                        {
                            ...
                            var optionalCondition = delete.WhereClause == null ? null : MapExpressionDeep(delete.WhereClause, mapContext);
                        ...
                        else if (action is OnMergeMatchedUpdateAction)
                        {
                            ...
                            var assignments = update.Assignments
                                ...
                                .Select(expr => new OnTriggerSetAssignment(expr))
                                .ToList();
                            var optionalCondition = update.WhereClause == null ? null : MapExpressionDeep(update.WhereClause, mapContext);
                        ...
                        else if (action is OnMergeMatchedInsertAction)
                        {
                            ...
                            var optionalCondition = insert.WhereClause == null ? null : MapExpressionDeep(insert.WhereClause, mapContext);
                            actionItem = new OnTriggerMergeActionInsert(optionalCondition, insert.OptionalStreamName, columnNames, select);
                        }
                        else
                        {
                    ...
                    var optionalConditionX = matchItem.OptionalCondition == null ? null : MapExpressionDeep(matchItem.OptionalCondition, mapContext);
        ...
        private static void MapRowLimit(RowLimitClause rowLimitClause, StatementSpecRaw raw, StatementSpecMapContext mapContext)
        {
            ...
            raw.RowLimitSpec = new RowLimitSpec(rowLimitClause.NumRows, rowLimitClause.OptionalOffsetRows,
        ...
        private static void MapForClause(ForClause clause, StatementSpecRaw raw, StatementSpecMapContext mapContext)
        {
            ...
            {
                var specItem = new ForClauseItemSpec(item.Keyword.Value.GetName(), MapExpressionDeep(item.Expressions, mapContext));
        ...
        private static void MapMatchRecognize(MatchRecognizeClause clause, StatementSpecRaw raw, StatementSpecMapContext mapContext)
        {
            ...
            var measures = clause.Measures
                .Select(item => new MatchRecognizeMeasureItem(MapExpressionDeep(item.Expression, mapContext), item.AsName))
                .ToList();
            ...
            var defines = clause.Defines
                .Select(define => new MatchRecognizeDefineItem(define.Name, MapExpressionDeep(define.Expression, mapContext)))
                .ToList();
            ...
            if (clause.IntervalClause != null)
            {
                ...
                {
                    throw new EPRuntimeException("Error validating time-period expression: " + e.Message, e);
                }
        ...
        private static GroupByClauseElement MapGroupByExpr(GroupByClauseExpression expr, StatementSpecMapContext mapContext)
        {
            ...
            if (expr is GroupByClauseExpressionRollupOrCube)
            {
                ...
                return new GroupByClauseElementRollupOrCube(rollup.IsCube, MapGroupByElements(rollup.Expressions, mapContext));
            }
            throw new IllegalStateException("Group by expression not recognized: " + expr);
        }

        private static IList<GroupByClauseElement> MapGroupByElements(IList<GroupByClauseExpression> elements, StatementSpecMapContext mapContext)
        {
            return elements.Select(element => MapGroupByExpr(element, mapContext)).ToList();
        }

        private static void UnmapGroupBy(IList<GroupByClauseElement> groupByExpressions, EPStatementObjectModel model, StatementSpecUnMapContext unmapContext)
        {
            if (groupByExpressions.Count == 0)
            {
                return;
            }
            var expressions = groupByExpressions.Select(element => UnmapGroupByExpression(element, unmapContext)).ToList();
        ...
        private static IList<GroupByClauseExpression> UnmapGroupByExpressions(IList<GroupByClauseElement> rollupExpressions, StatementSpecUnMapContext unmapContext)
        {
            return rollupExpressions.Select(e => UnmapGroupByExpression(e, unmapContext)).ToList();
        }
        ...
        private static void UnmapFrom(IList<StreamSpecRaw> streamSpecs, IList<OuterJoinDesc> outerJoinDescList, EPStatementObjectModel model, StatementSpecUnMapContext unmapContext)
        {
            ...
            foreach (var stream in streamSpecs)
            {
                ...
                else if (stream is MethodStreamSpec)
                {
                    ...
                    var methodStream = new MethodInvocationStream(method.ClassName, method.MethodName, method.OptionalStreamName);
        ...
        private static IList<SelectClauseElement> UnmapSelectClauseElements(IList<SelectClauseElementRaw> selectExprList, StatementSpecUnMapContext unmapContext)
        {
        ...
        private static InsertIntoClause UnmapInsertInto(InsertIntoDesc insertIntoDesc)
        {
            ...
            StreamSelector selector = SelectClauseStreamSelectorEnumExtensions.MapFromSODA(insertIntoDesc.StreamSelector);
            return InsertIntoClause.Create(insertIntoDesc.EventTypeName,
        ...
        private static ContextDetail MapCreateContextDetail(ContextDescriptor descriptor, StatementSpecMapContext mapContext)
        {
            ...
            if (descriptor is ContextDescriptorInitiatedTerminated)
            {
                ...
                {
                    distinctExpressions = ExprNodeUtility.ToArray(MapExpressionDeep(desc.OptionalDistinctExpressions, mapContext));
                }
            ...
            else if (descriptor is ContextDescriptorKeyedSegmented)
            {
                ...
                IList<ContextDetailPartitionItem> itemsdesc = new List<ContextDetailPartitionItem>();
                foreach (var item in seg.Items)
                {
            ...
            else if (descriptor is ContextDescriptorCategory)
            {
                ...
                IList<ContextDetailCategoryItem> itemsdesc = new List<ContextDetailCategoryItem>();
            ...
            else if (descriptor is ContextDescriptorHashSegmented)
            {
                ...
                IList<ContextDetailHashItem> itemsdesc = new List<ContextDetailHashItem>();
                foreach (var item in hash.Items)
                {
                    var rawSpec = MapFilter(item.Filter, mapContext);
                    var singleRowMethodExpression = (SingleRowMethodExpression)item.HashFunction;
                    ExprChainedSpec func = MapChains(Collections.SingletonList(singleRowMethodExpression.Chain[0]), mapContext)[0];
            ...
            else
            {
                ...
                foreach (var item in nested.Contexts)
                {
                    itemsdesc.Add(new CreateContextDesc(item.ContextName, MapCreateContextDetail(item.Descriptor, mapContext)));
                }
        ...
        private static ContextDetailCondition MapCreateContextRangeCondition(ContextDescriptorCondition condition, StatementSpecMapContext mapContext)
        {
            if (condition is ContextDescriptorConditionCrontab)
            {
                ...
                var expr = MapExpressionDeep(crontab.CrontabExpressions, mapContext);
        ...
        private static void MapCreateWindow(CreateWindowClause createWindow, FromClause fromClause, StatementSpecRaw raw, StatementSpecMapContext mapContext)
        {
            ...
            raw.CreateWindowDesc = new CreateWindowDesc(createWindow.WindowName, MapViews(createWindow.Views, mapContext), new StreamSpecOptions(), createWindow.IsInsert, insertFromWhereExpr, columns, asEventTypeName);
        }
        ...
        private static void MapUpdateClause(UpdateClause updateClause, StatementSpecRaw raw, StatementSpecMapContext mapContext)
        {
            ...
            var assignments = updateClause.Assignments.Select(pair => MapExpressionDeep(pair.Value, mapContext)).Select(expr => new OnTriggerSetAssignment(expr)).ToList();
            ExprNode whereClause = null;
            ...
            raw.StreamSpecs.Add(new FilterStreamSpecRaw(filterSpecRaw, ViewSpec.EMPTY_VIEWSPEC_ARRAY, null, new StreamSpecOptions()));
        }
        ...
        private static void MapCreateVariable(CreateVariableClause createVariable, StatementSpecRaw raw, StatementSpecMapContext mapContext)
        {
            ...
            raw.CreateVariableDesc = new CreateVariableDesc(createVariable.VariableType, createVariable.VariableName, assignment, createVariable.IsConstant, createVariable.IsArray, createVariable.IsArrayOfPrimitive);
        }
        ...
        private static void MapCreateTable(CreateTableClause createTable, StatementSpecRaw raw, StatementSpecMapContext mapContext)
        {
            ...
            {
                var optNode = desc.OptionalExpression != null ? MapExpressionDeep(desc.OptionalExpression, mapContext) : null;
                var annotations = MapAnnotations(desc.Annotations);
                cols.Add(new CreateTableColumn(desc.ColumnName, optNode, desc.OptionalTypeName, desc.OptionalTypeIsArray, desc.OptionalTypeIsPrimitiveArray, annotations, desc.PrimaryKey));
            }
        ...
        private static CreateSchemaDesc MapCreateSchemaInternal(CreateSchemaClause clause, StatementSpecRaw raw, StatementSpecMapContext mapContext)
        {
            ...
            return new CreateSchemaDesc(clause.SchemaName, clause.Types, columns, clause.Inherits, AssignedTypeExtensions.MapFrom(clause.TypeDefinition.Value), clause.StartTimestampPropertyName, clause.EndTimestampPropertyName, clause.CopyFrom);
        }
        ...
        private static void MapCreateGraph(CreateDataFlowClause clause, StatementSpecRaw raw, StatementSpecMapContext mapContext)
        {
            ...
            var schemas = clause.Schemas.Select(schema => MapCreateSchemaInternal(schema, raw, mapContext)).ToList();
            var ops = clause.Operators.Select(op => MapGraphOperator(op, mapContext)).ToList();
        ...
        private static GraphOperatorSpec MapGraphOperator(DataFlowOperator op, StatementSpecMapContext mapContext)
        {
            ...
            {
                input.StreamNamesAndAliases.Add(new GraphOperatorInputNamesAlias(@in.InputStreamNames.ToArray(), @in.OptionalAsName));
            }
        ...
        private static IList<GraphOperatorOutputItemType> MapGraphOpType(IList<DataFlowOperatorOutputType> typeInfos)
        {
            ...
            return typeInfos.Select(info => new GraphOperatorOutputItemType(info.IsWildcard, info.TypeOrClassname, MapGraphOpType(info.TypeParameters))).ToList();
        }
        ...
        private static IList<ColumnDesc> MapColumns(IList<SchemaColumnDesc> columns)
        {
            ...
            return columns.Select(col => new ColumnDesc(col.Name, col.Type, col.IsArray, col.IsPrimitiveArray)).ToList();
        }
        ...
        private static IList<SchemaColumnDesc> UnmapColumns(IList<ColumnDesc> columns)
        {
            ...
            return columns.Select(col => new SchemaColumnDesc(col.Name, col.Type, col.IsArray, col.IsPrimitiveArray)).ToList();
        }
        ...
        private static InsertIntoDesc MapInsertInto(InsertIntoClause insertInto)
        {
            ...
            var desc = new InsertIntoDesc(SelectClauseStreamSelectorEnumExtensions.MapFromSODA(insertInto.StreamSelector), eventTypeName);
        ...
        private static IList<SelectClauseElementRaw> MapSelectClauseElements(IList<SelectClauseElement> elements, StatementSpecMapContext mapContext)
        {
            ...
            foreach (var element in elements)
            {
                ...
                else if (element is SelectClauseExpression)
                {
                    ...
                    var rawElement = new SelectClauseExprRawSpec(exprNode, selectExpr.AsName, selectExpr.IsAnnotatedByEventFlag);
        ...
        private static IList<ExprNode> MapExpressionDeep(IList<Expression> expressions, StatementSpecMapContext mapContext)
        {
        ...
        private static ExprNode MapExpressionFlat(Expression expr, StatementSpecMapContext mapContext)
        {
            ...
            if (expr is ArithmaticExpression)
            {
                ...
                return new ExprMathNode(MathArithTypeEnumExtensions.ParseOperator(arith.Operator),
                        mapContext.Configuration.EngineDefaults.ExpressionConfig.IsIntegerDivision,
                        mapContext.Configuration.EngineDefaults.ExpressionConfig.IsDivisionByZeroReturnsNull);
            }
            else if (expr is PropertyValueExpression)
            {
                ...
                if (indexDot == -1)
                {
                    ...
                    {
                        ExprTableAccessNode tableNodeX = new ExprTableAccessNodeTopLevel(prop.PropertyName);
                    ...
                    if (variableMetaDataX != null)
                    {
                        ...
                        var message = VariableServiceUtil.CheckVariableContextName(mapContext.ContextName, variableMetaDataX);
                        if (message != null)
                        {
                ...
                var tableNode = ASTTableExprHelper.CheckTableNameGetExprForSubproperty(mapContext.TableService, stream, property);
                if (tableNode != null)
                {
                ...
                {
                    var contextDescriptor = mapContext.ContextManagementService.GetContextDescriptor(mapContext.ContextName);
                    if (contextDescriptor != null && contextDescriptor.ContextPropertyRegistry.IsContextPropertyPrefix(stream))
                    {
            ...
            else if (expr is RelationalOpExpression)
            {
                ...
                else if (op.Operator.ToUpper().Trim() == "IS")
                {
                    return new ExprEqualsNodeImpl(false, true);
                }
                else if (op.Operator.ToUpper().Trim() == "IS NOT")
                {
            ...
            else if (expr is ConstantExpression)
            {
                ...
                if (op.ConstantType != null)
                {
                    try
                    {
                        constantType = TypeHelper.ResolveType(op.ConstantType);
                    }
                    catch (TypeLoadException e)
                    {
                        throw new EPException("Error looking up class name '" + op.ConstantType + "' to resolve as constant type");
                    }
                }
            ...
            else if (expr is PreviousExpression)
            {
                ...
                return new ExprPreviousNode(EnumHelper.Parse<ExprPreviousNodePreviousType>(prev.ExpressionType.ToString()));
            }
            ...
            else if (expr is MinProjectionExpression)
            {
                ...
                return new ExprMinMaxAggrNode(method.IsDistinct, MinMaxTypeEnum.MIN, expr.Children.Count > 1, method.IsEver);
            }
            else if (expr is MaxProjectionExpression)
            {
                var method = (MaxProjectionExpression)expr;
                return new ExprMinMaxAggrNode(method.IsDistinct, MinMaxTypeEnum.MAX, expr.Children.Count > 1, method.IsEver);
            }
            ...
            else if (expr is SubstitutionParameterExpressionBase)
            {
                ...
                if (!(node.IsSatisfied))
                {
                    if (node is SubstitutionParameterExpressionIndexed)
                    {
                        ...
                        throw new EPException("Substitution parameter value for index " + indexed.Index + " not set, please provide a value for this parameter");
                    }
                    var named = (SubstitutionParameterExpressionNamed) node;
                    throw new EPException("Substitution parameter value for name '" + named.Name + "' not set, please provide a value for this parameter");
                }
            ...
            else if (expr is SingleRowMethodExpression)
            {
                ...
                string functionName = chain[0].Name;
                ...
                {
                    throw new ArgumentException("Function name '" + functionName + "' cannot be resolved to a single-row function: " + e.Message, e);
                }
            ...
            else if (expr is PlugInProjectionExpression)
            {
                ...
                var exprNode = ASTAggregationHelper.TryResolveAsAggregation(mapContext.EngineImportService, node.IsDistinct, node.FunctionName, mapContext.PlugInAggregations, mapContext.EngineURI);
                if (exprNode == null)
                {
            ...
            else if (expr is CrontabParameterExpression)
            {
                ...
                if (cronParam.ItemType == ScheduleItemType.LASTDAY)
                {
                    @operator = CronOperatorEnum.LASTDAY;
                }
                else if (cronParam.ItemType == ScheduleItemType.WEEKDAY)
                {
                    @operator = CronOperatorEnum.WEEKDAY;
                }
                else if (cronParam.ItemType == ScheduleItemType.LASTWEEKDAY)
                {
            ...
            else if (expr is DotExpression)
            {
                ...
                // determine table use
                IList<ExprChainedSpec> workChain = new List<ExprChainedSpec>(chain);
                string tableNameCandidate = workChain[0].Name;
                var pair = ASTTableExprHelper.CheckTableNameGetLibFunc(mapContext.TableService, mapContext.EngineImportService, mapContext.PlugInAggregations,
                ...
                if (chain.Count == 1)
                {
                    ...
                    var declared = ExprDeclaredHelper.GetExistsDeclaredExpr(name, chain[0].Parameters, mapContext.ExpressionDeclarations.Values, mapContext.ExprDeclaredService, mapContext.ContextDescriptor);
                    if (declared != null)
                    {
                        return declared;
                    }
                    var script = ExprDeclaredHelper.GetExistsScript(mapContext.Configuration.EngineDefaults.ScriptsConfig.DefaultDialect,
            ...
            else if (expr is TableAccessExpression)
            {
                ...
                {
                    ExprNode exprNode = MapExpressionDeep(b.OptionalAggregate, mapContext);
        ...
        private static IList<Expression> UnmapExpressionDeep(IList<ExprNode> expressions, StatementSpecUnMapContext unmapContext)
        {
        ...
        private static MatchRecognizeRegEx UnmapExpressionFlatRowregex(RowRegexExprNode expr, StatementSpecUnMapContext unmapContext)
        {
            ...
            else if (expr is RowRegexExprNodeAtom)
            {
                ...
                return new MatchRecognizeRegExAtom(atom.Tag, atom.NFAType.Xlate<MatchRecognizePatternElementType>(), repeat);
            }
        ...
        private static Expression UnmapExpressionFlat(ExprNode expr, StatementSpecUnMapContext unmapContext)
        {
            ...
            else if (expr is ExprContextPropertyNode)
            {
                ...
                return new PropertyValueExpression(ContextPropertyRegistryConstants.CONTEXT_PREFIX + "." + prop.PropertyName);
            }
            ...
            else if (expr is ExprConstantNodeImpl)
            {
                ...
                {
                    constantType = constNode.ConstantType.FullName;
                }
            ...
            else if (expr is ExprCastNode)
            {
                ...
                return new CastExpression(node.TypeIdentifier);
            }
            ...
            else if (expr is ExprTimePeriod)
            {
                ...
                return new TimePeriodExpression(node.HasYear, node.HasMonth, node.HasWeek, node.HasDay, node.HasHour, node.HasMinute, node.HasSecond, node.HasMillisecond);
            }
            ...
            else if (expr is ExprDeclaredNode)
            {
                ...
                var dotExpr = new DotExpression();
                dotExpr.Add(declNode.Prototype.Name,
            ...
            else if (expr is ExprTableAccessNode)
            {
                ...
                if (table is ExprTableAccessNodeTopLevel)
                {
                    ...
                    return new TableAccessExpression(topLevel.TableName, UnmapExpressionDeep(topLevel.ChildNodes, unmapContext), null, null);
                }
                if (table is ExprTableAccessNodeSubprop)
                {
                    ...
                    {
                        return new TableAccessExpression(sub.TableName, UnmapExpressionDeep(sub.ChildNodes, unmapContext), sub.SubpropName, null);
                    }
                }
                ...
                if (table is ExprTableAccessNodeSubpropAccessor)
                {
                    ...
                    {
                        var dotExpression = new DotExpression();
                        dotExpression.Add(table.TableName + "." + sub.SubpropName, Collections.GetEmptyList<Expression>(), true);
                        var @params = UnmapExpressionDeep(sub.AggregateAccessMultiValueNode.ChildNodes, unmapContext);
                    ...
                    else
                    {
                        ...
                        return new TableAccessExpression(sub.TableName, UnmapExpressionDeep(sub.ChildNodes, unmapContext), sub.SubpropName, aggregate);
                    }
                }
            }
            throw new ArgumentException("Could not map expression node of type " + expr.GetType().Name);
        ...
        private static void MapFrom(FromClause fromClause, StatementSpecRaw raw, StatementSpecMapContext mapContext)
        {
            ...
            foreach (var stream in fromClause.Streams)
            {
                ...
                else if (stream is SQLStream)
                {
                    ...
                    spec = new DBStatementStreamSpec(sqlStream.StreamName, views,
                ...
                else if (stream is PatternStream)
                {
                    ...
                    spec = new PatternStreamSpecRaw(child, views, patternStream.StreamName, options, flags.IsSuppressSameEventMatches, flags.IsDiscardPartialsOnMatch);
                }
                else if (stream is MethodInvocationStream)
                {
                    var methodStream = (MethodInvocationStream)stream;
                    var expressions = methodStream.ParameterExpressions.Select(expr => MapExpressionDeep(expr, mapContext)).ToList();
                    ...
                    spec = new MethodStreamSpec(methodStream.StreamName, views, "method",
        ...
        private static EvalFactoryNode MapPatternEvalFlat(PatternExpr eval, StatementSpecMapContext mapContext)
        {
            ...
            else if (eval is PatternFollowedByExpr)
            {
                ...
                return mapContext.PatternNodeFactory.MakeFollowedByNode(maxExpr, mapContext.Configuration.EngineDefaults.PatternsConfig.MaxSubexpressions != null);
            }
            ...
            else if (eval is PatternFilterExpr)
            {
                ...
                return mapContext.PatternNodeFactory.MakeFilterNode(filterSpec, filterExpr.TagName, filterExpr.OptionalConsumptionLevel);
            }
            else if (eval is PatternObserverExpr)
            {
                ...
                return mapContext.PatternNodeFactory.MakeObserverNode(new PatternObserverSpec(observer.Namespace, observer.Name, expressions));
            }
            else if (eval is PatternGuardExpr)
            {
                ...
                return mapContext.PatternNodeFactory.MakeGuardNode(new PatternGuardSpec(guard.Namespace, guard.Name, expressions));
            }
        ...
        private static PatternExpr UnmapPatternEvalFlat(EvalFactoryNode eval, StatementSpecUnMapContext unmapContext)
        {
            ...
            else if (eval is EvalMatchUntilFactoryNode)
            {
                ...
                var low = matchUntilNode.LowerBounds != null ? UnmapExpressionDeep(matchUntilNode.LowerBounds, unmapContext) : null;
                var high = matchUntilNode.UpperBounds != null ? UnmapExpressionDeep(matchUntilNode.UpperBounds, unmapContext) : null;
                var single = matchUntilNode.SingleBound != null ? UnmapExpressionDeep(matchUntilNode.SingleBound, unmapContext) : null;
        ...
        private static FilterSpecRaw MapFilter(Filter filter, StatementSpecMapContext mapContext)
        {
            ...
            if (filter.OptionalPropertySelects != null)
            {
                ...
                foreach (var propertySelect in filter.OptionalPropertySelects)
                {
                    ...
                    evalSpec.Add(new PropertyEvalAtom(splitterExpr, propertySelect.OptionalSplitExpressionTypeName, propertySelect.OptionalAsName, selectSpec, exprNodeWhere));
                }
            }
        ...
        private static Filter UnmapFilter(FilterSpecRaw filter, StatementSpecUnMapContext unmapContext)
        {
            ...
            if (filter.OptionalPropertyEvalSpec != null)
            {
                ...
                foreach (var atom in filter.OptionalPropertyEvalSpec.Atoms)
                {
                    SelectClause selectClause = null;
                    if (atom.OptionalSelectClause != null && !atom.OptionalSelectClause.SelectExprList.IsEmpty())
                    {
                        selectClause = UnmapSelect(atom.OptionalSelectClause, SelectClauseStreamSelectorEnum.ISTREAM_ONLY, unmapContext);
                    }
        ...
        private static IList<ExpressionDeclaration> UnmapExpressionDeclarations(ExpressionDeclDesc expr, StatementSpecUnMapContext unmapContext)
        {
        ...
        {
            return new ExpressionDeclaration(desc.Name, desc.ParametersNames, UnmapExpressionDeep(desc.Inner, unmapContext), desc.IsAlias);
        }

        private static IList<ScriptExpression> UnmapScriptExpressions(IList<ExpressionScriptProvided> scripts, StatementSpecUnMapContext unmapContext)
        {
            if (scripts == null || scripts.IsEmpty())
            {
                return Collections.GetEmptyList<ScriptExpression>();
            }
            return scripts.Select(script => UnmapScriptExpression(script, unmapContext)).ToList();
        }
        ...
        private static ScriptExpression UnmapScriptExpression(ExpressionScriptProvided script, StatementSpecUnMapContext unmapContext)
        {
            ...
            return new ScriptExpression(script.Name, script.ParameterNames, script.Expression, returnType, script.OptionalDialect);
        }
        ...
        public static IList<AnnotationDesc> MapAnnotations(IList<AnnotationPart> annotations)
        {
            ...
            if (annotations != null)
            {
                result = annotations.Select(MapAnnotation).ToList();
            }
            else
            {
        ...
        private static void MapExpressionDeclaration(IList<ExpressionDeclaration> expressionDeclarations, StatementSpecRaw raw, StatementSpecMapContext mapContext)
        {
        ...
        {
            return new ExpressionDeclItem(decl.Name,
        ...
        private static void MapScriptExpressions(IList<ScriptExpression> scriptExpressions, StatementSpecRaw raw, StatementSpecMapContext mapContext)
        {
        ...
        {
            var returnType = decl.OptionalReturnType != null ? decl.OptionalReturnType.Replace("[]", "") : null;
            var isArray = decl.OptionalReturnType != null && decl.OptionalReturnType.Contains("[]");
            return new ExpressionScriptProvided(decl.Name, decl.ExpressionText, decl.ParameterNames, returnType, isArray, decl.OptionalDialect);
        }
        ...
        private static void MapSQLParameters(FromClause fromClause, StatementSpecRaw raw, StatementSpecMapContext mapContext)
        {
            ...
            foreach (var stream in fromClause.Streams)
            {
                ...
                {
                    throw new EPRuntimeException("Error parsing SQL placeholder expression '" + sqlStream.SqlWithSubsParams + "': ");
                }
                ...
                foreach (var fragment in sqlFragments)
                {
                    ...
                    if (expression.ToUpper() == DatabasePollingViewableFactory.SAMPLE_WHERECLAUSE_PLACEHOLDER)
                    {
                        continue;
                    }
                    ...
                    var rawSqlExpr = EPAdministratorHelper.CompileEPL(toCompile, expression, false, null, SelectClauseStreamSelectorEnum.ISTREAM_ONLY,
                            mapContext.EngineImportService, mapContext.VariableService, mapContext.SchedulingService, mapContext.EngineURI, mapContext.Configuration, mapContext.PatternNodeFactory, mapContext.ContextManagementService, mapContext.ExprDeclaredService, mapContext.TableService);
        ...
        private static IList<ExprChainedSpec> MapChains(IList<DotExpressionItem> pairs, StatementSpecMapContext mapContext)
        {
            return pairs.Select(item => new ExprChainedSpec(item.Name, MapExpressionDeep(item.Parameters, mapContext), item.IsProperty)).ToList();
        }

        private static IList<DotExpressionItem> UnmapChains(IList<ExprChainedSpec> pairs, StatementSpecUnMapContext unmapContext, bool isProperty)
        {
            return pairs.Select(chain => new DotExpressionItem(chain.Name, UnmapExpressionDeep(chain.Parameters, unmapContext), chain.IsProperty)).ToList();
        }
    }
} // end of namespace
