...
namespace com.espertech.esper.epl.core
{
    ...
    public class MethodPollingViewableFactory
    {
                                                                   ...
                                                                   StatementContext statementContext)
        {
            ...
            if (methodStreamSpec.ClassName == null && methodStreamSpec.MethodName != null) {
                ...
                if (scriptsByName != null) {
                    scriptExpression = ExprDeclaredHelper.GetExistsScript(statementContext.ConfigSnapshot.EngineDefaults.Scripts.DefaultDialect, methodStreamSpec.MethodName, methodStreamSpec.Expressions, scriptsByName, statementContext.ExprDeclaredService);
                }
            }
            ...
            try {
                if (scriptExpression != null) {
                    ...
                    methodReflection = engineImportService.ResolveNonStaticMethodOverloadChecked(variableMetaData.VariableType, methodStreamSpec.MethodName);
                    ...
                    try {
                        ...
                        throw new ExprValidationException("Failed to find user-defined function '" + methodStreamSpec.MethodName + "': " + ex.Message, ex);
                    }
            ...
            if (methodReflection != null) {
                ...
                if ((beanClass == typeof(void)) || (beanClass == typeof(Void)) || (TypeHelper.IsBuiltinDataType(beanClass))) {
                    throw new ExprValidationException("Invalid return type for static method '" + methodReflection.Name + "' of class '" + methodStreamSpec.ClassName + "', expecting a Java class");
                }
                if (methodReflection.ReturnType.IsArray && methodReflection.ReturnType.GetElementType() != typeof(EventBean)) {
                    beanClass = methodReflection.ReturnType.GetElementType();
                }
    
                isCollection = TypeHelper.IsImplementsInterface(beanClass, typeof(Collection));
                ...
                if ((TypeHelper.IsImplementsInterface(methodReflection.ReturnType, typeof(Map))) ||
                        (methodReflection.ReturnType.IsArray && TypeHelper.IsImplementsInterface(methodReflection.ReturnType.GetElementType(), typeof(Map))) ||
                        (isCollection && TypeHelper.IsImplementsInterface(collectionClass, typeof(Map))) ||
                        (isIterator && TypeHelper.IsImplementsInterface(iteratorClass, typeof(Map)))) {
                    MethodMetadataDesc metadata;
                    if (variableMetaData != null) {
                        metadata = GetCheckMetadataVariable(methodStreamSpec.MethodName, variableMetaData, variableReader, engineImportService, typeof(Map));
                    } else {
                        metadata = GetCheckMetadataNonVariable(methodStreamSpec.MethodName, methodStreamSpec.ClassName, engineImportService, typeof(Map));
                    }
                ...
                if (methodReflection.ReturnType == typeof(Object[]) ||
                    ...
                    MethodMetadataDesc metadata;
                    if (variableMetaData != null) {
                        metadata = GetCheckMetadataVariable(methodStreamSpec.MethodName, variableMetaData, variableReader, engineImportService, typeof(LinkedHashMap));
                    } else {
                        metadata = GetCheckMetadataNonVariable(methodStreamSpec.MethodName, methodStreamSpec.ClassName, engineImportService, typeof(LinkedHashMap));
                    }
                ...
                if ((methodReflection.ReturnType.IsArray && methodReflection.ReturnType.GetElementType() == typeof(EventBean)) ||
                    ...
                    string typeName = methodStreamSpec.EventTypeName == null ? eventTypeNameProvidedUDFOrScript : methodStreamSpec.EventTypeName;
                    eventType = EventTypeUtility.RequireEventType("Method", methodReflection.Name, eventAdapterService, typeName);
                    ...
                    eventType = eventAdapterService.AddBeanType(beanClass.Name, beanClass, false, true, true);
                }
                ...
                string eventTypeName = methodStreamSpec.EventTypeName == null ? scriptExpression.EventTypeNameAnnotation : methodStreamSpec.EventTypeName;
                eventType = EventTypeUtility.RequireEventType("Script", scriptExpression.Script.Name, eventAdapterService, eventTypeName);
            }
    
            // get configuration for cache
            string configName = methodProviderClass != null ? methodProviderClass.Name : methodStreamSpec.MethodName;
            ...
            // metadata
            var meta = new MethodPollingViewableMeta(methodProviderClass, isStaticMethod, mapType, oaType, invocationTarget, strategy, isCollection, isIterator, variableReader, variableName, eventTypeWhenMethodReturnsEventBeans, scriptExpression);
        ...
        private static MethodMetadataDesc GetCheckMetadataVariable(
            ...
            Type metadataClass)
        {
            MethodInfo typeGetterMethod = GetRequiredTypeGetterMethodCanNonStatic(methodName, null, variableMetaData.VariableType, engineImportService, metadataClass);
    
            if (Modifier.IsStatic(typeGetterMethod.Modifiers)) {
        ...
        private static MethodMetadataDesc GetCheckMetadataNonVariable(
            ...
            Type metadataClass)
        {
            MethodInfo typeGetterMethod = GetRequiredTypeGetterMethodCanNonStatic(methodName, className, null, engineImportService, metadataClass);
            ...
            Type metadataClass)
        {
            ...
            try {
                if (clazzWhenAvailable != null) {
                    ...
                    typeGetterMethod = engineImportService.ResolveMethodOverloadChecked(classNameWhenNoClass, getterMethodName, new Type[0], new bool[0], new bool[0]);
                }
            } catch (Exception e) {
            ...
            if (fail) {
                throw new ExprValidationException("Getter method '" + typeGetterMethod.Name + "' does not return " + TypeHelper.GetTypeNameFullyQualPretty(metadataClass));
            }
