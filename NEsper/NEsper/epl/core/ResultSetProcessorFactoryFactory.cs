...
namespace com.espertech.esper.epl.core
{
    ...
    public class ResultSetProcessorFactoryFactory
    {
            ...
            ConfigurationInformation configurationInformation)
        {
            ...
            var allowRollup = statementSpec.GroupByExpressions != null && 
            ...
            var validationContext = new ExprValidationContext(
                typeService, 
                stmtContext.MethodResolutionService, viewResourceDelegate,
                ...
                false,
                allowRollup, true, resettableAggs, intoTableName, false); 
            ...
            if (optionalHavingNode != null) {
                optionalHavingNode = ExprNodeUtility.GetValidatedSubtree(
            ...
            for (var i = 0; i < orderByList.Count; i++)
            {
            	...
            	var validatedOrderBy = new OrderByItem(ExprNodeUtility.GetValidatedSubtree(
                    ExprNodeOrigin.ORDERBY, orderByNode, validationContext), isDescending);
            ...
            var havingAggregateExprNodes = new List<ExprAggregateNode>();
            ...
            // Analyze rollup
            var groupByInfo = AnalyzeValidateGroupBy(statementSpec.GroupByExpressions, validationContext);
            var groupByNodesValidated = groupByInfo == null ? new ExprNode[0] : groupByInfo.ExprNodes;
            var groupByRollupDesc = groupByInfo == null ? null : groupByInfo.RollupDesc;
            ...
            var aggregationServiceFactory = AggregationServiceFactoryFactory.GetService(
                ...
                hasGroupBy,
                statementSpec.Annotations, stmtContext.VariableService, 
                typeService.EventTypes.Length > 1, false,
                statementSpec.FilterRootNode, statementSpec.HavingExprRootNode,
                stmtContext.AggregationServiceFactoryService, 
                typeService.EventTypes, 
                stmtContext.MethodResolutionService, groupByRollupDesc,
            ...
            if (stmtContext.ConfigSnapshot != null) {
                useCollatorSort = stmtContext.ConfigSnapshot.EngineDefaults.LanguageConfig.IsSortUsingCollator;
            }
            ...
            if (allInGroupBy && allInSelect && localGroupByMatchesGroupBy)
            {
                ...
                var iterableUnboundConfig = configurationInformation.EngineDefaults.ViewResourcesConfig.IsIterableUnbound;
                var iterateUnbounded = noDataWindowSingleStream && (iterableUnboundConfig || AnnotationUtil.FindAttribute(statementSpec.Annotations, typeof(IterableUnboundAttribute)) != null);
    
                Log.Debug(".getProcessor Using ResultSetProcessorRowPerGroup");
                ResultSetProcessorFactory factoryX;
                if (groupByRollupDesc != null) {
                    var perLevelExpression = GetRollUpPerLevelExpressions(statementSpec, groupByNodesValidated, groupByRollupDesc, stmtContext, selectExprEventTypeRegistry, evaluatorContextStmt, insertIntoDesc, typeService, validationContext);
                    factoryX = new ResultSetProcessorRowPerGroupRollupFactory(perLevelExpression, groupByNodesValidated, groupByEval, isSelectRStream, isUnidirectional, outputLimitSpec, orderByProcessorFactory != null, noDataWindowSingleStream, groupByRollupDesc, typeService.EventTypes.Length > 1, isHistoricalOnly, iterateUnbounded);
                }
                else {
                    factoryX = new ResultSetProcessorRowPerGroupFactory(selectExprProcessor, groupByNodesValidated, groupByEval, optionHavingEval, isSelectRStream, isUnidirectional, outputLimitSpec, orderByProcessorFactory != null, noDataWindowSingleStream, isHistoricalOnly, iterateUnbounded);
                }
                return new ResultSetProcessorFactoryDesc(factoryX, orderByProcessorFactory, aggregationServiceFactory);
            }
            ...
            var factory = new ResultSetProcessorAggregateGroupedFactory(selectExprProcessor, groupByNodesValidated, groupByEval, optionHavingEval, isSelectRStream, isUnidirectional, outputLimitSpec, orderByProcessorFactory != null, isHistoricalOnly);
        ...
        private static GroupByRollupInfo AnalyzeValidateGroupBy(GroupByClauseExpressions groupBy, ExprValidationContext validationContext)
        {
            ...
            try {
                var hook = (GroupByRollupPlanHook) TypeHelper.GetAnnotationHook(validationContext.Annotations, HookType.INTERNAL_GROUPROLLUP_PLAN, typeof(GroupByRollupPlanHook), null);
                if (hook != null) {
            ...
            ExprValidationContext validationContext)
        {
            ...
            for (var i = 0; i < groupByNodesValidated.Length; i++) {
                propsPerGroupByExpr[i] = ExprNodeUtility.GetGroupByPropertiesValidateHasOne(
                    new ExprNode[] {groupByNodesValidated[i]});
            }
            ...
            for (var i = 0; i < numLevels; i++) {
                ...
                processors[i] = SelectExprProcessorFactory.GetProcessor(
                    Collections.GetEmptyList<int>(),
                    selectClause, false, insertIntoDesc, null, 
                    statementSpec.ForClauseSpec, typeService,
                    stmtContext.EventAdapterService, 
                    stmtContext.StatementResultService,
                    stmtContext.ValueAddEventService, 
        ...
        private static OrderByElement[] RewriteRollupOrderBy(
            OrderByItem[] items,
            ...
            foreach (var node in identVisitor.ExprProperties) {
                ...
                foreach (ExprNodePropOrStreamDesc rolledupProp in rolledupProps.Properties) {
                    ...
                    if (rolledupProp.StreamNum == node.Second.StreamId &&
                        Equals(prop.PropertyName, node.Second.ResolvedPropertyName))
                    {
                        rewrite = true;
                        break;
                    }
        ...
        private static int[] GetGroupExprCombination(ExprNode[] groupByNodes, ExprNode[] childNodes)
        {
            ...
            foreach (var child in childNodes) {
                ...
                for (var i = 0; i < groupByNodes.Length; i++) {
                    if (ExprNodeUtility.DeepEquals(child, groupByNodes[i])) {
                        if (indexes.Contains(i)) {
                            throw new ExprValidationException("Duplicate expression '" + child.ToExpressionStringMinPrecedenceSafe() + "' among grouping function parameters");
                        }
                        indexes.Add(i);
                        found = true;
                    }
                ...
                if (!found) {
                    throw new ExprValidationException("Failed to find expression '" + child.ToExpressionStringMinPrecedenceSafe() + "' among group-by expressions");
                }
            }
            ...
            ExprValidationContext validationContext)
        {
            for (var i = 0; i < selectClauseSpec.SelectExprList.Length; i++)
            {
                ...
                if (element is SelectClauseExprCompiledSpec)
                {
                    ...
                    var validatedExpression = ExprNodeUtility.GetValidatedSubtree(
        ...
        private static void ValidateHaving(
            ...
            ExprNode havingNode)
        {
            var aggregateNodesHaving = new List<ExprAggregateNode>();
            ...
            OrderByItem[] orderByUnexpanded)
        {
            ...
            // copy list to modify
            IList<OrderByItem> expanded = orderByUnexpanded.Select(item => item.Copy()).ToList();
        ...
        internal class GroupByRollupInfo
        {
            internal GroupByRollupInfo(ExprNode[] exprNodes, AggregationGroupByRollupDesc rollupDesc)
            {
                ExprNodes = exprNodes;
                RollupDesc = rollupDesc;
            }

            internal ExprNode[] ExprNodes;
            internal AggregationGroupByRollupDesc RollupDesc;
        }
