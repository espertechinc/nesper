	...
	public class ExprEnumExceptIntersectUnion {
	    ...
	    internal class ExprEnumInvalid : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            TryInvalidCompile(env, epl, "Failed to validate select-clause expression 'contained.union(prevwindow(s1))': Enumeration method 'union' expects event type '" + typeof(SupportBean_ST0).Name + "' but receives event type 'SupportBean'");
	    ...
	    private static IDictionary<string, object> MakeMap(string key, object value) {
	        IDictionary<string, object> map = new LinkedHashMap<string, object>();
	        map.Put(key, value);
	        return map;
	    }

	    private static IDictionary<string, object> MakeMap(string key, object value, string key2, object value2) {
	        IDictionary<string, object> map = MakeMap(key, value);
	        map.Put(key2, value2);
	        return map;
	    }

	    private static void SendAndAssert(RegressionEnvironment env, string metaOne, string metaTwo, bool expected) {
