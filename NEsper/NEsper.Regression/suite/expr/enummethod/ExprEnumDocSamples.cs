	...
	public class ExprEnumDocSamples
	{
		...
		internal class ExprEnumSubquery : RegressionExecution
		{
			public void Run(RegressionEnvironment env)
			{
				...
				Zone[] zones = env.Listener("s0").AssertOneGetNewAndReset().Get("zones").UnwrapIntoArray<Zone>();
			...
			object expected)
		{
			if (expected is object[]) {
				...
				ValidateWithVerifier(
					...
					result => {
						object[] returned = result.UnwrapIntoArray<object>();
						EPAssertionUtil.AssertEqualsExactOrder(expected.UnwrapIntoArray<object>(), returned);
					});
			}
			else {
