	...
	public class EPLOtherUpdateIStream
	{
		...
		internal class EPLOtherUpdateInsertDirectBeanTypeInheritance : RegressionExecution
		{
			public void Run(RegressionEnvironment env)
			{
				var path = new RegressionPath();
				var epl =
					"create schema BaseInterface as " + typeof(BaseInterface).FullName + ";\n" +
					"create schema BaseOne as " + typeof(BaseOne).FullName + ";\n" +
					"create schema BaseOneA as " + typeof(BaseOneA).FullName + ";\n" +
					"create schema BaseOneB as " + typeof(BaseOneB).FullName + ";\n" +
					"create schema BaseTwo as " + typeof(BaseTwo).FullName + ";\n";
		...
		private static void RunAssertionSetMapPropsBean(RegressionEnvironment env)
		{
			...
			var path = new RegressionPath();
			env.CompileDeployWBusPublicType("create schema MyMapPropEvent as " + typeof(MyMapPropEvent).FullName, path);
			...
			EventRepresentationChoice eventRepresentationEnum)
		{
			...
			if (eventRepresentationEnum.IsObjectArrayEvent()) {
				...
				var schema = SupportAvroUtil
					.GetAvroSchema(env.Runtime.EventTypeService.GetEventType(env.DeploymentId("type"), "MyNWInfraTypeWithMapProp"))
					.AsRecordSchema();
				var @event = new GenericRecord(schema);
				@event.Put("myarray", Arrays.AsList(0, 0, 0, 0, 0));
				@event.Put("mymap", new Dictionary<string, object>());
			...
			EPAssertionUtil.AssertPropsPerRow(env.GetEnumerator("window"), "simple,mymap('abc'),myarray[2]".SplitCsv(), new[] {
				new object[] {"A", 10, 10}
			});
