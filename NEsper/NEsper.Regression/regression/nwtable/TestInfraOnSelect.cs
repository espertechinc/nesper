...
namespace com.espertech.esper.regression.nwtable
{
    ...
    public class TestInfraOnSelect : IndexBackingTableInfo
    {
        ...
        private void PurgeServiceProvider()
        {
            if (_epService != null)
                _epService.Dispose();
        ...
        private void ServiceProviderScope(Action action)
        {
            PurgeServiceProvider();
        ...
        public void TestOnSelectIndexChoice() {
            ServiceProviderScope(() => RunAssertionOnSelectIndexChoice(true));
            ServiceProviderScope(() => RunAssertionOnSelectIndexChoice(false));
        }
        ...
        public void TestInvalid() {
            ServiceProviderScope(() => RunAssertionInvalid(true));
            ServiceProviderScope(() => RunAssertionInvalid(false));
        }
    
        [Test]
        public void TestSelectCondition() {
            ServiceProviderScope(() => RunAssertionSelectCondition(true));
            ServiceProviderScope(() => RunAssertionSelectCondition(false));
        }
    
        [Test]
        public void TestSelectJoinColumnsLimit() {
            ServiceProviderScope(() => RunAssertionSelectJoinColumnsLimit(true));
            ServiceProviderScope(() => RunAssertionSelectJoinColumnsLimit(false));
        }
    
        [Test]
        public void TestSelectAggregation() {
            ServiceProviderScope(() => RunAssertionSelectAggregation(true));
            ServiceProviderScope(() => RunAssertionSelectAggregation(false));
        }
    
        [Test]
        public void TestSelectAggregationCorrelated() {
            ServiceProviderScope(() => RunAssertionSelectAggregationCorrelated(true));
            ServiceProviderScope(() => RunAssertionSelectAggregationCorrelated(false));
        }
    
        [Test]
        public void TestSelectAggregationGrouping() {
            ServiceProviderScope(() => RunAssertionSelectAggregationGrouping(true));
            ServiceProviderScope(() => RunAssertionSelectAggregationGrouping(false));
        }
    
        [Test]
        public void TestSelectCorrelationDelete() {
            ServiceProviderScope(() => RunAssertionSelectCorrelationDelete(true));
            ServiceProviderScope(() => RunAssertionSelectCorrelationDelete(false));
        }
    
        [Test]
        public void TestPatternCorrelation() {
            ServiceProviderScope(() => RunAssertionPatternCorrelation(true));
            ServiceProviderScope(() => RunAssertionPatternCorrelation(false));
        }
        ...
        private void RunAssertionInvalid(bool namedWindow) {
            var stmtTextCreate = namedWindow ?
                    "create window MyInfra#keepall as select * from " + typeof(SupportBean).FullName :
            ...
            SupportMessageAssertUtil.TryInvalid(_epService, "on " + typeof(SupportBean_A).FullName + " select * from MyInfra where sum(IntPrimitive) > 100",
                    "Error validating expression: An aggregate function may not appear in a WHERE clause (use the HAVING clause) [on " + Name.Of<SupportBean_A>() + " select * from MyInfra where sum(IntPrimitive) > 100]");
    
            SupportMessageAssertUtil.TryInvalid(_epService, "on " + typeof(SupportBean_A).FullName + " insert into MyStream select * from DUMMY",
                    "Named window or table 'DUMMY' has not been declared [on " + Name.Of<SupportBean_A>() + " insert into MyStream select * from DUMMY]");
    
            SupportMessageAssertUtil.TryInvalid(_epService, "on " + typeof(SupportBean_A).FullName + " select prev(1, TheString) from MyInfra",
                    "Error starting statement: Failed to validate select-clause expression 'prev(1,TheString)': Previous function cannot be used in this context [on " + Name.Of<SupportBean_A>() + " select prev(1, TheString) from MyInfra]");
