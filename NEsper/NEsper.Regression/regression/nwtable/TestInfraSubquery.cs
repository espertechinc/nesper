    ...
    public class TestInfraSubquery 
    {
        ...
        private void RunAssertionUncorrelatedSubqueryAggregation(bool namedWindow)
        {
            ...
            string stmtTextCreate = namedWindow ?
                    "create window MyInfra#keepall as select TheString as a, LongPrimitive as b from " + typeof(SupportBean).FullName :
        ...
        public void RunAssertionInvalidSubquery(bool namedWindow)
        {
            string eplCreate = namedWindow ?
                "create window MyInfra#keepall as " + typeof(SupportBean).FullName :
                "create table MyInfra(TheString string)";
            epService.EPAdministrator.CreateEPL(eplCreate);
    
            try
            {
                epService.EPAdministrator.CreateEPL("select (select TheString from MyInfra#lastevent) from MyInfra");
                Assert.Fail();
            }
            catch (EPException ex)
            {
                if (namedWindow) {
                    Assert.AreEqual("Error starting statement: Failed to plan subquery number 1 querying MyInfra: Consuming statements to a named window cannot declare a data window view onto the named window [select (select TheString from MyInfra#lastevent) from MyInfra]", ex.Message);
                }
                else {
