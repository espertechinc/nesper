...
namespace com.espertech.esper.regression.nwtable
{
    ...
    public class TestNamedWindowJoin : IndexBackingTableInfo
    {
        ...
        public void TestWindowUnidirectionalJoin() {
            ...
            _epService.EPAdministrator.CreateEPL("select window(win.*) as c0 from S0#lastevent as s0, MyWindow as win");
        ...
        {
            EnumHelper.ForEach<EventRepresentationChoice>(RunAssertionInnerJoinLateStart);
        }
        ...
        private void SendPortfolio(EventRepresentationChoice eventRepresentationEnum, string portfolio, string product)
        {
            if (eventRepresentationEnum.IsObjectArrayEvent())
            {
                _epService.EPRuntime.SendEvent(new Object[] { portfolio, product }, "Portfolio");
            }
            else if (eventRepresentationEnum.IsMapEvent())
            {
                var theEvent = new LinkedHashMap<string, Object>();
            ...
            {
                var theEvent = new GenericRecord(SupportAvroUtil.GetAvroSchema(_epService, "Portfolio").AsRecordSchema());
                theEvent.Put("portfolio", portfolio);
                theEvent.Put("product", product);
                _epService.EPRuntime.SendEventAvro(theEvent, "Portfolio");
            }
            else
            {
                Assert.Fail();
        ...
        public void TestRightOuterJoinLateStart()
        {
            ...
            var stmtTextCreate = "create window WindowLeave#time(6000) as select timeLeave, id, location from " + typeof(SupportQueueLeave).FullName;
        ...
        {
            // create window
            var stmtTextCreate = "create window MyWindow#groupwin(TheString, IntPrimitive)#length(3) as select TheString, IntPrimitive, BoolPrimitive from " + typeof(SupportBean).FullName;
            ...
            var stmtTextSelect = "select TheString, IntPrimitive, count(BoolPrimitive) as cntBool, symbol " +
                                    "from MyWindow full outer join " + typeof(SupportMarketDataBean).FullName + "#keepall " +
                                    "on TheString = symbol " +
        ...
        {
            // create window
            var stmtTextCreate = "create window MyWindow#keepall as select TheString as a, IntPrimitive as b from " + typeof(SupportBean).FullName;
            var stmtCreate = _epService.EPAdministrator.CreateEPL(stmtTextCreate);
        ...
        public void TestJoinBetweenNamed()
        {
            ...
            // create window
            var stmtTextCreateOne = "create window MyWindowOne#keepall as select TheString as a1, IntPrimitive as b1 from " + typeof(SupportBean).FullName;
            var stmtCreateOne = _epService.EPAdministrator.CreateEPL(stmtTextCreateOne);
            stmtCreateOne.AddListener(_listenerWindow);
    
            // create window
            var stmtTextCreateTwo = "create window MyWindowTwo#keepall as select TheString as a2, IntPrimitive as b2 from " + typeof(SupportBean).FullName;
        ...
        public void TestJoinBetweenSameNamed()
        {
            ...
            // create window
            var stmtTextCreateOne = "create window MyWindow#keepall as select TheString as a, IntPrimitive as b from " + typeof(SupportBean).FullName;
            var stmtCreateOne = _epService.EPAdministrator.CreateEPL(stmtTextCreateOne);
            stmtCreateOne.AddListener(_listenerWindow);
        ...
        public void TestUnidirectional()
        {
            ...
            _epService.EPAdministrator.CreateEPL("create window MyWindow#keepall select * from SupportBean");
            _epService.EPAdministrator.CreateEPL("insert into MyWindow select * from SupportBean");
    
            var stmtOne = _epService.EPAdministrator.CreateEPL("select w.* from MyWindow w unidirectional, SupportBean_A#lastevent s where s.id = w.TheString");
