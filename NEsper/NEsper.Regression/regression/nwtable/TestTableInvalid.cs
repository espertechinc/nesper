    ...
    public class TestTableInvalid
    {
        ...
        public void TestInvalidAggMatchSingleFunc()
        {
            ...
            TryInvalidAggMatch("var1", "sum(double)", false, "sum(IntPrimitive)",
                    "Error starting statement: Incompatible aggregation function for table 'var1' column 'value', expecting 'sum(double)' and received 'sum(IntPrimitive)': The required parameter type is " + Name.Of<double>(false) + " and provided is " + Name.Of<int>(false) + " [");
        ...
        public void TestInvalid()
        {
            ...
            _epService.EPAdministrator.CreateEPL("create window MyNamedWindow#keepall as select * from SupportBean");
            ...
            SupportMessageAssertUtil.TryInvalid(_epService, "into table aggvar_ungrouped select count(*) as total from SupportBean unidirectional, SupportBean_S0#keepall",
            ...
            SupportMessageAssertUtil.TryInvalid(_epService, "select * from SupportBean#time(aggvar_ungrouped.total sec)",
                    "Error starting statement: Error in view 'time', Invalid parameter expression 0 for TimeInMillis view: Failed to validate view parameter expression 'aggvar_ungrouped.total seconds': Invalid use of table access expression, expression 'aggvar_ungrouped' is not allowed here [select * from SupportBean#time(aggvar_ungrouped.total sec)]");
            ...
            // view use
            SupportMessageAssertUtil.TryInvalid(_epService, "select * from aggvar_grouped_string#time(30)",
                    "Views are not supported with tables");
            SupportMessageAssertUtil.TryInvalid(_epService, "select (select * from aggvar_ungrouped#keepall) from SupportBean",
                    "Views are not supported with tables [");
            // contained use
