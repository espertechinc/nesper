...
namespace com.espertech.esper.regression.nwtable
{
	...
	public class TestTableRollup
    {
	    ...
	    public void TestRollupTwoDim()
        {
	        ...
	        _epService.EPAdministrator.CreateEPL("into table MyTable insert into MyStream select sum(col) as total from MyEvent#length(3) group by rollup(k0,k1)");
	    ...
	    public void TestGroupingSetThreeDim() {
	        ...
	        _epService.EPAdministrator.CreateEPL("into table MyTable insert into MyStream select sum(col) as total from MyEvent#length(3) group by grouping sets(k0,k1,k2)");
