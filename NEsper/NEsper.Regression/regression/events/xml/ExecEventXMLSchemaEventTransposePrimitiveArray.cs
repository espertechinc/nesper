...
namespace com.espertech.esper.regression.events.xml
{
    public class ExecEventXMLSchemaEventTransposePrimitiveArray : RegressionExecution {
        ...
        public override void Run(EPServiceProvider epService) {
            string schemaURI = typeof(ExecEventXMLSchemaEventTransposePrimitiveArray).ClassLoader.GetResource(CLASSLOADER_SCHEMA_URI).ToString();
            ...
            eventTypeMeta.SchemaResource = schemaURI;
            eventTypeMeta.EventSenderValidatesRoot = false;
            epService.EPAdministrator.Configuration.AddEventType("TestNested2", eventTypeMeta);
    
            // try array property in select
            EPStatement stmtInsert = epService.EPAdministrator.CreateEPL("select * from TestNested2#lastevent");
            ...
            EPAssertionUtil.AssertEqualsExactOrder((int?[]) theEvent.Get("prop3"), new Object[]{2, null, 4});
            SupportEventTypeAssertionUtil.AssertConsistency(theEvent);
    
            // try array property nested
            EPStatement stmtSelect = epService.EPAdministrator.CreateEPL("select nested3.* from ABCType#lastevent");
            SupportXML.SendDefaultEvent(epService.EPRuntime, "test");
