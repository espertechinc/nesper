...
{
    [TestFixture]
	public class TestBeanLegacyEvent
    {
	    ...
	    public void TestBeanAccessor() {
	        TryBeanAccessor(CodeGenerationEnum.ENABLED);
	    }

        [Test]
	    public void TestBeanAccessorNoCodeGen() {
	        TryBeanAccessor(CodeGenerationEnum.DISABLED);
	    }
	    ...
	    private void TryExplicitOnlyAccessors(CodeGenerationEnum codeGeneration) {
	        ...
	        try {
	            // invalid statement, object-style getters not exposed
	    ...
	    private void TryFinalClass(CodeGenerationEnum codeGeneration) {
	        ...
	        var statementText = "select IntPrimitive from " + typeof(SupportBeanFinal).FullName + "#length(5)";
