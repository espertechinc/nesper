...
namespace com.espertech.esper.regression.resultset
{
	...
	public class TestOutputLimitEventPerGroup 
	{
	    ...
	    public void TestOutputFirstHavingJoinNoJoin()
        {
	        ...
	        var stmtTextJoin = "select TheString, sum(IntPrimitive) as value from MyWindow mv, SupportBean_A#keepall a where a.id = mv.TheString " +
	        ...
	        var stmtTextOrderJoin = "select TheString, sum(IntPrimitive) as value from MyWindow mv, SupportBean_A#keepall a where a.id = mv.TheString " +
	    ...
	    public void TestGroupBy_All()
	    {
	    	...
	    	var statementString = "select irstream Symbol, sum(Price) from " + eventName + "#length(5) group by Symbol output all every 5 events";
	    ...
	    public void TestGroupBy_Default()
	    {
	        var fields = "Symbol,sum(Price)".Split(',');
	    	var eventName = typeof(SupportMarketDataBean).FullName;
	    	var statementString = "select irstream Symbol, sum(Price) from " + eventName + "#length(5) group by Symbol output every 5 events";
	    	var statement = _epService.EPAdministrator.CreateEPL(statementString);
	    	var updateListener = new SupportUpdateListener();
	    	statement.AddListener(updateListener);
