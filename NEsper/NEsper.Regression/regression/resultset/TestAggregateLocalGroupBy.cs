...
namespace com.espertech.esper.regression.resultset
{
	...
	public class TestAggregateLocalGroupBy 
	{
	    ...
	    public void TestInvalid()
        {
	        ...
	        var eplSubq = "select (select sum(IntPrimitive, group_by:TheString) from SupportBean#keepall) from SupportBean_S0";
	    ...
	    private void RunAssertionUngroupedUnidirectionalJoin() {
	        var epl = "select TheString, sum(IntPrimitive, group_by:TheString) as c0 from SupportBean#keepall, SupportBean_S0 unidirectional";
	        _epService.EPAdministrator.CreateEPL(epl).AddListener(_listener);
