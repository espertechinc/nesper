...
namespace com.espertech.esper.regression.resultset
{
    ...
    public class TestOutputLimitRowForAll 
	{
	    ...
	    private void RunAssertionOuputLastWithInsertInto(bool hinted)
        {
	        ...
	        var eplInsert = hint + "insert into MyStream select sum(IntPrimitive) as thesum from SupportBean#keepall " +
	    ...
	    {
	        var stmtText = "select count(*) as cnt from " + typeof(SupportBean).FullName + "#time(10 seconds) output every 10 seconds";
	        var stmt = epService.EPAdministrator.CreateEPL(stmtText);
	    ...
	    {
	        var stmtText = "select count(*) as cnt from " + typeof(SupportBean).FullName + "#time_batch(10 seconds) output every 10 seconds";
	        var stmt = epService.EPAdministrator.CreateEPL(stmtText);
	        var listener = new SupportUpdateListener();
	        stmt.AddListener(listener);
	    ...
	    public void TestLimitSnapshot()
	    {
	        ...
	        SendTimer(0);
	        var selectStmt = "select count(*) as cnt from " + typeof(SupportBean).FullName + "#time(10 seconds) where IntPrimitive > 0 output snapshot every 1 seconds";
