...
namespace com.espertech.esper.regression.resultset
{
	...
	public class TestOutputLimitCrontabWhen 
	{
	    ...
	    public void TestOutputCrontabAtVariable() {
	        ...
	        var expression = "select * from MarketData#lastevent output at (*/VFREQ, VMIN:VMAX, *, *, *)";
	    ...
	    public void TestOutputCrontabAt()
        {
            // every 15 minutes 8am to 5pm
	        SendTimeEvent(1, 17, 10, 0, 0);
	        var expression = "select * from MarketData#lastevent output at (*/15, 8:17, *, *, *)";
	        var stmt = _epService.EPAdministrator.CreateEPL(expression);
	        stmt.AddListener(_listener);
	        RunAssertionCrontab(1, stmt);
	    }
	    ...
	    public void TestOutputCrontabAtOMCreate()
        {
	        ...
	        model.FromClause = FromClause.Create(FilterStream.Create("MarketData").AddView("lastevent"));
	    ...
	    public void TestOutputWhenThenExpression()
	    {
	        ...
	        var expression = "select Symbol from MarketData#length(2) output when myvar=1 then set myvar=0, count_insert_var=count_insert";
	        var stmt =  _epService.EPAdministrator.CreateEPL(expression);
	        RunAssertion(1, stmt);

	        var model = new EPStatementObjectModel();
	        model.SelectClause = SelectClause.Create("Symbol");
	        model.FromClause = FromClause.Create(FilterStream.Create("MarketData").AddView("length", Expressions.Constant(2)));
	        ...
	        var outputLast = "select Symbol from MarketData#length(2) output last when myvar=1 ";
	        ...
	        var expressionTotal = "select TheString from SupportBean#length(2) output when count_insert_total = var_cnt_total or count_remove_total > 2";
	    ...
	    public void TestOutputWhenExpression()
	    {
	        ...
	        var expression = "select Symbol from MarketData#length(2) output when myint = 1 and mystring like 'F%'";
	        var stmt =  _epService.EPAdministrator.CreateEPL(expression);
	        var subscriber = new SupportSubscriber();
	        stmt.Subscriber = subscriber;
	    ...
	    {
	        var expression = "select Symbol from MarketData#length(2) output when count_insert >= 3";
	        var stmt =  _epService.EPAdministrator.CreateEPL(expression);
	        var subscriber = new SupportSubscriber();
	        stmt.Subscriber = subscriber;
	    ...
	    {
	        var expression = "select Symbol from MarketData#length(2) output when count_remove >= 2";
	        var stmt =  _epService.EPAdministrator.CreateEPL(expression);
	        var subscriber = new SupportSubscriber();
	        stmt.Subscriber = subscriber;
	    ...
	    {
	        SendTimeEvent(1, 8, 0, 0, 0);
	        var expression = "select Symbol from MarketData#length(2) output when current_timestamp - last_output_timestamp >= 2000";
	        var stmt =  _epService.EPAdministrator.CreateEPL(expression);
	        var subscriber = new SupportSubscriber();
	        stmt.Subscriber = subscriber;
