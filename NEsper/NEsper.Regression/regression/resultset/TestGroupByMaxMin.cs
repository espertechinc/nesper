...
namespace com.espertech.esper.regression.resultset
{
	...
	public class TestGroupByMaxMin 
	{
	    ...
	    {
	        var stmtText = "select Symbol from " + typeof(SupportMarketDataBean).FullName + "#time(5 sec) " +
	    ...
	    {
	        var stmtText = "select Symbol, min(Volume) as mymin from " + typeof(SupportMarketDataBean).FullName + "#length(5) " +
	                          "having Volume > min(Volume) * 1.3";
