...
namespace com.espertech.esper.regression.context
{
    public class ExecContextNested : RegressionExecution {
        ...
        private void RunAssertionIterateTargetedCP(EPServiceProvider epService) {
            ...
            EPAssertionUtil.AssertPropsPerRowAnyOrder(stmt.GetEnumerator(), stmt.SafeGetEnumerator(), fields, expectedAll);
            ...
            var allIds = new SupportSelectorById(new HashSet<>(Collections.List(0, 1, 2, 3, 4, 5)));
            ...
            var secondOne = new SupportSelectorCategory(Collections.Singleton("grp3"));
            var nestedSelector = new SupportSelectorNested(Collections.SingletonList(new ContextPartitionSelector[]{firstOne, secondOne}));
            EPAssertionUtil.AssertPropsPerRowAnyOrder(stmt.Iterator(nestedSelector), stmt.SafeIterator(nestedSelector), fields, new Object[][]{new object[] {"S0_2", "grp3", "E1", 2}});
    
            var firstTwo = new SupportSelectorFilteredInitTerm("S0_1");
            var secondTwo = new SupportSelectorCategory(Collections.Singleton("grp1"));
            ...
            EPAssertionUtil.AssertPropsPerRowAnyOrder(stmt.Iterator(nestedSelectorTwo), stmt.SafeIterator(nestedSelectorTwo), fields, new Object[][]{new object[] {"S0_2", "grp3", "E1", 2}, new object[] {"S0_1", "grp1", "E2", -1}});
            ...
            try {
                ...
                Assert.IsTrue("message: " + ex.Message, ex.Message.StartsWith("Invalid context partition selector, expected an implementation class of any of [ContextPartitionSelectorAll, ContextPartitionSelectorById, ContextPartitionSelectorNested] interfaces but received com."));
            }
            ...
            EPAssertionUtil.AssertPropsPerRowAnyOrder(stmtSelect.GetEnumerator(), stmtSelect.SafeGetEnumerator(), fieldsSelect, expectedRows);
            ...
            EPAssertionUtil.AssertPropsPerRowAnyOrder(stmtSelect.Iterator(nestedSelectorSelect), stmtSelect.SafeIterator(nestedSelectorSelect), fieldsSelect, new Object[][]{new object[] {"i3", "l1", "b2", 1L}});
        ...
        private void RunAssertionIterator(EPServiceProvider epService) {
            ...
            EPAssertionUtil.AssertPropsPerRow(stmtUser.SafeGetEnumerator(), fields, expected);
        ...
        private static void AssertFilters(EPServiceProvider epService, bool allowIsolation, string expected, EPStatementSPI spiStmt) {
            ...
            FilterSet set = filterSPI.Take(Collections.Singleton(spiStmt.StatementId));
