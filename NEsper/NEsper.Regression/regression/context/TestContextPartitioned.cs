...
namespace com.espertech.esper.regression.context
{
    ...
    public class TestContextPartitioned
    {
        ...
        public void TestIterateTargetedCP()
        {
            ...
            var stmt = _epService.EPAdministrator.CreateEPL("@Name('StmtOne') context PartitionedByString select context.key1 as c0, Sum(IntPrimitive) as c1 from SupportBean#length(5)");
        ...
        private void TryInvalidCreateWindow() {
            try {
                _epService.EPAdministrator.CreateEPL("context Ctx create window MyInvalidWindow#unique(p00) as SupportBean_S0");
                Assert.Fail();
            }
        ...
        public void TestSegmentedSubqueryNamedWindowIndexShared() {
            _epService.EPAdministrator.CreateEPL("@Name('context') create context SegmentedByString partition by TheString from SupportBean");
            _epService.EPAdministrator.CreateEPL("@Hint('enable_window_subquery_indexshare') create window MyWindow#keepall as SupportBean_S0");
        ...
        public void TestSegmentedSubqueryNamedWindowIndexUnShared() {
            _epService.EPAdministrator.CreateEPL("@Name('context') create context SegmentedByString partition by TheString from SupportBean");
            _epService.EPAdministrator.CreateEPL("create window MyWindow#keepall as SupportBean_S0");
            _epService.EPAdministrator.CreateEPL("insert into MyWindow select * from SupportBean_S0");
        ...
        public void TestSegmentedViews()
        {
            ...
            _epService.EPAdministrator.CreateEPL("@Name('Out') context MyCtx select * from SupportBean#expr(not trigger) for grouped_delivery(TheString)");
        ...
        {
            _epService.EPAdministrator.CreateEPL("create context MyContext partition by theString from SupportBean");
            EPStatement stmt = _epService.EPAdministrator.CreateEPL("context MyContext select Count(*) as cnt from SupportBean");
            stmt.AddListener(_listener);

            _epService.EPRuntime.SendEvent(new SupportBean(null, 10));
            Assert.AreEqual(1L, _listener.AssertOneGetNewAndReset().Get("cnt"));

            _epService.EPRuntime.SendEvent(new SupportBean(null, 20));
            Assert.AreEqual(2L, _listener.AssertOneGetNewAndReset().Get("cnt"));

            _epService.EPRuntime.SendEvent(new SupportBean("A", 30));
            Assert.AreEqual(1L, _listener.AssertOneGetNewAndReset().Get("cnt"));
        ...
        {
            _epService.EPAdministrator.CreateEPL("create context MyContext partition by theString, intBoxed, intPrimitive from SupportBean");
            EPStatement stmt = _epService.EPAdministrator.CreateEPL("context MyContext select Count(*) as cnt from SupportBean");
            stmt.AddListener(_listener);

            SendSBEvent(_epService, "A", null, 1);
            Assert.AreEqual(1L, _listener.AssertOneGetNewAndReset().Get("cnt"));

            SendSBEvent(_epService, "A", null, 1);
            Assert.AreEqual(2L, _listener.AssertOneGetNewAndReset().Get("cnt"));

            SendSBEvent(_epService, "A", 10, 1);
            Assert.AreEqual(1L, _listener.AssertOneGetNewAndReset().Get("cnt"));
        ...
        private static void SendSBEvent(EPServiceProvider engine, String theString, int? intBoxed, int intPrimitive)
        {
            SupportBean bean = new SupportBean(theString, intPrimitive);
