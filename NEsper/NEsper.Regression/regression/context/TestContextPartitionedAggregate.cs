...
namespace com.espertech.esper.regression.context
{
    ...
    public class TestContextPartitionedAggregate
    {
        ...
        public void TestAccessOnly()
        {
            ...
            var eplGroupedAccess = "@Name('S2') context SegmentedByString select TheString,IntPrimitive,window(LongPrimitive) as col1 from SupportBean#keepall sb group by IntPrimitive";
        ...
        public void TestGroupByEventPerGroupBatchContextProp()
        {
            ...
            var fieldsOne = "IntPrimitive,count(*)".Split(',');
            var stmtOne = _epService.EPAdministrator.CreateEPL("@Name('A') context SegmentedByString select IntPrimitive, count(*) from SupportBean#length_batch(2) group by IntPrimitive order by IntPrimitive asc");
            stmtOne.Events += _listener.Update;
            ...
            var fieldsTwo = "TheString,IntPrimitive,count(*)".Split(',');
            var stmtTwo = _epService.EPAdministrator.CreateEPL("@Name('B') context SegmentedByString select TheString, IntPrimitive, count(*) from SupportBean#length_batch(2) group by IntPrimitive order by TheString, IntPrimitive asc");
            stmtTwo.Events += _listener.Update;
