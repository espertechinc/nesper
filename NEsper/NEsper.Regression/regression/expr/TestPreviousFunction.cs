    ...
    public class TestPreviousFunction
    {
        ...
        public void TestPrevCountStarWithStaticMethod()
        {
            String text = "select irstream count(*) as total, " +
                    "prev(" + typeof(TestPreviousFunction).FullName + ".IntToLong(count(*)) - 1, Price) as firstPrice from " + typeof(SupportMarketDataBean).FullName + "#time(60)";
            EPStatement stmt = _epService.EPAdministrator.CreateEPL(text);
            stmt.Events += _listener.Update;
        ...
        public void TestPerGroupTwoCriteria()
        {
            ...
            _epService.EPAdministrator.Configuration.AddEventType<SupportBean>();
            _epService.EPAdministrator.CreateEPL("select prev(5,IntPrimitive) as val0 from SupportBean#groupwin(TheString)#length(5)").Events += _listener.Update;
