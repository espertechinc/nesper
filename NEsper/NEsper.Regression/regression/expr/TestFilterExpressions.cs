...
namespace com.espertech.esper.regression.expr
{
	...
	public class TestFilterExpressions 
	{
	    ...
	    public void TestShortCircuitEvalAndOverspecified()
        {
			...
			Assert.IsFalse(_listener.IsInvoked, "Subscriber should not have received result(s)");
	        stmt.Dispose();
	    ...
	    public void TestRelationalOpConstantFirst()
        {
	        ...
	        _epService.EPAdministrator.CreateEPL("@Name('A') select * from TestEvent where 4 < x").AddListener(_listener);
	    ...
	    private void RunAssertionRewriteWhereNamedWindow() {
	        var stmtWindow = _epService.EPAdministrator.CreateEPL("create window NamedWindowA#length(1) as SupportBean");
	    ...
	    {
	        // Test for Esper-114
	        var text = "select * from " + typeof(SupportBeanComplexProps).FullName + "(simpleProperty='1')#keepall as s0" +
	    ...
	    public void TestNotEqualsNull()
        {
	        ...
	        _epService.EPAdministrator.CreateEPL("select * from SupportBean where TheString != 'A'").AddListener(listeners[0]);
	        _epService.EPAdministrator.CreateEPL("select * from SupportBean where TheString != 'A' or IntPrimitive != 0").AddListener(listeners[1]);
	        _epService.EPAdministrator.CreateEPL("select * from SupportBean where TheString = 'A'").AddListener(listeners[2]);
	        _epService.EPAdministrator.CreateEPL("select * from SupportBean where TheString = 'A' or IntPrimitive != 0").AddListener(listeners[3]);
	        ...
	        _epService.EPAdministrator.CreateEPL("select " +
	                ...
	                "TheString = 'A' and IntPrimitive != 0 as val5 from SupportBean").AddListener(_listener);
	        ...
	        _epService.EPAdministrator.CreateEPL("select * from SupportBean where TheString is null").AddListener(listeners[0]);
	        _epService.EPAdministrator.CreateEPL("select * from SupportBean where TheString is null or IntPrimitive != 0").AddListener(listeners[1]);
	        _epService.EPAdministrator.CreateEPL("select * from SupportBean where TheString is not null").AddListener(listeners[2]);
	        _epService.EPAdministrator.CreateEPL("select * from SupportBean where TheString is not null or IntPrimitive != 0").AddListener(listeners[3]);
	        ...
	        // filter expression
	        _epService.EPAdministrator.CreateEPL("select * from SupportBean(TheString is null)").AddListener(listeners[0]);
	        _epService.EPAdministrator.CreateEPL("select * from SupportBean where TheString = null").AddListener(listeners[1]);
	        _epService.EPAdministrator.CreateEPL("select * from SupportBean(TheString = null)").AddListener(listeners[2]);
	        _epService.EPAdministrator.CreateEPL("select * from SupportBean(TheString is not null)").AddListener(listeners[3]);
	        _epService.EPAdministrator.CreateEPL("select * from SupportBean where TheString != null").AddListener(listeners[4]);
	        _epService.EPAdministrator.CreateEPL("select * from SupportBean(TheString != null)").AddListener(listeners[5]);
        ...
        private void TryPattern3Stream(
            string text,
            int?[] intBoxedA,
            double?[] doubleBoxedA,
            int?[] intBoxedB,
            ...
            bool[] expected)
	    {
	        ...
	        for (var i = 0; i < intBoxed.Length; i++)
	        {
                ...
                Assert.AreEqual(_listener.GetAndClearIsInvoked(), expected[i], "failed at index " + i);
	        }
	    ...
	    public void TestInvalid()
        {
            ...
            TryInvalid(
                "select * from pattern [a=" + typeof(SupportBean).FullName + " -> b=" +
            ...
            TryInvalid(
                "select * from pattern [a=" + typeof(SupportBean).FullName
	    ...
	    private void SendBean(string fieldName, object value)
	    {
	        ...
	        {
	            theEvent.BoolPrimitive = (Boolean) value;
	        }
	    ...
	    public class MyEvent
        {
	        ...
	        {
	            get { throw new EPRuntimeException("I should not have been called!"); }
	        }
