...
namespace com.espertech.esper.regression.expr
{
	...
	public class TestNewStructExpr 
	{
	    ...
	    {
            EnumHelper.ForEach<EventRepresentationChoice>(RunAssertionNewWRepresentation);
	    }
	    ...
	    public void TestInvalid() {
	        ...
	        TryInvalid(epl, "Error starting statement: Failed to validate select-clause expression 'case when true then new{col1=\"a\"} e...(54 chars)': Incompatible case-when return types by new-operator in case-when number 1: Type by name 'Case-when number 1' in property 'col1' expected System.String but receives " + Name.Of<int>() + " [select case when true then new { col1 = 'a' } else new { col1 = 1 } end from SupportBean]");
