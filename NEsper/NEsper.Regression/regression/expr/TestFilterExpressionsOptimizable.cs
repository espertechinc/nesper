...
namespace com.espertech.esper.regression.expr
{
	...
	public class TestFilterExpressionsOptimizable 
	{
	    ...
	    public void SetUp()
	    {
	        ...
	        func.FunctionClassName = this.GetType().FullName;
	        ...
	        config.PlugInSingleRowFunctions.Add(func);
        ...
        [Test]
	    public void TestInAndNotInKeywordMultivalue()
	    {
	        _epService.EPAdministrator.Configuration.AddEventType<MyEventInKeywordValue>();
	        ...
	        RunAssertionInKeyword("collOfInt", new MyEventInKeywordValue(Collections.List(1, 2)));
	        ...
	        RunAssertionNotInKeyword("collOfInt", new MyEventInKeywordValue(Collections.List(1, 2)));

	        RunAssertionInArrayContextProvided();

	        SupportMessageAssertUtil.TryInvalid(_epService, "select * from pattern[every a=MyEventInKeywordValue -> SupportBean(intPrimitive in (a.longs))]",
	            "Implicit conversion from datatype '" + Name.Of<long>() + "' to '" + Name.Of<int>() + "' for property 'intPrimitive' is not allowed (strict filter type coercion)");
	    }

        [Test]
	    public void TestOptimizablePerf()
        {
            _epService.EPAdministrator.Configuration.AddPlugInSingleRowFunction("libSplit", typeof(MyLib).FullName, "LibSplit", FilterOptimizableEnum.ENABLED);
            _epService.EPAdministrator.Configuration.AddPlugInSingleRowFunction("libE1True", typeof(MyLib).FullName, "LibE1True", FilterOptimizableEnum.ENABLED);
	    ...
	    private void RunAssertionVariableAndSeparateThread()
	    {
	        _epService.EPAdministrator.Configuration.AddVariable("myCheckServiceProvider", typeof(MyCheckServiceProvider), null);
	        _epService.EPRuntime.SetVariableValue("myCheckServiceProvider", new MyCheckServiceProvider());

	        EPStatement epStatement = _epService.EPAdministrator.CreateEPL("select * from SupportBean(myCheckServiceProvider.Check())");
	        epStatement.AddListener(_listener);
	        ...
	        Assert.IsTrue(latch.Await(TimeSpan.FromSeconds(10)));
	    }
	    ...
	    public void TestOptimizableInspectFilter()
        {
            ...
            _epService.EPAdministrator.Configuration.AddPlugInSingleRowFunction("funcOne", typeof(MyLib).FullName, "LibSplit", FilterOptimizableEnum.DISABLED);
            ...
            _epService.EPAdministrator.Configuration.AddPlugInSingleRowFunction("funcTwo", typeof(MyLib).FullName, "LibSplit", FilterOptimizableEnum.ENABLED);
	    ...
	    private void RunAssertionInKeyword(string field, MyEventInKeywordValue prototype)
	    {
	        RunAssertionInKeywordPlain(field, prototype);
	        RunAssertionInKeywordPattern(field, prototype);
	    }

	    private void RunAssertionNotInKeyword(string field, MyEventInKeywordValue prototype)
	    {
	        RunAssertionNotInKeywordPlain(field, prototype);
	        RunAssertionNotInKeywordPattern(field, prototype);
	    }

	    private void RunAssertionInKeywordPlain(string field, MyEventInKeywordValue prototype)
	    {

	        EPStatement stmt = _epService.EPAdministrator.CreateEPL("select * from MyEventInKeywordValue#keepall where 1 in (" + field + ")");
	        stmt.AddListener(_listener);

	        _epService.EPRuntime.SendEvent(SerializableObjectCopier.Copy(prototype));
	        Assert.IsTrue(_listener.IsInvokedAndReset());

	        stmt.Dispose();
	    }

	    private void RunAssertionNotInKeywordPlain(string field, MyEventInKeywordValue prototype)
	    {

	        EPStatement stmt = _epService.EPAdministrator.CreateEPL("select * from MyEventInKeywordValue#keepall where 1 not in (" + field + ")");
	        stmt.AddListener(_listener);
	    ...
	    private void RunAssertionInKeywordPattern(string field, MyEventInKeywordValue prototype)
	    {

	        EPStatementSPI stmt = (EPStatementSPI) _epService.EPAdministrator.CreateEPL("select * from pattern[every a=MyEventInKeywordValue -> SupportBean(intPrimitive in (a." + field + "))]");
	        stmt.AddListener(_listener);
	        ...
	        AssertFilterMulti("SupportBean", stmt, new FilterItem[][] {
	            new FilterItem[] {new FilterItem("intPrimitive", FilterOperator.IN_LIST_OF_VALUES)},
	        });

	        stmt.Dispose();
	    }

	    private void RunAssertionNotInKeywordPattern(string field, MyEventInKeywordValue prototype)
	    {

	        EPStatementSPI stmt = (EPStatementSPI) _epService.EPAdministrator.CreateEPL("select * from pattern[every a=MyEventInKeywordValue -> SupportBean(intPrimitive not in (a." + field + "))]");
	        stmt.AddListener(_listener);
	        ...
	        AssertFilterMulti("SupportBean", stmt, new FilterItem[][] {
	            new FilterItem[] {new FilterItem("intPrimitive", FilterOperator.NOT_IN_LIST_OF_VALUES)},
	        });

	        stmt.Dispose();
	    }

	    private void AssertInKeywordReceivedPattern(Object @event, int intPrimitive, bool expected)
	    {
	    ...
	    private void RunAssertionInArrayContextProvided()
	    {
	        ...
	        _epService.EPAdministrator.CreateEPL("create context MyContext initiated by MyEventInKeywordValue as mie terminated after 24 hours");

	        EPStatement statementOne = _epService.EPAdministrator.CreateEPL("context MyContext select * from SupportBean#keepall where intPrimitive in (context.mie.ints)");
	        statementOne.AddListener(listenerOne);

	        EPStatementSPI statementTwo = (EPStatementSPI) _epService.EPAdministrator.CreateEPL("context MyContext select * from SupportBean(intPrimitive in (context.mie.ints))");
	        statementTwo.AddListener(listenerTwo);

	        _epService.EPRuntime.SendEvent(new MyEventInKeywordValue(new int[] { 1, 2 }));
	    ...
	    private void RunAssertionSubquery() {
	        var epl = "select (select * from SupportBean_IntAlphabetic(a=1 or b=1)#keepall) as c0 from SupportBean";
	    ...
	    private void RunAssertionBooleanExprAnd() {
	        ...
	        foreach (var filter in filters) {
	            ...
	            var stmt = AssertFilterMulti("SupportBean_StringAlphabetic", epl, new FilterItem[][] {
	    ...
	    private void RunAssertionAndRewriteNotEqualsWithOrConsolidateSecond() {
	        ...
	        foreach (var filter in filters) {
	            ...
	            var stmt = AssertFilterMulti("SupportBean_IntAlphabetic", epl, new FilterItem[][] {
	    ...
	    private void RunAssertionAndRewriteInnerOr() {
	        ...
	        foreach (var filter in filtersAB) {
	            ...
	            var stmt = AssertFilterMulti("SupportBean", epl, new FilterItem[][] {
	    ...
	    {
            EPStatementSPI statementSPI = (EPStatementSPI) _epService.EPAdministrator.CreateEPL(epl);
	    ...
	    private void AssertFilterMulti(String eventTypeName, EPStatementSPI statementSPI, FilterItem[][] expected)
	    {
	        ...
	        FilterSet set = filterServiceSPI.Take(Collections.SingletonList(statementSPI.StatementId));

	        FilterSetEntry filterSetEntry = null;
	        foreach (FilterSetEntry entry in set.Filters)
	        {
	            if (entry.FilterValueSet.EventType.Name == eventTypeName)
	            {
	    ...
	    private void RunAssertionMethodInvocationContext()
	    {
	        ...
	        _epService.EPAdministrator.CreateEPL("select * from SupportBean e where MyCustomOkFunction(e) = \"OK\"");
	        _epService.EPRuntime.SendEvent(new SupportBean());
	        Assert.AreEqual("default", methodInvocationContextFilterOptimized.EngineURI);
	        ...
	        Assert.IsNull(methodInvocationContextFilterOptimized.StatementName);
	    ...
	    {
	        _epService.EPAdministrator.CreateEPL("@Name('ctx') create context MyContext initiated by SupportBean(theString='A' or intPrimitive=1) terminated after 24 hours");
	        _epService.EPAdministrator.CreateEPL("@Name('select') context MyContext select * from SupportBean").AddListener(_listener);
	    ...
	    {
	        _epService.EPAdministrator.CreateEPL("create context MyContext initiated by SupportBean terminated after 24 hours");
	        _epService.EPAdministrator.CreateEPL("context MyContext select * from SupportBean(theString='A' or intPrimitive=1)").AddListener(_listener);
