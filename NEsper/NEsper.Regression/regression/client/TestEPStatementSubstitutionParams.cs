...
namespace com.espertech.esper.regression.client
{
    ...
    public class TestEPStatementSubstitutionParams 
    {
        ...
        public void TestPattern()
        {
            ...
            Assert.AreEqual("select * from pattern [" + Name.Of<SupportBean>() + "(TheString=\"e1\")]", statement.Text);
    
            prepared.SetObject(1, "e2");
            statement = _epService.EPAdministrator.Create(prepared);
            statement.Events += _listenerTwo.Update;
            Assert.AreEqual("select * from pattern [" + Name.Of<SupportBean>() + "(TheString=\"e2\")]", statement.Text);
            ...
            statement.Dispose();
            prepared = _epService.EPAdministrator.PrepareEPL("create window MyWindow#time(?) as " + typeof(SupportBean).FullName);
            prepared.SetObject(1, 300);
            statement = _epService.EPAdministrator.Create(prepared);
            Assert.AreEqual("create window MyWindow#time(300) as select * from " + Name.Of<SupportBean>() + "", statement.Text);
        }
        ...
        private void RunSimpleTwoParameter(String stmtText, String statementName, bool compareText)
        {
            ...
            if (compareText)
            {
                Assert.AreEqual("select * from " + Name.Of<SupportBean>() + "(TheString=\"e1\" and IntPrimitive=1)", statement.Text);
            }
            ...
            if (compareText)
            {
                Assert.AreEqual("select * from " + Name.Of<SupportBean>() + "(TheString=\"e2\" and IntPrimitive=2)", statement.Text);
            }
        ...
        public void TestInvalidParameterType()
        {
            ...
            {
                SupportMessageAssertUtil.AssertMessage(ex, "Failed to validate filter expression 'TheString=-1': Implicit conversion from datatype '" + Name.Of<int>()  + "' to '" + Name.Of<string>() + "' is not allowed [");
            }
        }
