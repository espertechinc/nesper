	...
	public class TestSubscriberBind 
	{
	    ...
	    public void TestBindings()
        {
            ...
            EnumHelper.ForEach<EventRepresentationChoice>(
                rep => RunAssertionWidening(rep, new SupportSubscriberRowByRowSpecificNStmt()));
            EnumHelper.ForEach<EventRepresentationChoice>(
                rep => RunAssertionWidening(rep, new SupportSubscriberRowByRowSpecificWStmt()));
            ...
            // output limit
            EnumHelper.ForEach<EventRepresentationChoice>(
                rep => RunAssertionOutputLimitNoJoin(rep, new SupportSubscriberRowByRowSpecificNStmt()));
            ...
            // binding-to-map
            EnumHelper.ForEach<EventRepresentationChoice>(
                rep => RunAssertionBindMap(rep, new SupportSubscriberMultirowMapNStmt()));
            EnumHelper.ForEach<EventRepresentationChoice>(
                rep => RunAssertionBindMap(rep, new SupportSubscriberMultirowMapWStmt()));

            // binding-to-objectarray
            EnumHelper.ForEach<EventRepresentationChoice>(
                rep => RunAssertionBindObjectArr(rep, new SupportSubscriberMultirowObjectArrayNStmt()));
            EnumHelper.ForEach<EventRepresentationChoice>(
                rep => RunAssertionBindObjectArr(rep, new SupportSubscriberMultirowObjectArrayWStmt()));
	    ...
	    {
	        var stmtText = "select irstream TheString, IntPrimitive from " + typeof(SupportBean).FullName + "#length_batch(2)";
	    ...
	    {
	        var stmt = _epService.EPAdministrator.CreateEPL(eventRepresentationEnum.GetAnnotationText() + " select irstream TheString, IntPrimitive from SupportBean#unique(TheString)");
	        stmt.Subscriber = subscriber;
            Assert.IsTrue(eventRepresentationEnum.MatchesClass(stmt.EventType.UnderlyingType));
	    ...
	    private void RunAssertionStreamSelectWJoin(SupportSubscriberRowByRowSpecificBase subscriber) {
	        var stmt = _epService.EPAdministrator.CreateEPL("select null, s1, s0 from SupportBean#keepall() as s0, SupportMarketDataBean#keepall() as s1 where s0.TheString = s1.Symbol");
	        stmt.Subscriber = subscriber;
	    ...
	    private void RunAssertionBindWildcardJoin(SupportSubscriberRowByRowSpecificBase subscriber) {
	        var stmt = _epService.EPAdministrator.CreateEPL("select * from SupportBean#keepall() as s0, SupportMarketDataBean#keepall() as s1 where s0.TheString = s1.Symbol");
	        stmt.Subscriber = subscriber;
