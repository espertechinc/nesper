...
namespace com.espertech.esper.regression.epl
{
    ...
    public class TestInsertInto
    {
        ...
        public void TestVariantTwoWildcard() {
            var stmtText = "insert into event1 select * from " + typeof(SupportBean).FullName + "#length(100)";
            var otherText = "select * from default.event1#length(10)";
        ...
        public void TestVariantTwoJoinWildcard() {
            ...
            EnumHelper.ForEach<EventRepresentationChoice>(rep => RunAssertionJoinWildcard(false, rep));
        }
        ...
        public void TestStaggeredWithWildcard() {
            var statementOne = "insert into streamA select * from " + typeof(SupportBeanSimple).FullName + "#length(5)";
            var statementTwo = "insert into streamB select *, MyInt+MyInt as summed, myString||myString as concat from streamA#length(5)";
            var statementThree = "insert into streamC select * from streamB#length(5)";
        ...
        private void RunAssertionJoinWildcard(bool bean, EventRepresentationChoice? rep)
        {
            if (bean)
            {
                _epService.EPAdministrator.Configuration.AddEventType("S0", typeof(SupportBean));
                _epService.EPAdministrator.Configuration.AddEventType("S1", typeof(SupportBean_A));
            }
            else if (rep == null)
            {
                Assert.Fail();
            ...
            {
                _epService.EPAdministrator.Configuration.AddEventType("S0", Collections.SingletonDataMap("theString", typeof(string)));
                _epService.EPAdministrator.Configuration.AddEventType("S1", Collections.SingletonDataMap("id", typeof(string)));
            }
            else if (rep.Value.IsObjectArrayEvent())
            {
                _epService.EPAdministrator.Configuration.AddEventType("S0", new string[] { "theString" }, new Object[] { typeof(string) });
                _epService.EPAdministrator.Configuration.AddEventType("S1", new string[] { "id" }, new Object[] { typeof(string) });
            }
            else if (rep.Value.IsAvroEvent())
            {
                _epService.EPAdministrator.Configuration.AddEventTypeAvro("S0", new ConfigurationEventTypeAvro().SetAvroSchema(SchemaBuilder.Record("S0", TypeBuilder.RequiredString("theString"))));
                _epService.EPAdministrator.Configuration.AddEventTypeAvro("S1", new ConfigurationEventTypeAvro().SetAvroSchema(SchemaBuilder.Record("S1", TypeBuilder.RequiredString("id"))));
            }
            else
            {
                Assert.Fail();
            ...
            // Attach listener to feed
            EPStatement stmtOne = _epService.EPAdministrator.CreateEPL(textOne);
            var listenerOne = new SupportUpdateListener();
            stmtOne.AddListener(listenerOne);
            EPStatement stmtTwo = _epService.EPAdministrator.CreateEPL(textTwo);
            var listenerTwo = new SupportUpdateListener();
            ...
            if (bean)
            {
                ...
                _epService.EPRuntime.SendEvent(eventS1);
            }
            else if (rep.Value.IsMapEvent())
            {
                eventS1 = Collections.SingletonMap<string, object>("id", "myId");
                _epService.EPRuntime.SendEvent((IDictionary<string, object>) eventS1, "S1");
            }
            else if (rep.Value.IsObjectArrayEvent())
            {
                eventS1 = new Object[] { "myId" };
                _epService.EPRuntime.SendEvent((Object[])eventS1, "S1");
            }
            else if (rep.Value.IsAvroEvent())
            {
                var theEvent = new GenericRecord(SupportAvroUtil.GetAvroSchema(_epService, "S1").AsRecordSchema());
                theEvent.Put("id", "myId");
                eventS1 = theEvent;
                _epService.EPRuntime.SendEventAvro(theEvent, "S1");
            }
            else
            {
            ...
            if (bean)
            {
                ...
                _epService.EPRuntime.SendEvent(eventS0);
            }
            else if (rep.Value.IsMapEvent())
            {
                eventS0 = Collections.SingletonMap<string, object>("theString", "myId");
                _epService.EPRuntime.SendEvent((Map)eventS0, "S0");
            }
            else if (rep.Value.IsObjectArrayEvent())
            {
                eventS0 = new Object[] { "myId" };
                _epService.EPRuntime.SendEvent((Object[])eventS0, "S0");
            }
            else if (rep.Value.IsAvroEvent())
            {
                var theEvent = new GenericRecord(SupportAvroUtil.GetAvroSchema(_epService, "S0").AsRecordSchema());
                theEvent.Put("theString", "myId");
                eventS0 = theEvent;
                _epService.EPRuntime.SendEventAvro(theEvent, "S0");
            }
            else
            {
                throw new ArgumentException();
            ...
            stmtOne.Dispose();
            stmtTwo.Dispose();
            _epService.EPAdministrator.Configuration.RemoveEventType("S0", false);
            _epService.EPAdministrator.Configuration.RemoveEventType("S1", false);
            _epService.EPAdministrator.Configuration.RemoveEventType("event2", false);
        }
        ...
        private void RunAssertionWildcardRecast(bool sourceBean, EventRepresentationChoice? sourceType, bool targetBean, EventRepresentationChoice? targetType)
        {
            try
            {
        ...
        private void RunAssertionWildcardRecastInternal(bool sourceBean, EventRepresentationChoice? sourceType, bool targetBean, EventRepresentationChoice? targetType)
        {
            // declare source type
            if (sourceBean)
            {
                _epService.EPAdministrator.CreateEPL("create schema SourceSchema as " + TypeHelper.MaskTypeName<MyP0P1EventSource>());
            }
            else
            {
            ...
            if (targetBean)
            {
                _epService.EPAdministrator.CreateEPL("create schema TargetSchema as " + TypeHelper.MaskTypeName<MyP0P1EventTarget>());
            }
            else
            {
            ...
            EPStatement stmt = _epService.EPAdministrator.CreateEPL("select * from TargetSchema");
            var listener = new SupportUpdateListener();
            ...
            else if (sourceType.Value.IsAvroEvent())
            {
                ...
                record.Put("P0", "a");
                record.Put("P1", 10);
                _epService.EPRuntime.SendEventAvro(record, "SourceSchema");
            }
            else
            {
                Assert.Fail();
