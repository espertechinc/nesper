...
namespace com.espertech.esper.regression.enummethod
{
    ...
    public class TestEnumInvalid
    {
        ...
        public void TestInvalid()
        {
            ...
            SupportMessageAssertUtil.TryInvalid(_epService, epl, "Error starting statement: Failed to validate select-clause expression 'TheString.where()': Error validating enumeration method 'where' parameter 0: Failed to validate declared expression body expression 'x.BoolPrimitive': Failed to resolve property 'x.BoolPrimitive' (property 'x' is an indexed property and requires an index or enumeration method to access values) [select (select TheString from SupportBean#lastevent).where(x=>x.BoolPrimitive) from SupportBean_ST0]");
            ...
            SupportMessageAssertUtil.TryInvalid(_epService, epl, "Error starting statement: Failed to validate select-clause expression 'Contained.firstof().dummy()': Failed to resolve method 'dummy': Could not find enumeration method, date-time method or instance method named 'dummy' in class '" + Name.Of<SupportBean_ST0>() + "' taking no parameters [select Contained.firstof().dummy from SupportBean_ST0_Container]");
        }
    }
}
