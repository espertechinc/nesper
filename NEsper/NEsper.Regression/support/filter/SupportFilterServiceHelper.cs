...
namespace com.espertech.esper.regressionlib.support.filter
{
    public class SupportFilterServiceHelper
    {
            ...
            string stmtName)
        {
            ...
            Assert.AreEqual(count, GetFilterSvcCountAnyType(statement));
        }
        ...
        {
            var spi = ((EPRuntimeSPI) env.Runtime).ServicesContext.FilterServiceSPI;
            return spi.FilterCountApprox;
        }

        public static IDictionary<EventTypeIdPair, IDictionary<int, IList<FilterItem[]>>> GetFilterSvcAllStmt(EPRuntime runtime)
        {
            var deployments = runtime.DeploymentService.Deployments;
            var statements = new HashSet<int>();
            ...
            var filterService = ((EPRuntimeSPI) runtime).ServicesContext.FilterServiceSPI;
            ...
            string eventTypeName)
        {
            ...
            var deployments = runtime.DeploymentService.Deployments;
            var statements = new Dictionary<string, FilterItem>();
            foreach (var deployment in deployments) {
                var info = runtime.DeploymentService.GetDeployment(deployment);
                foreach (var statement in info.Statements) {
        ...
        public static IDictionary<EventTypeIdPair, IDictionary<int, IList<FilterItem[]>>> GetFilterSvcForStatement(EPStatement statement)
        {
            var spi = (EPStatementSPI) statement;
