	...
	public class ResultSetAggregateFirstLastWindow {
	    ...
	    internal class ResultSetAggregateFirstLastWindowGroup : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowNewOnly("s0", fields,
	                new object[][]{
	    ...
	    internal class ResultSetAggregateNoParamChainedAndProperty : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select first().property as val0, first().myMethod() as val1, window() as val2 from SupportEventPropertyWithMethod#lastevent";
	    ...
	    internal class ResultSetAggregateSubquery : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select id, (select window(sb.*) from SupportBean#length(2) as sb) as w from SupportBean_A";
	            env.CompileDeploy(epl).AddListener("s0");
	    ...
	    internal class ResultSetAggregateMethodAndAccessTogether : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@name('s0') select sum(intPrimitive) as si, window(sa.intPrimitive) as wi from SupportBean#length(2) as sa";
	            env.CompileDeploy(epl).AddListener("s0");
	    ...
	    internal class ResultSetAggregateOutputRateLimiting : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select sum(intPrimitive) as si, window(sa.intPrimitive) as wi from SupportBean#keepall as sa output every 2 events";
	            env.CompileDeploy(epl).AddListener("s0");
	    ...
	    internal class ResultSetAggregateTypeAndColNameAndEquivalency : RegressionExecution {
	        ...
	        private void TryAssertionType(RegressionEnvironment env, bool isCheckStatic, AtomicLong milestone) {
	    ...
	    internal class ResultSetAggregateBatchWindow : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsIRPair("s0", fields, new object[]{"E1", Split("E1,E2"), "E2"}, new object[]{null, null, null});
	            env.ListenerReset("s0");
	            ...
	            env.AssertPropsIRPair("s0", fields, new object[]{"E3", Split("E3,E4"), "E4"}, new object[]{"E1", Split("E1,E2"), "E2"});
	            env.ListenerReset("s0");
	            ...
	            env.AssertPropsIRPair("s0", fields, new object[]{"E5", Split("E5,E6"), "E6"}, new object[]{"E3", Split("E3,E4"), "E4"});
	            env.ListenerReset("s0");
	    ...
	    internal class ResultSetAggregateOnDemandQuery : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var qc = env.CompileFAF("select first(intPrimitive) as f, window(intPrimitive) as w, last(intPrimitive) as l from MyWindowFour as s", path);
	            var q = env.Runtime.FireAndForgetService.PrepareQuery(qc);
	            EPAssertionUtil.AssertPropsPerRow(q.Execute().Array, "f,w,l".SplitCsv(),
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertStatement("s0", statement =>{
	                ...
	                for (var i = 0; i < props.Count; i++) {
	                    Assert.AreEqual(i == 4 || i == 5 ? typeof(SupportBean[]) : typeof(SupportBean), props[i].PropertyType);
	                }
	            });
	    ...
	    internal class ResultSetAggregateWindowAndSumWGroup : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowLastNew("s0",
	                fields, new object[][]{new object[]{"E1", 15 + 18 + 19, new long?[]{2 * 15L, 3 * 18L, 4 * 19L}}, new object[] {"E2", null, null}});
	            ...
	            env.AssertPropsPerRowLastNew("s0",
	                fields, new object[][]{new object[]{"E1", 19 + 17, new long?[]{4 * 19L, -1 * 17L}}, new object[] {"E2", 1, new long?[]{1 * 1000L}}});

	            env.UndeployAll();
	        }
	    }

	    private static void TryAssertionGrouped(RegressionEnvironment env, AtomicLong milestone) {
	        var fields = "theString,firststring,firstint,laststring,lastint,allint".SplitCsv();
	    ...
	    private static void TryAssertionFirstLastIndexed(RegressionEnvironment env, AtomicLong milestone) {
	    ...
	    private static void TryAssertionStar(RegressionEnvironment env, AtomicLong milestone) {
	        var fields = "firststar,firststarsb,laststar,laststarsb,windowstar,windowstarsb,firsteverstar,lasteverstar".SplitCsv();
	    ...
	    private static void TryAssertionUngrouped(RegressionEnvironment env, AtomicLong milestone) {
	        var fields = "firststring,firstint,laststring,lastint,allint".SplitCsv();
