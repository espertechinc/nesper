	...
	public class ResultSetAggregationMethodWindow {
	    ...
	    private class ResultSetAggregateWindowNonTable : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@name('s0') select theString, window(*).first() as c0, window(*).last() as c1 from SupportBean#length(3) as sb group by theString";
	            env.CompileDeploy(epl).AddListener("s0");
	            ...
	            env.AssertPropsPerRowNewOnly("s0", fields, new object[][]{new object[] {"B", sb5, sb5}, new object[] {"A", sb3, sb4}});
