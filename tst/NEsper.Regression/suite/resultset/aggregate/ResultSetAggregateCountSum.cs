	...
	public class ResultSetAggregateCountSum {
	    ...
	    private class ResultSetAggregateCountSumInvalid : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var message = "Failed to validate select-clause expression 'XXX': Implicit conversion from datatype 'null' to numeric is not allowed for aggregation function '";
	    ...
	    private class ResultSetAggregateCountDistinctMultikeyWArray : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select count(distinct intOne) as c0, count(distinct {intOne, intTwo}) as c1 from SupportEventWithManyArray#length(3)";
	    ...
	    private class ResultSetAggregateCountHaving : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var statementText = "@name('s0') select irstream sum(intPrimitive) as mysum from SupportBean having sum(intPrimitive) = 2";
	            env.CompileDeploy(statementText).AddListener("s0");
	    ...
	    private class ResultSetAggregateSumHaving : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var statementText = "@name('s0') select irstream count(*) as mysum from SupportBean having count(*) = 2";
	            env.CompileDeploy(statementText).AddListener("s0");
	    ...
	    private class ResultSetAggregateGroupByCountNestedAggregationAvg : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            // test for ESPER-328
	            var epl = "@name('s0') select symbol, count(*) as cnt, avg(count(*)) as val from SupportMarketDataBean#length(3)" +
	            ...
	            env.AssertPropsPerRowNewOnly("s0", "symbol,cnt,val".SplitCsv(), new object[][]{new object[] {"DELL", 2L, 2d}, new object[] {"IBM", 1L, 1d}});
	    ...
	    private static void TryAssertionCount(RegressionEnvironment env, AtomicLong milestone) {
