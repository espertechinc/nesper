...
namespace com.espertech.esper.regressionlib.suite.resultset.outputlimit {
	public class ResultSetOutputLimitSimple {
	    ...
	    private static void RunAssertion9AllNoHavingNoJoin(RegressionEnvironment env, SupportOutputLimitOpt opt, AtomicLong milestone) {
	        var stmtText = opt.GetHint() + "@name('s0') select symbol, volume, price " +
	                       "from SupportMarketDataBean#time(5.5 sec) " +
	    ...
	    private static void RunAssertion10AllNoHavingJoin(RegressionEnvironment env, SupportOutputLimitOpt opt, AtomicLong milestone) {
	        var stmtText = opt.GetHint() + "@name('s0') select symbol, volume, price " +
	                       "from SupportMarketDataBean#time(5.5 sec), " +
	                       "SupportBean#keepall where theString=symbol " +
	                       "output all every 1 seconds";
	    ...
	    internal class ResultSetOutputEveryTimePeriod : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var stmtText = "@name('s0') select symbol from SupportMarketDataBean#keepall output snapshot every 1 day 2 hours 3 minutes 4 seconds 5 milliseconds";
	    ...
	    internal class ResultSetOutputEveryTimePeriodVariable : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var stmtText = "@name('s0') select symbol from SupportMarketDataBean#keepall output snapshot every D days H hours M minutes S seconds MS milliseconds";
	            env.CompileDeploy(stmtText).AddListener("s0");
	    ...
	    internal class ResultSetAggAllHaving : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[]{"S0", 20L}, new object[] {"YAH", 10L}});
	    ...
	    internal class ResultSetIterator : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowIteratorAnyOrder("s0", fields, new object[][]{new object[]{"CAT", 50d}, new object[] {"CAT", 60d}});

	            SendEvent(env, "IBM", 70);
	            env.AssertPropsPerRowIteratorAnyOrder("s0", fields, new object[][]{new object[]{"CAT", 50d}, new object[] {"CAT", 60d}, new object[] {"IBM", 70d}});

	            SendEvent(env, "IBM", 90);
	            env.AssertPropsPerRowIteratorAnyOrder("s0", fields, new object[][]{new object[]{"CAT", 50d}, new object[] {"CAT", 60d}, new object[] {"IBM", 70d}, new object[] {"IBM", 90d}});
	    ...
	    internal class ResultSetSimpleNoJoinAll : RegressionExecution {
	        ...
	        private static void TryAssertionSimpleNoJoinAll(RegressionEnvironment env, SupportOutputLimitOpt opt) {
	            var epl = opt.GetHint() + "@name('s0') select longBoxed " +
	    ...
	    internal class ResultSetLimitSnapshot : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            SendTimer(env, 2000);
	            SendEvent(env, "YAH");
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[]{"IBM"}, new object[] {"MSFT"}, new object[] {"YAH"}});
	            ...
	            SendTimer(env, 10000);
	            SendEvent(env, "s6");
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[]{"IBM"}, new object[] {"MSFT"}, new object[] {"YAH"}, new object[] {"s4"}, new object[] {"s5"}, new object[] {"s6"}});
	            ...
	            SendEvent(env, "s9");
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[]{"YAH"}, new object[] {"s4"}, new object[] {"s5"}, new object[] {"s6"}, new object[] {"s7"}, new object[] {"s8"}, new object[] {"s9"}});

	            SendTimer(env, 14000);
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[]{"s6"}, new object[] {"s7"}, new object[] {"s8"}, new object[] {"s9"}});
	            ...
	            SendTimer(env, 23000);
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[]{"s10"}, new object[] {"s11"}});
	    ...
	    internal class ResultSetLimitSnapshotJoin : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            SendTimer(env, 2000);
	            SendEvent(env, "s2");
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[]{"s0"}, new object[] {"s1"}, new object[] {"s2"}});
	            ...
	            SendTimer(env, 10000);
	            SendEvent(env, "s6");
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[]{"s0"}, new object[] {"s1"}, new object[] {"s2"}, new object[] {"s4"}, new object[] {"s5"}, new object[] {"s6"}});
	            ...
	            SendEvent(env, "s9");
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[]{"s2"}, new object[] {"s4"}, new object[] {"s5"}, new object[] {"s6"}, new object[] {"s7"}, new object[] {"s8"}, new object[] {"s9"}});
	    ...
	    private static void TryAssertion17IStream(RegressionEnvironment env, string stmtText, string outputLimit, AtomicLong milestone) {
	        SendTimer(env, 0);
	        env.CompileDeploy(stmtText).AddListener("s0");
	        ...
	        var execution = new ResultAssertExecution(stmtText, env, expected, ResultAssertExecutionTestSelector.TEST_ONLY_AS_PROVIDED);
	        execution.Execute(false, milestone);
	    }

	    private static void TryAssertion17IRStream(RegressionEnvironment env, string stmtText, string outputLimit, AtomicLong milestone) {
	        SendTimer(env, 0);
	        env.CompileDeploy(stmtText).AddListener("s0");
