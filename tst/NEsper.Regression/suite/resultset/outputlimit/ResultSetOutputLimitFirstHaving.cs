	...
	public class ResultSetOutputLimitFirstHaving {
	    ...
	    private class ResultSetHavingNoAvgOutputFirstEvents : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var query = "@name('s0') select doublePrimitive from SupportBean having doublePrimitive > 1 output first every 2 events";
	    ...
	    private class ResultSetHavingNoAvgOutputFirstMinutes : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var query = "@name('s0') select sum(doublePrimitive) as val0 from SupportBean#length(5) having sum(doublePrimitive) > 100 output first every 2 seconds";
	            env.CompileDeploy(query).AddListener("s0");
	    ...
	    private class ResultSetHavingAvgOutputFirstEveryTwoMinutes : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var query = "@name('s0') select doublePrimitive, avg(doublePrimitive) from SupportBean having doublePrimitive > 2*avg(doublePrimitive) output first every 2 minutes";
	            env.CompileDeploy(query).AddListener("s0");
