	...
	public class ResultSetOutputLimitRowPerGroup {
	    ...
	    internal class ResultSetOutputSnapshotMultikeyWArray : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@name('s0') select theString as c0, longPrimitive as c1, sum(intPrimitive) as c2 from SupportBean group by theString, longPrimitive " +
	    ...
	    internal class ResultSetOutputLastMultikeyWArray : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@name('s0') select theString, longPrimitive, sum(intPrimitive) as thesum from SupportBean#keepall " +
	    ...
	    internal class ResultSetOutputFirstMultikeyWArray : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@name('s0') select sum(value) as thesum from SupportEventWithIntArray group by array output first every 10 seconds";
	            env.CompileDeploy(epl).AddListener("s0");
	    ...
	    internal class ResultSetOutputFirstHavingJoinNoJoin : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var stmtText = "@name('s0') select theString, sum(intPrimitive) as value from MyWindow group by theString having sum(intPrimitive) > 20 output first every 2 events";
	            TryOutputFirstHaving(env, stmtText);

	            var stmtTextJoin = "@name('s0') select theString, sum(intPrimitive) as value from MyWindow mv, SupportBean_A#keepall a where a.id = mv.theString " +
	                               "group by theString having sum(intPrimitive) > 20 output first every 2 events";
	            TryOutputFirstHaving(env, stmtTextJoin);

	            var stmtTextOrder = "@name('s0') select theString, sum(intPrimitive) as value from MyWindow group by theString having sum(intPrimitive) > 20 output first every 2 events order by theString asc";
	            TryOutputFirstHaving(env, stmtTextOrder);

	            var stmtTextOrderJoin = "@name('s0') select theString, sum(intPrimitive) as value from MyWindow mv, SupportBean_A#keepall a where a.id = mv.theString " +
	    ...
	    internal class ResultSetOutputFirstWhenThen : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            SendBeanEvent(env, "E1", 12);
	            env.AssertPropsNew("s0", fields, new object[]{"E1", 33});
	            env.AssertRuntime(runtime => Assert.AreEqual(false, env.Runtime.VariableService.GetVariableValue(null, "varoutone")));
	    ...
	    private static void RunAssertion11AllHavingNoJoin(RegressionEnvironment env, SupportOutputLimitOpt opt, AtomicLong milestone) {
	        var stmtText = opt.GetHint() + "@name('s0') select symbol, sum(price) " +
	                       "from SupportMarketDataBean#time(5.5 sec) " +
	                       "group by symbol " +
	                       "having sum(price) > 50 " +
	                       "output all every 1 seconds";
	    ...
	    internal class ResultSetLimitSnapshot : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[]{"ABC", 14d}, new object[] {"IBM", 16d}});
	            ...
	            SendTimer(env, 10000);
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[]{"ABC", 14d}, new object[] {"IBM", 16d}, new object[] {"MSFT", 30d}});

	            SendTimer(env, 11000);
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[]{"IBM", 18d}, new object[] {"MSFT", 30d}});
	    ...
	    internal class ResultSetGroupByAll : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var statementString = "@name('s0') select irstream symbol, sum(price) from SupportMarketDataBean#length(5) group by symbol output all every 5 events";
	    ...
	    internal class ResultSetGroupByDefault : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var fields = "symbol,sum(price)".SplitCsv();
	            var epl = "@name('s0') select irstream symbol, sum(price) from SupportMarketDataBean#length(5) group by symbol output every 5 events";
	            env.CompileDeploy(epl).AddListener("s0");
	    ...
	    internal class ResultSetNoJoinLast : RegressionExecution {
	        ...
	        private static void TryAssertionNoJoinLast(RegressionEnvironment env, SupportOutputLimitOpt opt) {
	            var epl = opt.GetHint() + "@name('s0') select irstream symbol," +
	                      "sum(price) as mySum," +
	                      "avg(price) as myAvg " +
	    ...
	    private static void TryAssertion13_14(RegressionEnvironment env, string stmtText, string outputLimit, bool assertAllowAnyOrder, AtomicLong milestone) {
	        SendTimer(env, 0);
	        env.CompileDeploy(stmtText).AddListener("s0");
