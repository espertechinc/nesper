	...
	public class ResultSetOutputLimitRowPerGroupRollup {
	    ...
	    internal class ResultSetOutputSnapshotOrderWLimit : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@name('s0') select theString as c0, sum(intPrimitive) as c1 from SupportBean group by rollup(theString) " +
	            ...
	            env.AssertPropsPerRowLastNew("s0", "c0,c1".SplitCsv(), new object[][]{new object[]{"E3", 10}, new object[] {"E1", 12}, new object[] {"E4", 13}});
	    ...
	    private static void RunAssertion3OutputLimitAll(RegressionEnvironment env, SupportOutputLimitOpt outputLimitOpt, AtomicLong milestone) {
	        var stmtText = outputLimitOpt.GetHint() + "@name('s0') select symbol, sum(price) " +
	                       "from SupportMarketDataBean#time(5.5 sec)" +
	                       "group by rollup(symbol)" +
	    ...
	    private static void RunAssertion4OutputLimitLast(RegressionEnvironment env, SupportOutputLimitOpt opt, AtomicLong milestone) {
	    ...
	    private static void RunAssertionOutputAll(RegressionEnvironment env, bool join, SupportOutputLimitOpt opt) {
	        var fields = "c0,c1,c2".SplitCsv();
	        env.AdvanceTime(0);

	        var epl = opt.GetHint() + "@name('s0')" +
	                  "select irstream theString as c0, intPrimitive as c1, sum(longPrimitive) as c2 " +
	                  "from SupportBean#time(3.5 sec) " + (join ? ", SupportBean_S0#lastevent " : "") +
	                  "group by rollup(theString, intPrimitive) " +
	    ...
	    private static void RunAssertionOutputLast(RegressionEnvironment env, bool join, SupportOutputLimitOpt opt, AtomicLong milestone) {
	        var fields = "c0,c1,c2".SplitCsv();
	        env.AdvanceTime(0);
