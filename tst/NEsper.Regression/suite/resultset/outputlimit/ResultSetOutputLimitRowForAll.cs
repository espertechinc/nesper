...
namespace com.espertech.esper.regressionlib.suite.resultset.outputlimit
{
	public class ResultSetOutputLimitRowForAll {
	    ...
	    private static void RunAssertion9AllNoHavingNoJoin(RegressionEnvironment env, SupportOutputLimitOpt opt, AtomicLong milestone) {
	        var stmtText = opt.GetHint() + "@name('s0') select sum(price) " +
	                       "from SupportMarketDataBean#time(5.5 sec) " +
	    ...
	    private static void RunAssertion14LastNoHavingJoin(RegressionEnvironment env, SupportOutputLimitOpt opt, AtomicLong milestone) {
	        var stmtText = opt.GetHint() + "@name('s0') select sum(price) " +
	                       "from SupportMarketDataBean#time(5.5 sec), " +
	                       "SupportBean#keepall where theString=symbol " +
	                       "output last every 1 seconds";
	    ...
	    internal class ResultSetTimeWindowOutputCountLast : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select count(*) as cnt from SupportBean#time(10 seconds) output every 10 seconds";
	            env.CompileDeploy(stmtText).AddListener("s0");
	    ...
	    internal class ResultSetTimeBatchOutputCount : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select count(*) as cnt from SupportBean#time_batch(10 seconds) output every 10 seconds";
	            env.CompileDeploy(stmtText).AddListener("s0");
	    ...
	    internal class ResultSetLimitSnapshot : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            SendTimer(env, 0);
	            var selectStmt = "@name('s0') select count(*) as cnt from SupportBean#time(10 seconds) where intPrimitive > 0 output snapshot every 1 seconds";
	    ...
	    private static void TryAssertionOutputSnapshotGetValue(RegressionEnvironment env, bool join) {
	    ...
	    private static void TryAssertionOuputLastWithInsertInto(RegressionEnvironment env, SupportOutputLimitOpt opt) {
	        var epl = opt.GetHint() + "insert into MyStream select sum(intPrimitive) as thesum from SupportBean#keepall " +
