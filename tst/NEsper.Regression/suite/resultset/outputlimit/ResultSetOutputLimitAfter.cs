	...
	public class ResultSetOutputLimitAfter {
	    ...
	    private class ResultSetEveryPolicy : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            SendTimer(env, 0);
	            var stmtText = "select theString from SupportBean#keepall output after 0 days 0 hours 0 minutes 20 seconds 0 milliseconds every 0 days 0 hours 0 minutes 5 seconds 0 milliseconds";
	            ...
	            model.OutputLimitClause = OutputLimitClause.Create(Expressions.TimePeriod(0, 0, 0, 5, 0)).WithAfterTimePeriodExpression(Expressions.TimePeriod(0, 0, 0, 20, 0));
	    ...
	    private static void TryAssertionSnapshotVar(RegressionEnvironment env) {
	        ...
	        SendEvent(env, "E4");
	        var fields = "theString".SplitCsv();
	        env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[] {"E1"}, new object[] {"E2"}, new object[] {"E3"}, new object[] {"E4"}});
	    ...
	    private static void TryAssertionEveryPolicy(RegressionEnvironment env, AtomicLong milestone) {
	        var fields = "theString".SplitCsv();
	        SendTimer(env, 1);
	        SendEvent(env, "E1");
	    ...
	    private static void RunAssertionAfterWithOutputLast(RegressionEnvironment env, SupportOutputLimitOpt opt, AtomicLong milestone) {
	        var epl = opt.GetHint() + "@name('s0') select sum(intPrimitive) as thesum " +
