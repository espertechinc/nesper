...
namespace com.espertech.esper.regressionlib.suite.resultset.outputlimit
{
	public class ResultSetOutputLimitRowPerEvent {
	    ...
	    private static void RunAssertion10AllNoHavingJoin(RegressionEnvironment env, SupportOutputLimitOpt hint, AtomicLong milestone) {
	        var stmtText = hint.GetHint() + "@name('s0') select symbol, sum(price) " +
	                       "from SupportMarketDataBean#time(5.5 sec), " +
	                       "SupportBean#keepall where theString=symbol " +
	                       "output all every 1 seconds";
	        TryAssertion56(env, stmtText, "all", milestone);
	    ...
	    private static void RunAssertion11AllHavingNoJoinHinted(RegressionEnvironment env, SupportOutputLimitOpt hint, AtomicLong milestone) {
	        var stmtText = hint.GetHint() + "@name('s0') select symbol, sum(price) " +
	                       "from SupportMarketDataBean#time(5.5 sec) " +
	                       "having sum(price) > 100" +
	                       "output all every 1 seconds";
	    ...
	    private static void RunAssertion12AllHavingJoin(RegressionEnvironment env, SupportOutputLimitOpt hint, AtomicLong milestone) {
	        var stmtText = hint.GetHint() + "@name('s0') select symbol, sum(price) " +
	                       "from SupportMarketDataBean#time(5.5 sec), " +
	                       "SupportBean#keepall where theString=symbol " +
	                       "having sum(price) > 100" +
	    ...
	    private static void RunAssertion13LastNoHavingNoJoin(RegressionEnvironment env, SupportOutputLimitOpt opt, AtomicLong milestone) {
	        var stmtText = opt.GetHint() + "@name('s0') select symbol, sum(price) " +
	                       "from SupportMarketDataBean#time(5.5 sec)" +
	    ...
	    private static void RunAssertion14LastNoHavingJoin(RegressionEnvironment env, SupportOutputLimitOpt hint, AtomicLong milestone) {
	        var stmtText = hint.GetHint() + "@name('s0') select symbol, sum(price) " +
	                       "from SupportMarketDataBean#time(5.5 sec), " +
	                       "SupportBean#keepall where theString=symbol " +
	                       "output last every 1 seconds";
	    ...
	    private static void RunAssertion15LastHavingNoJoin(RegressionEnvironment env, SupportOutputLimitOpt hint, AtomicLong milestone) {
	        var stmtText = hint.GetHint() + "@name('s0') select symbol, sum(price) " +
	                       "from SupportMarketDataBean#time(5.5 sec)" +
	                       "having sum(price) > 100 " +
	                       "output last every 1 seconds";
	    ...
	    private class ResultSetLimitSnapshot : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowIRPair("s0",  fields, new object[][]{new object[] {"ABC", 50d}, new object[] {"IBM", 50d}, new object[] {"MSFT", 50d}}, null);
	            ...
	            SendTimer(env, 10000);
	            env.AssertPropsPerRowIRPair("s0", fields, new object[][]{new object[] {"ABC", 98d}, new object[] {"IBM", 98d}, new object[] {"MSFT", 98d}, new object[] {"YAH", 98d}, new object[] {"s4", 98d}}, null);

	            SendTimer(env, 11000);
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[] {"YAH", 48d}, new object[] {"s4", 48d}});
	    ...
	    private class ResultSetLimitSnapshotJoin : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            SendTimer(env, 0);
	            var selectStmt = "@name('s0') select irstream symbol, sum(price) as sumprice from SupportMarketDataBean" +
	            ...
	            SendTimer(env, 1000);
	            var fields = new string[]{"symbol", "sumprice"};
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[] {"ABC", 50d}, new object[] {"IBM", 50d}, new object[] {"MSFT", 50d}});
	            ...
	            SendTimer(env, 10000);
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[] {"ABC", 98d}, new object[] {"IBM", 98d}, new object[] {"MSFT", 98d}, new object[] {"YAH", 98d}, new object[] {"s4", 98d}});
	    ...
	    private class ResultSetRowPerEventNoJoinLast : RegressionExecution {
	        ...
	        private static void TryAssertionRowPerEventNoJoinLast(RegressionEnvironment env, SupportOutputLimitOpt opt) {
	            var epl = opt.GetHint() + "@name('s0') select longBoxed, sum(longBoxed) as result " +
	    ...
	    private class ResultSetTime : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@name('s0') select symbol, sum(volume) from " + EVENT_NAME + "#length(5) output first every 3 seconds";
	            env.CompileDeploy(epl).AddListener("s0");
	            env.ListenerReset("s0");
	    ...
	    private class ResultSetCount : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            // Create the EPL statement and add a listener
	            var statementText = "@name('s0') select symbol, sum(volume) from " + EVENT_NAME + "#length(5) output first every 3 events";
	    ...
	    private static void TryAssertion17IStreamOnly(RegressionEnvironment env, string stmtText, string outputLimit, AtomicLong milestone) {
	        SendTimer(env, 0);
	        env.CompileDeploy(stmtText).AddListener("s0");
