	...
	public class ResultSetQueryTypeWTimeBatch {
	    ...
	    private class ResultSetQueryTypeTimeBatchRowForAllNoJoin : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            SendTimer(env, 0);
	            var stmtText = "@name('s0') select irstream sum(price) as sumPrice from SupportMarketDataBean#time_batch(1 sec)";
	    ...
	    private class ResultSetQueryTypeTimeBatchRowForAllJoin : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            SendTimer(env, 0);
	            var stmtText = "@name('s0') select irstream sum(price) as sumPrice from SupportMarketDataBean#time_batch(1 sec) as S0, SupportBean#keepall as S1 where S0.symbol = S1.theString";
	            env.CompileDeploy(stmtText).AddListener("s0");
	    ...
	    private class ResultSetQueryTypeTimeBatchRowPerEventNoJoin : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            SendTimer(env, 0);
	            var stmtText = "@name('s0') select irstream symbol, sum(price) as sumPrice from SupportMarketDataBean#time_batch(1 sec)";
	            env.CompileDeploy(stmtText).AddListener("s0");
	    ...
	    private class ResultSetQueryTypeTimeBatchRowPerEventJoin : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            SendTimer(env, 0);
	            var stmtText = "@name('s0') select irstream symbol, sum(price) as sumPrice from SupportMarketDataBean#time_batch(1 sec) as S0, SupportBean#keepall as S1 where S0.symbol = S1.theString";
	            env.CompileDeploy(stmtText).AddListener("s0");
	    ...
	    private class ResultSetQueryTypeTimeBatchRowPerGroupNoJoin : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            SendTimer(env, 0);
	            var stmtText = "@name('s0') select irstream symbol, sum(price) as sumPrice from SupportMarketDataBean#time_batch(1 sec) group by symbol order by symbol asc";
	            env.CompileDeploy(stmtText).AddListener("s0");
	    ...
	    private class ResultSetQueryTypeTimeBatchRowPerGroupJoin : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowLastNewAnyOrder("s0", fields, new object[][]{new object[] {"DELL", 30d}, new object[] {"IBM", 15d}});
	    ...
	    private class ResultSetQueryTypeTimeBatchAggrGroupedNoJoin : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            SendTimer(env, 0);
	            var stmtText = "@name('s0') select irstream symbol, sum(price) as sumPrice, volume from SupportMarketDataBean#time_batch(1 sec) group by symbol";
	            env.CompileDeploy(stmtText).AddListener("s0");
