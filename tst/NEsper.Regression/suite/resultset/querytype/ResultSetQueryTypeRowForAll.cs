	...
	public class ResultSetQueryTypeRowForAll {
	    ...
	    private class ResultSetQueryTypeRowForAllStaticMethodDoubleNested : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "import " + typeof(MyHelper).FullName + ";\n" +
	    ...
	    private class ResultSetQueryTypeRowForAllSelectExprGroupWin : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            SendEvent(env, "A", 1);
	            env.AssertListener("s0", listener => Assert.AreEqual(1.0, env.Listener("s0").LastNewData[0].Get("price")));
	    ...
	    private static void TryAssert(RegressionEnvironment env) {
	        ...
	        env.AssertPropsPerRowIteratorAnyOrder("s0", new string[]{"mySum"}, new object[][]{new object[] {null}});

	        SendTimerEvent(env, 0);
	        SendEvent(env, 10);
	        AssertMySum(env, 10L);
	        env.AssertPropsPerRowIteratorAnyOrder("s0", new string[]{"mySum"}, new object[][]{new object[] {10L}});

	        SendTimerEvent(env, 5000);
	        SendEvent(env, 15);
	        AssertMySum(env, 25L);
	        env.AssertPropsPerRowIteratorAnyOrder("s0", new string[]{"mySum"}, new object[][]{new object[] {25L}});

	        SendTimerEvent(env, 8000);
	        SendEvent(env, -5);
	        AssertMySum(env, 20L);
	        env.AssertPropsPerRowIteratorAnyOrder("s0", new string[]{"mySum"}, new object[][]{new object[] {20L}});
	        ...
	        env.AssertPropsPerRowIteratorAnyOrder("s0", new string[]{"mySum"}, new object[][]{new object[] {-5L}});
	    ...
	    private static void AssertMySum(RegressionEnvironment env, long expected) {
	        env.AssertListener("s0", listener => Assert.AreEqual(expected, listener.GetAndResetLastNewData()[0].Get("mySum")));
	    }

	    private static void AssertAPrice(RegressionEnvironment env, double expected) {
	        env.AssertListener("s0", listener => Assert.AreEqual(expected, listener.GetAndResetLastNewData()[0].Get("aprice")));
	    }
