	...
	public class ResultSetQueryTypeRollupGroupingFuncs {
	    ...
	    private class ResultSetQueryTypeDocSampleCarEventAndGroupingFunc : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            // try simple
	            var epl = "@name('s0') select name, place, sum(count), grouping(name), grouping(place), grouping_id(name,place) as gid " +
	        ...
	        private static void TryAssertionDocSampleCarEvent(RegressionEnvironment env, AtomicLong milestone) {
	            var fields = new string[]{"name", "place", "sum(count)", "grouping(name)", "grouping(place)", "gid"};
	    ...
	    private class ResultSetQueryTypeGroupingFuncExpressionUse : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var eplTwo = "@name('s0') select prev(1, name) as c0, prior(1, name) as c1, name as c2, sum(count) as c3 " +
