...
namespace com.espertech.esper.regressionlib.suite.rowrecog
{
	public class RowRecogRepetition {
	    ...
	    private static void RunDocSampleUpToN(RegressionEnvironment env, AtomicLong milestone) {
	    ...
	    private static void RunDocSampleNOrMore_and_BetweenNandM(RegressionEnvironment env, string pattern, AtomicLong milestone) {
	    ...
	    private static void RunDocSampleExactlyN(RegressionEnvironment env, AtomicLong milestone) {
	    ...
	    private class RowRecogRepetitionInvalid : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.TryInvalidCompile(template.RegexReplaceAll("REPLACE", "A{}"),
	                "Invalid match-recognize quantifier '{}', expecting an expression");
	            env.TryInvalidCompile(template.RegexReplaceAll("REPLACE", "A{null}"),
	                "Pattern quantifier 'null' must return an integer-type value");
	            env.TryInvalidCompile(template.RegexReplaceAll("REPLACE", "A{myvariable}"),
	                "Pattern quantifier 'myvariable' must return a constant value");
	            env.TryInvalidCompile(template.RegexReplaceAll("REPLACE", "A{prev(A)}"),
	            ...
	            env.TryInvalidCompile(template.RegexReplaceAll("REPLACE", "A{-1,}"), expected);
	            env.TryInvalidCompile(template.RegexReplaceAll("REPLACE", "A{5,3}"),
	    ...
	    private static void RunAssertionNestedRepeatSingle(RegressionEnvironment env, bool soda) {
	    ...
	    private static void RunAssertionNestedRepeatRange(RegressionEnvironment env, bool soda) {
	        RunOnceOrTwiceABThenC(env, soda, "(A B) (A B)? C");
	        RunOnceOrTwiceABThenC(env, soda, "(A B){1,2} C");
	    }

	    private static void RunAssertionsAtLeast(RegressionEnvironment env, bool soda) {
	        RunAtLeast2AThenB(env, soda, "A A A* B");
	        RunAtLeast2AThenB(env, soda, "A{2,} B");
	        RunAtLeast2AThenB(env, soda, "A{2,4} B");
	    }

	    private static void RunAssertionsUpTo(RegressionEnvironment env, bool soda) {
	        RunUpTo2AThenB(env, soda, "A? A? B");
	        RunUpTo2AThenB(env, soda, "A{,2} B");
	    }

	    private static void RunAssertionsRepeatRange(RegressionEnvironment env, bool soda) {
	        Run2To3AThenB(env, soda, "A A A? B");
	        Run2To3AThenB(env, soda, "A{2,3} B");
	    }

	    private static void RunAssertionsNestedUpTo(RegressionEnvironment env, bool soda) {
	        RunUpTo2ABThenC(env, soda, "(A B)? (A B)? C");
	        RunUpTo2ABThenC(env, soda, "(A B){,2} C");
	    }

	    private static void RunAssertionsNestedAtLeast(RegressionEnvironment env, bool soda) {
	        RunAtLeast2ABThenC(env, soda, "(A B) (A B) (A B)* C");
	        RunAtLeast2ABThenC(env, soda, "(A B){2,} C");
	    }

	    private static void RunAssertionRepeatSingleBound(RegressionEnvironment env, bool soda) {
	    ...
	    private static void RunAtLeast2ABThenC(RegressionEnvironment env, bool soda, string pattern) {
	        RunAssertion(env, soda, pattern, "a,b,c", new bool[]{true, true, false}, new string[]{
	            "A1,B1,A2,B2,C1",
	            "A1,B1,A2,B2,A3,B3,C1"
	        }, new string[]{"A1,B1,C1", "A1,B1,A2,C1", "B1,A1,B2,C1"});
	    }

	    private static void RunOnceOrTwiceABThenC(RegressionEnvironment env, bool soda, string pattern) {
	        RunAssertion(env, soda, pattern, "a,b,c", new bool[]{true, true, false}, new string[]{
	            "A1,B1,C1",
	            "A1,B1,A2,B2,C1"
	    ...
	    private static void RunAtLeast2AThenB(RegressionEnvironment env, bool soda, string pattern) {
	        RunAssertion(env, soda, pattern, "a,b", new bool[]{true, false}, new string[]{
	            "A1,A2,B1",
	            "A1,A2,A3,B1"
	        }, new string[]{"A1,B1", "B1"});
	    }

	    private static void RunUpTo2AThenB(RegressionEnvironment env, bool soda, string pattern) {
	        RunAssertion(env, soda, pattern, "a,b", new bool[]{true, false}, new string[]{
	            "B1",
	            "A1,B1",
	            "A1,A2,B1"
	    ...
	    private static void Run2AThen2B(RegressionEnvironment env, bool soda, string pattern) {
	        RunAssertion(env, soda, pattern, "a,b", new bool[]{true, true}, new string[]{
	            "A1,A2,B1,B2",
	        }, new string[]{"A1,A2,B1", "B1,B2,A1,A2", "A1,B1,A2,B2"});
	    }

	    private static void RunUpTo2ABThenC(RegressionEnvironment env, bool soda, string pattern) {
	        RunAssertion(env, soda, pattern, "a,b,c", new bool[]{true, true, false}, new string[]{
	            "C1",
	            "A1,B1,C1",
	            "A1,B1,A2,B2,C1",
	    ...
	    private static void Run2To3AThenB(RegressionEnvironment env, bool soda, string pattern) {
	        RunAssertion(env, soda, pattern, "a,b", new bool[]{true, false}, new string[]{
	            "A1,A2,A3,B1",
	            "A1,A2,B1",
	    ...
	    private static void RunAThen2BOr2C(RegressionEnvironment env, bool soda, string pattern) {
	        RunAssertion(env, soda, pattern, "a,b,c", new bool[]{false, true, true}, new string[]{
	            "A1,C1,C2",
	            "A2,B1,B2",
	        }, new string[]{"B1,B2", "C1,C2", "A1,B1,C1", "A1,C1,B1"});
	    }

	    private static void RunTwiceAB(RegressionEnvironment env, bool soda, string pattern) {
	        RunAssertion(env, soda, pattern, "a,b", new bool[]{true, true}, new string[]{
	            "A1,B1,A2,B2",
	        }, new string[]{"A1,A2,B1", "A1,A2,B1,B2", "A1,B1,B2,A2"});
	    }

	    private static void RunAThenTwiceBC(RegressionEnvironment env, bool soda, string pattern) {
	        RunAssertion(env, soda, pattern, "a,b,c", new bool[]{false, true, true}, new string[]{
	            "A1,B1,C1,B2,C2",
	        }, new string[]{"A1,B1,C1,B2", "A1,B1,C1,C2", "A1,B1,B2,C1,C2"});
	    }

	    private static void RunAThen2BThenC(RegressionEnvironment env, bool soda, string pattern) {
	        RunAssertion(env, soda, pattern, "a,b,c", new bool[]{false, true, false}, new string[]{
	            "A1,B1,B2,C1",
	        }, new string[]{"B1,B2,C1", "A1,B1,C1", "A1,B1,B2"});
	    }

	    private static void RunExactly2A(RegressionEnvironment env, bool soda, string pattern) {
	        RunAssertion(env, soda, pattern, "a", new bool[]{true}, new string[]{
	            "A1,A2",
	            "A3,A4",
	        }, new string[]{"A5"});
	    }

	    private static void RunAssertion(RegressionEnvironment env, bool soda, string pattern, string propertyNames, bool[] arrayProp,
	    ...
	    private static void RunAssertionSequence(RegressionEnvironment env, bool match, string[] propertyNames, bool[] arrayProp, int sequenceNum, string sequence) {
	    ...
	    internal static void RunEquivalent(RegressionEnvironment env, string before, string after) {
	        var hook = "@Hook(type=" + typeof(HookType).FullName + ".INTERNAL_COMPILE,hook='" + SupportStatementCompileHook.ResetGetClassName() + "')";
	        var epl = hook + "@name('s0') select * from SupportBean#keepall " +
	                  "match_recognize (" +
	                  " measures A as a" +
	                  " pattern (" + before + ")" +
	                  " define" +
