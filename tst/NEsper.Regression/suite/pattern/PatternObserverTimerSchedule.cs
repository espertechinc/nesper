	...
	public class PatternObserverTimerSchedule {
	    ...
	    public class PatternTimerScheduleLimitedWDateAndPeriod : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@name('s0') select * from pattern[every timer:schedule(iso: 'R3/2012-10-01T05:52:00Z/PT2S')]";
	            env.CompileDeploy(epl).AddListener("s0");
	    ...
	    public class PatternTimerScheduleDateWithPeriod : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            // Repeat 3 times, starting "2012-10-01T05:52:00Z" (UTC), period of 2 seconds
	            var epl = "@name('s0') select * from pattern[every timer:schedule(period: 1 day, date: '2012-10-02T00:00:00Z')]";
	            env.CompileDeploy(epl).AddListener("s0");
	    ...
	    private static void RunAssertionFollowedByDynamicallyComputed(RegressionEnvironment env, AtomicLong milestone) {

	        SendCurrentTime(env, "2012-10-01T05:51:07.000GMT-0:00");

	        var epl = "@name('s0') select * from pattern[every sb=SupportBean -> timer:schedule(iso: computeISO8601String(sb))]";
	        env.CompileDeploy(epl).AddListener("s0");
	    ...
	    private static void RunAssertionFollowedBy(RegressionEnvironment env, AtomicLong milestone) {
	        SendCurrentTime(env, "2012-10-01T05:51:07.000GMT-0:00");

	        var epl = "@name('s0') select * from pattern[every sb=SupportBean -> timer:schedule(iso: 'R/1980-01-01T00:00:00Z/PT15S')]";
	        env.CompileDeploy(epl).AddListener("s0");
	        ...
	        env.AssertPropsPerRowLastNew("s0", "sb".SplitCsv(), new object[][]{new object[] {b2}, new object[] {b3}});
	    ...
	    private static void RunAssertionEquivalent(RegressionEnvironment env) {
	        ...
	        var first = "@name('s0') select * from pattern[every timer:schedule(iso: 'R2/2008-03-01T13:00:00Z/P1Y2M10DT2H30M')]";
	    ...
	    private static void TryAssertionDateWithPeriod(RegressionEnvironment env, AtomicLong milestone) {
	    ...
	    private static void TryAssertionDateWithPeriod(RegressionEnvironment env, string parameters, AtomicLong milestone) {
	    ...
	    private static void RunAssertionFullFormLimitedFutureDated(RegressionEnvironment env, AtomicLong milestone) {
	    ...
	    private static void TryAssertionFullFormLimitedFutureDated(RegressionEnvironment env, bool audit, string parameters, AtomicLong milestone) {

	        SendCurrentTime(env, "2012-10-01T05:51:00.000GMT-0:00");

	        // Repeat 3 times, starting "2012-10-01T05:52:00Z" (UTC), period of 2 seconds
	        var epl = (audit ? "@Audit " : "") + "@name('s0') select * from pattern[every timer:schedule(" + parameters + ")]";
	        env.CompileDeploy(epl).AddListener("s0");
	    ...
	    private static void RunAssertionJustFutureDate(RegressionEnvironment env, AtomicLong milestone) {
	    ...
	    private static void TryAssertionJustFutureDate(RegressionEnvironment env, bool hasEvery, string parameters, AtomicLong milestone) {
	        SendCurrentTime(env, "2012-10-01T05:51:00.000GMT-0:00");

	        // Fire once at "2012-10-01T05:52:00Z" (UTC)
	        var epl = "@name('s0') select * from pattern[" + (hasEvery ? "every " : "") + "timer:schedule(" + parameters + ")]";
	        env.CompileDeploy(epl).AddListener("s0");
	    ...
	    private static void RunAssertionJustPastDate(RegressionEnvironment env, AtomicLong milestone) {
	        TryAssertionJustPastDate(env, true, milestone);
	        TryAssertionJustPastDate(env, false, milestone);
	    }

	    private static void TryAssertionJustPastDate(RegressionEnvironment env, bool hasEvery, AtomicLong milestone) {
	        SendCurrentTime(env, "2012-10-01T05:51:00.000GMT-0:00");

	        // Fire once at "2012-10-01T05:52:00Z" (UTC)
	        var epl = "@name('s0') select * from pattern[" + (hasEvery ? "every " : "") + "timer:schedule(iso: '2010-10-01T05:52:00Z')]";
	        env.CompileDeploy(epl).AddListener("s0");
	    ...
	    private static void RunAssertionJustPeriod(RegressionEnvironment env, AtomicLong milestone) {
	        TryAssertionJustPeriod(env, "iso:'P1DT2H'", milestone);
	        TryAssertionJustPeriod(env, "period: 1 day 2 hours", milestone);
	    }

	    private static void TryAssertionJustPeriod(RegressionEnvironment env, string parameters, AtomicLong milestone) {
	        SendCurrentTime(env, "2012-10-01T05:51:00.000GMT-0:00");
	    ...
	    private static void RunAssertionRecurringLimitedWithPeriod(RegressionEnvironment env, AtomicLong milestone) {
	    ...
	    private static void TryAssertionRecurringLimitedWithPeriod(RegressionEnvironment env, string parameters, AtomicLong milestone) {
	    ...
	    private static void RunAssertionRecurringUnlimitedWithPeriod(RegressionEnvironment env, AtomicLong milestone) {
	    ...
	    private static void RunAssertionFullFormUnlimitedPastDated(RegressionEnvironment env, AtomicLong milestone) {
	    ...
	    private static void RunAssertionFullFormUnlimitedPastDatedAnchoring(RegressionEnvironment env, AtomicLong milestone) {
	    ...
	    private static void RunAssertionRecurringAnchoring(RegressionEnvironment env, AtomicLong milestone) {
	        // Repeat unlimited number of times, reference-dated to "1980-01-01T00:00:00Z" (UTC), period of 1 second
	        SendCurrentTime(env, "2012-01-01T00:0:00.000GMT-0:00");
	    ...
	    private static void RunAssertionFullFormLimitedPastDated(RegressionEnvironment env, AtomicLong milestone) {
	    ...
	    private static void RunAssertionFullFormUnlimitedFutureDated(RegressionEnvironment env, AtomicLong milestone) {
	    ...
	    private static void AssertReceivedAtTime(RegressionEnvironment env, string time) {
	        ...
	        env.AssertListener("s0", listener => Assert.IsTrue(listener.IsInvokedAndReset(), "expected but not received at " + time));
	    }
