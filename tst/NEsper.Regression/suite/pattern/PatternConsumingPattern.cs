	...
	public class PatternConsumingPattern {
	    ...
	    private class PatternInvalid : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var expected = "Discard-partials and suppress-matches is not supported in a joins, context declaration and on-action ";
	            env.TryInvalidCompile(path, "select * from pattern " + GetText(TargetEnum.DISCARD_AND_SUPPRESS) + "[SupportIdEventA]#keepall, A#keepall",
	            ...
	            env.TryInvalidCompile(path, "on pattern " + GetText(TargetEnum.DISCARD_AND_SUPPRESS) + "[SupportIdEventA] select * from AWindow",
	    ...
	    private class PatternCombination : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            // test order-by
	            var epl = "@name('s0') select * from pattern @DiscardPartialsOnMatch [every a=SupportIdEventA -> SupportIdEventB] order by a.id desc";
	            ...
	            env.AssertPropsPerRowNewOnly("s0", "a.id".SplitCsv(), new object[][]{new object[] {"A2"}, new object[] {"A1"}});
	    ...
	    private class PatternObserverOp : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@name('s0') select * from pattern " + GetText(TargetEnum.DISCARD_ONLY) + " [" +
	    ...
	    private static void TryAssertionEveryChildState(RegressionEnvironment env, AtomicLong milestone, string everySuffix, bool matchDiscard) {
	        var fields = "a.id,b.id,c.id".SplitCsv();
	    ...
	    private static void TryAssertionEveryBeginState(RegressionEnvironment env, AtomicLong milestone, string distinct) {
	        var fields = "a.id,b.id".SplitCsv();
	    ...
	    private static void RunFollowedByOp(RegressionEnvironment env, AtomicLong milestone, string pattern, bool matchDiscard) {
	        var fields = "a1.id,a2.id".SplitCsv();

	        var epl = "@name('s0') select * from pattern "
	                  + (matchDiscard ? GetText(TargetEnum.DISCARD_ONLY) : "") + "[" + pattern + "]";
	        env.CompileDeploy(epl).AddListener("s0");
	    ...
	    private static void TryAssertionTargetNextMatch(RegressionEnvironment env, bool testSoda, TargetEnum target) {

	        var fields = "a.id,b.id,c.id".SplitCsv();
	        var epl = "@name('s0') select * from pattern " + GetText(target) + "[every a=SupportIdEventA -> b=SupportIdEventB -> c=SupportIdEventC(pc=a.pa)]";
	    ...
	    private static void TryAssertionMatchUntilBoundOp(RegressionEnvironment env, AtomicLong milestone, bool matchDiscard) {
	    ...
	    private static void TryAssertionMatchUntilWChildMatcher(RegressionEnvironment env, AtomicLong milestone, bool matchDiscard) {
	    ...
	    private static void TryAssertionMatchUntilRangeOpWTime(RegressionEnvironment env, AtomicLong milestone) {
	    ...
	    private static void TryAssertionTargetCurrentMatch(RegressionEnvironment env, bool testSoda, TargetEnum target) {

	        var fields = "a1.id,aarr[0].id,b.id".SplitCsv();
	        var epl = "@name('s0') select * from pattern " + GetText(target) + "[every a1=SupportIdEventA -> [:10] aarr=SupportIdEventA until b=SupportIdEventB]";
	        env.CompileDeploy(testSoda, epl).AddListener("s0");
	    ...
	    private static void RunAndWAndState(RegressionEnvironment env, AtomicLong milestone, bool matchDiscard) {
	        var fields = "a.id,b.id,c.id".SplitCsv();
	    ...
	    private static void RunAndWChild(RegressionEnvironment env, AtomicLong milestone, bool matchDiscard) {
	        var fields = "a.id,b.id,c.id".SplitCsv();
	    ...
	    private static void RunGuardOpBeginState(RegressionEnvironment env, AtomicLong milestone, bool matchDiscard) {
	        var fields = "a.id,b.id,c.id".SplitCsv();
	    ...
	    private static void RunGuardOpChildState(RegressionEnvironment env, AtomicLong milestone, bool matchDiscard) {
	        var fields = "a.id,b.id,c.id".SplitCsv();
	    ...
	    private static void SendAEvent(string id, string pa, int? mysec, RegressionEnvironment env) {
