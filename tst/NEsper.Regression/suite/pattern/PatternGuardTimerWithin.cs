	...
	public class PatternGuardTimerWithin {
	    ...
	    private class PatternInterval10MinVariable : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            // Set up a timer:within
	            var stmtText = "@name('s0') select * from pattern [(every SupportBean) where timer:within(D days H hours M minutes S seconds MS milliseconds)]";
	    ...
	    private class PatternIntervalPrepared : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            // Set up a timer:within
	            var compiled = env.Compile("@name('s0') select * from pattern [(every SupportBean) where timer:within(?::int days ?::int hours ?::int minutes ?::int seconds ?::int milliseconds)]");
	            ...
	            env.Deploy(compiled, new DeploymentOptions().WithStatementSubstitutionParameter(supportPortableDeploySubstitutionParams.SetStatementParameters));
	            env.AddListener("s0");
	    ...
	    private class PatternPatternNotFollowedBy : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var stmtText = "@name('s0') select * from pattern [ every(SupportBean -> (SupportMarketDataBean where timer:within(5 sec))) ]";
	            env.CompileDeploy(stmtText).AddListener("s0");
	    ...
	    private static void TryAssertionWithinMayMaxMonthScoped(RegressionEnvironment env, bool hasMax, AtomicLong milestone) {
