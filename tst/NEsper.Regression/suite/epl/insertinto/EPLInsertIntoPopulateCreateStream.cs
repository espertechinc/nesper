...
namespace com.espertech.esper.regressionlib.suite.epl.insertinto
{
	public class EPLInsertIntoPopulateCreateStream : RegressionExecution {
	    ...
	    private static void RunAssertPopulateFromNamedWindow(RegressionEnvironment env, EventRepresentationChoice type) {
	        ...
	        var schemaEPL = type.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedNode)) + "@Public @buseventtype create schema Node(nid string)";
	        ...
	        env.CompileDeploy(type.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedNodePlus)) + "create schema NodePlus(npid string, node Node)", path);
	    ...
	    private static void RunAssertionCreateStream(RegressionEnvironment env, EventRepresentationChoice representation) {
	        var epl = representation.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedMyEvent)) + " @buseventtype @public create schema MyEvent(myId int);\n" +
	                  representation.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedCompositeEvent)) + " @buseventtype @public create schema CompositeEvent(c1 MyEvent, c2 MyEvent, rule string);\n" +
	                  ...
	                  representation.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedCompositeEvent)) + " @Name('Target') select * from CompositeEvent;\n";
	    ...
	    private static void RunAssertionCreateStreamTwo(RegressionEnvironment env, EventRepresentationChoice eventRepresentationEnum) {
	        var path = new RegressionPath();
	        var epl = eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedMyEvent)) + " @public @buseventtype create schema MyEvent(myId int)\n;" +
	                  eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedAllMyEvent)) + " @public @buseventtype create schema AllMyEvent as (myEvent MyEvent, clazz String, reverse boolean);\n" +
	                  eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedSuspectMyEvent)) + " @public @buseventtype create schema SuspectMyEvent as (myEvent MyEvent, clazz String);\n";
	        ...
	        env.AssertStatement("s0", statement => Assert.IsTrue(eventRepresentationEnum.MatchesClass(statement.EventType.UnderlyingType)));
