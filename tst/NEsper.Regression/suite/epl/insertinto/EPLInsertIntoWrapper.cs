	...
	public class EPLInsertIntoWrapper {
	    ...
	    public class EPLInsertIntoOnSplitForkJoin : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('A') \n" +
	                      "on SupportBean_S0 event insert into AStream select transpose(" + typeof(EPLInsertIntoWrapper).FullName + ".transpose(event));\n" +
	                      "\n" +
	    ...
	    public class EPLInsertInto3StreamWrapper : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var statementOne = "@name('s0') @public insert into StreamA select irstream * from SupportBeanSimple#length(2)";
	            var statementTwo = "@name('s1') @public insert into StreamB select irstream *, myString||'A' as propA from StreamA#length(2)";
	            var statementThree = "@name('s2') @public insert into StreamC select irstream *, propA||'B' as propB from StreamB#length(2)";
	    ...
	    public class MyEvent {
	        ...
	        public MyEvent(int id, bool propOne, bool propTwo, bool propThree) {
	            this.id = id;
