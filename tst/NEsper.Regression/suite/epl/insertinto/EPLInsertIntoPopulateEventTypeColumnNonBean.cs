	...
	public class EPLInsertIntoPopulateEventTypeColumnNonBean {
	    ...
	    private class EPLInsertIntoColNonBeanSingleToMulti : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowIterator("s0", "aArray[0].id".Split(","), new object[][] {new object[] {"x1"}});
	    ...
	    private class EPLInsertIntoColNonBeanFromSubqueryMultiFilter : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertEventNew("s0", @event => EPAssertionUtil.AssertPropsPerRow((EventBean[]) @event.Get("ez"), fields, new object[][]{new object[]{"x2"}, new object[]{"x3"}}));
	    ...
	    private class EPLInsertIntoColNonBeanCaseNew : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            var path = new RegressionPath();
	            env.CompileDeploy(representation.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedNested)) + "@public create schema Nested(p0 string, p1 int)", path);
	            env.CompileDeploy(representation.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedOuterType)) + "@public create schema OuterType(n0 Nested)", path);
	    ...
	    private class EPLInsertIntoColNonBeanFromSubquerySingle : RegressionExecution {
	        ...
	        public EPLInsertIntoColNonBeanFromSubquerySingle(string typeType, bool filter) {
	            this.typeType = typeType;
	            this.filter = filter;
	        }
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsNew("s0", fields, expected);
	            if (!filter) {
	                env.AssertPropsPerRowIterator("s1", "ez.e0_0".Split(","), new object[][]{new object[] {"x1"}, new object[] {"x2"}, new object[] {"x3"}});
	            }
	    ...
	    private class EPLInsertIntoColNonBeanFromSubqueryMulti : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowIterator("s1", "ez[0].e0_0,ez[1].e0_0".Split(","), new object[][]{new object[] {"x1", null}, new object[] {"x1", "x2"}});
