	...
	public class EPLInsertIntoPopulateEventTypeColumnBean {
	    ...
	    private class EPLInsertIntoColBeanMultiToSingle : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertEventNew("s0", @event => {
	                Assert.AreSame(bean, @event.Get("sb"));
	            });

	            env.Milestone(0);

	            env.AssertPropsPerRowIterator("s0", new string[] {"sb.theString"}, new object[][] {new object[] {"E1"}});
	    ...
	    private class EPLInsertIntoColBeanSingleToMulti : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowIterator("s0", new string[] {"sbarr[0].theString"}, new object[][] {new object[] {"E1"}});
	    ...
	    private class EPLInsertIntoColBeanFromSubqueryMulti : RegressionExecution {
	        ...
	        public EPLInsertIntoColBeanFromSubqueryMulti(string typeType, bool filter) {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowIterator("s1", "sbarr[0].id,sbarr[1].id".Split(","), new object[][]{new object[] {1, null}, new object[] {1, 2}});
	    ...
	    private class EPLInsertIntoColBeanFromSubquerySingle : RegressionExecution {
	        ...
	        public EPLInsertIntoColBeanFromSubquerySingle(string typeType, bool filter) {
	            this.typeType = typeType;
	            this.filter = filter;
	        }
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            if (!filter) {
	                env.AssertPropsPerRowIterator("s1", "sb.id".Split(","), new object[][]{new object[] {1}, new object[] {null}});
	            }
	    ...
	    private class EPLInsertIntoColBeanInvalid : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.TryInvalidCompile(path, "insert into TypeOne select (select * from SupportBean_S0#keepall) as sbs from SupportBean_S1",
	                "Incompatible type detected attempting to insert into column 'sbs' type '" + typeof(SupportBean).Name + "' compared to selected type 'SupportBean_S0'");
	    ...
	    private static void AssertS0(EventBean @event, params SupportBean_S0[] expected) {
