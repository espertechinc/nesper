...
namespace com.espertech.esper.regressionlib.suite.epl.insertinto
{
	public class EPLInsertIntoPopulateUndStreamSelect {
	    ...
	    private static void TryAssertionNamedWindow(RegressionEnvironment env, EventRepresentationChoice rep) {
	        ...
	        var schema = rep.GetAnnotationText() + "@name('schema') @public @buseventtype create schema A as (myint int, mystr string);\n" +
	    ...
	    private static void TryAssertionStreamInsertWWidenMap(RegressionEnvironment env, EventRepresentationChoice rep) {
	        ...
	        var path = new RegressionPath();
	        var schemaSrc = rep.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedSrc)) + "@name('schema') @public @buseventtype create schema Src as (myint int, mystr string)";
	        env.CompileDeploy(schemaSrc, path);

	        env.CompileDeploy(rep.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedD1)) + "@public create schema D1 as (myint int, mystr string, addprop long)", path);
	        ...
	        env.CompileDeploy(rep.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedD2)) + "@public create schema D2 as (mystr string, myint int, addprop double)", path);
	        ...
	        env.CompileDeploy(rep.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedD3)) + "@public create schema D3 as (mystr string, addprop int)", path);
	        ...
	        env.CompileDeploy(rep.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedD4)) +  "@public create schema D4 as (myint int, mystr string)", path);
	    ...
	    private static void TryAssertionInvalid(RegressionEnvironment env, EventRepresentationChoice rep) {
	        var path = new RegressionPath();
	        env.CompileDeploy(rep.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedSrc)) + "@public create schema Src as (myint int, mystr string)", path);

	        // mismatch in type
	        env.CompileDeploy(rep.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedE1)) + "@public create schema E1 as (myint long)", path);
	        ...
	        env.CompileDeploy(rep.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedE2)) + "@public create schema E2 as (someprop long)", path);
	    ...
	    private static IDictionary<string, object> MakeMap(int myint, string mystr) {
	    ...
	    private static GenericRecord MakeAvro(RegressionEnvironment env, int myint, string mystr) {
