	...
	public class EPLInsertIntoEventPrecedence {
	    ...
	    public class EPLInsertIntoEventPrecNonConstInsertIntoContainedEvent : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl =
	                    "@public @buseventtype create schema LvlA as " + typeof(LvlA).FullName + ";\n" +
	    ...
	    public class EPLInsertIntoEventPrecConstantOnSplit : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowNewFlattened("s0", new string[]{"id"}, new object[][]{new object[]{3}, new object[]{2}, new object[]{1}});
	    ...
	    private class EPLInsertIntoEventPrecConstantInsertInto : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.SendEventBean(new SupportBean());
	            env.AssertPropsPerRowIterator("window", new string[]{"id"}, new object[][]{new object[]{5}, new object[]{4}, new object[]{3}, new object[]{2}, new object[]{1}, new object[]{0}});
	    ...
	    private class EPLInsertIntoEventPrecSubqueryInsertIntoSODA : RegressionExecution {
	        ...
	        private void SendSBAssert(RegressionEnvironment env, string first, string second) {
	            env.SendEventBean(new SupportBean());
	            env.AssertPropsPerRowNewFlattened("s0", new string[]{"id"}, new object[][]{new object[]{first}, new object[]{second}});
	        }
	    }
	    ...
	    private class EPLInsertIntoEventPrecConstantInfraMergeInsertInto : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.SendEventBean(new SupportBean());
	            env.AssertPropsPerRowIterator("WindowOut", new string[]{"outid"}, new object[][]{new object[]{"c"}, new object[]{"a"}, new object[]{"b"}});
	    ...
	    private class EPLInsertIntoEventPrecSubqueryOnInsertSODA : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var eplOne = "on SupportBean insert into Out event-precedence((select intOne from SupportBeanNumeric#lastevent)) select \"a\" as id from MyWindow";
	            env.CompileDeploy(true, eplOne, path);

	            var eplTwo = "on SupportBean insert into Out event-precedence((select intTwo from SupportBeanNumeric#lastevent)) select \"b\" as id from MyWindow;\n" +
	                         "@name('s0') select * from Out;\n";
	        ...
	        public void Run(RegressionEnvironment env) {
	            var epl =
	                    "create schema Out(id int);\n" +
	                            ...
	                            "insert into Out event-precedence(" + typeof(EPLInsertIntoEventPrecedence).FullName + ".computeEventPrecedence(3, *)) select 3 + intPrimitive * 10 as id from SupportBean output every 2 events;\n" +
	                            "@name('s0') select * from Out;\n";
	            env.CompileDeploy(epl).AddListener("s0");

	            env.SendEventBean(new SupportBean("E1", 1));
	            env.SendEventBean(new SupportBean("E2", 2));
	            env.AssertPropsPerRowNewFlattened("s0", new string[]{"id"}, new object[][]{new object[]{13}, new object[]{23}, new object[]{12}, new object[]{22}, new object[]{11}, new object[]{21}});
