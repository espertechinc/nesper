	...
	public class EPLOtherPlanExcludeHint : IndexBackingTableInfo {
	    ...
	    private class EPLOtherDocSample : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var schema =
	                "@public create schema AEvent as " + typeof(AEvent).FullName + ";\n" +
	                    "@public create schema BEvent as " + typeof(BEvent).FullName + ";\n";
	    ...
	    private class EPLOtherJoin : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var types = new EventType[]{env.Runtime.EventTypeService.GetEventTypePreconfigured("SupportBean_S0"), env.Runtime.EventTypeService.GetEventTypePreconfigured("SupportBean_S1")};
