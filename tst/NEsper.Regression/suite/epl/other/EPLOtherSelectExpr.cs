	...
	public class EPLOtherSelectExpr {
	        ...
	        private static void TryPrecedenceNoColumnName(RegressionEnvironment env, string selectColumn, string expectedColumn, object value) {
	            ...
	            env.AssertStatement("s0", statement => {
	                if (!statement.EventType.PropertyNames[0].Equals(expectedColumn)) {
	                    Assert.Fail("Expected '" + expectedColumn + "' but was " + statement.EventType.PropertyNames[0]);
	                }
	            });
	    ...
	    private class EPLOtherKeywordsAllowed : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertStatement("s0", statement => EPAssertionUtil.AssertEqualsExactOrder(statement.EventType.PropertyNames, fields.SplitCsv()));
	    ...
	    private class EPLOtherEscapeString : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertStatement("A\'B", statement => {
	                ...
	                var desc = (com.espertech.esper.common.client.annotation.DescriptionAttribute) statement.Annotations[1];
	    ...
	    private class EPLOtherGetEventType : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select theString, boolBoxed aBool, 3*intPrimitive, floatBoxed+floatPrimitive result" +
	    ...
	    private class EPLOtherWindowStats : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select theString, boolBoxed as aBool, 3*intPrimitive, floatBoxed+floatPrimitive as result" +
	                      " from SupportBean#length(3) " +
	                      " where boolBoxed = true";
	            env.CompileDeploy(epl).AddListener("s0");
	    ...
	    private static void SendEvent(RegressionEnvironment env, string s, bool b, int i, float f1, float f2) {
