	...
	public class EPLOtherPlanInKeywordQuery : IndexBackingTableInfo {
	    ...
	    private class EPLOtherNotIn : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = IndexBackingTableInfo.INDEX_CALLBACK_HOOK + "select * from SupportBean_S0 as s0 unidirectional, SupportBean_S1#keepall as s1 " +
	    ...
	    private class EPLOtherMultiIdxMultipleInAndMultirow : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            // assert join
	            SupportQueryPlanIndexHook.Reset();
	            var epl = "@name('s0') " + IndexBackingTableInfo.INDEX_CALLBACK_HOOK + "select * from SupportBean_S0 as s0 unidirectional, SupportBean_S1#keepall as s1 " +
	            ...
	            var eplNamedWindow = "@name('s0') " + IndexBackingTableInfo.INDEX_CALLBACK_HOOK + "on SupportBean_S0 as s0 select * from S1Window as s1 " +
	                                 "where p00 in (p10, p11) and p01 in (p12, p13)";
	            ...
	            var eplTable = "@name('s0') " + IndexBackingTableInfo.INDEX_CALLBACK_HOOK + "on SupportBean_S0 as s0 select * from S1Table as s1 " +
	                           "where p00 in (p10, p11) and p01 in (p12, p13)";
	    ...
	    private class EPLOtherMultiIdxSubquery : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@name('s0') " + IndexBackingTableInfo.INDEX_CALLBACK_HOOK + "select s0.id as c0," +
	            ...
	            var eplCoercion = IndexBackingTableInfo.INDEX_CALLBACK_HOOK + "select *," +
	    ...
	    private class EPLOtherSingleIdxMultipleInAndMultirow : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            // assert join
	            SupportQueryPlanIndexHook.Reset();
	            var epl = "@name('s0') " + IndexBackingTableInfo.INDEX_CALLBACK_HOOK + "select * from SupportBean_S0#keepall as s0, SupportBean_S1 as s1 unidirectional " +
	                      "where p00 in (p10, p11) and p01 in (p12, p13)";
	            env.CompileDeploy(epl).AddListener("s0");
	            ...
	            var eplNamedWindow = "@name('s0') " + IndexBackingTableInfo.INDEX_CALLBACK_HOOK + "on SupportBean_S1 as s1 select * from S0Window as s0 " +
	                                 "where p00 in (p10, p11) and p01 in (p12, p13)";
	            env.CompileDeploy(eplNamedWindow, path).AddListener("s0");
	            ...
	            var eplTable = "@name('s0') " + IndexBackingTableInfo.INDEX_CALLBACK_HOOK + "on SupportBean_S1 as s1 select * from S0Table as s0 " +
	                           "where p00 in (p10, p11) and p01 in (p12, p13)";
	            env.CompileDeploy(eplTable, path).AddListener("s0");
	    ...
	    private class EPLOtherSingleIdxSubquery : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            SupportQueryPlanIndexHook.Reset();
	            var epl = "@name('s0') " + IndexBackingTableInfo.INDEX_CALLBACK_HOOK + "select s1.id as c0," +
	    ...
	    private class EPLOtherSingleIdxConstants : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowLastNewAnyOrder("s0", fields, new object[][]{new object[] {2, 101}, new object[] {2, 102}});
	        ...
	        public void Run(RegressionEnvironment env) {
	                ...
	                SupportQueryPlanBuilder.Start(3)
	                    ...
	                    .SetLookupPlanInstruction(0, "s0", new LookupInstructionPlanForge[]{
	                        new LookupInstructionPlanForge(0, "s0", new int[]{1},
	                            new TableLookupPlanForge[]{planInMidx}, null, new bool[3]),
	                        new LookupInstructionPlanForge(0, "s0", new int[]{2},
	                            new TableLookupPlanForge[]{new FullTableScanLookupPlanForge(1, 2, false, types, GetIndexKey("i2"))}, null, new bool[3])
	                    })
	                    .Get());
	            ...
	            TryAssertion(env, epl + " where p00 in (p10, p20)",
	                SupportQueryPlanBuilder.Start(3)
	                    ...
	                    .SetLookupPlanInstruction(0, "s0", new LookupInstructionPlanForge[]{
	                        new LookupInstructionPlanForge(0, "s0", new int[]{1},
	                            new TableLookupPlanForge[]{new FullTableScanLookupPlanForge(0, 1, false, types, GetIndexKey("i1"))}, null, new bool[3]),
	                        new LookupInstructionPlanForge(0, "s0", new int[]{2},
	                            new TableLookupPlanForge[]{new FullTableScanLookupPlanForge(1, 2, false, types, GetIndexKey("i2"))}, null, new bool[3])
	                    })
	                    .Get());
	    ...
	    private static QueryPlanForge GetSingleIndexPlan(EventType[] types, InKeywordTableLookupPlanSingleIdxForge plan) {
	        return SupportQueryPlanBuilder.Start(3)
	            ...
	            .SetLookupPlanInstruction(0, "s0", new LookupInstructionPlanForge[]{
	                new LookupInstructionPlanForge(0, "s0", new int[]{1},
	                    new TableLookupPlanForge[]{plan}, null, new bool[3]),
	                new LookupInstructionPlanForge(0, "s0", new int[]{2},
	                    new TableLookupPlanForge[]{new FullTableScanLookupPlanForge(1, 2, false, types, GetIndexKey("i2"))}, null, new bool[3])
	            })
	            .Get();
	    ...
	    private static void TryAssertionMultiIdx(RegressionEnvironment env) {
	        ...
	        env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[] {11, 101}, new object[] {11, 102}});
	        ...
	        env.SendEventBean(new SupportBean_S0(21, "a", "d"));
	        env.AssertPropsPerRowLastNewAnyOrder("s0", fields, new object[][]{new object[] {21, 101}, new object[] {21, 102}, new object[] {21, 103}});
	    ...
	    private static void TryAssertionSingleIdx(RegressionEnvironment env) {
	        ...
	        env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[] {100, 10}, new object[] {101, 10}});
	        ...
	        env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[] {100, 20}, new object[] {101, 20}, new object[] {102, 20}});

	        env.SendEventBean(new SupportBean_S1(21, "b", "a", "x", "c"));
	        env.AssertPropsPerRowLastNewAnyOrder("s0", fields, new object[][]{new object[] {100, 21}, new object[] {102, 21}});

	        env.SendEventBean(new SupportBean_S1(22, "b", "a", "d", "c"));
	        env.AssertPropsPerRowLastNewAnyOrder("s0", fields, new object[][]{new object[] {100, 22}, new object[] {101, 22}, new object[] {102, 22}});
