...
namespace com.espertech.esper.regressionlib.suite.epl.dataflow
{
	public class EPLDataflowOpBeaconSource {
	    ...
	    private static void RunAssertionFields(RegressionEnvironment env, EventRepresentationChoice representationEnum, bool eventbean) {
	        EPDataFlowInstantiationOptions options;

	        var path = new RegressionPath();
	        env.CompileDeploy(representationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedMyEvent)) + "@public create schema MyEvent(p0 string, p1 long, p2 double)", path);
	        env.CompileDeploy("@name('flow') create dataflow MyDataFlowOne " +
	            "" +
	    ...
	    private class EPLDataflowBeaconNoType : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var instance = env.Runtime.DataFlowService.Instantiate(env.DeploymentId("flow"), "MyDataFlowFour", options);
	            instance.Start();
	            try {
