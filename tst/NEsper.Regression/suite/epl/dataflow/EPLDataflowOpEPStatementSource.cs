	...
	public class EPLDataflowOpEPStatementSource {
	    ...
	    private class EPLDataflowStmtNameDynamic : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            EPAssertionUtil.AssertProps(env.Container, captureOp.GetCurrentAndReset()[0], "id".SplitCsv(), new object[]{"E2"});
	            ...
	            captureOp.WaitForInvocation(100, 1);
	            EPAssertionUtil.AssertProps(env.Container, captureOp.GetCurrentAndReset()[0], "id".SplitCsv(), new object[]{"E4"});
	            ...
	            captureOp.WaitForInvocation(100, 1);
	            EPAssertionUtil.AssertProps(env.Container, captureOp.GetCurrentAndReset()[0], "id".SplitCsv(), new object[]{"XE6X"});
	    ...
	    private class EPLDataflowAllTypes : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            RunAssertionStatementNameExists(env, DefaultSupportGraphEventUtil.EVENTTYPENAME, DefaultSupportGraphEventUtil.GetPONOEvents());
	    ...
	    private class EPLDataflowStatementFilter : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            options.WithParameterProvider(new DefaultSupportGraphParamProvider(Collections.SingletonDataMap("statementFilter", myFilter)));
	            ...
	            EPAssertionUtil.AssertProps(env.Container, captureOp.GetCurrentAndReset()[0], "id".SplitCsv(), new object[]{"B1"});

	            env.CompileDeploy("select theString, intPrimitive from SupportBean");
	            env.SendEventBean(new SupportBean("E1", 1));
	            captureOp.WaitForInvocation(200, 1);
	            EPAssertionUtil.AssertProps(env.Container, captureOp.GetCurrentAndReset()[0], "theString,intPrimitive".SplitCsv(), new object[]{"E1", 1});

	            env.CompileDeploy("@name('s2') select id from SupportBean_A");
	            env.SendEventBean(new SupportBean_A("A1"));
	            captureOp.WaitForInvocation(200, 1);
	            EPAssertionUtil.AssertProps(env.Container, captureOp.GetCurrentAndReset()[0], "id".SplitCsv(), new object[]{"A1"});
	            ...
	            captureOp.WaitForInvocation(200, 1);
	            EPAssertionUtil.AssertProps(env.Container, captureOp.GetCurrentAndReset()[0], "id".SplitCsv(), new object[]{"A3"});

	            env.SendEventBean(new SupportBean_B("B2"));
	            captureOp.WaitForInvocation(200, 1);
	            EPAssertionUtil.AssertProps(env.Container, captureOp.GetCurrentAndReset()[0], "id".SplitCsv(), new object[]{"B2"});
