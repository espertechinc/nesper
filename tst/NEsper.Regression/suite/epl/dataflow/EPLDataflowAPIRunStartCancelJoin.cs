	...
	public class EPLDataflowAPIRunStartCancelJoin {
	    ...
	    private class EPLDataflowNonBlockingJoinCancel : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var options = new EPDataFlowInstantiationOptions().WithOperatorProvider(new DefaultSupportGraphOpProvider(src, output));
	    ...
	    private class EPLDataflowBlockingCancel : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var options = new EPDataFlowInstantiationOptions().WithOperatorProvider(new DefaultSupportGraphOpProviderByOpName(ops));
	            var dfOne = env.Runtime.DataFlowService.Instantiate(env.DeploymentId("flow"), "MyDataFlowOne", options);
	    ...
	    private class EPLDataflowInvalidJoinRun : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var options = new EPDataFlowInstantiationOptions().WithOperatorProvider(new DefaultSupportGraphOpProvider(source));
	            var dfOne = env.Runtime.DataFlowService.Instantiate(env.DeploymentId("flow"), "MyDataFlowOne", options);
	            ...
	            try {
	                ...
	                Assert.AreEqual("Data flow 'MyDataFlowOne' instance has not been executed, please use join after start or run", ex.Message);
	            ...
	            try {
	                ...
	                Assert.AreEqual("Data flow 'MyDataFlowOne' instance has been cancelled and cannot be run or started", ex.Message);
	            }
	    ...
	    private class EPLDataflowBlockingMultipleRunnable : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            try {
	                ...
	                Assert.AreEqual("The data flow 'MyDataFlowOne' has zero or multiple sources and requires the use of the start method instead", ex.Message);
	            }
	    ...
	    private class EPLDataflowNonBlockingJoinSingleRunnable : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var options = new EPDataFlowInstantiationOptions().WithOperatorProvider(new DefaultSupportGraphOpProvider(source, future));
	            var dfOne = env.Runtime.DataFlowService.Instantiate(env.DeploymentId("flow"), "MyDataFlowOne", options);
	    ...
	    private class EPLDataflowFastCompleteBlocking : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            // instantiate
	            var future = new DefaultSupportCaptureOp<object>(1, env.Container.LockManager());
	            var options = new EPDataFlowInstantiationOptions().WithOperatorProvider(new DefaultSupportGraphOpProvider(future));
	            var dfOne = env.Runtime.DataFlowService.Instantiate(env.DeploymentId("flow"), "MyDataFlowOne", options);
	            Assert.AreEqual("MyDataFlowOne", dfOne.DataFlowName);
	            Assert.AreEqual(EPDataFlowState.INSTANTIATED, dfOne.State);
	    ...
	    private static void TryAssertionAfterExec(EPDataFlowInstance df) {
	        ...
	        try {
	            ...
	            Assert.AreEqual("Data flow 'MyDataFlowOne' instance has already completed, please use instantiate to run the data flow again", ex.Message);
	        }
