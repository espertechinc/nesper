    ...
    public class EPLContainedEventExample
    {
	    ...
	    internal class EPLContainedSolutionPatternFinancial : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowLastNew("out", "foreignSymbol,localSymbol,value".SplitCsv(),
	                new object[][]{new object[] {"ABC", "123", 600d}, new object[] {"DEF", "456", 300d}, new object[] {"GHI", "789", 200d}, new object[] {"JKL", "666", 400d}});
        ...
        internal class EPLContainedExample : RegressionExecution
        {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            var stmtTextSix = "@name('s6') select * from pattern [c=Cancel -> o=MediaOrder(OrderId = c.OrderId)[Books.Book]]";
	            env.CompileDeploy(stmtTextSix).AddListener("s6");

	            var stmtTextSeven = "@name('s7') select * from MediaOrder[select OrderId, BookId from Books.Book][select * from Review]";
	            env.CompileDeploy(stmtTextSeven).AddListener("s7");

	            var stmtTextEight = "@name('s8') select * from MediaOrder[select * from Books.Book][select ReviewId, Comment from Review]";
	            env.CompileDeploy(stmtTextEight).AddListener("s8");

	            var stmtTextNine = "@name('s9') select * from MediaOrder[Books.Book as Book][select Book.*, ReviewId, Comment from Review]";
	            env.CompileDeploy(stmtTextNine).AddListener("s9");

	            var stmtTextTen = "@name('s10') select * from MediaOrder[Books.Book as Book][select MediaOrder.*, BookId, ReviewId from Review] as MediaOrder";
	            env.CompileDeploy(stmtTextTen).AddListener("s10");

	            var path = new RegressionPath();
	            var stmtTextElevenZero = "@name('s11_0') @public insert into ReviewStream select * from MediaOrder[Books.Book as Book]\n" +
	            ...
	            var stmtTextElevenOne = "@name('s11') select MediaOrder.OrderId, Book.BookId, Review.ReviewId from ReviewStream";
	            env.CompileDeploy(stmtTextElevenOne, path).AddListener("s11");

	            var stmtTextTwelve = "@name('s12') select * from MediaOrder[Books.Book where Author = 'Orson Scott Card'][Review]";
	            env.CompileDeploy(stmtTextTwelve).AddListener("s12");

	            env.SendEventXMLDOM(eventDocOne, "MediaOrder");

	            env.AssertPropsNew("s1", "OrderId,Items.Item[0].ItemId".SplitCsv(), new object[]{"PO200901", "100001"});
	            env.AssertPropsPerRowNewOnly("s2", "BookId".SplitCsv(), new object[][]{new object[] {"B001"}, new object[] {"B002"}});
	            env.AssertPropsPerRowNewOnly("s3", "BookId".SplitCsv(), new object[][]{new object[] {"B001"}, new object[] {"B002"}});
	            ...
	            env.AssertPropsPerRowNewOnly("s7", "OrderId,BookId,ReviewId".SplitCsv(), new object[][]{new object[] {"PO200901", "B001", "1"}});
	            env.AssertPropsPerRowNewOnly("s8", "ReviewId,BookId".SplitCsv(), new object[][]{new object[] {"1", "B001"}});
	            env.AssertPropsPerRowNewOnly("s9", "ReviewId,BookId".SplitCsv(), new object[][]{new object[] {"1", "B001"}});
	            env.AssertPropsPerRowNewOnly("s10", "ReviewId,BookId".SplitCsv(), new object[][]{new object[] {"1", "B001"}});
	            env.AssertPropsPerRowNewOnly("s11", "MediaOrder.OrderId,Book.BookId,Review.ReviewId".SplitCsv(), new object[][]{new object[] {"PO200901", "B001", "1"}});
        ...
        internal class EPLContainedJoinSelfJoin : RegressionExecution
        {
	        ...
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select Book.BookId,Item.ItemId from MediaOrder[Books.Book] as Book, MediaOrder[Items.Item] as Item where ProductId = BookId order by BookId, Item.ItemId asc";
	            ...
	            env.AssertListener("s0", listener => {
	                ...
	                EPAssertionUtil.AssertPropsPerRow(listener.LastNewData, fieldsItems, new object[][]{new object[] {"B001", "100001"}});
	            });
	            ...
	            env.AssertPropsPerRowLastNew("s0", fieldsItems, new object[][]{new object[] {"B005", "200002"}, new object[] {"B005", "200004"}, new object[] {"B006", "200001"}});
        ...
        internal class EPLContainedJoinSelfLeftOuterJoin : RegressionExecution
        {
	        ...
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select Book.BookId,Item.ItemId from MediaOrder[Books.Book] as Book left outer join MediaOrder[Items.Item] as Item on ProductId = BookId order by BookId, Item.ItemId asc";
	            env.CompileDeploy(stmtText).AddListener("s0");

	            var fieldsItems = "Book.BookId,Item.ItemId".SplitCsv();
	            env.SendEventXMLDOM(eventDocTwo, "MediaOrder");
	            env.AssertPropsPerRowLastNew("s0", fieldsItems, new object[][]{new object[] {"B005", "200002"}, new object[] {"B005", "200004"}, new object[] {"B006", "200001"}, new object[] {"B008", null}});

	            env.SendEventXMLDOM(eventDocOne, "MediaOrder");
	            env.AssertListener("s0", listener => {
	                PrintRows(env, listener.LastNewData);
	                EPAssertionUtil.AssertPropsPerRow(listener.GetAndResetLastNewData(), fieldsItems, new object[][]{new object[] {"B001", "100001"}, new object[] {"B002", null}});
	            });
        ...
        internal class EPLContainedJoinSelfFullOuterJoin : RegressionExecution
        {
	        ...
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select OrderId, Book.BookId,Item.ItemId from MediaOrder[Books.Book] as Book full outer join MediaOrder[select OrderId, * from Items.Item] as Item on ProductId = BookId order by BookId, Item.ItemId asc";
	            env.CompileDeploy(stmtText).AddListener("s0");

	            var fieldsItems = "Book.BookId,Item.ItemId".SplitCsv();
	            env.SendEventXMLDOM(eventDocTwo, "MediaOrder");
	            env.AssertPropsPerRowLastNew("s0", fieldsItems, new object[][]{new object[] {null, "200003"}, new object[] {"B005", "200002"}, new object[] {"B005", "200004"}, new object[] {"B006", "200001"}, new object[] {"B008", null}});
	    ...
	    internal class EPLContainedSolutionPattern : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var stmtText = "@name('s0') select Category, SubEventType, avg(ResponseTimeMillis) as AvgTime from SupportResponseEvent[select Category, * from subEvents]#time(1 min) group by Category, SubEventType order by Category, SubEventType";
	            env.CompileDeploy(stmtText).AddListener("s0");
	            ...
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[] {"svcOne", "typeA", 1000.0}, new object[] {"svcOne", "typeB", 800.0}});
	            ...
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[] {"svcOne", "typeA", 750.0}, new object[] {"svcOne", "typeB", 600.0}});
