	...
	public class EPLContainedEventNested {
	    ...
	    private class EPLContainedNamedWindowFilter : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var stmtText = "@name('s0') select reviewId from OrderWindowNWF[books][reviews] bookReviews order by reviewId asc";
	            env.CompileDeploy(stmtText, path).AddListener("s0");

	            env.SendEventBean(OrderBeanFactory.MakeEventOne());
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[] {1}, new object[] {2}, new object[] {10}});
	    ...
	    private class EPLContainedNamedWindowSubquery : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var stmtText = "@name('s0') select *, (select sum(price) from OrderWindowNWS[books]) as totalPrice from SupportBean";
	            env.CompileDeploy(stmtText, path).AddListener("s0");
	    ...
	    private class EPLContainedNamedWindowOnTrigger : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var stmtText = "@name('s0') on OrderWindowNWOT[books] owb select sbw.* from SupportBeanWindow sbw where theString = title";
	            env.CompileDeploy(stmtText, path).AddListener("s0");
	    ...
	    private class EPLContainedSimple : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var stmtText = "@name('s0') select reviewId from OrderBean[books][reviews] bookReviews order by reviewId asc";
	    ...
	    private class EPLContainedWhere : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            // try where in root
	            var stmtText = "@name('s0') select reviewId from OrderBean[books where title = 'Enders Game'][reviews] bookReviews order by reviewId asc";
	            env.CompileDeploy(stmtText).AddListener("s0");
	    ...
	    private class EPLContainedColumnSelect : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            // columns supplied
	            var stmtText = "@name('s0') select * from OrderBean[select bookId, orderdetail.orderId as orderId from books][select reviewId from reviews] bookReviews order by reviewId asc";
	            env.CompileDeploy(stmtText).AddListener("s0");
	    ...
	    private class EPLContainedUnderlyingSelect : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var stmtText = "@name('s0') select orderdetail.orderId as orderId, bookFrag.bookId as bookId, reviewFrag.reviewId as reviewId " +
	                           "from OrderBean[books as book][select myorder.*, book.* as bookFrag, review.* as reviewFrag from reviews as review] as myorder";
	            env.CompileDeploy(stmtText).AddListener("s0");
