	...
	public class EPLOuterJoinChain4Stream {
	    ...
	    private class EPLJoinLeftOuterJoinRootS2 : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@name('s0') select * from " +
	                      ...
	                      " right outer join " + "SupportBean_S0#length(1000) as s0 on s1.p10 = s0.p00 " +
	                      " left outer join SupportBean_S3#length(1000) as s3 on s2.p20 = s3.p30 ";
	    ...
	    private static void AssertListenerUnd(RegressionEnvironment env, object[][] expected) {
	        env.AssertListener("s0", listener => {
	            var und = ArrayHandlingUtil.GetUnderlyingEvents(listener.GetAndResetLastNewData(), new string[]{"s0", "s1", "s2", "s3"});
