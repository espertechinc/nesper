	...
	public class EPLOuterJoinVarA3Stream {
	    ...
	    private class EPLJoinMapLeftJoinUnsortedProps : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select t1.col1, t1.col2, t2.col1, t2.col2, t3.col1, t3.col2 from Type1#keepall as t1" +
	    ...
	    private class EPLJoinLeftOuterJoinRootS0OM : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            Assert.AreEqual("select * from SupportBean_S0#keepall as s0 left outer join SupportBean_S1#keepall as s1 on s0.p00 = s1.p10 left outer join SupportBean_S2#keepall as s2 on s0.p00 = s2.p20", model.ToEPL());
	    ...
	    private static void AssertListenerUnd(RegressionEnvironment env, object[][] expected) {
	        env.AssertListener("s0", listener => {
	            var und = ArrayHandlingUtil.GetUnderlyingEvents(listener.GetAndResetLastNewData(), new string[]{"s0", "s1", "s2"});
