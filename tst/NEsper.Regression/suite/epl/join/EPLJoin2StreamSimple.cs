...
namespace com.espertech.esper.regressionlib.suite.epl.join
{
	public class EPLJoin2StreamSimple : RegressionExecution {
	    public void Run(RegressionEnvironment env) {
	        var text = "@name('s0') select irstream s0.price, s1.price from SupportMarketDataBean(symbol='S0')#length(3) as s0," +
	        ...
	        env.AssertPropsPerRowIRPairFlattened("s0", new string[]{"s0.price", "s1.price"},
	            new object[][]{new object[] {100.0, 20.0}}, null);

	        env.Milestone(2);

	        env.SendEventBean(MakeMarketDataEvent("S1", 21, 1));
	        env.AssertPropsPerRowIRPairFlattened("s0", new string[]{"s0.price", "s1.price"},
	            new object[][]{new object[] {100.0, 21.0}}, null);
	        ...
	        env.AssertPropsPerRowIRPairFlattened("s0", new string[]{"s0.price", "s1.price"}, null, new object[][]{new object[] {100.0, 20.0}});
