	...
	public class EPLSubselectAggregatedInExistsAnyAll {
	    ...
	    private class EPLSubselectUngroupedWHavingWIn : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@name('s0') select value in (select sum(intPrimitive) from SupportBean#keepall having last(theString) != 'E1') as c0," +
	    ...
	    private class EPLSubselectGroupedWHavingWIn : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var fields = "c0,c1".SplitCsv();
	            var epl = "@name('s0') select value in (select sum(intPrimitive) from SupportBean#keepall group by theString having last(theString) != 'E1') as c0," +
	    ...
	    private class EPLSubselectGroupedWOHavingWIn : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var fields = "c0,c1".SplitCsv();
	            var epl = "@name('s0') select value in (select sum(intPrimitive) from SupportBean#keepall group by theString) as c0," +
	    ...
	    private class EPLSubselectUngroupedWHavingWExists : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var fields = "c0,c1".SplitCsv();
	            var epl = "@name('s0') select exists (select sum(intPrimitive) from SupportBean having sum(intPrimitive) < 15) as c0," +
	    ...
	    private class EPLSubselectExistsSimple : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select id from SupportBean_S0 where exists (select max(id) from SupportBean_S1#length(3))";
	            env.CompileDeployAddListenerMileZero(epl, "s0");
	    ...
	    private class EPLSubselectInSimple : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select id from SupportBean_S0 where id in (select max(id) from SupportBean_S1#length(2))";
	            env.CompileDeployAddListenerMileZero(epl, "s0");
