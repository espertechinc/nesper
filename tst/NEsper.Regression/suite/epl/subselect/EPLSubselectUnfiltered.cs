...
namespace com.espertech.esper.regressionlib.suite.epl.subselect
{
	public class EPLSubselectUnfiltered {
	    ...
	    private class EPLSubselectStartStopStatement : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select id from SupportBean_S0 where (select true from SupportBean_S1#length(1000))";
	    ...
	    private class EPLSubselectWhereClauseWithExpression : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select id from SupportBean_S0 where (select p10='X' from SupportBean_S1#length(1000))";
	    ...
	    private class EPLSubselectJoinUnfiltered : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select (select id from SupportBean_S3#length(1000)) as idS3, (select id from SupportBean_S4#length(1000)) as idS4 from SupportBean_S0#keepall as s0, SupportBean_S1#keepall as s1 where s0.id = s1.id";
	            env.CompileDeployAddListenerMileZero(stmtText, "s0");
	    ...
	    private class EPLSubselectInvalidSubselect : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.TryInvalidCompile("select id in (select * from SupportBean_S1#length(1000)) as value from SupportBean_S0",
	                "Failed to validate select-clause expression subquery number 1 querying SupportBean_S1: Implicit conversion from datatype '" + typeof(SupportBean_S1).FullName + "' to 'Integer' is not allowed [select id in (select * from SupportBean_S1#length(1000)) as value from SupportBean_S0]");
	        }
	    }
	    ...
	    private class EPLSubselectUnfilteredStreamPriorOM : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            subquery.FromClause = FromClause.Create(FilterStream.Create("SupportBean_S1").AddView("length", Expressions.Constant(1000)));
	            ...
	            var stmtText = "select (select prior(0,id) from SupportBean_S1#length(1000)) as idS1 from SupportBean_S0";
	    ...
	    private class EPLSubselectUnfilteredStreamPriorCompile : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select (select prior(0,id) from SupportBean_S1#length(1000)) as idS1 from SupportBean_S0";
	    ...
	    private class EPLSubselectCustomFunction : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select (select " + typeof(SupportStaticMethodLib).FullName + ".minusOne(id) from SupportBean_S1#length(1000)) as idS1 from SupportBean_S0";

	            env.CompileDeployAddListenerMileZero(stmtText, "s0");

	            // check type
	            env.AssertStatement("s0", statement => Assert.AreEqual(typeof(double?), statement.EventType.GetPropertyType("idS1")));
	    ...
	    private class EPLSubselectComputedResult : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select 100*(select id from SupportBean_S1#length(1000)) as idS1 from SupportBean_S0";

	            env.CompileDeployAddListenerMileZero(stmtText, "s0");

	            // check type
	            env.AssertStatement("s0", statement => Assert.AreEqual(typeof(int?), statement.EventType.GetPropertyType("idS1")));
	    ...
	    private class EPLSubselectFilterInside : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select (select id from SupportBean_S1(p10='A')#length(1000)) as idS1 from SupportBean_S0";
	    ...
	    private class EPLSubselectUnfilteredUnlimitedStream : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select (select id from SupportBean_S1#length(1000)) as idS1 from SupportBean_S0";
	    ...
	    private class EPLSubselectUnfilteredLengthWindow : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select (select id from SupportBean_S1#length(2)) as idS1 from SupportBean_S0";
	            TryAssertMultiRowUnfiltered(env, stmtText, "idS1");
	        }
	    }
	    ...
	    private class EPLSubselectUnfilteredAsAfterSubselect : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select (select id from SupportBean_S1#lastevent) as idS1 from SupportBean_S0";
	    ...
	    private class EPLSubselectUnfilteredWithAsWithinSubselect : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select (select id as myId from SupportBean_S1#lastevent) from SupportBean_S0";
	    ...
	    public class EPLSubselectUnfilteredLastEvent : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@name('s0') select theString, (select p00 from SupportBean_S0#lastevent()) as col from SupportBean";
	            env.CompileDeployAddListenerMileZero(epl, "s0");
	    ...
	    private class EPLSubselectUnfilteredExpression : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select (select p10 || p11 from SupportBean_S1#lastevent) as value from SupportBean_S0";

	            env.CompileDeployAddListenerMileZero(stmtText, "s0");

	            // check type
	            env.AssertStatement("s0", statement => Assert.AreEqual(typeof(string), statement.EventType.GetPropertyType("value")));
	    ...
	    private static void TryAssertSingleRowUnfiltered(RegressionEnvironment env, string stmtText, string columnName) {
	        ...
	        // check type
	        env.AssertStatement("s0", statement => Assert.AreEqual(typeof(int?), statement.EventType.GetPropertyType(columnName)));
