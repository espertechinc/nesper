...
namespace com.espertech.esper.regressionlib.suite.epl.subselect
{
	public class EPLSubselectFiltered {
	    ...
	    private class EPLSubselectWhereClauseMultikeyWArrayPrimitive : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select (select id from SupportEventWithManyArray#keepall as sm where sm.intOne = se.array) as value from SupportEventWithIntArray as se";
	            env.CompileDeploy(epl).AddListener("s0");
	    ...
	    private class EPLSubselectSameEventCompile : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select (select * from SupportBean_S1#length(1000)) as events1 from SupportBean_S1";
	            env.EplToModelCompileDeploy(stmtText).AddListener("s0");

	            env.AssertStatement("s0", statement => Assert.AreEqual(typeof(SupportBean_S1), statement.EventType.GetPropertyType("events1")));
	    ...
	    private class EPLSubselectSameEventOM : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            subquery.FromClause = FromClause.Create(FilterStream.Create("SupportBean_S1").AddView(View.Create("length", Expressions.Constant(1000))));
	    ...
	    private class EPLSubselectSelectWildcard : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select (select * from SupportBean_S1#length(1000)) as events1 from SupportBean_S0";
	            env.CompileDeploy(stmtText).AddListener("s0");
	    ...
	    private class EPLSubselectSelectWildcardNoName : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertStatement("s0", statement => Assert.AreEqual(typeof(SupportBean_S1), statement.EventType.GetPropertyType("subselect_1")));
	    ...
	    private class EPLSubselectWhereConstant : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var stmtText = "@name('s0') select (select id from SupportBean_S1#length(1000) where p10='X') as ids1 from SupportBean_S0";
	    ...
	    private class EPLSubselectWherePrevious : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select (select prev(1, id) from SupportBean_S1#length(1000) where id=s0.id) as value from SupportBean_S0 as s0";
	            env.CompileDeployAddListenerMileZero(stmtText, "s0");
	    ...
	    private class EPLSubselectWherePreviousOM : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var stmtText = "select (select prev(1,id) from SupportBean_S1#length(1000) where id=s0.id) as value from SupportBean_S0 as s0";
	            Assert.AreEqual(stmtText, model.ToEPL());
	    ...
	    public class EPLSubselectSelectSceneOne : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowIRPairFlattened("s0", fields, new object[][]{new object[] {100.0, null}}, null);
	            ...
	            env.AssertPropsPerRowIRPairFlattened("s0", fields, new object[][]{new object[] {200.0, -10.0}}, null);
	            ...
	            env.AssertPropsPerRowIRPairFlattened("s0", fields, new object[][]{new object[] {300.0, -20.0}}, new object[][]{new object[] {100.0, null}});
	    ...
	    private class EPLSubselectSelectWithWhereJoined : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select (select id from SupportBean_S1#length(1000) where p10=s0.p00) as ids1 from SupportBean_S0 as s0";
	            env.CompileDeployAddListenerMileZero(stmtText, "s0");
	    ...
	    private class EPLSubselectSelectWhereJoined2Streams : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select (select id from SupportBean_S0#length(1000) where p00=s1.p10 and p00=s2.p20) as ids0 from SupportBean_S1#keepall as s1, SupportBean_S2#keepall as s2 where s1.id = s2.id";
	            env.CompileDeployAddListenerMileZero(stmtText, "s0");
	    ...
	    private class EPLSubselectSelectWhereJoined3Streams : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select (select id from SupportBean_S0#length(1000) where p00=s1.p10 and p00=s3.p30) as ids0 " +
	    ...
	    private class EPLSubselectSelectWhereJoined3SceneTwo : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var stmtText = "@name('s0') select (select id from SupportBean_S0#length(1000) where p00=s1.p10 and p00=s3.p30 and p00=s2.p20) as ids0 " +
	                           "from SupportBean_S1#keepall as s1, SupportBean_S2#keepall as s2, SupportBean_S3#keepall as s3 where s1.id = s2.id and s2.id = s3.id";
	            env.CompileDeployAddListenerMileZero(stmtText, "s0");
	    ...
	    private class EPLSubselectHavingNoAggWFilterWWhere : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select (select intPrimitive from SupportBean(intPrimitive < 20) #keepall where intPrimitive > 15 having theString = 'ID1') as c0 from SupportBean_S0";
	            env.CompileDeployAddListenerMileZero(epl, "s0");
	    ...
	    private class EPLSubselectHavingNoAggWWhere : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select (select intPrimitive from SupportBean#keepall where intPrimitive > 15 having theString = 'ID1') as c0 from SupportBean_S0";
	            env.CompileDeployAddListenerMileZero(epl, "s0");
	    ...
	    private class EPLSubselectHavingNoAggNoFilterNoWhere : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select (select intPrimitive from SupportBean#keepall having theString = 'ID1') as c0 from SupportBean_S0";
	            env.CompileDeployAddListenerMileZero(epl, "s0");
	    ...
	    private static void TrySelectWhereJoined4CoercionBack(RegressionEnvironment env, AtomicLong milestone, string stmtText) {
	        env.CompileDeployAddListenerMile(stmtText, "s0", milestone.GetAndIncrement());
	    ...
	    private static void TrySelectWhereJoined4Coercion(RegressionEnvironment env, AtomicLong milestone, string stmtText) {
	        env.CompileDeployAddListenerMile(stmtText, "s0", milestone.GetAndIncrement());
	    ...
	    private static void SendSBAndS0Assert(RegressionEnvironment env, string theString, int intPrimitive, int? expected) {
	        env.SendEventBean(new SupportBean(theString, intPrimitive));
	        SendS0AndAssert(env, expected);
	    }

	    private static void SendS0AndAssert(RegressionEnvironment env, int? expected) {
	        env.SendEventBean(new SupportBean_S0(0));
