...
namespace com.espertech.esper.regressionlib.suite.epl.script
{
	public class EPLScriptExpression {
	    ...
	    private class EPLScriptGenericResultType : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') expression System.Collections.Generic.IList<String> js:myJSFunc(stringvalue) [\n" +
	            ...
	            env.AssertStatement("s0", statement => Assert.AreEqual(typeof(IList<string>), statement.EventType.GetPropertyType("c0")));
	    ...
	    private class EPLScriptInvalidScriptJS : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            try {
	                ...
	                Assert.IsTrue(ex.Message.Contains("Unexpected exception in statement 'ABC': "), "Message is: " + ex.Message);
	            }
	            env.UndeployAll();
	        }
	    ...
	    private class EPLScriptInvalidScriptMVEL : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.TryInvalidCompile("expression System.String mvel:abc[10] select * from SupportBean where abc()",
	    ...
	    private class EPLScriptScripts : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
                ...
                TryImports(env, "expression MyImportedClass js:callOne() [ " +
                    "var MyJavaClass = Java.type('" + typeof(MyImportedClass).FullName + "');" +
                    "new MyJavaClass() ] ");

                if (TEST_MVEL) {
	                TryImports(env, "expression MyImportedClass mvel:callOne() [ import " + typeof(MyImportedClass).FullName + "; new MyImportedClass() ] ");
	            }
	    ...
	    private class EPLScriptJavaScriptStatelessReturnPassArgs : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            TrySelect(env, path, "expression double js:abc(num) [ " + expression + " ]", "abc(intPrimitive)", typeof(double?), testData);
	            path.Clear();
	            ...
	            TrySelect(env, path, "expression js:abc(myint) [ myint * 10 ]", "abc(intPrimitive)", typeof(object), testData);
	            path.Clear();
	        }
	    }
	    ...
	    private class EPLScriptMVELStatelessReturnPassArgs : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            TrySelect(env, path, "expression mvel:abc(myint) [ myint * 10 ]", "abc(intPrimitive)", typeof(int?), testData);
	            path.Clear();
	            ...
	            TrySelect(env, path, "expression mvel:abc(myint, theString) [" + expression + "]", "abc(intPrimitive, theString)", typeof(object), testData);
	            path.Clear();

	            TrySelect(env, path, "expression int mvel:abc(myint, theString) [" + expression + "]", "abc(intPrimitive, theString)", typeof(int?), testData);
	            path.Clear();
	            ...
	            TrySelect(env, path, "expression mvel:abc(a, b) [" + expression + "]", "abc(theString, intPrimitive)", typeof(string), testData);
	        }
	    }

	    private static void RunAssertionScriptReturningEvents(RegressionEnvironment env, bool soda) {
	        var path = new RegressionPath();
	        env.CompileDeploy("@name('type') @public create schema ItemEvent(id string)", path);

	        var script = "@name('script') @public create expression EventBean[] @type(ItemEvent) js:myScriptReturnsEvents() [\n" +
	        ...
	        env.AssertStatement("script", statement => {
	            Assert.AreEqual(StatementType.CREATE_EXPRESSION, statement.GetProperty(StatementProperty.STATEMENTTYPE));
	    ...
	    private static void TryReturnObject(RegressionEnvironment env, string dialect) {
	        ...
	        var expression = "@name('s0') expression " + typeof(SupportBean).FullName + " " + dialect + ":callIt() [ new " + typeof(SupportBean).FullName + "('E1', 10); ]";
	        env.CompileDeploy(expression + " select callIt() as val0, callIt().getTheString() as val1 from SupportBean as sb").AddListener("s0");
	        env.AssertStatement("s0", statement => Assert.AreEqual(typeof(SupportBean), statement.EventType.GetPropertyType("val0")));

	        env.SendEventBean(new SupportBean());
	        env.AssertPropsNew("s0", "val0.theString,val0.intPrimitive,val1".SplitCsv(), new object[]{"E1", 10, "E1"});
	    ...
	    private static void TryDatetime(RegressionEnvironment env, string dialect) {
	        ...
	        env.CompileDeploy(epl).AddListener("s0");
	        env.AssertStatement("s0", statement => Assert.AreEqual(typeof(int?), statement.EventType.GetPropertyType("val0")));
	    ...
	    private static void TryNested(RegressionEnvironment env, string dialect) {
	        ...
	        var epl = "@name('s0') expression int " + dialect + ":abc(p1, p2) [p1*p2*10]\n" +
	    ...
	    private static void TryReturnTypes(RegressionEnvironment env, string dialect) {
	        ...
	        var epl = "@name('s0') expression string " + dialect + ":one() ['x']\n" +
	    ...
	    private static void TryOverloaded(RegressionEnvironment env, string dialect) {
	        ...
	        var epl = "@name('s0') expression int " + dialect + ":abc() [10]\n" +
	                  "expression int " + dialect + ":abc(p1) [p1*10]\n" +
	    ...
	    private static void TryUnnamedInSelectClause(RegressionEnvironment env, string dialect) {
	        ...
	        var epl = "@name('s0') " + expressionOne + expressionTwo + expressionThree + " select callOne(),callTwo(1),callThree(1, 2) from SupportBean";
	        env.CompileDeploy(epl).AddListener("s0");

	        env.SendEventBean(new SupportBean());
	        env.AssertEventNew("s0", outEvent => {
	    ...
	    private static void TrySelect(RegressionEnvironment env, RegressionPath path, string scriptPart, string selectExpr, Type expectedType, object[][] testdata) {
	        env.CompileDeploy("@name('s0') " + scriptPart +
	            " select " + selectExpr + " as val from SupportBean as sb", path).AddListener("s0");
	        env.AssertStatement("s0", statement => Assert.AreEqual(expectedType, statement.EventType.GetPropertyType("val")));
	    ...
	    private static void TryParseJS(RegressionEnvironment env, string js, Type type, object value) {
	        ...
	        env.SendEventBean(new SupportBean());
	        env.AssertStatement("s0", statement => Assert.AreEqual(type, statement.EventType.GetPropertyType("getResultOne()")));
	    ...
	    private static void TryParseMVEL(RegressionEnvironment env, string mvelExpression, Type type, object value) {
