	...
	public class EPLSpatialMXCIFQuadTreeFilterIndex {
	    ...
	    private class EPLSpatialMXCIFFilterIndexTypeAssertion : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var eplNoIndex = "@name('s0') select * from SupportSpatialEventRectangle(rectangle(0, 0, 1, 1).intersects(rectangle(x, y, width, height)))";
	            env.CompileDeploy(eplNoIndex);
	            SupportFilterServiceHelper.AssertFilterSvcByTypeMulti(env, "s0", "SupportSpatialEventRectangle", new FilterItem[][]{new []{FilterItem.BoolExprFilterItem}});
	            env.UndeployAll();
	            ...
	            SupportFilterServiceHelper.AssertFilterSvcByTypeMulti(env, "s0", "SupportSpatialEventRectangle", new FilterItem[][]{new []{new FilterItem("x,y,width,height/myindex/mxcifquadtree/0.0,0.0,100.0,100.0,4.0,20.0", FilterOperator.ADVANCED_INDEX)}});
	    ...
	    public class EPLSpatialMXCIFFilterIndexWContext : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "create context RectangleContext initiated by SupportSpatialEventRectangle ssr terminated by SupportBean(theString=ssr.id);\n" +
	        ...
	        private void RemoveRectangles(RegressionEnvironment env, IList<SupportSpatialEventRectangle> points) {
	        ...
	        private void Query(RegressionEnvironment env, string stmtName, IList<SupportSpatialEventRectangle> points) {
	        ...
	        private void AddRectangles(RegressionEnvironment env, IList<SupportSpatialEventRectangle> rectangles, AtomicLong rectangleCount) {
	            var random = new Random();
	    ...
	    internal static void SendAssertEventRectangle(RegressionEnvironment env, double x, double y, double width, double height, bool expected) {
	        env.SendEventBean(new SupportSpatialEventRectangle(null, x, y, width, height));
	        env.AssertListenerInvokedFlag("s0", expected);
	    }

	    internal static void SendSpatialEventRectanges(RegressionEnvironment env, int numX, int numY) {
	        for (var x = 0; x < numX; x++) {
	            for (var y = 0; y < numY; y++) {
	                env.SendEventBean(new SupportSpatialEventRectangle(Convert.ToString(x) + "_" + Convert.ToString(y), x, y, 0.1, 0.2));
	            }
	        }
	    }
	}
