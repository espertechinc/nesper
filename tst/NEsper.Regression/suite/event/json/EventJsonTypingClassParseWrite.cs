	...
	public class EventJsonTypingClassParseWrite
	{
		...
		internal class EventJsonTypingNestedRecursive : RegressionExecution
		{
				...
				JObject json)
			{
				...
				string rendered = env.Runtime.RenderEventService.GetJSONRenderer(@event.EventType).Render(@event);
			...
			{
				string[] split = csv.SplitCsv();
				if (split.Length == 0) {
					return new JObject();
				}

				JObject parent = new JObject(new JProperty("id", split[0]));
				JObject current = parent;
				for (int i = 1; i < split.Length; i++) {
					JObject child = new JObject(new JProperty("id", split[i]));
		...
		internal class EventJsonTypingClassWArrayAndColl : RegressionExecution
		{
			public void Run(RegressionEnvironment env)
			{
				...
				JObject jsonFilled = new JObject(
				...
				JObject jsonNulled = new JObject(
				...
				JObject localHalfFilled = new JObject(
					new JProperty("c0", MakeJson("E1", 1)),
					new JProperty(
						"c0Arr",
						new JArray(
				...
				JObject jsonHalfFilled = new JObject(
				...
				JObject localFilledMultiple = new JObject(
					new JProperty("c0", MakeJson("E1", 1)),
					new JProperty(
						"c0Arr",
						new JArray(
				...
				JObject jsonFilledMultiple = new JObject(
		...
		internal class EventJsonTypingVMClass : RegressionExecution
		{
			public void Run(RegressionEnvironment env)
			{
				...
				JValue valueOfOne = new JValue(1);
				...
				DateTimeEx dateTimeEx = DateTimeEx.NowUtc();
				DateTimeOffset dateTimeOffset = dateTimeEx.DateTime;
				...
				Uri uri = new Uri("ftp://ftp.is.co.za/rfc/rfc1808.txt");
