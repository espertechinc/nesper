	...
	public class EventJsonTypingClassParseWrite {
		...
		internal class EventJsonTypingNestedRecursive : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				var epl = "@public @buseventtype create json schema JsonEvent(local " + typeof (MyLocalEventNestedRecursive).FullName + ");\n" +
			...
			private void AssertDepthTwo (RegressionEnvironment env, EventBean @event, JObject json) {
			...
			private void AssertDepthThree (RegressionEnvironment env, EventBean @event, JObject json) {
				var result = (MyLocalEventNestedRecursive) @event.Get ("local");
				Assert.AreEqual ("a", result.id);
				Assert.AreEqual ("b", result.child.id);
		...
		internal class EventJsonTypingClassWArrayAndColl : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				var epl = "@public @buseventtype create json schema JsonEvent(local " + typeof (MyLocalEventWArrayColl).FullName + ");\n" +
				          "@name('s0') select * from JsonEvent#keepall;\n";
				env.CompileDeploy (epl).AddListener ("s0");
			...
			private void AssertEmpty(
				...
				JObject json)
			{
				AssertEvent(env, @event, json, null, Array.Empty<object>(), Array.Empty<object[]>(), Array.Empty<object>());
			}
			...
			private void AssertHalfFilled (RegressionEnvironment env, EventBean @event, JObject json) {
				AssertEvent (env, @event, json,
					MakeLocal ("E1", 1), new object[] { null, MakeLocal ("E2", 2) }, new object[][] { null, new object[] { MakeLocal ("E3", 3), null }, null }, new object[] { MakeLocal ("E4", 4), null });
			}

			private void AssertEvent (RegressionEnvironment env, EventBean @event, JObject json, object c0, object[] c0Arr, object[][] c0Arr2Dim, object[] c0Coll) {
			...
			private JObject MakeJson (string theString, int intPrimitive) {
				return new JObject(
		...
		internal class EventJsonTypingVMClass : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				...
				RunAssertion(env, typeof(MyLocalVMTypeUUID), typeof(Guid), uuid.ToString(), uuid, valueOfOne, milestone);

				var dateTimeEx = DateTimeEx.NowUtc();
				var dateTimeOffset = dateTimeEx.DateTime;
				var dateTime = dateTimeOffset.DateTime;

				RunAssertion(env, typeof(MyLocalVMTypeDateTimeEx), typeof(DateTimeEx), dateTimeEx.ToString(), dateTimeEx, valueOfOne, milestone);
				RunAssertion(env, typeof(MyLocalVMTypeDateTimeOffset), typeof(DateTimeOffset), dateTimeOffset.ToString(), dateTimeOffset, valueOfOne, milestone);
				RunAssertion(env, typeof(MyLocalVMTypeDateTime), typeof(DateTime), dateTime.ToString(), dateTime, valueOfOne, milestone);

				var uri = new Uri("ftp://ftp.is.co.za/rfc/rfc1808.txt");
				Assert.AreEqual(uri, new Uri(uri.ToString()));

				RunAssertion(env, typeof(MyLocalVMTypeURI), typeof(Uri), uri.ToString(), uri, new JValue("a b"), milestone);
			}

			private void RunAssertion (RegressionEnvironment env, Type localType, Type fieldType, string jsonText, object expected, JValue invalidJson, AtomicLong milestone) {
				var epl = "@public @buseventtype create json schema JsonEvent(local " + localType.FullName + ");\n" +
				          "@name('s0') select * from JsonEvent#keepall;\n";
				env.CompileDeploy (epl).AddListener ("s0");
			...
			private void AssertEvent (RegressionEnvironment env, EventBean @event, string json, object expected) {
				...
				EPAssertionUtil.AssertEqualsExactOrder (new object[][] { new object[] { expected } }, result.C0Array2Dim);
		...
		internal class EventJsonTypingListEnumType : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				var epl = "@public @buseventtype create json schema JsonEvent(local " + typeof (MyLocalEventCollectionEnumType).FullName + ");\n" +
				          "@name('s0') select * from JsonEvent#keepall;\n";
			...
			{
				EPAssertionUtil.AssertEqualsExactOrder(values, CollectionValue(@event, function).UnwrapIntoArray<object>());
			}
		...
		internal class EventJsonTypingListBuiltinType : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				var epl = "@public @buseventtype create json schema JsonEvent(local " + typeof (MyLocalEventCollectionBuiltinType).FullName + ");\n" +
				          "@name('s0') select * from JsonEvent#keepall;\n";
		...
		internal class EventJsonTypingClassSimple : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				var epl = "@public @buseventtype create json schema JsonEvent(local " + typeof (MyLocalEvent).FullName + ");\n" +
				          "@name('s0') select * from JsonEvent#keepall;\n";
				env.CompileDeploy (epl).AddListener ("s0");
				...
				env.AssertIterator ("s0", iterator => EPAssertionUtil.AssertProps (iterator.Advance(), "local".Split (","), new object[] { new MyLocalEvent ("abc", 10) }));
		...
		[Serializable] public class MyLocalEvent {
		...
		[Serializable] public class MyLocalEventCollectionBuiltinType {
		...
		[Serializable] public class MyLocalEventCollectionEnumType {
		...
		[Serializable] public class MyLocalEventWArrayColl {
		...
		[Serializable] public class MyLocalEventNestedRecursive {
			public string id;
