	...
	public class EventJsonAdapter {
	    ...
	    private class EventJsonAdapterDocSample : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@Public @buseventtype @JsonSchemaField(name=myDate, adapter='" + typeof(MyDateJSONParser).FullName + "')\n" +
	    ...
	    private class EventJsonAdapterInvalid : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.TryInvalidCompile("@JsonSchemaField(name=mydate, adapter='System.String') create json schema JsonEvent(mydate Date)",
	                "Json schema field adapter class does not implement interface 'JsonFieldAdapterString");

	            env.TryInvalidCompile("@JsonSchemaField(name=mydate, adapter='" + typeof(InvalidAdapterJSONDate).FullName + "') create json schema JsonEvent(mydate Date)",
	                "Json schema field adapter class '" + typeof(InvalidAdapterJSONDate).FullName + "' does not have a default constructor");

	            env.TryInvalidCompile("@JsonSchemaField(name=mydate, adapter='" + nameof(SupportJsonFieldAdapterStringDate) + "') create json schema JsonEvent(mydate String)",
	                "Json schema field adapter class '" + typeof(SupportJsonFieldAdapterStringDate).FullName + "' mismatches the return type of the parse method, expected 'String' but found 'Date'");
	        }
	    }
	    ...
	    private class EventJsonAdapterInsertInto : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.SendEventBean(new LocalEvent(new Point(7, 14), DateTimeParsingFunctions.ParseDefaultEx("2002-05-01T08:00:01.999")));

	            var jsonFilled = "{\"point\":\"7,14\",\"mydate\":\"2002-05-01T08:00:01.999\"}";
	            DoAssert(env, jsonFilled, new object[]{new Point(7, 14), DateTimeParsingFunctions.ParseDefaultEx("2002-05-1T08:00:01.999")});
	    ...
	    private class EventJsonAdapterCreateSchemaWStringTransform : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var jsonFilled = "{\"point\":\"7,14\",\"mydate\":\"2002-05-01T08:00:01.999\"}";
	            SendAssert(env, jsonFilled, new object[]{new Point(7, 14), DateTimeParsingFunctions.ParseDefaultEx("2002-05-1T08:00:01.999")});
	    ...
	    public class LocalEvent {
	        ...
	        public LocalEvent(Point point, DateTimeEx mydate) {
	            this.point = point;
	            this.mydate = mydate;
	        }
	    ...
	    public class InvalidAdapterJSONDate : JsonFieldAdapterString<DateTimeEx> {
	        ...
	        public void Write(DateTimeEx value, Utf8JsonWriter writer) {
	            throw new UnsupportedOperationException();
	        }
	    }
