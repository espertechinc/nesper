...
namespace com.espertech.esper.regressionlib.suite.@event.avro
{
	public class EventAvroEventBean : RegressionExecution {
	    ...
	    public static readonly RecordSchema RECORD_SCHEMA = SchemaBuilder.Record("RecordSchema", Field("i", INNER_SCHEMA));
	    ...
	    private void RunAssertionDynamicProp(RegressionEnvironment env) {
	        ...
	        var innerSchema = SchemaBuilder.Record("InnerSchema", Field("b", StringType(Property(PROP_STRING_KEY, PROP_STRING_VALUE))));
