...
namespace com.espertech.esper.regressionlib.suite.@event.infra
{
	public class EventInfraPropertyUnderlyingSimple : RegressionExecution {
	    ...
	    public void Run(RegressionEnvironment env) {
	        ...
	        var eplJson =
	            "@public @buseventtype @name('schema') create json schema " + JSON_TYPENAME + "(myInt int, myString string);\n" +
	                "@public @buseventtype @name('schema') @JsonSchema(className='" + typeof(MyLocalJsonProvided).FullName + "') create json schema " + JSONPROVIDEDBEAN_TYPENAME + "();\n";
	    ...
	    private void RunAssertionTypeValidProp(RegressionEnvironment env, string typeName, bool boxed) {
	        env.AssertThat(() => {
	            ...
	            var expectedType = new object[][]{new object[] {"myInt", boxed ? typeof(int?) : typeof(int), null, null}, new object[] {"myString", typeof(string), null, null}};
	            ...
	            SupportEventPropUtil.AssertPropEquals(new SupportEventPropDesc("myString", typeof(string)), eventType.GetPropertyDescriptor("myString"));
	        });
	    }

	    private void RunAssertionTypeInvalidProp(RegressionEnvironment env, string typeName, bool xml) {
	        env.AssertThat(() => {
	    ...
	    private static readonly FunctionSendEventIntString FMAP = (eventTypeName, env, a, b) => {
	    ...
	    private static readonly FunctionSendEventIntString FOA = (eventTypeName, env, a, b) => {
	        var oa = new object[]{a, b};
	        env.SendEventObjectArray(oa, eventTypeName);
	        return oa;
	    };

	    private static readonly FunctionSendEventIntString FBEAN = (eventTypeName, env, a, b) => {
	        var bean = new SupportBeanSimple(b, a);
	        env.SendEventBean(bean);
	        return bean;
	    };

	    private static readonly FunctionSendEventIntString FXML = (eventTypeName, env, a, b) => {
	    ...
	    private static readonly FunctionSendEventIntString FAVRO = (eventTypeName, env, a, b) => {
	    ...
	    private static readonly FunctionSendEventIntString FJSON = (eventTypeName, env, a, b) => {
