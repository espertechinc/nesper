...
namespace com.espertech.esper.regressionlib.suite.@event.infra
{
	public class EventInfraPropertyDynamicNested : RegressionExecution {
	    ...
	    private void RunAssertion(RegressionEnvironment env, EventRepresentationChoice outputEventRep, string additionalAnnotations) {
	        ...
	        RunAssertion(env, outputEventRep, additionalAnnotations, BEAN_TYPENAME, FBEAN, null, beanTests, typeof(object), path);

	        // Map
	        var mapTests = new Pair<object, object>[]{
	            new Pair<object, object>(Collections.EmptyDataMap, NotExists()),
	            new Pair<object, object>(Collections.SingletonMap("item", Collections.SingletonMap("id", 101)), Exists(101)),
	            new Pair<object, object>(Collections.SingletonMap("item", Collections.EmptyDataMap), NotExists()),
	        };
	        RunAssertion(env, outputEventRep, additionalAnnotations, MAP_TYPENAME, FMAP, null, mapTests, typeof(object), path);
	        ...
	        RunAssertion(env, outputEventRep, additionalAnnotations, OA_TYPENAME, FOA, null, oaTests, typeof(object), path);
	        ...
	        if (!outputEventRep.IsAvroOrJsonEvent()) {
	            RunAssertion(env, outputEventRep, additionalAnnotations, XML_TYPENAME, FXML, xmlToValue, xmlTests, typeof(XmlNode), path);
	        }
	        ...
	        RunAssertion(env, outputEventRep, additionalAnnotations, AVRO_TYPENAME, FAVRO, null, avroTests, typeof(object), path);
	        ...
	        RunAssertion(env, outputEventRep, additionalAnnotations, JSON_TYPENAME, FJSON, null, jsonTests, typeof(object), path);

	        // Json-Provided (class is provided)
	        var schemasJsonProvided =
	            "@JsonSchema(className='" + typeof(MyLocalJsonProvidedItem).FullName + "') @public @buseventtype @name('schema') create json schema Item();\n" +
	            "@JsonSchema(className='" + typeof(MyLocalJsonProvided).FullName + "') @public @buseventtype @name('schema') create json schema " + JSONPROVIDED_TYPENAME + "(item Item)";
	        env.CompileDeploy(schemasJsonProvided, path);
	        RunAssertion(env, outputEventRep, additionalAnnotations, JSONPROVIDED_TYPENAME, FJSON, null, jsonTests, typeof(object), path);
	    }
	    ...
	    private void RunAssertion(RegressionEnvironment env,
	                              ...
	                              RegressionPath path) {
	        var stmtText = "@name('s0') " + eventRepresentationEnum.GetAnnotationText() + additionalAnnotations + " select " +
	                       "item.id? as myid, " +
	    ...
	    private static readonly SupportEventInfra.FunctionSendEvent FAVRO = (env, value, typeName) => {
