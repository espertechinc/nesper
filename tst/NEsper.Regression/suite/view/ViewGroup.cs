	...
	public class ViewGroup {
	    ...
	    internal class ViewGroupEscapedPropertyText : RegressionExecution{
	        public void Run(RegressionEnvironment env) {
	            var epl = "create schema event as " + typeof(EventWithTags).FullName + ";\n" +
	                      "\n" +
	                      "insert into stream1\n" +
	    ...
	    internal class ViewGroupReclaimTimeWindow : RegressionExecution{
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertStatement("s0", statement =>Assert.AreEqual(10, SupportScheduleHelper.ScheduleCount(statement)));
	            ...
	            env.AssertStatement("s0", statement => Assert.AreEqual(1, SupportScheduleHelper.ScheduleCount(statement)));
	    ...
	    internal class ViewGroupStats : RegressionExecution{
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowIterator("priceAllStats", fields, new object[][] {new object[] {SYMBOL_CISCO, 26.5d}, new object[] {SYMBOL_GE, averageGE}, new object[] {SYMBOL_IBM, 10.875d}});
	            env.AssertPropsPerRowIterator("priceLast3Stats", fields, new object[][] {new object[] {SYMBOL_CISCO, 27d}, new object[] {SYMBOL_GE, 86d + 2d / 3d}, new object[] {SYMBOL_IBM, 11d + 1 / 6d}});
	    ...
	    internal class ViewGroupExpressionGrouped : RegressionExecution{
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select irstream * from SupportBeanTimestamp#groupwin(timestamp.getDayOfWeek())#length(2)";
	            env.CompileDeploy(epl).AddListener("s0");

	            env.SendEventBean(new SupportBeanTimestamp("E1", DateTimeParsingFunctions.ParseDefaultMSec("2002-01-01T09:0:00.000")));
	            env.SendEventBean(new SupportBeanTimestamp("E2", DateTimeParsingFunctions.ParseDefaultMSec("2002-01-08T09:0:00.000")));
	            env.SendEventBean(new SupportBeanTimestamp("E3", DateTimeParsingFunctions.ParseDefaultMSec("2002-01-015T09:0:00.000")));
	            env.AssertListener("s0", listener => {
	                Assert.AreEqual(1, listener.DataListsFlattened.Second.Length);
	            });
	    ...
	    internal class ViewGroupCorrel : RegressionExecution{
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@name('s0') select * from SupportMarketDataBean#groupwin(symbol)#length(1000000)#correl(price, volume, feed)";
	            env.CompileDeploy(epl).AddListener("s0");
	    ...
	    internal class ViewGroupLinest : RegressionExecution{
	        public void Run(RegressionEnvironment env) {
	            // further math tests can be found in the view unit test
	            var epl = "@name('s0') select * from SupportMarketDataBean#groupwin(symbol)#length(1000000)#linest(price, volume, feed)";
	            env.CompileDeploy(epl).AddListener("s0");
	    ...
	    internal class ViewGroupLengthWinWeightAvg : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            if (useGroup) {
	                // 0.69 sec for 100k
	                var stmtString = "@name('s0') select * from SupportSensorEvent#groupwin(type)#length(10000000)#weighted_avg(measurement, confidence)";
	                env.CompileDeploy(stmtString).AddListener("s0");
	            } else {
	                // 0.53 sec for 100k
	                for (var i = 0; i < 10; i++) {
	                    var stmtString = "SELECT * FROM SupportSensorEvent(type='A" + i + "')#length(1000000)#weighted_avg(measurement,confidence)";
	                    env.CompileDeploy(stmtString).AddListener("s0");
	                }
	            }
	    ...
	    internal class ViewGroupReclaimWithFlipTime : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@name('s0') @Hint('reclaim_group_aged=1,reclaim_group_freq=5') select * from SupportBean#groupwin(theString)#keepall";
	            env.CompileDeploy(epl).AddListener("s0");
	    ...
	    internal class ViewGroupTimeAccum : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            var text = "@name('s0') select irstream * from  SupportMarketDataBean#groupwin(symbol)#time_accum(10 sec)";
	            ...
	            env.AssertPropsPerRowIRPair("s0", fields, null, new object[][]{new object[]{10d}, new object[] {20d}});
	    ...
	    internal class ViewGroupTimeBatch : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var text = "@name('s0') select irstream * from  SupportMarketDataBean#groupwin(symbol)#time_batch(10 sec)";
	            env.CompileDeploy(text).AddListener("s0");
	            ...
	            env.AdvanceTime(11000);
	            env.AssertPropsPerRowIRPair("s0", fields, new object[][]{new object[]{10d}, new object[] {20d}}, null);
	    ...
	    internal class ViewGroupTimeOrder : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var text = "@name('s0') select irstream * from SupportBeanTimestamp#groupwin(groupId)#time_order(timestamp, 10 sec)";
	    ...
	    internal class ViewGroupTimeLengthBatch : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var text = "@name('s0') select irstream * from  SupportMarketDataBean#groupwin(symbol)#time_length_batch(10 sec, 100)";
	            env.CompileDeploy(text).AddListener("s0").Milestone(0);
	    ...
	    internal class ViewGroupLengthBatch : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            var text = "@name('s0') select irstream * from SupportMarketDataBean#groupwin(symbol)#length_batch(3) order by symbol asc";
	            env.CompileDeploy(text).AddListener("s0");
	            ...
	            // test iterator
	            env.AssertPropsPerRowIterator("s0", new string[]{"price"}, new object[][]{new object[]{1.0}, new object[] {2.0}, new object[] {20.0}, new object[] {21.0}});

	            env.SendEventBean(MakeMarketDataEvent("S2", 22));
	            env.AssertPropsPerRowIRPairFlattened("s0", fields, new object[][]{new object[]{"S2", 20.0}, new object[] {"S2", 21.0}, new object[] {"S2", 22.0}}, null);
	            ...
	            env.AssertPropsPerRowIRPairFlattened("s0", fields, new object[][]{new object[]{"S1", 1.0}, new object[] {"S1", 2.0}, new object[] {"S1", 3.0}}, null);
	            ...
	            env.AssertPropsPerRowIRPairFlattened("s0", fields, new object[][]{new object[]{"S2", 23.0}, new object[] {"S2", 24.0}, new object[] {"S2", 25.0}}, new object[][]{new object[]{"S2", 20.0}, new object[] {"S2", 21.0}, new object[] {"S2", 22.0}});
	            ...
	            env.AssertPropsPerRowIRPairFlattened("s0", fields, new object[][]{new object[]{"S1", 4.0}, new object[] {"S1", 5.0}, new object[] {"S1", 6.0}}, new object[][]{new object[]{"S1", 1.0}, new object[] {"S1", 2.0}, new object[] {"S1", 3.0}});
	    ...
	    internal class ViewGroupLengthWin : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@name('s0') select irstream theString as c0,intPrimitive as c1 from SupportBean#groupwin(theString)#length(3)";
	            env.CompileDeploy(epl).AddListener("s0");
	            ...
	            env.AssertPropsPerRowNewFlattened("s0", fields, new object[][]{new object[]{"E1", 2}, new object[] {"E2", 21}, new object[] {"E2", 22}, new object[] {"E1", 3}});

	            env.Milestone(4);

	            env.AssertPropsPerRowIteratorAnyOrder("s0", fields, new object[][]{new object[]{"E1", 1}, new object[] {"E1", 2}, new object[] {"E1", 3}, new object[] {"E2", 20}, new object[] {"E2", 21}, new object[] {"E2", 22}});
	            ...
	            env.AssertPropsPerRowIteratorAnyOrder("s0", fields, new object[][]{new object[]{"E1", 1}, new object[] {"E1", 2}, new object[] {"E1", 3}, new object[] {"E2", 23}, new object[] {"E2", 21}, new object[] {"E2", 22}});
	            SendSupportBean(env, "E1", -1);
	    ...
	    private static void AssertLastNewRow(RegressionEnvironment env, string statementName, string symbol, double average) {
	        ...
	        env.AssertListener(statementName, listener => EPAssertionUtil.AssertPropsPerRow(listener.LastNewData, fields, new object[][]{new object[]{symbol, average}}));
	    }
	    ...
	    [Serializable] internal class EventWithTags {
