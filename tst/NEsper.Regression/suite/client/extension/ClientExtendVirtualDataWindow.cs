...
{
	public class ClientExtendVirtualDataWindow : RegressionExecution, IndexBackingTableInfo {
	    ...
	    private void RunAssertionJoinAndLifecyle(RegressionEnvironment env) {
	        ...
	        env.AddListener("s0");
	        AssertIndexSpec(window.RequestedLookups[1], "theString=(String)|longPrimitive=(Long)", "intPrimitive[,](Integer)");
	    ...
	    private void RunAssertionInvalid(RegressionEnvironment env) {
	        ...
	        env.TryInvalidCompile(epl, "Failed to validate data window declaration: Virtual data window forge class " + typeof(SupportBean).FullName + " does not implement the interface " + typeof(VirtualDataWindowForge).FullName);
	    ...
	    private void TryAssertionVirtualDW(RegressionEnvironment env, CaseEnum caseEnum, string uniqueFields, string whereClause, bool unique, IndexAssertionEventSend assertion) {
