...
namespace com.espertech.esper.regressionlib.suite.client.extension
{
	public class ClientExtendAggregationMultiFunctionInlinedClass {
	    ...
	    const string INLINEDCLASS_PREFIXMAP = "inlined_class \"\"\"\n" +
	        ...
	        "\n" +
	        "            public AggregationMultiFunctionAgentMode getAgentMode() {\n" +
	        "                if (parameters.length != 1 || (parameters[0].getForge().EvaluationType != typeof(string))) {\n" +
	        ...
	        "            public EPChainableType getReturnType() {\n" +
	        "                return EPChainableTypeHelper.SingleValue(typeof(IDictionary<string, object>));\n" +
	        "            }\n" +
	        "\n" +
	        "            public AggregationMultiFunctionStateKey getAggregationStateUniqueKey() {\n" +
	    ...
	    private class ClientExtendAggregationMFInlinedOneModule : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            AssertReceived(env, CollectionUtil.BuildMap("Andreas", Collections.SingletonList(p1), "Andras", Arrays.AsList(p2, p3), "And", Collections.SingletonList(p4)));
	        ...
	        private void AssertReceived(RegressionEnvironment env, IDictionary<string, object> expected) {
	    ...
	    private static IDictionary<string, object> MakeSendPerson(RegressionEnvironment env, string name, string id) {
