	    ...
	    private class ClientExtendUDFInlinedWOptions : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') inlined_class \"\"\"\n" +
	                      ...
	                      "      valueCache=" + typeof(ConfigurationCompilerPlugInSingleRowFunction).FullName + ".ValueCache.DISABLED,\n" +
	                      "      filterOptimizable=" + typeof(ConfigurationCompilerPlugInSingleRowFunction).FullName + ".FilterOptimizable.DISABLED,\n" +
