	...
	public class ClientRuntimeStatementAnnotation
	{
		...
		public class ClientRuntimeStatementAnnotationAppSimple : RegressionExecution {
			...
			public void Run (RegressionEnvironment env) {
				var stmtText =
					...
					"@MyAnnotationValueDefaulted " +
					"@MyAnnotationValueEnum(supportEnum=" + typeof (SupportEnum).Name + ".ENUM_VALUE_3) " +
				...
				var stmtTextFormatted = "@MyAnnotationSimple" + NEWLINE +
				                        ...
				                        "@MyAnnotationValueEnum(supportEnum=" + typeof (SupportEnum).Name + ".ENUM_VALUE_3)" + NEWLINE +
		...
		public class ClientRuntimeStatementAnnotationAppNested : RegressionExecution {
			...
			private void RunNestedArray (RegressionEnvironment env) {
				var stmtText = "@MyAnnotationWArrayAndClass(priorities = {@Priority(1), @Priority(3)}, classOne = java.lang.typeof(String), classTwo = typeof(Integer)) @name('s0') select * from SupportBean";
				env.CompileDeploy (stmtText);
		...
		public class ClientRuntimeStatementAnnotationBuiltin : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				...
				env.AssertStatement ("MyTestStmt", statement => {
					...
					var name = (NameAttribute) AnnotationUtil.FindAnnotation (statement.Annotations, typeof (NameAttribute));
				...
				// try fully-qualified
				epl = "@" + typeof (NameAttribute).Name + "('MyTestStmt') @Description('MyTestStmt description') @Tag(name=\"UserId\", value=\"value\") select * from SupportBean";
				env.CompileDeploy (epl).AddListener ("MyTestStmt");
				env.AssertStatement ("MyTestStmt", ClientRuntimeStatementAnnotation.TryAssertion);
				env.UndeployAll ();
				...
				env.AssertStatement ("s0", statement => Assert.AreEqual ("DISABLE_RECLAIM_GROUP", HintEnum.DISABLE_RECLAIM_GROUP.GetHint (statement.Annotations).Value));

				env.CompileDeploy ("@Hint('ITERATE_ONLY,ITERATE_ONLY,DISABLE_RECLAIM_GROUP,ITERATE_ONLY') @name('s1') select * from SupportBean");
				env.AssertStatement ("s1", statement => Assert.AreEqual ("ITERATE_ONLY,ITERATE_ONLY,DISABLE_RECLAIM_GROUP,ITERATE_ONLY", HintEnum.DISABLE_RECLAIM_GROUP.GetHint (statement.Annotations).Value));
				...
				env.AssertStatement ("s4", statement => Assert.AreEqual ("one, two", HintEnum.INDEX.GetHintAssignedValues (statement.Annotations)[0]));

				env.UndeployAll ();

				// NoLock
				env.CompileDeploy ("@name('s0') @NoLock select * from SupportBean");
				env.AssertStatement ("s0", statement => Assert.AreEqual (1, AnnotationUtil.FindAnnotations (statement.Annotations, typeof (NoLockAttribute)).Count));
		...
		private static void TryInvalidAnnotation (RegressionEnvironment env, string stmtText, bool isSyntax, string message) {
			try {
