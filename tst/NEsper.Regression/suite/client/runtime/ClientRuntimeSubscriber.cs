...
namespace com.espertech.esper.regressionlib.suite.client.runtime
{
	public class ClientRuntimeSubscriber {
	    ...
	    private static void TryAssertionBindObjectArr(RegressionEnvironment env, EventRepresentationChoice eventRepresentationEnum, SupportSubscriberMultirowObjectArrayBase subscriber) {
	        var stmtText = eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedStringInt)) + " @name('s0') select irstream theString, intPrimitive from SupportBean" + "#length_batch(2)";
	        var stmt = env.CompileDeploy(stmtText).Statement("s0");
	        stmt.SetSubscriber(subscriber);
	    ...
	    private static void TryAssertionWidening(RegressionEnvironment env, EventRepresentationChoice eventRepresentationEnum, SupportSubscriberRowByRowSpecificBase subscriber) {
	        var stmt = env.CompileDeploy(eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedWidenedEvent)) + " @name('s0') select bytePrimitive, intPrimitive, longPrimitive, floatPrimitive from SupportBean(theString='E1')").Statement("s0");
	        stmt.SetSubscriber(subscriber);
	        Assert.IsTrue(eventRepresentationEnum.MatchesClass(stmt.EventType.UnderlyingType));
	    ...
	    private static void TryAssertionObjectArrayDelivery(RegressionEnvironment env, EventRepresentationChoice eventRepresentationEnum, SupportSubscriberRowByRowObjectArrayBase subscriber) {
	        var stmt = env.CompileDeploy(eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedStringInt)) + " @name('s0') select theString, intPrimitive from SupportBean#unique(theString)").Statement("s0");
	        stmt.SetSubscriber(subscriber);
	        Assert.IsTrue(eventRepresentationEnum.MatchesClass(stmt.EventType.UnderlyingType));
	    ...
	    private static void TryAssertionRowMapDelivery(RegressionEnvironment env, EventRepresentationChoice eventRepresentationEnum, SupportSubscriberRowByRowMapBase subscriber) {
	        var stmt = env.CompileDeploy(eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedStringInt)) + " @name('s0') select irstream theString, intPrimitive from SupportBean#unique(theString)").Statement("s0");
	        stmt.SetSubscriber(subscriber);
	        Assert.IsTrue(eventRepresentationEnum.MatchesClass(stmt.EventType.UnderlyingType));
	    ...
	    private static void TryAssertionStreamSelectWJoin(RegressionEnvironment env, SupportSubscriberRowByRowSpecificBase subscriber) {
	        var stmt = env.CompileDeploy("@name('s0') select null, s1, s0 from SupportBean#keepall as s0, SupportMarketDataBean#keepall as s1 where s0.theString = s1.symbol").Statement("s0");
	        stmt.SetSubscriber(subscriber);
	    ...
	    private static void TryAssertionBindWildcardJoin(RegressionEnvironment env, SupportSubscriberRowByRowSpecificBase subscriber) {
	        var stmt = env.CompileDeploy("@name('s0') select * from SupportBean#keepall as s0, SupportMarketDataBean#keepall as s1 where s0.theString = s1.symbol").Statement("s0");
	        stmt.SetSubscriber(subscriber);
	    ...
	    private static void TryAssertionStreamWildcardJoin(RegressionEnvironment env, SupportSubscriberRowByRowSpecificBase subscriber) {
	        var stmt = env.CompileDeploy("@name('s0') select theString || '<', s1.* as s1, s0.* as s0 from SupportBean#keepall as s0, SupportMarketDataBean#keepall as s1 where s0.theString = s1.symbol").Statement("s0");
	        stmt.SetSubscriber(subscriber);
	    ...
	    private static void TryAssertionOutputLimitNoJoin(RegressionEnvironment env, EventRepresentationChoice eventRepresentationEnum, SupportSubscriberRowByRowSpecificBase subscriber) {
	        var stmt = env.CompileDeploy(eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedStringInt)) + " @name('s0') select theString, intPrimitive from SupportBean output every 2 events").Statement("s0");
	        stmt.SetSubscriber(subscriber);
	        Assert.IsTrue(eventRepresentationEnum.MatchesClass(stmt.EventType.UnderlyingType));
	    ...
	    private static void TryAssertionOutputLimitJoin(RegressionEnvironment env, SupportSubscriberRowByRowSpecificBase subscriber) {
	        var stmt = env.CompileDeploy("@name('s0') select theString, intPrimitive from SupportBean#keepall, SupportMarketDataBean#keepall where symbol = theString output every 2 events").Statement("s0");
	        stmt.SetSubscriber(subscriber);
	    ...
	    private static void TryAssertionStaticMethod(RegressionEnvironment env) {
	        ...
	        EPAssertionUtil.AssertEqualsExactOrder(new object[][]{new object[] {"E2", 200}}, SupportSubscriberRowByRowStaticWStatement.Indicate);
	    ...
	    private class ClientRuntimeSubscriberInvocationTargetEx : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            stmt.AddListener(new DelegateUpdateListener((sender, args) => 
		            throw new EPRuntimeException("test exception 2")));
	            stmt.AddListenerWithReplay(new DelegateUpdateListener((sender, args) =>
	    ...
	    private static void TryAssertionNamedWindow(RegressionEnvironment env, EventRepresentationChoice eventRepresentationEnum) {
	        ...
	        var stmtTextCreate = eventRepresentationEnum.GetAnnotationText() + " @name('create') @public create window MyWindow#keepall as select theString as key, intPrimitive as value from SupportBean";
	    ...
	    private class ClientRuntimeSubscriberPerformanceSynthetic : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            UpdateListener listener = new DelegateUpdateListener((sender, args) => {
