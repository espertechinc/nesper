	...
	public class ClientStageSendEvent {
	    ...
	    private class ClientStageSendEventContextNestedInitiatedOverKeyed : ClientStageRegressionExecution {
	        public override void Run(RegressionEnvironment env) {
	            ...
	            AssertPropsPerRow(env.ListenerStage("ST", "s0").GetAndResetLastNewData(), fields, new object[][]{new object[] {1000, "A", 10 + 20}});
	            SendEvent(env, "ST", "B", 21);
	            AssertPropsPerRow(env.ListenerStage("ST", "s0").GetAndResetLastNewData(), fields, new object[][]{new object[] {1000, "B", 21}});

	            env.Milestone(2);

	            SendEventS0(env, "ST", 2000);
	            SendEvent(env, "ST", "A", 30);
	            AssertPropsPerRow(env.ListenerStage("ST", "s0").GetAndResetLastNewData(), fields, new object[][]{new object[] {1000, "A", 10 + 20 + 30}, new object[] {2000, "A", 30}});
	            ...
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[] {1000, "A", 10 + 20 + 30 + 40}, new object[] {2000, "A", 30 + 40}});

	            SendEventS0(env, null, 3000);

	            SendEvent(env, null, "B", 41);
	            env.AssertPropsPerRowLastNew("s0", fields, new object[][]{new object[] {1000, "B", 21 + 41}, new object[] {2000, "B", 41}, new object[] {3000, "B", 41}});
	    ...
	    private class ClientStageSendEventContextInitiatedNoTerminated : ClientStageRegressionExecution {
	        public override void Run(RegressionEnvironment env) {
	            ...
	            AssertPropsPerRowAnyOrder(env.ListenerStage("ST", "s0").GetAndResetLastNewData(), fields, new object[][]{new object[] {100, 701}, new object[] {300, 300}});
	            ...
	            env.AssertPropsPerRowLastNewAnyOrder("s0", fields, new object[][]{new object[] {100, 1101}, new object[] {300, 700}});

	            SendEvent(env, null, "init", 401);
	            env.AssertPropsPerRowLastNewAnyOrder("s0", fields, new object[][]{new object[] {100, 1101 + 401}, new object[] {300, 700 + 401}, new object[] {401, 401}});
	    ...
	    private class ClientStageSendEventPatternWNot : ClientStageRegressionExecution {
	        public override void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select * from pattern[SupportBean(theString='a') and not SupportBean(theString='b')]";
	            env.CompileDeploy(epl).AddListener("s0");
	            var deploymentId = env.DeploymentId("s0");
	            env.StageService.GetStage("ST");
	    ...
	    private class ClientStageSendEventPatternWUntil : ClientStageRegressionExecution {
	        public override void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select * from pattern[SupportBean(theString='a') until SupportBean(theString='b')]";
	            env.CompileDeploy(epl).AddListener("s0");
	            var deploymentId = env.DeploymentId("s0");
	            env.StageService.GetStage("ST");
	    ...
	    private class ClientStageSendEventPatternWAnd : ClientStageRegressionExecution {
	        public override void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select * from pattern[SupportBean(theString='a') and SupportBean(theString='b')]";
	            env.CompileDeploy(epl).AddListener("s0");
	            var deploymentId = env.DeploymentId("s0");
	    ...
	    private class ClientStageSendEventPatternWFollowedBy : ClientStageRegressionExecution {
	        public override void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select * from pattern[SupportBean(theString='a') -> SupportBean(theString='b') ->" +
