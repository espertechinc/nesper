	...
	public class InfraNWTableFAFSubstitutionParams : IndexBackingTableInfo {
	    ...
	    private class InfraParameterizedQueryNamedParameter : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            RunParameterizedQueryWCompile(env, path, eplOneParam, Collections.SingletonDataMap("p0", 5), new string[]{"E5"});

	            var eplTwiceUsed = "select * from MyInfra where intPrimitive = ?:p0:int or intBoxed = ?:p0:int";
	            RunParameterizedQueryWCompile(env, path, eplTwiceUsed, Collections.SingletonDataMap("p0", 12), new string[]{"E2"});
	            ...
	            RunParameterizedQuery(env, query, CollectionUtil.PopulateNameValueMap("p0", 3, "p1", 3), Array.Empty<string>());
	            RunParameterizedQuery(env, query, CollectionUtil.PopulateNameValueMap("p0", 13, "p1", 13), Array.Empty<string>());
	    ...
	    private class InfraParameterizedQueryInvalidInsufficientValues : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            try {
	                ...
	                SupportMessageAssertUtil.AssertMessage(ex.Message, "Missing values for substitution parameters, use prepare-parameterized instead");
	            }
	    ...
	    private class InfraParameterizedQueryInvalidParametersTyped : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            TryInvalidSetObject(env, compiled, query => query.SetObject(1, 10), "Failed to set substitution parameter 1, expected a value of type 'System.String': " + typeof(string));

	            // name, typed
	            compiled = env.CompileFAF("select * from MyWindow where theString=?:p0:string", path);
	            TryInvalidSetObject(env, compiled, query => query.SetObject("p0", 10), "Failed to set substitution parameter 'p0', expected a value of type 'System.String': " + typeof(string));
	    ...
	    private static void RunParameterizedQueryWCompile(RegressionEnvironment env, RegressionPath path, string eplOneParam, IDictionary<string, object> @params, string[] expected) {
	    ...
	    private static void RunParameterizedQuery(RegressionEnvironment env, EPFireAndForgetPreparedQueryParameterized parameterizedQuery, IDictionary<string, object> parameters, string[] expected) {
