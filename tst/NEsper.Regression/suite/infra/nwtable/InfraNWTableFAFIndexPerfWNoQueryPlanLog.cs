	...
	public class InfraNWTableFAFIndexPerfWNoQueryPlanLog : IndexBackingTableInfo {
	    ...
	    private class InfraFAFKeyBTreePerformance : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            var eplIdx2One = "select intPrimitive as sumi from MyInfraFAFKB where intPrimitive = 5501 and theString = 'A'";
	            RunFAFAssertion(env, path, eplIdx2One, 5501);

	            var eplIdx2Two = "select sum(intPrimitive) as sumi from MyInfraFAFKB where intPrimitive in [5000:5004) and theString = 'A'";
	            RunFAFAssertion(env, path, eplIdx2Two, 5000 + 5001 + 5003 + 5002);

	            var eplIdx2Three = "select sum(intPrimitive) as sumi from MyInfraFAFKB where intPrimitive=5001 and theString between 'A' and 'B'";
	    ...
	    private static void RunFAFAssertion(RegressionEnvironment env, RegressionPath path, string epl, int? expected) {
	    ...
	    private static void RunFAFQuery(EPFireAndForgetPreparedQuery query, int? expectedValue) {
	        var result = query.Execute();
	        Assert.AreEqual(1, result.Array.Length);
