	...
	public class InfraNWTableOnSelect : IndexBackingTableInfo {
	    ...
	    private class InfraOnSelectMultikeyWArray : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            var stmtTextSelect = "@name('s0') on SupportBean select array, sum(value) as thesum from MyInfraPC group by array";
	            ...
	            env.SendEventBean(new SupportBean());
	            env.AssertPropsPerRowLastNewAnyOrder("s0", "thesum".SplitCsv(), new object[][] {new object[]{42}, new object[]{22}});
	    ...
	    private class InfraPatternCorrelation : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            // create select stmt
	            var stmtTextSelect = "@name('select') on pattern [every ea=SupportBean_A or every eb=SupportBean_B] select mywin.* from MyInfraPC as mywin where a = coalesce(ea.id, eb.id)";
	            env.CompileDeploy(stmtTextSelect, path).AddListener("select");

	            // create insert into
	            var stmtTextInsertOne = "insert into MyInfraPC select theString as a, intPrimitive as b from SupportBean";
	            ...
	            env.AssertListenerNotInvoked("select");
	            env.AssertPropsPerRowIteratorAnyOrder("create", fields, new object[][]{new object[]{"E1", 1}, new object[]{"E2", 2}, new object[]{"E3", 3}});
	            if (namedWindow) {
	    ...
	    private class InfraSelectCorrelationDelete : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            SendSupportBean_A(env, "E1");
	            env.AssertListenerNotInvoked("select");
	            env.AssertPropsPerRowIteratorAnyOrder("create", fields, new object[][]{new object[]{"E2", 2}, new object[]{"E3", 3}});
	    ...
	    private class InfraSelectAggregationGrouping : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            // fire trigger
	            SendSupportBean_A(env, "A1");
	            env.AssertPropsPerRowNewOnly("select", fields, new object[][]{new object[]{"E2", 2}, new object[]{"E1", 6}});
	            ...
	            env.AssertPropsPerRowNewOnly("select", fields, new object[][]{new object[]{"E4", -1}, new object[]{"E2", 12}, new object[]{"E1", 106}});
	            ...
	            env.AssertPropsPerRowNewOnly("select", fields, new object[][]{new object[]{"E4", -1}, new object[]{"E1", 106}});
	    ...
	    private class InfraSelectAggregation : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            // create insert into
	            var stmtTextInsertOne = "insert into MyInfraSA select theString as a, intPrimitive as b from SupportBean";
	            env.CompileDeploy(stmtTextInsertOne, path);
	    ...
	    private class InfraSelectJoinColumnsLimit : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            // create select stmt
	            var stmtTextSelect = "@name('select') on SupportBean_A as trigger select trigger.id as triggerid, win.a as wina, b from MyInfraSA as win order by wina";
	            env.CompileDeploy(stmtTextSelect, path).AddListener("select");
	    ...
	    private class InfraSelectCondition : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            // create select stmt
	            var stmtTextSelect = "@name('select') on SupportBean_A select mywin.*, id from " + infraName + " as mywin where " + infraName + ".b < 3 order by a asc";
	            env.CompileDeploy(stmtTextSelect, path).AddListener("select");
	            env.AssertStatement("select", statement => Assert.AreEqual(StatementType.ON_SELECT, statement.GetProperty(StatementProperty.STATEMENTTYPE)));

	            // create insert into
	            var stmtTextInsertOne = "@name('insert') insert into " + infraName + " select theString as a, intPrimitive as b from SupportBean";
	            env.CompileDeploy(stmtTextInsertOne, path);
	            ...
	            // fire trigger
	            SendSupportBean_A(env, "A1");
	            env.AssertPropsPerRowNewOnly("select", fieldsCreate, new object[][]{new object[]{"E1", 1}, new object[]{"E2", 2}});
	            env.AssertPropsPerRowIteratorAnyOrder("create", fieldsCreate, new object[][]{new object[]{"E1", 1}, new object[]{"E2", 2}, new object[]{"E3", 3}});
	            ...
	            env.AssertListener("select", listener => {
	                ...
	                EPAssertionUtil.AssertProps(listener.LastNewData[0], fieldsOnSelect, new object[]{"E1", 1, "A2"});
	                EPAssertionUtil.AssertProps(listener.LastNewData[1], fieldsOnSelect, new object[]{"E2", 2, "A2"});
	                EPAssertionUtil.AssertProps(listener.GetAndResetLastNewData()[2], fieldsOnSelect, new object[]{"E4", 0, "A2"});
	            });
	            env.AssertPropsPerRowIteratorAnyOrder("create", fieldsCreate, new object[][]{new object[]{"E1", 1}, new object[]{"E2", 2}, new object[]{"E3", 3}, new object[]{"E4", 0}});
	            env.AssertIterator("select", iterator => Assert.IsFalse(iterator.MoveNext()));
	    ...
	    private class InfraPatternTimedSelect : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            var stmtTextOnSelect = "@name('select') on pattern [every timer:interval(10 sec)] select theString from MyInfraPTS having count(theString) > 0";
	    ...
	    private class InfraSelectAggregationHavingStreamWildcard : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            var stmtTextSelect = "@name('select') on SupportBean_A select mwc.* as mwcwin from MyInfraSHS mwc where id = a group by a having sum(b) = 20";
	            env.CompileDeploy(stmtTextSelect, path).AddListener("select");
	            env.AssertStatement("select", statement => Assert.IsFalse(((EPStatementSPI) statement).StatementContext.IsStatelessSelect));
	    ...
	    private class InfraWindowAgg : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            AssertReceived(env, namedWindow, beans, new int[]{0, 1}, new int[]{0, 1}, "E0,E1".SplitCsv(), new object[]{0, 1});

	            // add bean
	            env.SendEventBean(beans[2]);
	            env.SendEventBean(new SupportBean_S0(10));
	            AssertReceived(env, namedWindow, beans, new int[]{0, 1, 2}, new int[]{0, 1}, "E0,E1,E2".SplitCsv(), new object[]{0, 1, 2});
	            ...
	            AssertReceived(env, namedWindow, beans, new int[]{0, 2}, new int[]{0}, "E0,E2".SplitCsv(), new object[]{0, 2});

	            // delete another bean
	            env.SendEventBean(new SupportBean_S1(13, "E0"));
	            env.SendEventBean(new SupportBean_S0(14));
	            AssertReceived(env, namedWindow, beans, new int[]{2}, Array.Empty<int>(), "E2".SplitCsv(), new object[]{2});
	    ...
	    private class InfraOnSelectIndexChoice : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            AssertIndexChoice(env, namedWindow, Array.Empty<string>(), preloadedEventsOne, "std:unique(s1)",
	                new IndexAssertion[]{
	            ...
	            AssertIndexChoice(env, namedWindow, Array.Empty<string>(), preloadedEventsRelOp, "win:keepall()",
	                new IndexAssertion[]{
	    ...
	    private static void AssertIndexChoice(RegressionEnvironment env, bool namedWindow, string[] indexes, object[] preloadedEvents, string datawindow,
	        ...
	        foreach (var assertion in assertions) {
	            ...
	            if (assertion.EventSendAssertion == null) {
	                ...
	                env.AssertThat(() => SupportQueryPlanIndexHook.AssertOnExprTableAndReset(assertion.ExpectedIndexName, assertion.IndexBackingClass));
	    ...
	    private static void AssertReceived(RegressionEnvironment env, bool namedWindow, SupportBean[] beans, int[] indexesAll, int[] indexesWhere, string[] mapKeys, object[] mapValues) {
	        env.AssertListener("select", listener => {
