	...
	public class InfraNWTableCreateIndex {
	    ...
	    private class InfraOnSelectReUse : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertThat(() => Assert.AreEqual(namedWindow ? 1 : 2, GetIndexCount(env, namedWindow, "create", "MyInfraONR")));
	            ...
	            env.AssertThat(() => Assert.AreEqual(1, SupportInfraUtil.GetIndexCountNoContext(env, true, "cw", "MyInfraFour")));
	    ...
	    public class InfraLateCreate : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            // create insert into
	            var stmtTextInsertOne = "@name('Insert') insert into MyInfra select theString, intPrimitive from SupportBean";
	            ...
	            env.AssertThat(() => {
	                var result = env.CompileExecuteFAF("select * from MyInfra where theString = 'B2' order by intPrimitive asc", path);
	                EPAssertionUtil.AssertPropsPerRow(result.Array, fields, new object[][]{new object[] {"B2", 1}, new object[] {"B2", 2}});
	            });
	    ...
	    private class InfraMultiRangeAndKey : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            ...
	            var query1 = "select * from MyInfraMRAK where rangeStartLong > 1 and rangeEndLong > 2 and keyLong=1 and key='K1' order by id asc";
	            ...
	            RunQueryAssertion(env, path, query1, fields, new object[][]{new object[] {"E1"}, new object[] {"E2"}});

	            env.Milestone(0);

	            env.SendEventBean(SupportBeanRange.MakeLong("E3", "K1", 1L, 3L, 3L));
	            RunQueryAssertion(env, path, query1, fields, new object[][]{new object[] {"E1"}, new object[] {"E2"}, new object[] {"E3"}});

	            var query2 = "select * from MyInfraMRAK where rangeStartLong > 1 and rangeEndLong > 2 and keyLong=1 order by id asc";
	            RunQueryAssertion(env, path, query2, fields, new object[][]{new object[] {"E1"}, new object[] {"E2"}, new object[] {"E3"}});
	    ...
	    private static int GetIndexCount(RegressionEnvironment env, bool namedWindow, string infraStmtName, string infraName) {
	        return SupportInfraUtil.GetIndexCountNoContext(env, namedWindow, infraStmtName, infraName);
	    }

	    private static void AssertIndexesRef(RegressionEnvironment env, bool namedWindow, string name, string csvNames) {
	    ...
	    private static void AssertIndexCountInstance(RegressionEnvironment env, bool namedWindow, string name, int count) {
	        var repo = GetIndexInstanceRepo(env, namedWindow, name);
	        Assert.AreEqual(count, repo.Tables.Count);
	    }

	    private static EventTableIndexRepository GetIndexInstanceRepo(RegressionEnvironment env, bool namedWindow, string name) {
	        if (namedWindow) {
	    ...
	    private static EventTableIndexMetadataEntry GetIndexEntry(RegressionEnvironment env, bool namedWindow, string name) {
	    ...
	    private static EventTableIndexMetadata GetIndexMetaRepo(RegressionEnvironment env, bool namedWindow, string name) {
	        if (namedWindow) {
