	...
	public class InfraNamedWindowOnMerge {
	    ...
	    private class InfraOnMergeSetRHSEvent : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('window') create window RecentWindow#time(30 seconds) (id string, currentSewid SimpleEventWithId, prevSewid SimpleEventWithId);\n" +
	    ...
	    private class InfraPropertyInsertBean : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@name('merge') on SupportBean as up merge MergeWindow as mv where mv.theString=up.theString when not matched then insert select intPrimitive";
	            env.CompileDeploy(epl, path);
	            env.SendEventBean(new SupportBean("E1", 10));

	            env.AssertIterator("window", iterator => {
	                var theEvent = iterator.Advance();
	                EPAssertionUtil.AssertProps(theEvent, "theString,intPrimitive".SplitCsv(), new object[]{null, 10});
	            });
	            ...
	            env.AssertPropsPerRowIterator("window", "theString,intPrimitive".SplitCsv(), new object[][]{new object[] {null, 10}, new object[] {"E2", 20}});
	    ...
	    private static void TryAssertionSubselect(RegressionEnvironment env, EventRepresentationChoice eventRepresentationEnum) {
	        ...
	        var epl = eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedMyEvent)) + " @public @buseventtype create schema MyEvent as (in1 string, in2 int);\n";
	        epl += eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedMySchema)) + " @public @buseventtype create schema MySchema as (col1 string, col2 int);\n";
	    ...
	    private static void TryAssertionDocExample(RegressionEnvironment env, EventRepresentationChoice eventRepresentationEnum) {
	        var path = new RegressionPath();
	        var baseModuleEPL = eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedOrderEvent)) +
	        ...
	        var appModuleOne = eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedProductTotalRec)) + " create schema ProductTotalRec as (productId string, totalPrice double);" +
	                           "" +
	        ...
	        env.AssertPropsPerRowIteratorAnyOrder("nwProd", "productId,totalPrice".SplitCsv(), new object[][]{new object[] {"P1", 21d}, new object[] {"P2", 3d}});
	        env.AssertPropsPerRowIteratorAnyOrder("nwOrd", "orderId,quantity".SplitCsv(), new object[][]{new object[] {"O1", 200}, new object[] {"O2", 300}});
	    ...
	    private static void SendOrderEvent(RegressionEnvironment env, EventRepresentationChoice eventRepresentationEnum, string orderId, string productId, double price, int quantity, bool deletedFlag) {
	        if (eventRepresentationEnum.IsObjectArrayEvent()) {
	    ...
	    private static void RunAssertionInsertSelectStar(RegressionEnvironment env, string onInsert) {
	        ...
	        env.AssertPropsPerRowIterator("window", "theString,intPrimitive".SplitCsv(), new object[][]{new object[] {"E1", 10}, new object[] {"E2", 20}});
	    ...
	    [Serializable] public class MyLocalJsonProvidedMySchema {
	        public string col1;
	        public int col2;
	    }

	    [Serializable] public class MyLocalJsonProvidedOrderEvent {
	    ...
	    [Serializable] public class MyLocalJsonProvidedProductTotalRec {
	        public string productId;
