	...
	public class InfraNamedWindowTypes {
	    ...
	    private class InfraEventTypeColumnDef : RegressionExecution {
	        ...
	        public void Run(RegressionEnvironment env) {
	            var epl = eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedSchemaOne)) + " @name('schema') @buseventtype @public create schema SchemaOne(col1 int, col2 int);\n";
	            epl += eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedSchemaWindow)) + " @name('create') @public create window SchemaWindow#lastevent as (s1 SchemaOne);\n";
	            ...
	            foreach (var name in new string[] {"schema", "create"}) {
	                env.AssertStatement(name, statement => Assert.IsTrue(eventRepresentationEnum.MatchesClass(statement.EventType.UnderlyingType)));
	            }
	    ...
	    private class InfraMapTranspose : RegressionExecution {
	        ...
	        private void TryAssertionMapTranspose(RegressionEnvironment env, EventRepresentationChoice eventRepresentationEnum) {
	            ...
	            // create window
	            var epl = eventRepresentationEnum.GetAnnotationText() + " @name('create') create window MyWindowMT#keepall as select one, two from OuterType;\n" +
	    ...
	    private class InfraNoWildcardWithAs : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('create') create window MyWindowNW#keepall as select theString as a, longPrimitive as b, longBoxed as c from SupportBean;\n" +
	    ...
	    private class InfraNoWildcardNoAs : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('create') create window MyWindowNWNA#keepall as select theString, longPrimitive, longBoxed from SupportBean;\n" +
	    ...
	    private class InfraConstantsAs : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('create') create window MyWindowCA#keepall as select '' as theString, 0L as longPrimitive, 0L as longBoxed from MyMapWithKeyPrimitiveBoxed;\n" +
	    ...
	    private class InfraCreateSchemaModelAfter : RegressionExecution {
	        ...
	        private void TryAssertionCreateSchemaModelAfter(RegressionEnvironment env, EventRepresentationChoice eventRepresentationEnum) {
	            var epl = eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedEventTypeOne)) + " @public @buseventtype create schema EventTypeOne (hsi int);\n" +
	                      eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedEventTypeTwo)) + " @public @buseventtype create schema EventTypeTwo (event EventTypeOne);\n" +
	            ...
	            if (eventRepresentationEnum.IsObjectArrayEvent()) {
	                ...
	                var theEvent = new GenericRecord(env.RuntimeAvroSchemaPreconfigured("EventTypeOne").AsRecordSchema());
	                theEvent.Put("hsi", 10);
	    ...
	    private class InfraCreateTableSyntax : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('create') create window MyWindowCTS#keepall (stringValOne varchar, stringValTwo string, intVal int, longVal long);\n" +
	    ...
	    private class InfraModelAfterMap : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('create') create window MyWindowMAM#keepall select * from MyMapWithKeyPrimitiveBoxed;\n" +
	    ...
	    private class InfraWildcardWithFields : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('create') create window MyWindowWWF#keepall as select *, id as myid from SupportBean_A;\n" +
	    ...
	    [Serializable] public class MyLocalJsonProvidedSchemaOne {
	        public int col1;
	        public int col2;
	    }

	    [Serializable] public class MyLocalJsonProvidedSchemaWindow {
	        public MyLocalJsonProvidedSchemaOne s1;
	    }

	    [Serializable] public class MyLocalJsonProvidedEventTypeOne {
	        public int hsi;
	    }

	    [Serializable] public class MyLocalJsonProvidedEventTypeTwo {
