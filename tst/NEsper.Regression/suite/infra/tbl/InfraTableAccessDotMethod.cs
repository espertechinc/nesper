...
namespace com.espertech.esper.regressionlib.suite.infra.tbl
{
	...
	public class InfraTableAccessDotMethod {
	    ...
	    private class InfraPlainPropDatetimeAndEnumerationAndMethod : RegressionExecution {
	        ...
	        private static void RunPlainPropertyWDatetimeEtc(RegressionEnvironment env, bool grouped, bool soda, AtomicLong milestone) {
	            ...
	            var eplDeclare = "@public create table varaggPWD (key string" + (grouped ? " primary key" : "") +
	                             ", ts long" +
	                             ", mb " + myBean +
	                             ", mbarr " + myBean + "[]" +
	                             ", me MyEvent, mearr MyEvent[])";
	            ...
	            env.AssertEventNew("s0", output => {
	                EPAssertionUtil.AssertProps(output, "c0,c1,c3".SplitCsv(),
	    ...
	    private static void TryAssertionNestedDotMethod(RegressionEnvironment env, bool grouped, bool soda, AtomicLong milestone) {
	    ...
	    private static void RunAggregationWDatetimeEtc(RegressionEnvironment env, bool grouped, bool soda, AtomicLong milestone) {

	        var path = new RegressionPath();
	        var eplDeclare = "@public create table varaggWDE (" + (grouped ? "key string primary key, " : "") +
	        ...
	        env.AssertStatement("into", statement => SupportEventTypeAssertionUtil.AssertEventTypeProperties(expectedAggType, statement.EventType, SupportEventTypeAssertionEnum.NAME, SupportEventTypeAssertionEnum.TYPE));

	        var key = grouped ? "[\"E1\"]" : "";
	        var eplGet = "@name('s0') select varaggWDE" + key + ".a1.after(150L) as c0, " +
	        ...
	        env.AssertStatement("s0", statement => SupportEventTypeAssertionUtil.AssertEventTypeProperties(expectedGetType, statement.EventType, SupportEventTypeAssertionEnum.NAME, SupportEventTypeAssertionEnum.TYPE));
