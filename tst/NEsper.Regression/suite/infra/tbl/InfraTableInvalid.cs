...
namespace com.espertech.esper.regressionlib.suite.infra.tbl
{
	...
	public class InfraTableInvalid {
	    ...
	    private static void TryInvalidAggMatch(RegressionEnvironment env, string name, string declared, bool unbound, string provided, string messageOrNull) {
	        var path = new RegressionPath();
	        env.CompileDeploy("@name('create') @public create table " + name + "(value " + declared + ")", path);

	        try {
	            var epl = "into table " + name + " select " + provided + " as value from SupportBean" +
	                      (unbound ? "#time(1000)" : "");
