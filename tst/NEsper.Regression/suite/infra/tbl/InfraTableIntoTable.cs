...
namespace com.espertech.esper.regressionlib.suite.infra.tbl {
	...
	public class InfraTableIntoTable {
		...
		private class InfraIntoTableMultikeyWArrayTwoKeyed : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				...
				env.AssertPropsPerRowIteratorAnyOrder ("tbl", "k1,k2,thesum".Split (","), new object[][] {
					new object[] { new int[] { 10 }, new int[] { 1, 2 }, 100 }, new object[] { new int[] { 10 }, new int[] { 1, 1 }, 102 + 104 },
						new object[] { new int[] { 10, 20 }, new int[] { 1, 2 }, 101 + 103 }, new object[] { new int[] { 10, 20 }, new int[] { 1, 1 }, 105 },
				});
		...
		private class InfraIntoTableMultikeyWArraySingleArrayKeyed : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				...
				env.AssertPropsPerRowIteratorAnyOrder ("tbl", "k,thesum".Split (","), new object[][] {
					new object[] { new int[] { 1, 2 }, 10 }, new object[] { new int[] { 0, 2 }, 11 + 13 }, new object[] { new int[] { 1, 1 }, 12 + 15 }, new object[] { new int[] { 1 }, 14 },
				});
		...
		private static void TryAssertionLastFirstWindow (RegressionEnvironment env, bool soda, AtomicLong milestone) {
			...
			var eplUnboundInto = "into table varagg select lastever(*) as lasteveru, firstever(*) as firsteveru from SupportBean";
		...
		private static void TryAssertionSortedMinMaxBy (RegressionEnvironment env, bool soda, AtomicLong milestone) {
			...
			var eplUnboundInto = "into table varagg select maxbyever() as maxbyeveru, minbyever() as minbyeveru from SupportBean";
			env.CompileDeploy (soda, eplUnboundInto, path);
		...
		public class InfraTableIntoTableNoKeys : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				...
				var eplIntoTable = "@name('Into-Table') into table MyTable select sum(intPrimitive) as sumint from SupportBean";
			...
			private static void AssertValue (RegressionEnvironment env, int? value) {
				env.SendEventBean (new SupportBean_S0 (0));
		...
		public class InfraTableIntoTableWithKeys : RegressionExecution {
			...
			public void Run (RegressionEnvironment env) {
				...
				var eplCreateTable = "@name('Create-Table') @public create table MyTable(pkey string primary key, sumint sum(int))";
				env.CompileDeploy (eplCreateTable, path);

				var eplIntoTable = "@name('Into-Table') into table MyTable select sum(intPrimitive) as sumint from SupportBean group by theString";
				env.CompileDeploy (eplIntoTable, path);

				var eplQueryTable = "@name('s0') select (select sumint from MyTable where pkey = s0.p00) as c0 from SupportBean_S0 as s0";
				env.CompileDeploy (eplQueryTable, path).AddListener ("s0");
				...
				env.AssertPropsPerRowIteratorAnyOrder ("Create-Table", fields, new object[][] { new object[] { "E1", 10 } });
				MakeSendBean (env, "E2", 200);
				AssertValues (env, valueList, new int?[] { 10, 200, null });

				env.Milestone (3);

				env.AssertPropsPerRowIteratorAnyOrder ("Create-Table", fields, new object[][] { new object[] { "E1", 10 }, new object[] { "E2", 200 } });
				MakeSendBean (env, "E1", 11);
				AssertValues (env, valueList, new int?[] { 21, 200, null });

				env.Milestone (4);

				env.AssertPropsPerRowIteratorAnyOrder ("Create-Table", fields, new object[][] { new object[] { "E1", 21 }, new object[] { "E2", 200 } });
				MakeSendBean (env, "E3", 3000);
				...
				env.AssertPropsPerRowIteratorAnyOrder ("Create-Table", fields, new object[][] { new object[] { "E1", 21 }, new object[] { "E2", 200 }, new object[] { "E3", 3000 } });
				MakeSendBean (env, "E2", 201);
				env.AssertPropsPerRowIteratorAnyOrder ("Create-Table", fields, new object[][] { new object[] { "E1", 21 }, new object[] { "E2", 401 }, new object[] { "E3", 3000 } });
				MakeSendBean (env, "E3", 3001);
				env.AssertPropsPerRowIteratorAnyOrder ("Create-Table", fields, new object[][] { new object[] { "E1", 21 }, new object[] { "E2", 401 }, new object[] { "E3", 6001 } });

				MakeSendBean (env, "E1", 12);
				env.AssertPropsPerRowIteratorAnyOrder ("Create-Table", fields, new object[][] { new object[] { "E1", 33 }, new object[] { "E2", 401 }, new object[] { "E3", 6001 } });
			...
			private static void AssertValues (RegressionEnvironment env, string keys, int?[] values) {
				...
				for (var i = 0; i < keyarr.Length; i++) {
					...
					var index = i;
					env.AssertEventNew ("s0", @event => Assert.AreEqual (values[index], @event.Get ("c0"), "Failed for key '" + keyarr[index] + "'"));
				}
			}
		}
		...
		private class InfraTableBigNumberAggregation : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				...
				env.AssertIterator ("tbl", iterator => {
					...
					EPAssertionUtil.AssertProps (result, fields, new object[] { 5m, 100m, BigInteger.Parse ("5"), 100m });
				});

				env.SendEventBean (new SupportBeanNumeric (BigInteger.Parse ("4"), 200m));
				env.AssertIterator ("tbl", iterator => {
					var result = env.GetEnumerator ("tbl").Advance ();
					EPAssertionUtil.AssertProps (result, fields, new object[] { 4m, 200m, BigInteger.Parse ("4"), 200m });
				});
		...
		private static void TryAssertionMinMax (RegressionEnvironment env, bool soda, AtomicLong milestone) {
		...
		private static void AssertResults (IEnumerator<EventBean> it, string[] fields, object[] values) {
