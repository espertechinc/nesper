	...
	public class InfraTableCountMinSketch {
	    ...
	    private class InfraDocSamples : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.CompileDeploy("@public create table WordCountTable2(wordcms countMinSketch({\n" +
	                ...
	                "  seed: 38576,\n" +
	                "  topk: 20,\n" +
	                "  agent: '" + typeof(CountMinSketchAgentStringUTF16Forge).FullName + "'" +
	                "}))", path);
	    ...
	    private class InfraNonStringType : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var eplTable = "@public create table MyApproxNS(bytefreq countMinSketch({" +
	                           ...
	                           "  confidence: 0.98," +
	                           "  topk: null," +
	                           "  agent: '" + typeof(MyBytesPassthruAgentForge).FullName + "'" +
	                           "}))";
	            env.CompileDeploy(eplTable, path);

	            var eplInto = "into table MyApproxNS select countMinSketchAdd(body) as bytefreq from SupportByteArrEventStringId(id='A')";
	            env.CompileDeploy(eplInto, path);

	            var eplRead = "@name('s0') select MyApproxNS.bytefreq.countMinSketchFrequency(body) as freq from SupportByteArrEventStringId(id='B')";
	    ...
	    private class InfraFrequencyAndTopk : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            // test join
	            var eplJoin = "@name('join') select wordapprox.countMinSketchFrequency(s2.p20) as c0 from MyApproxFT, SupportBean_S2 s2 unidirectional";
	            ...
	            // test subquery
	            var eplSubquery = "@name('subq') select (select wordapprox.countMinSketchFrequency(s2.p20) from MyApproxFT) as c0 from SupportBean_S2 s2";
	    ...
	    private class InfraInvalid : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.TryInvalidCompile(path, "create table MyTable(cms countMinSketch({agent:'System.String'}))",
	    ...
	    private static void AssertTopk(RegressionEnvironment env, string topkList) {
	        ...
	        env.AssertEventNew("topk", @event => {
	            ...
	            foreach (var pair in pairs) {
	                ...
	                Assert.IsFalse(foundIndex == -1, "failed to find '" + expectedValue + "=" + expectedFrequency + "' among remaining " + Arrays.AsList(arr));
	                arr[foundIndex] = null;
	            }
	        });
	    }
