	...
	public class InfraTableOnMerge {
	    ...
	    private class InfraTableArrayAssignmentBoxed : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertEventNew("s0", @event => CollectionAssert.AreEqual(new double?[] {null, 1d, null}, (double?[]) @event.Get("c0")));
	    ...
	    private class InfraTableOnMergeSimple : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowIteratorAnyOrder("tbl", fields, new object[][]{new object[] {"E1", 11}, new object[] {"E2", 100}});

	            env.Milestone(2);

	            env.SendEventBean(new SupportBean("E2", 101));
	            env.AssertPropsPerRowIteratorAnyOrder("tbl", fields, new object[][]{new object[] {"E1", 11}, new object[] {"E2", 101}});
	    ...
	    private static void RunOnMergeInsertUpdDeleteUngrouped(RegressionEnvironment env, bool soda, AtomicLong milestone) {
	        var path = new RegressionPath();
	        var eplDeclare = "@public create table varaggIUD (p0 string, sumint sum(int))";
	        env.CompileDeploy(soda, eplDeclare, path);

	        var fields = "c0,c1".SplitCsv();
	        var eplRead = "@name('s0') select varaggIUD.p0 as c0, varaggIUD.sumint as c1, varaggIUD as c2 from SupportBean_S0";
	        ...
	        env.AssertEventNew("s0", received => {
	            ...
	            EPAssertionUtil.AssertPropsMap((IDictionary<string, object>) received.Get("c2"), "p0,sumint".SplitCsv(), new object[]{"updated", 50});
	        });
	    ...
	    private static void RunOnMergeInsertUpdDeleteSingleKey(RegressionEnvironment env, bool soda, AtomicLong milestone) {
	        var fieldsTable = "key,p0,p1,p2,sumint".SplitCsv();
	        var path = new RegressionPath();
	        var eplDeclare = "@public create table varaggMIU (key int primary key, p0 string, p1 int, p2 int[], sumint sum(int))";
	        env.CompileDeploy(soda, eplDeclare, path);

	        var fields = "c0,c1,c2,c3".SplitCsv();
	        var eplRead = "@name('s0') select varaggMIU[id].p0 as c0, varaggMIU[id].p1 as c1, varaggMIU[id].p2 as c2, varaggMIU[id].sumint as c3 from SupportBean_S0";
	        env.CompileDeploy(soda, eplRead, path).AddListener("s0");
	        ...
	        env.SendEventBean(new SupportBean("U2", 10));
	        env.AssertPropsIRPair("merge", fieldsTable, new object[]{10, "v2", 2000, new int?[]{3, 4}, 50}, new object[]{10, "v1", 1000, new int?[]{1, 2}, 50});
	    ...
	    private static void RunOnMergeInsertUpdDeleteTwoKey(RegressionEnvironment env, bool soda, AtomicLong milestone) {
	        var path = new RegressionPath();
	        var eplDeclare = "@public create table varaggMIUD (keyOne int primary key, keyTwo string primary key, prop string)";
	        env.CompileDeploy(soda, eplDeclare, path);

	        var fields = "c0,c1,c2".SplitCsv();
	        var eplRead = "@name('s0') select varaggMIUD[id,p00].keyOne as c0, varaggMIUD[id,p00].keyTwo as c1, varaggMIUD[id,p00].prop as c2 from SupportBean_S0";
	        env.CompileDeploy(soda, eplRead, path).AddListener("s0");
	        ...
	        env.AssertStatement("merge", statement => SupportEventTypeAssertionUtil.AssertEventTypeProperties(expectedType, statement.EventType, SupportEventTypeAssertionEnum.NAME, SupportEventTypeAssertionEnum.TYPE));
	    ...
	    private static void AssertResultAggRead(RegressionEnvironment env, object[] objects, int total) {
	        ...
	        env.AssertEventNew("s0", @event => {
	            ...
	            EPAssertionUtil.AssertEqualsExactOrder(new object[]{objects[^1]}, ((ICollection<object>) @event.Get("c0")).ToArray());
	        });
	    }

	    private static void AssertKeyFound(RegressionEnvironment env, string keyCsv, bool[] expected) {
	        var split = keyCsv.SplitCsv();
	        for (var i = 0; i < split.Length; i++) {
	            var key = split[i];
	            env.SendEventBean(new SupportBean_S0(0, key));
	            var expectedString = expected[i] ? key : null;
	            env.AssertEventNew("s0", @event =>Assert.AreEqual(expectedString, @event.Get("c0"), "failed for key '" + key + "'"));
	        }
	    }
