	...
	public class InfraTableResetAggregationState {
	    ...
	    private class InfraTableResetInvalid : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var invalidSelectAggReset = prefix + "on SupportBean_S0 merge MyTable when matched then insert into MyStream select asum.reset()";
	            ...
	            var invalidSelectRowReset = prefix + "on SupportBean_S0 merge MyTable as mt when matched then insert into MyStream select mt.reset()";
	            env.TryInvalidCompile(invalidSelectRowReset, "Failed to validate select-clause expression 'mt.reset()'");

	            var invalidAggResetWParams = prefix + "on SupportBean_S0 merge MyTable as mt when matched then update set asum.reset(1)";
	            ...
	            var invalidRowResetWParams = prefix + "on SupportBean_S0 merge MyTable as mt when matched then update set mt.reset(1)";
	    ...
	    private class InfraTableResetVariousAggs : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var fieldSetOne = "myAvedev,myCount,myCountDistinct,myMax,myMedian,myStddev,myFirstEver,myCountEver,myMaxByEver".SplitCsv();
	    ...
	    private static void AssertTableSum(RegressionEnvironment env, int? expected) {
