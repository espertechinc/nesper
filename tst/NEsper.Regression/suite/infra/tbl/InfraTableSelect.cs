	...
	public class InfraTableSelect {
	    ...
	    private class InfraTableSelectMultikeyWArrayComposite : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@public create table MyTable(k0 string primary key, k1 string primary key, k2 string primary key, v string);\n" +
	    ...
	    private class InfraTableSelectMultikeyWArrayTwoArray : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var path = new RegressionPath();
	            var epl = "@public create table MyTable(k1 int[primitive] primary key, k2 int[primitive] primary key, value int);\n" +
	    ...
	    private static void RunAssertionOutputSnapshot(RegressionEnvironment env, RegressionPath path, object[][] expectedType, object[] rowValues, AtomicLong currentTime) {
	        ...
	        env.AdvanceTime(currentTime.Get());
	        env.AssertEventNew("s0", @event =>AssertEventTypeAndEvent(@event.EventType, expectedType, @event.Underlying, rowValues));
	    ...
	    private static void RunAssertionSubquerySelectWEnumMethod(RegressionEnvironment env, RegressionPath path, object[] rowValues) {
	        ...
	        env.AssertStatement("s0", statement => Assert.AreEqual(typeof(ICollection<object>), statement.EventType.GetPropertyType("mt")));
	    ...
	    private static void RunAssertionSubquerySelectStar(RegressionEnvironment env, RegressionPath path, object[] rowValues) {
	        ...
	        env.AssertStatement("s0", statement => {
	            ...
	            Assert.AreSame(env.Statement("create").EventType, statement.EventType.GetFragmentType("mt").FragmentType);
	        });
	    ...
	    private static void RunAssertionSubquerySelectStar(RegressionEnvironment env, RegressionPath path, object[] rowValues, string epl) {
	        ...
	        env.AssertStatement("s0", statement => Assert.AreEqual(typeof(object[]), statement.EventType.GetPropertyType("mt")));
	    ...
	    private static void RunAssertionJoinSelectStreamStarNamed(RegressionEnvironment env, RegressionPath path, object[][] expectedType, object[] rowValues) {
	        ...
	        // listener assertion
	        env.SendEventBean(new SupportBean_S2(0, "G1"));
	        env.AssertEventNew("s0", @event =>AssertEventTypeAndEvent(@event.EventType.GetFragmentType("mymt").FragmentType,
	    ...
	    private static void RunAssertionJoinSelectStreamName(RegressionEnvironment env, RegressionPath path, object[][] expectedType, object[] rowValues) {
	        ...
	        env.AssertStatement("s0", statement => AssertEventType(statement.EventType.GetFragmentType("mt").FragmentType, expectedType));

	        env.SendEventBean(new SupportBean_S2(0, "G1"));
	        env.AssertEventNew("s0", @event =>AssertEventTypeAndEvent(@event.EventType.GetFragmentType("mt").FragmentType,
	    ...
	    private static void RunAssertionJoinSelectStar(RegressionEnvironment env, RegressionPath path, object[][] expectedType, object[] rowValues) {
	        ...
	        env.AssertStatement("s0", statement => AssertEventType(statement.EventType.GetFragmentType("stream_0").FragmentType, expectedType));

	        // listener assertion
	        env.SendEventBean(new SupportBean_S2(0, "G1"));
	        env.AssertEventNew("s0", @event =>AssertEventTypeAndEvent(@event.EventType.GetFragmentType("stream_0").FragmentType,
