...
namespace com.espertech.esper.regressionlib.suite.infra.tbl
{
	...
	public class InfraTableNonAccessDotSubqueryAndJoin : RegressionExecution {
	    ...
	    private static void RunAssertionUse(RegressionEnvironment env, bool soda) {
	        ...
	        var eplMerge = "@name('merge') on SupportBean merge MyTable when matched then update set col3={1,2,4,2}, col0=\"x\"";
	        ...
	        env.AssertStatement("s0", statement => SupportEventTypeAssertionUtil.AssertEventTypeProperties(expectedType, statement.EventType, SupportEventTypeAssertionEnum.NAME, SupportEventTypeAssertionEnum.TYPE));

	        MakeSendSupportBean(env, null, -1);
	        env.AssertEventNew("s0", @event => {
	            EPAssertionUtil.AssertProps(@event, "c0_1,c0_2,c1_1,c1_2".SplitCsv(), new object[]{"x", "x", 41, 41});
	        ...
	        env.AssertStatement("s1", statement => SupportEventTypeAssertionUtil.AssertEventTypeProperties(expectedTypeUnnamed, statement.EventType, SupportEventTypeAssertionEnum.NAME, SupportEventTypeAssertionEnum.TYPE));
