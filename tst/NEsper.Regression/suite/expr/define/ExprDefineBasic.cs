	...
	public class ExprDefineBasic {
		...
		internal class ExprDefineExpressionSimpleSameStmt : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				...
				env.AssertStatement ("s0", statement => Assert.AreEqual (StatementType.SELECT, statement.GetProperty (StatementProperty.STATEMENTTYPE)));
				env.SendEventBean (new SupportBean ());
				env.AssertEqualsNew ("s0", "c0", 1);
				env.UndeployAll ();
			}
		...
		internal class ExprDefineCaseNewMultiReturnNoElse : RegressionExecution {
			...
			public void Run (RegressionEnvironment env) {
				...
				env.AssertStatement ("s0", statement => Assert.AreEqual (typeof (IDictionary<string, object>), statement.EventType.GetPropertyType ("val0")));
			...
			private void AssertEvents (RegressionEnvironment env, object[] expected) {
				env.AssertEventNew ("s0", @event => EPAssertionUtil.AssertPropsMap ((IDictionary<string, object>) @event.Get ("val0"), FIELDS_INNER, expected));
		...
		internal class ExprDefineSubqueryCross : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				...
				var eplAlias = "@name('s0') expression subq alias for { (select theString from SupportBean#keepall where theString = one.id and intPrimitive = two.p10) }" +
				               "select subq as val1 " +
				               "from SupportBean_ST0#lastevent as one, SupportBean_ST1#lastevent as two";
		...
		internal class ExprDefineSubqueryCorrelated : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				...
				var eplAlias = "@name('s0') expression subqOne alias for {(select id from SupportBean_ST0#keepall where p00 = t.intPrimitive)} " +
		...
		internal class ExprDefineSubqueryUncorrelated : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				...
				var eplAlias = "@name('s0') expression subqOne alias for {(select id from SupportBean_ST0#lastevent)} " +
				               "select theString as val0, subqOne as val1 from SupportBean as t";
		...
		internal class ExprDefineSubqueryNamedWindowUncorrelated : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				var eplDeclare = "@name('s0') expression subqnamedwin { MyWindow.where(x => x.val1 > 10).orderBy(x => x.val0) } " +
				...
				var eplAlias = "@name('s0') expression subqnamedwin alias for {MyWindow.where(x => x.val1 > 10).orderBy(x => x.val0)}" +
			...
			private void TryAssertionSubqueryNamedWindowUncorrelated (RegressionEnvironment env, string epl) {
				...
				env.AssertListener ("s0", listener => {
					EPAssertionUtil.AssertPropsPerRow (ToArrayMap (listener.AssertOneGetNew ().Get ("c0").Unwrap<object>()), fieldsInside, null);
					EPAssertionUtil.AssertPropsPerRow (ToArrayMap (listener.AssertOneGetNew ().Get ("c1").Unwrap<object>()), fieldsInside, null);
					listener.Reset ();
				});
				...
				env.AssertListener ("s0", listener => {
					EPAssertionUtil.AssertPropsPerRow (ToArrayMap (listener.AssertOneGetNew ().Get ("c0").Unwrap<object>()), fieldsInside, new object[][] { new object[] { "E1" } });
					EPAssertionUtil.AssertPropsPerRow (ToArrayMap (listener.AssertOneGetNew ().Get ("c1").Unwrap<object>()), fieldsInside, new object[][] { new object[] { "E1" } });
					listener.Reset ();
				});
				...
				env.AssertListener ("s0", listener => {
					EPAssertionUtil.AssertPropsPerRow (ToArrayMap (listener.AssertOneGetNew ().Get ("c0").Unwrap<object>()), fieldsInside, new object[][] { new object[] { "E1" }, new object[] { "E2" } });
					EPAssertionUtil.AssertPropsPerRow (ToArrayMap (listener.AssertOneGetNew ().Get ("c1").Unwrap<object>()), fieldsInside, new object[][] { new object[] { "E1" } });
					listener.Reset ();
				});
		...
		internal class ExprDefineSubqueryNamedWindowCorrelated : RegressionExecution {
			...
			private void AssertC0 (RegressionEnvironment env, string[] fieldInside, object[][] expecteds) {
				env.AssertEventNew ("s0", @event => EPAssertionUtil.AssertPropsPerRow (ToArrayMap (@event.Get ("c0").Unwrap<object>()), fieldInside, expecteds));
			}
		}
		...
		internal class ExprDefineAggregationNoAccess : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				...
				env.AssertStmtTypes ("s0", fields, new Type[] { typeof(int?), typeof(int?), typeof(double?), typeof(long?) });
		...
		internal class ExprDefineScalarReturn : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				...
				var eplCaseDeclare = "@name('s0') expression theExpression { myEvent => " + inner + "} " +
				...
				var eplCaseAlias = "@name('s0') expression theExpression alias for {" + inner + "}" +
		...
		internal class ExprDefineEventTypeAndSODA : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				...
				var eplFormatted = "@name('s0')" + NEWLINE +
		...
		internal class ExprDefineWhereClauseExpression : RegressionExecution {
			public void Run (RegressionEnvironment env) {
				var eplNoAlias = "@name('s0') expression one {x=>x.boolPrimitive} select * from SupportBean as sb where one(sb)";
				TryAssertionWhereClauseExpression (env, eplNoAlias);

				var eplAlias = "@name('s0') expression one alias for {boolPrimitive} select * from SupportBean as sb where one";
		...
		private static SupportBean GetSupportBean (int intPrimitive, int? intBoxed) {
