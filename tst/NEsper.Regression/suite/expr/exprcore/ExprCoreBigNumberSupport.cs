	...
	public class ExprCoreBigNumberSupport {
	    ...
	    private class ExprCoreBigNumberEquals : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            // test equals decimal
	            var epl = "@name('s0') select * from SupportBeanNumeric where bigdec = 1 or bigdec = intOne or bigdec = doubleOne";
	    ...
	    private class ExprCoreBigNumberBetween : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select * from SupportBeanNumeric where bigdec between 10 and 20 or bigint between 100 and 200";
	            env.CompileDeploy(epl).AddListener("s0");
	    ...
	    private class ExprCoreBigNumberIn : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select * from SupportBeanNumeric where bigdec in (10, 20d) or bigint in (0x02, 3)";
	            env.CompileDeploy(epl).AddListener("s0");
	    ...
	    private class ExprCoreBigNumberMath : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            SendBigNumEvent(env, 1, 2);
	            env.AssertPropsNew("s0", "v1,v2,v3,v4,v5".SplitCsv(),
	            ...
	            env.AssertStatement("s0", statement => Assert.AreEqual(typeof(decimal), statement.EventType.GetPropertyType("avgRate")));
	    ...
	    private class ExprCoreBigNumberCastAndUDF : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select SupportStaticMethodLib.myBigIntFunc(cast(2, BigInteger)) as v1, SupportStaticMethodLib.myBigDecFunc(cast(3d, decimal)) as v2 from SupportBeanNumeric";
	            env.CompileDeploy(epl).AddListener("s0");
	    ...
	    private static void SendBigNumEvent(RegressionEnvironment env, int bigInt, decimal decimalPrimitive) {
