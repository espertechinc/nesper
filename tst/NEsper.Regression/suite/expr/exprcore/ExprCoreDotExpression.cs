	...
	public class ExprCoreDotExpression {
	    ...
	    private class ExprCoreDotObjectSimpleEventProperty : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "create constant variable string MYVAR = " + typeof(ExprCoreDotExpression).FullName + ".supportDotExpressionReturningSB('X').theString;\n" +
	                      "@name('s0') select " + typeof(ExprCoreDotExpression).FullName + ".supportDotExpressionReturningSB(p00).theString as c0, MYVAR as c1 from SupportBean_S0";
	    ...
	    private class ExprCoreDotAggregationSimpleValueMethod : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsNew("s0", "c0".SplitCsv(), new object[] {
		            1m
	            });
	    ...
	    private class ExprCoreDotCollectionSelectFromGetAndSize : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select p01.split(',').selectFrom(v -> v).size() as sz from SupportBean_S0(p00=p01.split(',').selectFrom(v -> v).get(2))";
	            env.CompileDeploy(epl).AddListener("s0");
	        ...
	        private void SendAssert(RegressionEnvironment env, string p00, string p01, int? sizeExpected) {
	    ...
	    private class ExprCoreDotMapIndexPropertyRooted : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertStatement("s0", statement => {
	                Assert.AreEqual(typeof(SupportEventInnerTypeWGetIds), statement.EventType.GetPropertyType("c0"));
	                Assert.AreEqual(typeof(SupportEventInnerTypeWGetIds), statement.EventType.GetPropertyType("c1"));
	            ...
	            var @event = new SupportEventTypeErasure("key1", 2, Collections.SingletonMap("key1", new SupportEventInnerTypeWGetIds(new int[]{20, 30, 40})), new SupportEventInnerTypeWGetIds[]{new SupportEventInnerTypeWGetIds(new int[]{2, 3}), new SupportEventInnerTypeWGetIds(new int[]{4, 5}), new SupportEventInnerTypeWGetIds(new int[]{6, 7, 8})});
	            env.SendEventBean(@event);
	            env.AssertPropsNew("s0", "c0,c1,c2,c3,c4,c5,c6,c7".SplitCsv(), new object[]{@event.InnerTypes.Get("key1"), @event.InnerTypes.Get("key1"), 30, 40, 5, 8, 999999, 999999});
	    ...
	    private class ExprCoreDotNestedPropertyInstanceExpr : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsNew("s0", "val0,val1,val2".SplitCsv(), new object[]{"level1:10", "level2:20", "level3:30"});
	    ...
	    private class ExprCoreDotChainedUnparameterized : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsNew("s0", "nested.getNestedValue()".SplitCsv(), new object[]{bean.Nested.NestedValue});
	    ...
	    private class ExprCoreDotArrayPropertySizeAndGet : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.SendEventBean(bean);
	            env.AssertPropsNew("s0", "size,get0,get1,get2,get3".SplitCsv(),
	    ...
	    private class ExprCoreDotArrayPropertySizeAndGetChained : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.SendEventBean(bean);
	            env.AssertPropsNew("s0", "size,get0".SplitCsv(),
	    ...
	    private static void SendAssertDotObjectEquals(RegressionEnvironment env, int intPrimitive, bool expected) {
