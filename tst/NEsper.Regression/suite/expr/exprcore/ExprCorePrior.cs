	...
	public class ExprCorePrior {
	    ...
	    internal class ExprCorePriorUnboundSceneOne : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowNewFlattened("s0", new string[]{"prior1"}, new object[][]{new object[]{null}});

	            env.Milestone(1);

	            env.SendEventBean(MakeMarketDataEvent("E1"));
	            env.AssertPropsPerRowNewFlattened("s0", new string[]{"prior1"}, new object[][]{new object[]{"E0"}});

	            env.Milestone(2);

	            for (var i = 2; i < 9; i++) {
	                env.SendEventBean(MakeMarketDataEvent("E" + i));
	                env.AssertPropsPerRowNewFlattened("s0", new string[]{"prior1"}, new object[][]{new object[]{"E" + (i - 1)}});
	    ...
	    internal class ExprCorePriorLengthWindowSceneTwo : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.SendEventBean(MakeMarketDataEvent("E0"));
	            env.AssertPropsPerRowNewFlattened("s0",  new string[]{"prior1", "prior2"}, new object[][]{new object[]{null, null}});

	            env.MilestoneInc(milestone);

	            env.SendEventBean(MakeMarketDataEvent("E1"));
	            env.AssertPropsPerRowNewFlattened("s0",  new string[]{"prior1", "prior2"}, new object[][]{new object[]{"E0", null}});

	            env.MilestoneInc(milestone);

	            env.SendEventBean(MakeMarketDataEvent("E2"));
	            env.AssertPropsPerRowNewFlattened("s0",  new string[]{"prior1", "prior2"}, new object[][]{new object[]{"E1", "E0"}});
	    ...
	    private static void TryAssertionPriorStreamAndVariable(RegressionEnvironment env, RegressionPath path, string priorIndex, AtomicLong milestone) {
	        ...
	        env.AssertStatement("s0", statement =>Assert.AreEqual(typeof(SupportBean_S0), statement.EventType.GetPropertyType("result")));
