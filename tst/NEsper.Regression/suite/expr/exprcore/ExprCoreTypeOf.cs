	...
	public class ExprCoreTypeOf {
	    ...
	    private class ExprCoreTypeOfDynamicProps : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var schema = MAP.GetAnnotationText() + " @public @buseventtype create schema MyDynoPropSchema as (key string);\n";
	    ...
	    private static void TryAssertionVariantStream(RegressionEnvironment env, EventRepresentationChoice eventRepresentationEnum) {
	        ...
	        var eplSchemas =
	            eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedWKey)) + " @public @buseventtype create schema EventOne as (key string);\n" +
	                eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedWKey)) + " @public @buseventtype create schema EventTwo as (key string);\n" +
	                " @public @buseventtype create schema S0 as " + typeof(SupportBean_S0).FullName + ";\n" +
	    ...
	    private static void TryAssertionFragment(RegressionEnvironment env, EventRepresentationChoice eventRepresentationEnum) {
	        var epl = eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedInnerSchema)) + " @public create schema InnerSchema as (key string);\n" +
	                  eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedMySchema)) + " @buseventtype @public create schema MySchema as (inside InnerSchema, insidearr InnerSchema[]);\n" +
	                  eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedOut)) + " @name('s0') select typeof(s0.inside) as t0, typeof(s0.insidearr) as t1 from MySchema as s0;\n";
	    ...
	    private static IDictionary<string, object> MakeSchemaEvent(object prop, string key) {
	        IDictionary<string, object> theEvent = new Dictionary<string, object>();
