	...
	public class ExprCorePrevious
	{
		...
		private class ExprCorePreviousExprNameAndTypeAndSODA : RegressionExecution
		{
			public void Run(RegressionEnvironment env)
			{
				...
				for (var i = 0; i < rows.Length; i++) {
					...
					Assert.That(resultType.GetBoxedType(), Is.EqualTo(prop.PropertyType.GetBoxedType()));
				}
		...
		private class ExprCorePreviousPrevCountStarWithStaticMethod : RegressionExecution
		{
			public void Run(RegressionEnvironment env)
			{
				...
				var epl = 
					"@Name('s0')select irstream count(*) as total, " +
					$"prev({exprCorePrevious}.IntToLong(count(*)) - 1, Price) as firstPrice from SupportMarketDataBean#time(60)";
				env.CompileDeploy(epl).AddListener("s0");
