    ...
    public class ExprFilterExpressions {
        ...
        private class ExprFilterRelationalOpRange : RegressionExecution {
            public void Run (RegressionEnvironment env) {
                ...
                MultiStmtAssertUtil.RunIsInvokedWTestdata (env, assertions, new object[] { 1, 2, 3, 4 },
                    data => SendBeanIntDouble (env, (int?) data, 0D), milestone);
            }
        }
        ...
        private class ExprFilterIn3ValuesAndNull : RegressionExecution {
            public void Run (RegressionEnvironment env) {
                ...
                Try3Fields (env, milestone, text, new int[] { 1, 1, 1 }, new int?[] { 0, 1, 0 }, new double?[] { 2d, 2d, 1d }, new bool[] { false, true, true });

                text = "select * from SupportBean(intPrimitive in (intBoxed, " +
                    typeof (SupportStaticMethodLib).FullName + ".minusOne(doubleBoxed)))";
                Try3Fields (env, milestone, text, new int[] { 1, 1, 1 }, new int?[] { 0, 1, 0 }, new double?[] { 2d, 2d, 1d }, new bool[] { true, true, false });

                text = "select * from SupportBean(intPrimitive not in (intBoxed, doubleBoxed))";
                Try3Fields (env, milestone, text, new int[] { 1, 1, 1 }, new int?[] { 0, 1, 0 }, new double?[] { 2d, 2d, 1d }, new bool[] { true, false, false });

                text = "select * from SupportBean(intBoxed = doubleBoxed)";
                Try3Fields (env, milestone, text, new int[] { 1, 1, 1 }, new int?[] { null, 1, null }, new double?[] { null, null, 1d }, new bool[] { false, false, false });
                ...
                Try3Fields (env, milestone, text, new int[] { 1, 1, 1 }, new int?[] { null, 1, 2 }, new double?[] { null, null, 1d }, new bool[] { false, false, true });

                text = "select * from SupportBean(intBoxed not in [doubleBoxed:10))";
                Try3Fields (env, milestone, text, new int[] { 1, 1, 1 }, new int?[] { null, 1, 2 }, new double?[] { null, null, 1d }, new bool[] { false, true, false });
            }
        }
        ...
        private class ExprFilterNotEqualsNull : RegressionExecution {
            ...
            public void Run (RegressionEnvironment env) {
                ...
                env.AssertStatement ("s0", statement => EPAssertionUtil.AssertEqualsExactOrder (new string[] {
                    "intBoxed is null",
        ...
        private class ExprFilterOverInClause : RegressionExecution {
            public void Run (RegressionEnvironment env) {
                var epl = "@name('s0') select * from pattern[every event1=SupportTradeEvent(userId in ('100','101'),amount>=1000)]";
                env.CompileDeployAddListenerMileZero (epl, "s0");

                env.SendEventBean (new SupportTradeEvent (1, "100", 1001));
                env.AssertEqualsNew ("s0", "event1.id", 1);

                var eplTwo = "@name('s1') select * from pattern [every event1=SupportTradeEvent(userId in ('100','101'))]";
        ...
        private class ExprFilterPromoteIndexToSetNotIn : RegressionExecution {
            public void Run (RegressionEnvironment env) {
                var eplOne = "@name('s0') select * from SupportBean(theString != 'x' and theString != 'y' and doubleBoxed is not null)";
                var eplTwo = "@name('s1') select * from SupportBean(theString != 'x' and theString != 'y' and longBoxed is not null)";
        ...
        private class ExprFilterShortCircuitEvalAndOverspecified : RegressionExecution {
            ...
            public void Run (RegressionEnvironment env) {
                var epl = "@name('s0') select * from SupportRuntimeExBean(SupportRuntimeExBean.property2 = '4' and SupportRuntimeExBean.property1 = '1')";
                env.CompileDeployAddListenerMileZero (epl, "s0");

                env.SendEventBean (new SupportRuntimeExBean ());
                env.AssertListener ("s0", listener => Assert.IsFalse (
                    listener.IsInvoked, "Subscriber should not have received result(s)"));
        ...
        private class ExprFilterRelationalOpConstantFirst : RegressionExecution {
            public void Run (RegressionEnvironment env) {
                ...
                assertions.Add (new EPLWithInvokedFlags ("select * from SupportInstanceMethodBean where 4 < x", new bool[] { false, false, true }));
                assertions.Add (new EPLWithInvokedFlags ("select * from SupportInstanceMethodBean where 4 <= x", new bool[] { false, true, true }));
                assertions.Add (new EPLWithInvokedFlags ("select * from SupportInstanceMethodBean where 4 > x", new bool[] { true, false, false }));
                assertions.Add (new EPLWithInvokedFlags ("select * from SupportInstanceMethodBean where 4 >= x", new bool[] { true, true, false }));

                MultiStmtAssertUtil.RunIsInvokedWTestdata (env, assertions, new object[] { 3, 4, 5 },
                    data => env.SendEventBean (new SupportInstanceMethodBean (data.AsInt32())), milestone);
            }
        }
        ...
        private class ExprFilterConstant : RegressionExecution {
            public void Run (RegressionEnvironment env) {
                var epl = "@name('s0') select * from pattern [SupportBean(intPrimitive=" + typeof (ISupportA).FullName + ".VALUE_1)]";
                env.CompileDeployAddListenerMileZero (epl, "s0");
        ...
        private class ExprFilterEnumSyntaxOne : RegressionExecution {
            public void Run (RegressionEnvironment env) {
                var epl = "@name('s0') select * from pattern [SupportBeanWithEnum(supportEnum=" + typeof (SupportEnum).FullName + ".valueOf('ENUM_VALUE_1'))]";
                env.CompileDeployAddListenerMileZero (epl, "s0");
        ...
        private class ExprFilterEnumSyntaxTwo : RegressionExecution {
            public void Run (RegressionEnvironment env) {
                var epl = "@name('s0') select * from pattern[SupportBeanWithEnum(supportEnum=" + typeof (SupportEnum).FullName + ".ENUM_VALUE_2)]";
                ...
                // test where clause
                epl = "@name('s0') select * from SupportBeanWithEnum where supportEnum=" + typeof (SupportEnum).FullName + ".ENUM_VALUE_2";
                env.CompileDeployAddListenerMile (epl, "s0", 1);
        ...
        private class ExprFilterPatternFunc3Stream : RegressionExecution {
            public void Run (RegressionEnvironment env) {
                ...
                TryPattern3Stream (env, text, milestone, new int?[] { null, 2, 1, null, 8, 1, 2 }, new double?[] { 0d, 0d, 0d, 0d, 0d, 0d, 0d },
                ...
                TryPattern3Stream (env, text, milestone, new int?[] { 2, 8, null, 2, 1, null, 1 }, new double?[] { 0d, 0d, 0d, 0d, 0d, 0d, 0d },
                ...
                TryPattern3Stream (env, text, milestone, new int?[] { 2, 2, 1, 2, 1, 7, 1 }, new double?[] { 0d, 0d, 0d, 0d, 0d, 0d, 0d },
                ...
                TryPattern3Stream (env, text, milestone, new int?[] { 2, 1, 1, null, 1, null, 1 }, new double?[] { 0d, 0d, 0d, 0d, 0d, 0d, 0d },
        ...
        private class ExprFilterPatternFunc : RegressionExecution {
            public void Run (RegressionEnvironment env) {
                ...
                text = "select * from pattern [a=SupportBean -> b=" +
                    typeof (SupportBean).Name + "(intBoxed = a.intBoxed and doubleBoxed = a.doubleBoxed)]";
                TryPattern (env, text, milestone, new int?[] { null, 2, 1, null, 8, 1, 2 }, new double?[] { 2d, 2d, 2d, 1d, 5d, 6d, 7d },
                    new int?[] { null, 3, 1, 8, null, 1, 2 }, new double?[] { 2d, 3d, 2d, 1d, 5d, 6d, 8d },
                    new bool[] { false, false, true, false, false, true, false });

                text = "select * from pattern [a=SupportBean -> b=" +
                    typeof (SupportBean).Name + "(intBoxed is a.intBoxed and doubleBoxed = a.doubleBoxed)]";
                TryPattern (env, text, milestone, new int?[] { null, 2, 1, null, 8, 1, 2 }, new double?[] { 2d, 2d, 2d, 1d, 5d, 6d, 7d },
                    new int?[] { null, 3, 1, 8, null, 1, 2 }, new double?[] { 2d, 3d, 2d, 1d, 5d, 6d, 8d },
                    new bool[] { true, false, true, false, false, true, false });

                text = "select * from pattern [a=SupportBean -> b=" +
                    typeof (SupportBean).Name + "(a.doubleBoxed = doubleBoxed)]";
                TryPattern (env, text, milestone, new int?[] { 0, 0 }, new double?[] { 2d, 2d },
                    new int?[] { 0, 0 }, new double?[] { 2d, 3d },
                    new bool[] { true, false });

                text = "select * from pattern [a=SupportBean -> b=" +
                    typeof (SupportBean).Name + "(a.doubleBoxed = b.doubleBoxed)]";
                TryPattern (env, text, milestone, new int?[] { 0, 0 }, new double?[] { 2d, 2d },
                    new int?[] { 0, 0 }, new double?[] { 2d, 3d },
                    new bool[] { true, false });
                ...
                text = "select * from pattern [a=SupportBean -> b=" +
                    typeof (SupportBean).Name + "(doubleBoxed in [a.doubleBoxed:a.intBoxed])]";
                TryPattern (env, text, milestone, new int?[] { 1, 1, 1, 1, 1, 1 }, new double?[] { 10d, 10d, 10d, 10d, 10d, 10d },
                ...
                text = "select * from pattern [a=SupportBean -> b=" +
                    typeof (SupportBean).Name + "(b.doubleBoxed in (a.doubleBoxed:a.intBoxed))]";
                TryPattern (env, text, milestone, new int?[] { 1, 1, 1, 1, 1, 1 }, new double?[] { 10d, 10d, 10d, 10d, 10d, 10d },
                    new int?[] { 0, 0, 0, 0, 0, 0 }, new double?[] { 0d, 1d, 2d, 9d, 10d, 11d },
                ...
                text = "select * from pattern [a=SupportBean -> b=" +
                    typeof (SupportBean).Name + "(doubleBoxed not in [a.doubleBoxed:a.intBoxed])]";
                TryPattern (env, text, milestone, new int?[] { 1, 1, 1, 1, 1, 1 }, new double?[] { 10d, 10d, 10d, 10d, 10d, 10d },
                    new int?[] { 0, 0, 0, 0, 0, 0 }, new double?[] { 0d, 1d, 2d, 9d, 10d, 11d },
                ...
                text = "select * from pattern [a=SupportBean -> b=" +
                    typeof (SupportBean).Name + "(b.doubleBoxed not in (a.doubleBoxed:a.intBoxed))]";
                TryPattern (env, text, milestone, new int?[] { 1, 1, 1, 1, 1, 1 }, new double?[] { 10d, 10d, 10d, 10d, 10d, 10d },
                    new int?[] { 0, 0, 0, 0, 0, 0 }, new double?[] { 0d, 1d, 2d, 9d, 10d, 11d },
                ...
                text = "select * from pattern [a=SupportBean -> b=" +
                    typeof (SupportBean).Name + "(doubleBoxed not in (a.doubleBoxed, a.intBoxed, 9))]";
                TryPattern (env, text, milestone, new int?[] { 1, 1, 1, 1, 1, 1 }, new double?[] { 10d, 10d, 10d, 10d, 10d, 10d },
                    new int?[] { 0, 0, 0, 0, 0, 0 }, new double?[] { 0d, 1d, 2d, 9d, 10d, 11d },
                    new bool[] { true, false, true, false, false, true });

                text = "select * from pattern [a=SupportBean -> b=" +
                    typeof (SupportBean).Name + "(doubleBoxed in (a.doubleBoxed, a.intBoxed, 9))]";
                TryPattern (env, text, milestone, new int?[] { 1, 1, 1, 1, 1, 1 }, new double?[] { 10d, 10d, 10d, 10d, 10d, 10d },
                    new int?[] { 0, 0, 0, 0, 0, 0 }, new double?[] { 0d, 1d, 2d, 9d, 10d, 11d },
                    new bool[] { false, true, false, true, true, false });

                text = "select * from pattern [a=SupportBean -> b=" +
                    typeof (SupportBean).Name + "(b.doubleBoxed in (doubleBoxed, a.intBoxed, 9))]";
                TryPattern (env, text, milestone, new int?[] { 1, 1, 1, 1, 1, 1 }, new double?[] { 10d, 10d, 10d, 10d, 10d, 10d },
                    new int?[] { 0, 0, 0, 0, 0, 0 }, new double?[] { 0d, 1d, 2d, 9d, 10d, 11d },
                    new bool[] { true, true, true, true, true, true });

                text = "select * from pattern [a=SupportBean -> b=" +
                    typeof (SupportBean).Name + "(doubleBoxed not in (doubleBoxed, a.intBoxed, 9))]";
                TryPattern (env, text, milestone, new int?[] { 1, 1, 1, 1, 1, 1 }, new double?[] { 10d, 10d, 10d, 10d, 10d, 10d },
                    new int?[] { 0, 0, 0, 0, 0, 0 }, new double?[] { 0d, 1d, 2d, 9d, 10d, 11d },
                    new bool[] { false, false, false, false, false, false });

                text = "select * from pattern [a=SupportBean -> b=" +
                    typeof (SupportBean).Name + "(doubleBoxed = " + typeof (SupportStaticMethodLib).FullName + ".minusOne(a.doubleBoxed))]";
                TryPattern (env, text, milestone, new int?[] { 0, 0, 0 }, new double?[] { 10d, 10d, 10d },
                    new int?[] { 0, 0, 0 }, new double?[] { 9d, 10d, 11d },
                    new bool[] { true, false, false });

                text = "select * from pattern [a=SupportBean -> b=" +
                    typeof (SupportBean).Name + "(doubleBoxed = " + typeof (SupportStaticMethodLib).FullName + ".minusOne(a.doubleBoxed) or " +
                    "doubleBoxed = " + typeof (SupportStaticMethodLib).FullName + ".minusOne(a.intBoxed))]";
                TryPattern (env, text, milestone, new int?[] { 0, 0, 12 }, new double?[] { 10d, 10d, 10d },
                    new int?[] { 0, 0, 0 }, new double?[] { 9d, 10d, 11d },
        ...
        private class ExprFilterNotEqualsOp : RegressionExecution {
            public void Run (RegressionEnvironment env) {
                ...
                env.AssertListener ("s0", listener => Assert.AreSame (theEvent, listener.GetAndResetLastNewData () [0].Underlying));
        ...
        private class ExprFilterInvalid : RegressionExecution {
            public void Run (RegressionEnvironment env) {
                ...
                env.TryInvalidCompile ("select * from SupportBeanWithEnum(theString=" + typeof (SupportEnum).FullName + ".ENUM_VALUE_1)",
                    "Failed to validate filter expression 'theString=ENUM_VALUE_1': Implicit conversion from datatype '" + typeof (SupportEnum).FullName + "' to 'String' is not allowed [");
                ...
                env.TryInvalidCompile ("select * from pattern [a=SupportBean -> b=" +
                    typeof (SupportBean).Name + "(doubleBoxed not in (doubleBoxed, x.intBoxed, 9))]",
        ...
        private class ExprFilterInstanceMethodWWildcard : RegressionExecution {
            public void Run (RegressionEnvironment env) {
                TryFilterInstanceMethod (env, "select * from SupportInstanceMethodBean(s0.myInstanceMethodAlwaysTrue()) as s0", new bool[] { true, true, true });
                TryFilterInstanceMethod (env, "select * from SupportInstanceMethodBean(s0.myInstanceMethodEventBean(s0, 'x', 1)) as s0", new bool[] { false, true, false });
                TryFilterInstanceMethod (env, "select * from SupportInstanceMethodBean(s0.myInstanceMethodEventBean(*, 'x', 1)) as s0", new bool[] { false, true, false });
            }

            private void TryFilterInstanceMethod (RegressionEnvironment env, string epl, bool[] expected) {
        ...
        private static void SendBeanIntDoubleString (RegressionEnvironment env, int? intBoxed, double? doubleBoxed, string theString) {
            var theEvent = new SupportBean ();
        ...
        private static void SendBeanIntDouble (RegressionEnvironment env, int? intBoxed, double? doubleBoxed) {
            var theEvent = new SupportBean ();
            theEvent.IntBoxed = (intBoxed);
            theEvent.DoubleBoxed = (doubleBoxed);
            env.SendEventBean (theEvent);
        ...
        private static void SendBeanIntIntDouble (RegressionEnvironment env, int intPrimitive, int? intBoxed, double? doubleBoxed) {
            var theEvent = new SupportBean ();
            theEvent.IntPrimitive = (intPrimitive);
            theEvent.IntBoxed = (intBoxed);
            theEvent.DoubleBoxed = (doubleBoxed);
        ...
        private static void AssertListeners (RegressionEnvironment env, string[] statementNames, bool[] invoked) {
            for (var i = 0; i < invoked.Length; i++) {
                var index = i;
                env.AssertListener (statementNames[i], listener => Assert.AreEqual (
                    invoked[index], listener.GetAndClearIsInvoked (),
        ...
        private static void TryPattern3Stream (RegressionEnvironment env,
            ...
            for (var i = 0; i < intBoxedA.Length; i++) {
                ...
                var index = i;
                env.AssertListener ("s0", listener => Assert.AreEqual (
                    expected[index], listener.GetAndClearIsInvoked (), "failed at index " + index));

                env.UndeployAll ();
            }
        }

        private static void Try3Fields (RegressionEnvironment env, AtomicLong milestone,
            string text,
            int[] intPrimitive,
            int?[] intBoxed,
            double?[] doubleBoxed,
        ...
        private static void TryPattern (RegressionEnvironment env, string text,
            AtomicLong milestone, int?[] intBoxedA,
            double?[] doubleBoxedA,
            int?[] intBoxedB,
            double?[] doubleBoxedB,
            bool[] expected) {
        ...
        private static void TryPatternWithExpr (RegressionEnvironment env, string text, AtomicLong milestone) {
        ...
        private static void TryRewriteWhere (RegressionEnvironment env, string prefix, AtomicLong milestone) {
