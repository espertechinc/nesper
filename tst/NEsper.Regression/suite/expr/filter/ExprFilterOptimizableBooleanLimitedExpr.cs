...
namespace com.espertech.esper.regressionlib.suite.expr.filter
{
	public class ExprFilterOptimizableBooleanLimitedExpr {
	    ...
	    public class ExprFilterOptReboolDuplicateRegexp : RegressionExecution {
	        ...
	        private void SendAssert(RegressionEnvironment env, string theString, bool expected) {
	    ...
	    private class ExprFilterOptReboolWithEquals : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select * from pattern[s0=SupportBean_S0 -> SupportBean(intPrimitive+5=s0.id and theString='a')]";
	            env.CompileDeploy(epl).AddListener("s0");
	    ...
	    private class ExprFilterOptReboolPatternValueWithConst : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            var epl = "@name('s0') select * from pattern[s0=SupportBean_S0 -> SupportBean_S1(p10 || 'abc' regexp s0.p00)];\n";
	            env.CompileDeploy(epl).AddListener("s0");
	    ...
	    private class ExprFilterOptReboolMixedValueRegexpRHS : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            if (HasFilterIndexPlanAdvanced(env)) {
	                var filters = SupportFilterServiceHelper.GetFilterSvcAllStmtForTypeSingleFilter(env.Runtime, "SupportBean");
	                var s0 = filters.Get("s0");
	    ...
	    private static void SendS0Assert(RegressionEnvironment env, string p00, string p01, bool receivedS0) {
	        env.SendEventBean(new SupportBean_S0(0, p00, p01));
	        env.AssertListenerInvokedFlag("s0", receivedS0);
	    }

	    private static void SendSBAssert(RegressionEnvironment env, string theString, bool receivedS0, bool receivedS1, bool receivedS2) {
	        env.SendEventBean(new SupportBean(theString, 0));
	        env.AssertListenerInvokedFlag("s0", receivedS0);
	    ...
	    private static void SendSBAssert(RegressionEnvironment env, string theString, int intPrimitive, bool receivedS0) {
	        env.SendEventBean(new SupportBean(theString, intPrimitive));
	        env.AssertListenerInvokedFlag("s0", receivedS0);
	    }

	    private static void SendSBAssert(RegressionEnvironment env, string theString, ICollection<string> names, bool expected) {
	        env.SendEventBean(new SupportBean(theString, 0));
	        AssertReceived(env, names, expected);
	    }

	    private static void AssertReceived(RegressionEnvironment env, ICollection<string> names, bool expected) {
	    ...
	    private static void AssertSameFilterEntry(RegressionEnvironment env, string eventTypeName) {
	        env.AssertThat(() => {
	            var filters = SupportFilterServiceHelper.GetFilterSvcAllStmtForTypeSingleFilter(env.Runtime, eventTypeName);
	            var s0 = filters.Get("s0");
	            var s1 = filters.Get("s1");
	            Assert.AreEqual(FilterOperator.REBOOL, s0.Op);
	            Assert.IsNotNull(s0.Index);
	    ...
	    private static void SendS1Assert(RegressionEnvironment env, string p10, string p11, bool expected) {
	        env.SendEventBean(new SupportBean_S1(1, p10, p11));
	        env.AssertListenerInvokedFlag("s0", expected);
	    }

	    private static void SendS1Assert(RegressionEnvironment env, string p10, bool expected) {
