...
namespace com.espertech.esper.regressionlib.suite.context
{
	public class ContextHashSegmented {
	    ...
	    private class ContextHashScoringUseCase : RegressionExecution {
	        ...
	        private static void TryAssertionScoringUseCase(RegressionEnvironment env, EventRepresentationChoice eventRepresentationEnum, AtomicLong milestone) {
	            var fields = "userId,keyword,sumScore".SplitCsv();
	            var epl =
	                    eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedScoreCycle)) + "@buseventtype @public create schema ScoreCycle (userId string, keyword string, productId string, score long);\n" +
	                    eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvidedUserKeywordTotalStream)) + "@buseventtype @public create schema UserKeywordTotalStream (userId string, keyword string, sumScore long);\n" +
	                    "\n" +
	                eventRepresentationEnum.GetAnnotationTextWJsonProvided(typeof(MyLocalJsonProvided)) + " create context HashByUserCtx as " +
	    ...
	    public class ContextHashPartitionSelection : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowIteratorAnyOrder("s0", fields, new object[][]{new object[] {5, "E1", 6}, new object[] {15, "E2", 10}, new object[] {9, "E3", 201}});
	            ...
	            env.AssertStatement("s0", statement => {
	                ...
	                EPAssertionUtil.AssertPropsPerRowAnyOrder(statement.GetEnumerator(selector), statement.GetSafeEnumerator(selector), fields, new object[][]{new object[] {15, "E2", 10}});
	                selector = new SupportSelectorByHashCode(new HashSet<int>(Arrays.AsList(1, 9, 5)));
	                EPAssertionUtil.AssertPropsPerRowAnyOrder(statement.GetEnumerator(selector), statement.GetSafeEnumerator(selector), fields, new object[][]{new object[] {5, "E1", 6}, new object[] {9, "E3", 201}});
	                Assert.IsFalse(statement.GetEnumerator(new SupportSelectorByHashCode(Collections.SingletonSet(99))).MoveNext());
	                Assert.IsFalse(statement.GetEnumerator(new SupportSelectorByHashCode(EmptySet<int>.Instance)).MoveNext());
	                ...
	                EPAssertionUtil.AssertPropsPerRowAnyOrder(statement.GetEnumerator(filtered), statement.GetSafeEnumerator(filtered), fields, new object[][]{new object[] {15, "E2", 10}});
	                filtered = new MySelectorFilteredHash(new HashSet<int>(Arrays.AsList(1, 9, 5)));
	                EPAssertionUtil.AssertPropsPerRowAnyOrder(statement.GetEnumerator(filtered), statement.GetSafeEnumerator(filtered), fields, new object[][]{new object[] {5, "E1", 6}, new object[] {9, "E3", 201}});
	                ...
	                try {
	                    ...
	                    Assert.IsTrue(
		                    ex.Message.StartsWith("Invalid context partition selector, expected an implementation class of any of [ContextPartitionSelectorAll, ContextPartitionSelectorFiltered, ContextPartitionSelectorById, ContextPartitionSelectorHash] interfaces but received com."),
		                    "message: " + ex.Message);
	                }
	            });
	    ...
	    private class ContextHashSegmentedFilter : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var eplCtx = "@name('context') @public create context " + ctx + " as " +
	                         "coalesce " +
	            ...
	            var eplStmt = "@name('s0') context " + ctx + " " + "select context.name as c0, intPrimitive as c1 from SupportBean#lastevent";
	    ...
	    private class ContextHashSegmentedManyArg : RegressionExecution {
	        ...
	        private static void TryHash(RegressionEnvironment env, AtomicLong milestone, string hashFunc) {
	            var path = new RegressionPath();
	    ...
	    private class ContextHashSegmentedMulti : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var eplStmt = "@name('s0') context " + ctx + " " +
	            ...
	            AssertIterator(env, "s0", fields, new object[][]{new object[] {ctx, 10, 3}, new object[] {ctx, 12, 1}});
	    ...
	    private class ContextHashSegmentedBasic : RegressionExecution {
	        ...
	        private static void TryAssertionHash(RegressionEnvironment env, AtomicLong milestone, string stmtNameIterate, string stmtNameContext) {
	            ...
	            AssertIterator(env, stmtNameIterate, fields, new object[][]{new object[] {stmtNameContext, "E1", 5}});

	            env.MilestoneInc(milestone);

	            env.SendEventBean(new SupportBean("E2", 6));
	            env.AssertPropsNew("s0", fields, new object[]{stmtNameContext, "E2", 6});
	            AssertIterator(env, stmtNameIterate, fields, new object[][]{new object[] {stmtNameContext, "E1", 5}, new object[] {stmtNameContext, "E2", 6}});

	            env.SendEventBean(new SupportBean("E3", 7));
	            env.AssertPropsNew("s0", fields, new object[]{stmtNameContext, "E3", 7});
	            AssertIterator(env, stmtNameIterate, fields, new object[][]{new object[] {stmtNameContext, "E1", 5}, new object[] {stmtNameContext, "E3", 7}, new object[] {stmtNameContext, "E2", 6}});

	            env.MilestoneInc(milestone);

	            env.SendEventBean(new SupportBean("E4", 8));
	            env.AssertPropsNew("s0", fields, new object[]{stmtNameContext, "E4", 8});
	            AssertIterator(env, stmtNameIterate, fields, new object[][]{new object[] {stmtNameContext, "E1", 5}, new object[] {stmtNameContext, "E3", 7}, new object[] {stmtNameContext, "E4", 8}, new object[] {stmtNameContext, "E2", 6}});

	            env.SendEventBean(new SupportBean("E5", 9));
	            env.AssertPropsNew("s0", fields, new object[]{stmtNameContext, "E5", 9});
	            AssertIterator(env, stmtNameIterate, fields, new object[][]{new object[] {stmtNameContext, "E5", 9}, new object[] {stmtNameContext, "E1", 5}, new object[] {stmtNameContext, "E3", 7}, new object[] {stmtNameContext, "E4", 8}, new object[] {stmtNameContext, "E2", 6}});

	            env.MilestoneInc(milestone);

	            env.SendEventBean(new SupportBean("E1", 10));
	            env.AssertPropsNew("s0", fields, new object[]{stmtNameContext, "E1", 15});
	            AssertIterator(env, stmtNameIterate, fields, new object[][]{new object[] {stmtNameContext, "E5", 9}, new object[] {stmtNameContext, "E1", 15}, new object[] {stmtNameContext, "E3", 7}, new object[] {stmtNameContext, "E4", 8}, new object[] {stmtNameContext, "E2", 6}});

	            env.SendEventBean(new SupportBean("E4", 11));
	            env.AssertPropsNew("s0", fields, new object[]{stmtNameContext, "E4", 19});
	            AssertIterator(env, stmtNameIterate, fields, new object[][]{new object[] {stmtNameContext, "E5", 9}, new object[] {stmtNameContext, "E1", 15}, new object[] {stmtNameContext, "E3", 7}, new object[] {stmtNameContext, "E4", 19}, new object[] {stmtNameContext, "E2", 6}});
	    ...
	    private class ContextHashSegmentedBySingleRowFunc : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var eplStmt = "@name('s0') context HashSegmentedContext select context.id as c1, myHash(*) as c2, mySecond(*, theString) as c3, "
	                          + nameof(ContextHashSegmented) + ".mySecondFunc(*, theString) as c4 from SupportBean";
	    ...
	    private static void AssertIterator(RegressionEnvironment env, string statementName, string[] fields, object[][] expected) {
	        ...
	        env.AssertSafeEnumerator(statementName, iterator => {
