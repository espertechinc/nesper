	...
	public class ContextKeySegmentedInfra {
	    ...
	    private class ContextKeySegmentedInfraNWConsumeAll : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertIterator("s0", iterator => EPAssertionUtil.AssertProps(iterator.Advance(), "cnt".SplitCsv(), new object[]{0L}));
	    ...
	    public class ContextKeyedSegmentedTable : RegressionExecution {
	        ...
	        private void AssertValues(RegressionEnvironment env, string group, object[][] expected) {
	            env.AssertStatement("table", statement => {
		            var it =
			            statement.GetEnumerator(new ProxyContextPartitionSelectorSegmented(
	    ...
	    private static void TryAssertionSegmentedOnSelect(RegressionEnvironment env, AtomicLong milestone, bool namedWindow) {
	        var path = new RegressionPath();
	        ...
	        env.SendEventBean(new SupportBean_S0(0, "G1"));
	        env.AssertPropsPerRowLastNewAnyOrder("s0", fieldsNW, new object[][]{new object[] {"G1", 1}, new object[] {"G1", 3}});
	    ...
	    private static void TryAssertionCreateIndex(RegressionEnvironment env, AtomicLong milestone, bool namedWindow) {
	        var path = new RegressionPath();
	        ...
	        env.AssertThat(() => {
	            ...
	            var result = env.Runtime.FireAndForgetService.ExecuteQuery(faf, new ContextPartitionSelector[]{new SupportSelectorById(1)});
	            EPAssertionUtil.AssertPropsPerRow(result.Array, "theString,intPrimitive".SplitCsv(), new object[][]{new object[] {"E1", 1}});
	        });
	    ...
	    private static void TryAssertionOnDeleteAndUpdate(RegressionEnvironment env, AtomicLong milestone, bool namedWindow) {
	        var path = new RegressionPath();
	        env.CompileDeploy("@name('context') @public create context SegmentedByString " +
	        ...
	        env.SendEventBean(new SupportBean_S0(0, "G2"));
	        if (namedWindow) {
	            env.AssertListener("s0", listener => EPAssertionUtil.AssertPropsPerRow(listener.LastOldData, fieldsNW, new object[][]{new object[] {"G2", 20}, new object[] {"G2", 21}}));
	        }
	    ...
	    private static void TryAssertionAggregatedSubquery(RegressionEnvironment env, AtomicLong milestone, bool namedWindow) {
	        var epl = "";
