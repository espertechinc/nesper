	...
	public class ContextVariables {
	    ...
	    private class ContextVariablesIterateAndListen : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.AssertPropsPerRowIterator("upd", fields, new object[][]{new object[] {100}, new object[] {101}});
	            env.AssertPropsPerRowIteratorAnyOrder("var", fields, new object[][]{new object[] {100}, new object[] {101}});
	    ...
	    private class ContextVariablesGetSetAPI : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.Runtime.VariableService.SetVariableValue(Collections.SingletonMap<DeploymentIdNamePair, object>(namePairVariable, 10), 0);
	            AssertVariableValues(env, 0, 10);

	            env.SendEventBean(new SupportBean_S0(0, "P2"));    // allocate partition P2
	            AssertVariableValues(env, 1, 5);

	            env.Runtime.VariableService.SetVariableValue(Collections.SingletonMap<DeploymentIdNamePair, object>(namePairVariable, 11), 1);
	            ...
	            try {
	                env.Runtime.VariableService.SetVariableValue(Collections.SingletonMap<DeploymentIdNamePair, object>(nameGlobalVar, 11), 0);
	                Assert.Fail();
	            ...
	            try {
	                env.Runtime.VariableService.GetVariableValue(Collections.SingletonSet(nameGlobalVar), new SupportSelectorById(1));
	                Assert.Fail();
	            } catch (VariableNotFoundException ex) {
	                Assert.AreEqual("Variable by name 'myglobarvar' is a global variable and not context-partitioned", ex.Message);
	            }
	    ...
	    private static void AssertVariableValues(RegressionEnvironment env, int agentInstanceId, int expected) {
	        var namePairVariable = new DeploymentIdNamePair(env.DeploymentId("var"), "mycontextvar");
	        var states = env.Runtime.VariableService.GetVariableValue(Collections.SingletonSet(namePairVariable), new SupportSelectorById(agentInstanceId));
