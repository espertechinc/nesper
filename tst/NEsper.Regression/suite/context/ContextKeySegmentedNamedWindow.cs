	...
	public class ContextKeySegmentedNamedWindow {
	    ...
	    private class ContextKeyedNamedWindowFAF : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            EPAssertionUtil.AssertPropsPerRow(env.Runtime.FireAndForgetService.ExecuteQuery(compiled).Array, "theString".SplitCsv(), new object[][]{new object[] {"G1"}});

	            env.SendEventBean(new SupportBean("G2", 0));

	            env.Milestone(1);

	            EPAssertionUtil.AssertPropsPerRowAnyOrder(env.Runtime.FireAndForgetService.ExecuteQuery(compiled).Array, "theString".SplitCsv(), new object[][]{new object[] {"G1"}, new object[] {"G2"}});
	    ...
	    private class ContextKeyedSubqueryNamedWindowIndexUnShared : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var epl = "@name('context') create context SegmentedByString partition by theString from SupportBean;\n" +
