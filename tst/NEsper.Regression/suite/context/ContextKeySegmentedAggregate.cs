	...
	public class ContextKeySegmentedAggregate {
	    ...
	    private class ContextKeySegmentedAccessOnly : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var eplContext = "@name('CTX') @public create context SegmentedByString partition by theString from SupportBean";
	            env.CompileDeploy(eplContext, path);

	            var fieldsGrouped = "theString,intPrimitive,col1".SplitCsv();
	            var eplGroupedAccess = "@name('s0') context SegmentedByString select theString,intPrimitive,window(longPrimitive) as col1 from SupportBean#keepall sb group by intPrimitive";
	    ...
	    private class ContextKeySegmentedRowPerGroupBatchContextProp : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            env.SendEventBean(new SupportBean("G1", 11));
	            env.AssertPropsPerRowNewOnly("s0", fieldsOne, new object[][] {new object[] {10, 1L}, new object[] {11, 1L}});
	            ...
	            env.SendEventBean(new SupportBean("G1", 10));
	            env.AssertPropsPerRowNewOnly("s0", fieldsOne, new object[][]{new object[] {10, 2L}, new object[] {11, 0L}});

	            env.Milestone(4);

	            env.SendEventBean(new SupportBean("G2", 10));
	            env.SendEventBean(new SupportBean("G2", 10));
	            env.AssertPropsPerRowNewOnly("s0", fieldsOne, new object[][]{new object[] {10, 2L}, new object[] {200, 0L}});
	            ...
	            env.SendEventBean(new SupportBean("G1", 11));
	            env.AssertPropsPerRowNewOnly("s0", fieldsTwo, new object[][]{new object[] {"G1", 10, 1L}, new object[] {"G1", 11, 1L}});
	            ...
	            env.SendEventBean(new SupportBean("G1", 10));
	            env.AssertPropsPerRowNewOnly("s0", fieldsTwo, new object[][]{new object[] {"G1", 10, 2L}, new object[] {"G1", 11, 0L}});

	            env.Milestone(7);

	            env.SendEventBean(new SupportBean("G2", 10));
	            env.SendEventBean(new SupportBean("G2", 10));
	            env.AssertPropsPerRowNewOnly("s0", fieldsTwo, new object[][]{new object[] {"G2", 10, 2L}, new object[] {"G2", 200, 0L}});
	    ...
	    private class ContextKeySegmentedRowForAll : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var eplCtx = "@name('context') @public create context SegmentedByString partition by theString from SupportBean";
	    ...
	    public class ContextKeySegmentedRowPerEvent : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var eplUngrouped = "@name('S1') context SegmentedByString select theString,sum(intPrimitive) as col1 from SupportBean";
	            env.CompileDeploy(eplUngrouped, path).AddListener("S1");

	            var eplGroupedAccess = "@name('S2') context SegmentedByString select theString,window(intPrimitive) as col1 from SupportBean#keepall() sb";
	        ...
	        private void AssertPartitionInfo(RegressionEnvironment env) {
	            env.AssertThat(() => {
	                ...
	                var partitions = partitionAdmin.GetContextPartitions(env.DeploymentId("CTX"), "SegmentedByString", ContextPartitionSelectorAll.INSTANCE);
	    ...
	    public class ContextKeySegmentedRowPerGroup3Stmts : RegressionExecution {
	        public void Run(RegressionEnvironment env) {
	            ...
	            var eplGrouped = "@name('S1') context SegmentedByString select theString,intPrimitive,sum(longPrimitive) as col1 from SupportBean group by intPrimitive";
	            ...
	            var eplGroupedDistinct = "@name('S3') context SegmentedByString select theString,intPrimitive,sum(distinct longPrimitive) as col1 from SupportBean.win:keepall() sb group by intPrimitive";
